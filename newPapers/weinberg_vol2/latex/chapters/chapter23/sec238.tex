...
\begin{equation}
 S[\phi_R]=\int_0^\infty 2\pi^2\rho^3d\rho
 \left[
 \frac{R^2}{2}\left(\frac{d\phi}{d\rho}\right)^2+R^4V(\phi)
 \right].
 \tag{23.8.9}\label{eq:23.8.9}
\end{equation}
If $\phi(\rho)$ is the solution of Eq.~\eqref{eq:23.8.7} then the action must be stationary with respect to any variation in $\phi$, so that $dS[\phi_R]/dR$ must vanish at $R=1$, and therefore
\begin{equation}
 \int_0^\infty\rho^3d\rho\left(\frac{d\phi}{d\rho}\right)^2
 =-4\int_0^\infty\rho^3d\rho\,V(\phi).
 \tag{23.8.10}\label{eq:23.8.10}
\end{equation}
It follows then that
\begin{equation}
 B=\frac{\pi^2}{2}\int_0^\infty\rho^3d\rho
 \left(\frac{d\phi}{d\rho}\right)^2>0.
...
\begin{equation}
 \mathcal S=\int_0^{\langle\phi\rangle}\sqrt{2V(f)}\,df.
