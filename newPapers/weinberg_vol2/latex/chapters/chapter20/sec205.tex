...
\begin{align}
&\sum_N\delta^4(p-p_N)
\bra{\Omega}J_\alpha^\mu(0)\ket{N}
\left[\bra{\Omega}J_\beta^\nu(0)\ket{N}\right]^*
\notag\\
&\qquad=(2\pi)^{-3}\theta(p^0)
\left[
\left(\eta^{\mu\nu}-\frac{p^\mu p^\nu}{p^2}\right)
\rho_{\alpha\beta}^{(1)}(-p^2)
+p^\mu p^\nu\rho_{\alpha\beta}^{(0)}(-p^2)
\right],
...
\begin{align}
\bra{\Omega}J_\alpha^\mu(x)J_\beta^\nu(0)\ket{\Omega}
={}&\int d\mu^2
\left[
\eta^{\mu\nu}\rho_{\alpha\beta}^{(1)}(\mu^2)
\right.
\notag\\
&\left.
\qquad-
\left(
\rho_{\alpha\beta}^{(0)}(\mu^2)
+\frac{\rho_{\alpha\beta}^{(1)}(\mu^2)}{\mu^2}
\right)
\partial^\mu\partial^\nu
\right]\Delta_+(x;\mu^2),
...
\end{equation}
Assuming the currents to have been chosen as Hermitian operators, it is immediately apparent from Eq.~\eqref{eq:20.5.1} that $\rho_{\alpha\beta}^{(1)}(\mu^2)$ and $\rho_{\alpha\beta}^{(0)}(\mu^2)$ are positive Hermitian matrices. Also, taking $x^\mu$ to be spacelike (for which $\Delta_+(x)$ is even) and using translation invariance and causality in Eq.~\eqref{eq:20.5.2}, we see that $\rho_{\alpha\beta}^{(1)}(\mu^2)$ and $\rho_{\alpha\beta}^{(0)}(\mu^2)$ are also symmetric.
...
\end{equation}
where $\gamma$ is the Euler constant. The first few terms in the vacuum expectation value of the expansion of $J_\alpha^\mu(x)J_\beta^\nu(0)$ are thus
\begin{align}
\bra{\Omega}J_\alpha^\mu(x)J_\beta^\nu(0)\ket{\Omega}
\longrightarrow{}&
-\frac{1}{2\pi^2}
...
\notag\\
&-\frac{\eta_{\mu\nu}}{4\pi^2x^2}
\int d\mu^2\,\rho_{\alpha\beta}^{(0)}(\mu^2)\mu^2
\notag\\
&+\frac{x_\mu x_\nu}{2\pi^2(x^2)^2}
\int d\mu^2
\left(
\rho_{\alpha\beta}^{(0)}(\mu^2)\mu^2
+\rho_{\alpha\beta}^{(1)}(\mu^2)
\right)
+O(\ln x^2).
\tag{20.5.5}\label{eq:20.5.5}
\end{align}
Hence if some linear combination $\sum_{\alpha\beta}c_{\alpha\beta}\bra{\Omega}J_\alpha^\mu(x)J_\beta^\nu(0)\ket{\Omega}$ of the two-point functions has a singularity as $x\to0$ which can be shown to be weaker than of order $1/x^4$, we have
\begin{equation}
...
\begin{equation}
\sum_{\alpha\beta}c_{\alpha\beta}
\int d\mu^2\,\rho_{\alpha\beta}^{(1)}(\mu^2)=0
\tag{20.5.7}\label{eq:20.5.7}
\end{equation}
and
\begin{equation}
\sum_{\alpha\beta}c_{\alpha\beta}
\int d\mu^2\,\rho_{\alpha\beta}^{(0)}(\mu^2)\mu^2=0.
...
In order to tell which operators appear in the expansion of the product of two currents, we need to classify the $SU(N)\cdot SU(N)$ representations contained in the product, and to tell whether these operators have non-vanishing vacuum expectation values, we have to ask which of them are invariant under the subgroup of $SU(N)\cdot SU(N)$ that is not spontaneously broken. To answer these questions, we note that the currents $J_{La}^\mu(x)$ and $J_{Ra}^\mu(x)$ transform under $SU(N)\cdot SU(N)$ respectively according to the $(A,\mathbf{1})$ and $(\mathbf{1},A)$ representations of $SU(N)\cdot SU(N)$, where $A$ and $\mathbf{1}$ are the adjoint and identity representations of $SU(N)$.

Also, we shall assume that the subgroup of $SU(N)\cdot SU(N)$ that is not spontaneously broken is the vectorial $SU(N)_V$ whose currents are $J_{La}^\mu(x)+J_{Ra}^\mu(x)$, as is the case in quantum chromodynamics and (as we saw in Section 19.9) a wide range of other theories. We also assume that parity conservation is not spontaneously broken. These unbroken symmetries have the consequence that
\begin{equation}
\rho_{La,Lb}^{(1)}(\mu^2)
=\rho_{Ra,Rb}^{(1)}(\mu^2)
=\delta_{ab}\left[
...
\qquad
J_{Aa}^\mu=-i\bar\psi\gamma^\mu\gamma^5\lambda_a\psi,
\tag{20.5.14}\label{eq:20.5.14}
\end{equation}
with generators $\lambda_a$ and $\lambda_a\gamma^5$, respectively. Also
\begin{equation}
...
The products $J_{La}^\mu(x)J_{Lb}^\nu(0)$ and $J_{Ra}^\mu(x)J_{Rb}^\nu(0)$ transform respectively as the $(A\cdot A,\mathbf{1})$ and $(\mathbf{1},A\cdot A)$ representations of $SU(N)\cdot SU(N)$, where $A$ and $\mathbf{1}$ are the adjoint and identity representations, respectively. For any group, $A\cdot A$ contains the identity representation, so the unit operator appears in the expansions of these products, with equal coefficients proportional to $\delta_{ab}$. Hence only the traceless parts of the operator product can satisfy spectral function sum rules. But as we have seen, the spectral functions have no traceless part, so the like-chirality spectral functions cannot satisfy any sum rules.

\noindent\textbf{Unlike Chirality:}\quad
The products $J_{La}^\mu(x)J_{Rb}^\nu(0)$ and $J_{Ra}^\mu(x)J_{Lb}^\nu(0)$ both transform as the $(A,A)$ representation of $SU(N)\cdot SU(N)$. The unit operator (and operators like $F_{\alpha\mu\nu}F_\alpha^{\mu\nu}$) are of course $SU(N)\cdot SU(N)$ singlets, and therefore cannot appear in the expansion of these products. Non-derivative fermion bilinears like $\bar\psi\psi$ transform according to the $(\overline N,N)$ and $(N,\overline N)$ representations of $SU(N)\cdot SU(N)$, so they also cannot appear in the expansions of the products of currents with unlike chirality. The only gauge- and Lorentz-invariant fermion bilinears with a single derivative involve the gauge-covariant derivative operator $\gamma^\mu D_\mu$ acting on $\psi$, which the field equations tell us vanishes. Thus these spectral functions satisfy both the first and second spectral function sum rules, which here read
\begin{equation}
\int d\mu^2\,
...
In the original work on spectral function sum rules~\hyperref[ch20-ref-5]{[5]}, the $SU(2)\cdot SU(2)$ spectral functions were assumed to be sharply peaked at values of $\mu$ that for $\rho_V^{(1)}(\mu^2)$ could be assumed to be $m_\rho=770$ MeV, and for $\rho_A^{(1)}(\mu^2)$ was taken to be at an unknown mass $m_A$. That is,
\[
