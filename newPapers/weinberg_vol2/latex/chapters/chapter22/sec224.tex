...
current $J_\alpha^\mu$. Where this current is itself coupled to a gauge
...
the completely symmetric constant factor $D_{\alpha\beta\gamma}$ defined
by Eq.~\eqref{eq:22.3.12}, so for gauge currents we must
have~\hyperref[ch22-ref-11]{[11]}
\begin{equation}
 D_{\alpha\beta\gamma}\equiv
 \frac{1}{2}\operatorname{tr}
 \bigl[\{T_\alpha,T_\beta\}T_\gamma\bigr]=0,
 \tag{22.4.1}\label{eq:22.4.1}
\end{equation}
where $T_\alpha$ is the representation of the gauge algebra on the set of
...
\]
or equivalently (since we always take $T_\alpha$ Hermitian)
\begin{equation}
 T_\alpha^{\mathrm T}=-S T_\alpha S^{-1}.
 \tag{22.4.2}\label{eq:22.4.2}
\end{equation}
(Inserting Eq.~\eqref{eq:22.4.2} in Eq.~\eqref{eq:22.3.12} gives
$D_{\alpha\beta\gamma}=-D_{\alpha\beta\gamma}$.) Such a representation
may be either \emph{real}, in which case it is possible by a similarity
transformation $T'_\alpha=R T_\alpha R^{-1}$ to convert the representation
to a form in which $T'_\alpha$ is imaginary and antisymmetric, or
...
$D_{\alpha\beta\gamma}$ vanishes, even though some representations are
...
group $SU(3)\cdot SU(2)\cdot U(1)$, so we must rely on cancellations among
...
We can now check whether $D_{\alpha\beta\gamma}$ vanishes when $T_\alpha$,
$T_\beta$, and $T_\gamma$ run over all of the generators of
$SU(3)\cdot SU(2)\cdot U(1)$. We need only consider those combinations of
generators for which the product of $T_\alpha$, $T_\beta$, and $T_\gamma$
is neutral under $SU(3)\cdot SU(2)\cdot U(1)$, since
$D_{\alpha\beta\gamma}$ obviously vanishes for all the others. We can make
invariants out of either zero, two, or three $SU(3)$ generators (because
$\mathbf8\cdot\mathbf8$ and
$\mathbf8\cdot\mathbf8\cdot\mathbf8$ both contain singlets), zero, two, or
...
Here $D_{\alpha\beta\gamma}$ vanishes because the left-handed fermions
furnish a representation
$\mathbf3+\mathbf3+\overline{\mathbf3}+\overline{\mathbf3}
+\mathbf1+\mathbf1+\mathbf1$ of $SU(3)$, which is real.
...
by noting that $SU(3)\cdot SU(2)\cdot U(1)$ may be embedded in
...
reducible representation of $SU(3)\cdot SU(2)\cdot U(1)$ that furnishes a
...
one additional $(SU(3)\cdot SU(2)\cdot U(1))$-singlet forms a complete
...
$SU(3)\cdot SU(2)\cdot U(1)$ anomalies. The singlet would not contribute
...
$\overline\chi T_\alpha\gamma^\mu\chi$ (where $\chi$ is a column of all
...
$\partial_\mu\langle\overline\chi T_\alpha\gamma^\mu\chi\rangle$
proportional to
\[
 \operatorname{tr}\{T_\alpha\}\epsilon^{\mu\nu\rho\sigma}
...
($\operatorname{tr}\{T_\alpha\}$ is just a number which commutes with all
the $T_\beta$, so if it is non-zero then the algebra is not simple.) Hence
...
hypercharges $y$ for various $SU(3)\cdot SU(2)$ multiplets were originally
...
$SU(3)\cdot SU(2)\cdot U(1)$ gauge bosons of the standard model may couple
...
that couple to other undetected $(SU(3)\cdot SU(2)\cdot U(1))$-neutral
...
$(SU(3)\cdot SU(2)\cdot U(1))$-neutral fermions, the requirement of
