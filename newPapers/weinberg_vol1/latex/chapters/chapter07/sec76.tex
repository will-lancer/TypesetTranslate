...
cancel.}
\begin{equation}
H_0=\sum_\sigma\int d^3p\,p^0\left[
...
$[\Psi^a,\Pi_b]=i\delta^a_b$, $[\Psi^a,\Psi^b]=[\Pi_a,\Pi_b]=0$, then the
commutator of any two functions of the $\Psi$s and $\Pi$s would be just
$[A,B]=i[A,B]_{\mathrm P}$. But the constraints do not always allow this.
...
\notag\\
[V^i(\mathbf x),V^0(\mathbf y)]&=-im^{-2}\partial_i\delta^3(\mathbf x-\mathbf y),
\notag\\
[V^i(\mathbf x),\Pi_j(\mathbf y)]&=i\delta^i_j\delta^3(\mathbf x-\mathbf y),
