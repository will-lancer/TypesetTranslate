...
&=\int\!\left[\prod_{s,y}\dd\phi^s(y)\right]
 \exp\!\left(\ii I[\phi]+\ii\int \dd^4x\,\phi^r(x)J_r(x)+\epsilon\text{ terms}\right).
\label{eq:16.1.1}
\end{align}
...
&=\ii\int\!\left[\prod_{r,x}\dd\phi^r(x)\right]
 \phi^r(y)\exp\{\ii I[\phi]+\epsilon\text{ terms}\} \notag\\
&=\ii\,\OutBra{\Omega}\Phi^r(y){\InKet{\Omega}}_{J=0}.
\label{eq:16.1.2}
\end{align}
...
\begin{equation}
Z[J]=\sum_{N=0}^{\infty}\frac{1}{N!}\bigl(\ii W[J]\bigr)^N
=\exp\bigl(\ii W[J]\bigr),
\label{eq:16.1.3}
\end{equation}
...
=-\frac{\ii}{Z[J]}\frac{\delta}{\delta J_r(x)}Z[J]
\label{eq:16.1.4}
\end{equation}
...
\begin{equation}
\Gamma[\phi]\equiv-\int \dd^4x\,\phi^r(x)J_{\phi r}(x)+W[J_\phi].
\label{eq:16.1.6}
\end{equation}
...
={}&-\int \dd^4x\,\phi^r(x)\frac{\delta J_{\phi r}(x)}{\delta\phi^s(y)}-J_{\phi s}(y)\\
&+\int \dd^4x\left[\frac{\delta W[J]}{\delta J_r(x)}\right]_{J=J_\phi}
...
\begin{align}
\exp\{\ii W_\Gamma[J,g]\}
\equiv{}&\int\prod_{r,x}\dd\phi^r(x)\,
\exp\!\left\{ig^{-1}\left[\Gamma[\phi]+\int \dd^4x\,\phi^r(x)J_r(x)\right]
...
\begin{equation}
\exp\{\ii W_\Gamma[J,g]\}\propto
\exp\!\left\{ig^{-1}\left[\Gamma[\phi_J]+\int \dd^4x\,\phi_J^r(x)J_r(x)\right]\right\},
\label{eq:16.1.12}
\end{equation}
...
\begin{equation}
W_\Gamma^{(0)}[J]=\Gamma[\phi_J]+\int \dd^4x\,\phi_J^r(x)J_r(x).
\label{eq:16.1.14}
\end{equation}
...
\left[\prod_{r,x}\dd\phi^r(x)\right]
\exp\!\left\{\ii\Gamma[\phi]+\ii\sum_r\int\phi^r(x)J_r(x)\,\dd^4x\right\}.
\label{eq:16.1.16}
\end{equation}
...
Eq.\ \eqref{eq:16.1.16} to be correct, $\ii\Gamma[\phi]$ must be the sum of all
...
Equivalently, $\ii\Gamma[\phi_0]$ for some fixed field $\phi_0^r(x)$ may be expressed as the
sum of one-particle-irreducible graphs for the vacuum--vacuum amplitude,
calculated with a shifted action $I[\phi+\phi_0]$:
\begin{equation}
\ii\Gamma[\phi_0]=\int_{\substack{\mathrm{1PI}\\\mathrm{CONNECTED}}}
\left[\prod_{r,x}\dd\phi^r(x)\right]\exp\{\ii I[\phi+\phi_0]\}.
\label{eq:16.1.17}
\end{equation}
...
\begin{equation}
\exp\{\ii\Gamma[\phi_0]\}=\int_{\mathrm{1PI}}
\left[\prod_{r,x}\dd\phi^r(x)\right]\exp\{\ii I[\phi+\phi_0]\},
\label{eq:16.1.18}
\end{equation}
