...
\exp\!\left(\ii q_n\Lambda(x)/\hbar\right)\psi_n(x),
...
\begin{equation}
\psi_n(x)=\exp\!\left(\ii q_n\phi(x)/\hbar\right)\widetilde\psi_n(x).
...
\begin{equation}
\int_{\mathcal A}\mathbf B\cdot \dd\mathbf S
=\oint_{\mathcal C}\mathbf A\cdot \dd\mathbf x
=\oint_{\mathcal C}\boldsymbol\nabla\phi\cdot \dd\mathbf x
=\frac{n\pi\hbar}{e}.
...
\[
\Delta_A\phi=\int \dd\mathbf x\cdot
...
\left[-\ii\frac{\partial}{\partial t}+E(\mathbf p)\right]
a_{\mathbf p,s}(t)
\notag\\
&+\sum_{s_1s_2s_3s_4}
\int \dd^3p_1\,\dd^3p_2\,\dd^3p_3\,\dd^3p_4\,
V_{s_1s_2s_3s_4}
...
\begin{equation}
\frac{1}{\omega-v_F(\mathbf k)\ell+\ii\epsilon}.
...
\left[
-\ii\frac{\partial}{\partial t}-A_0(\mathbf x,t)
+E\{-\ii\boldsymbol\nabla+\mathbf A(\mathbf x,t)\}
\right]\psi_s(\mathbf x,t)
\notag\\
&+\sum_{s_1s_2s_3s_4}
\int \dd^3x_1\,\dd^3x_2\,\dd^3x_3\,\dd^3x_4\,
...
\int \dd^3p\,\exp(\ii\mathbf p\cdot\mathbf x)\,a_{\mathbf p,s}(t).
...
\Gamma[\Psi]
={}&\frac14\sum_{s_1s_2s_3s_4}
\int \dd t\int \dd^3x_1\,\dd^3x_2\,\dd^3x_3\,\dd^3x_4\,
V_{s_1s_2s_3s_4}(\mathbf x_1,\mathbf x_2,\mathbf x_3,\mathbf x_4)
\notag\\
&\quad\cdot
\Psi^\dagger_{s_2s_1}(\mathbf x_2,\mathbf x_1,t)
...
\notag\\
&-\ii\ln\operatorname{Det}
\begin{pmatrix}
\mathcal A&\mathcal B\\
\mathcal B^\dagger&-\mathcal A^{\mathsf T}
\end{pmatrix},
...
=\int \dd^3p\,e^{\ii\mathbf p\cdot(\mathbf x'-\mathbf x)}\Psi(\mathbf p),
\tag{21.6.63}\label{eq:21.6.63}
\end{equation}
\begin{equation}
\Delta(\mathbf x',\mathbf x)
=(2\pi)^{-3}\int \dd^3p\,e^{\ii\mathbf p\cdot(\mathbf x'-\mathbf x)}
\Delta(\mathbf p),
...
\begin{align}
&\int \dd^3x_1\,\dd^3x_2\,\dd^3x_3\,\dd^3x_4\,
e^{\ii\mathbf p'\cdot(\mathbf x_1-\mathbf x_2)}
e^{\ii\mathbf p\cdot(\mathbf x_3-\mathbf x_4)}
V(\mathbf x_1,\mathbf x_2,\mathbf x_3,\mathbf x_4)
\notag\\
...
\begin{equation}
\mathcal V_4\equiv\int \dd^3x\int \dd t\,1.
...
\notag\\
&+\frac{\ii}{(2\pi)^4}\int \dd\omega\,\dd^3p\,
\ln\left(
1+\frac{|\Delta(\mathbf p)|^2}
{\omega^2-E^2(\mathbf p)+\ii\epsilon}
\right)
\tag{21.6.67}\label{eq:21.6.67}
\end{align}
with
\begin{equation}
\Delta(\mathbf p)=-\int \dd^3p'\,
V(\mathbf p,\mathbf p')\Psi(\mathbf p').
\tag{21.6.68}\label{eq:21.6.68}
\end{equation}
(The $\ii\epsilon$ term can be inferred from the Feynman rules for the electron loop graphs, and as shown in \hyperref[sec:9.2]{Section 9.2}, it ultimately arises from the conditions on the electron field at $t\to\pm\infty$.) Wick rotating, integrating over $\omega$, and expressing $\Psi$ in terms of $\Delta$, this becomes
\begin{align}
V[\Delta]={}&-\int \dd^3p\,\dd^3p'\,
\Delta^*(\mathbf p')V^{-1}(\mathbf p',\mathbf p)\Delta(\mathbf p)
\notag\\
&-\frac{1}{(2\pi)^3}\int \dd^3p\,
\left[
...
\begin{align}
V[\Delta]
={}&-\kappa^2\int_{\mathcal S}\dd^2k\,\dd^2k'\,
\Delta^*(\mathbf k')V^{-1}(\mathbf k',\mathbf k)\Delta(\mathbf k)
\notag\\
&-\frac{1}{(2\pi)^3}\int_0^\kappa \dd\ell
\int_{\mathcal S}\dd^2k\,
\left[
...
\Delta^*(\mathbf k')V_\mu^{-1}(\mathbf k',\mathbf k)\Delta(\mathbf k)
\notag\\
&+\frac{1}{2(2\pi)^3}\int_{\mathcal S}\dd^2k\,
...
\[
\mu\frac{\dd}{\dd\mu}V_\mu^{-1}(\mathbf k',\mathbf k)
=-\frac{\delta^2(\mathbf k'-\mathbf k)}
{2(2\pi)^3v_F(\mathbf k)}
\]
or, equivalently,
\begin{equation}
\mu\frac{\dd}{\dd\mu}V_\mu(\mathbf k',\mathbf k)
=\frac{1}{2(2\pi)^3}
\int \dd^2k''\,
...
as
\begin{equation}
\mu\frac{\dd}{\dd\mu}K_\mu=K_\mu^2.
