...
\notag\\
&-\bar\psi'\bigl(\sl{\bar D}-it_a\sl{A}'_a+m\bigr)\psi',
\label{eq:17.5.2}
\end{align}
\begin{equation}
...
\notag\\
&-\int d^4x\,d^4y\,\omega_a^{\prime *}(x)\omega'_b(y)
...
\begin{align}
\mathcal M^A_{a\mu,b\nu}(q)={}&
\eta_{\mu\nu}\bigl(-iq^\lambda\delta_{ca}
+A_d^\lambda C_{cda}\bigr)
\bigl(iq_\lambda\delta_{cb}
+A_{e\lambda}C_{ceb}\bigr)
\notag\\
&-\bigl(-iq_\nu\delta_{ca}
+A_{d\nu}C_{cda}\bigr)
\bigl(iq_\mu\delta_{cb}
+A_e^\mu C_{ceb}\bigr)
+F_{c\mu\nu}C_{cab}
\notag\\
&+\frac1\xi\bigl(-iq_\mu\delta_{ca}
+A_{d\mu}C_{cda}\bigr)
\bigl(iq_\nu\delta_{cb}
+A_{e\nu}C_{ceb}\bigr)
+\epsilon\text{ terms},
...
=\bigl(i\sl q-it_a\sl A_a+m\bigr)_{k\ell}
+\epsilon\text{ terms},
\label{eq:17.5.16}
\end{equation}
\begin{align}
\mathcal M^\omega_{ab}(q)={}&
\bigl(-iq_\lambda\delta_{ca}
+A_{d\lambda}C_{cda}\bigr)
\bigl(iq^\lambda\delta_{cb}
...
\[
[\mathcal A_\lambda]_{ab}\equiv-iC_{abc}A_{c\lambda}
\]
for which
\begin{align*}
...
-\mathcal A^\lambda\mathcal A^\eta\mathcal A_\lambda\mathcal A_\eta]
\notag\\
={}&\frac1{12}\mathcal I\,C_{cab}C_{dab}
F_{c\mu\nu}F_d^{\mu\nu}.
\label{eq:17.5.28}
\end{align}
Finally, the vertices in the matter loop are
\[
[\mathcal M_1^\psi(q)]_{k\ell}=-i(t_a\sl A_a)_{k\ell},
\qquad
...
\left\{\left[(i\sl q+m)^{-1}t_a\sl A_a\right]^4\right\}.
\]
...
=\frac{-i\mathcal I}{(2\pi)^4}\int d^4x\,F_{c\mu\nu}F_d^{\mu\nu}
...
case
\begin{equation}
C_{cab}C_{dab}=g^2C_1\delta_{cd},
\label{eq:17.5.33}
\end{equation}
\begin{equation}
...
\begin{equation}
\Gamma_{A^4}^{(1\ \mathrm{loop})}
=\frac{-ig^2\mathcal I}{(2\pi)^4}\int d^4x\,F_{c\mu\nu}F_c^{\mu\nu}
