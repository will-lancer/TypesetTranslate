...
\notag\\
&\qquad\quad{}\cdot
\exp\left(\ii I_{\mathrm{gauge}}[A]
+\ii I_{\mathrm{Dirac}}[\psi,\psi^\dagger;A]\right),
\tag{19.9.1}\label{eq:19.9.1}
\end{align}
where $k_1\ldots k_r$ and $l_1\ldots l_r$ are Dirac spin indices, $u_1\ldots u_r$ and $v_1\ldots v_r$ are flavor indices, $I_{\mathrm{gauge}}[A]$ is the action for a pure gauge theory, $I_{\mathrm{Dirac}}[\psi,\psi^\dagger;A]$ is the action for the Dirac fields in the presence of a gauge field $A^\mu_a(x)$, and $Z$ is the vacuum--vacuum amplitude
\begin{equation}
...
It will be necessary here to work in a Euclidean spacetime, with $x^4=x_4=\ii x^0$, $y^4=y_4=\ii y^0$, and $A^4_a=A_{4a}=\ii A_{0a}$, all real. (See Appendix A of Chapter 23.) In this case, the Dirac action is
\begin{equation}
I_{\mathrm{Dirac}}[\psi,\psi^\dagger;A]
=\ii\int \dd^3x\int \dd x^4\,
...
\begin{equation}
\sl{D}
=\sum_{i=1}^4(\partial_i-\ii t^a A_{ia})\gamma_i,
\tag{19.9.4}\label{eq:19.9.4}
\end{equation}
where as usual $\gamma_4=\ii\gamma^0$. Because the action is quadratic in fermion fields, we can explicitly perform the integral over these fields
\begin{align}
&\bra{\Omega}T\left\{
\Psi_{u_1k_1}(x_1)\cdots\Psi_{u_rk_r}(x_r)
\Psi^\dagger_{v_1l_1}(y_1)\cdots\Psi^\dagger_{v_rl_r}(y_r)
...
\exp\left(\ii I_{\mathrm{gauge}}[A]\right)
\notag\\
&\qquad\quad{}\cdot
\left[
...
\begin{equation}
\Psi_u[f]\equiv\int \dd^4x\,f^{k\dagger}(x)\Psi_{uk}(x),
...
\end{align}
In this basis, the fermion propagators for a given gauge field are not only well defined, but bounded uniformly in $A^\mu_a(x)$:\footnote{To see this, we may expand in the eigenvectors of $M$
\[
...
\begin{equation}
\int f_i^\dagger(x)f_i(x)\,\dd^4x
=\int g_j^\dagger(y)g_j(y)\,\dd^4y
=1.
...
&\quad=
\exp\left(
-\frac{1}{4}\int \dd^3x\int \dd x^4
\sum_{i=1}^4\sum_{j=1}^4F_{ij}^2
\right)
\cdot
\sqrt{\operatorname{Det}\left[(\ii\sl{D})^2+M^2\right]}.
