...
\begin{equation}
\xi'_a(x)=\sum_b\mathcal D_{ab}(h)\xi_b(x),
...
\end{align}
Eq.~\eqref{eq:19.6.12} gives the quantity $\gamma^{-1}\partial_\mu\gamma$ as a linear combination of $x$s and $t$s. Also, the second term on the right-hand side of Eq.~\eqref{eq:19.6.25} is a linear combination of $t$s alone. Since the $x_a$ and $t_i$ are all independent, the coefficients of each generator on both sides of Eq.~\eqref{eq:19.6.25} must all be equal:
\begin{equation}
\sum_a x_aD'_{a\mu}
