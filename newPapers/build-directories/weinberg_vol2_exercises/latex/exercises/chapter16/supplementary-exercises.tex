...
\begin{enumerate}
  ...
  \SupplementaryExercise{11}{The Exact Background-Field Effective-Action Identity}
  {Inspired by Cambridge Part III, Advanced Quantum Field Theory, 2019 Paper 304, the background-field material.}
  {cambridge-part-iii-aqft-2019-p304-q1-v2ch16-p11}
  For a bosonic theory with field \(\phi=\bar\phi+\eta\), define the
  effective action by a Legendre transform and choose the source so that
  \(\langle\eta\rangle=0\).  Derive
  \[
    e^{i\Gamma[\bar\phi]}
      =\int\mathcal D\eta\,
        \exp i\left(S[\bar\phi+\eta]
          -\int d^dx\,\eta(x)
             \frac{\delta\Gamma}{\delta\bar\phi(x)}\right).
  \]
  Expand this exact identity through one loop, show that a linear symmetry
  of the action and measure is inherited by \(\Gamma\), and explain why the
  identity is implicit rather than an ordinary integral formula.
