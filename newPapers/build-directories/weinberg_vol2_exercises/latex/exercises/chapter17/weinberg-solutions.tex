...
\begin{enumerate}
  ...
  antisymmetric Hermitian generators \(T_\alpha\), including the coupling
  in the same way as the chapter's \(t_\alpha\), and to add to the BRST
  transformations
  \[
    s\phi_i=i\omega^\alpha(T_\alpha)_i{}^j\phi_j,\qquad
    [T_\alpha,T_\beta]
      =iC_{\gamma\alpha\beta}T_\gamma .
  \]
  ...
  \[
    s'\phi_i=i\mathcal Z\omega^\alpha
       (T'_\alpha)_i{}^j\phi_j ,
  \]
  ...
  \[
    [T'_\alpha,T'_\beta]
       =iC'_{\gamma\alpha\beta}T'_\gamma .
  \]
  Hence \(T'_\alpha\) is the same representation after a change of scalar
  ...
  \begin{split}
    \mathcal L_{\phi}={}&
      ...
      -a_i\phi_i-\frac12m^2_{ij}\phi_i\phi_j\\
  ...
  \[
    (T_\alpha)_i{}^{i'}\lambda_{i'jkl}
    +(T_\alpha)_j{}^{j'}\lambda_{ij'kl}
    +(T_\alpha)_k{}^{k'}\lambda_{ijk'l}
    +(T_\alpha)_l{}^{l'}\lambda_{ijkl'}=0,
  \]
  with analogous equations for \(a,m^2,\kappa\), and \(Y\).  These are
  ...
  \(\omega_\alpha^*u_{\alpha i}\phi_i\) or
  \(\omega_\alpha^*u_{\alpha ij}\phi_i\phi_j\); after applying \(s\) these
  ...
  \(\mathcal A_{\alpha\mu}=A_{\alpha\mu}+A'_{\alpha\mu}\), where \(A\)
  ...
  \begin{aligned}
    ...
    s\omega_\alpha
      &=-\frac12C_{\alpha\beta\gamma}\omega_\beta\omega_\gamma,\\
    s\omega_\alpha^*&=-h_\alpha,\qquad &sh_\alpha&=0 .
  \end{aligned}
  \]
  Here \(\bar D_\mu\) contains only \(A\).  On a quantum matter
  fluctuation,
  \(s\psi'=it_\alpha\omega_\alpha(\psi+\psi')\), while
  ...
  \end{split}
  \]
  Eliminating \(h_\alpha\) gives
  \(-(\bar D^\mu A'_{\alpha\mu})^2/(2\xi)\), while the last line is
  ...
  \[
    \Gamma^{(1)}_{AA,\infty}
      =-\frac14L_A\int d^4x\,F_{\alpha\mu\nu}F_\alpha{}^{\mu\nu},
  \]
  with
  \[
    L_A=
  ...
  \[
    {\rm tr}_r(T_\alpha T_\beta)
       =g^2 C_s(r)\delta_{\alpha\beta}.
  \]
