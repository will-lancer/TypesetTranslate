...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \[
      \boxed{\int \dd^4x\,
        \frac{\delta\Gamma}{\delta\omega_a^*(x)}=0,}
    \]
    so every local dependence on \(\omega_a^*\) must be through a
    derivative.  Independently, the source equation for the linear
    transformation \(s\omega_a^*=-h_a\) is exact:
    \[
      \frac{\delta_L\Gamma}{\delta K_a^*(x)}
        =-h_a(x).
    \]
    ...
    except, provisionally, \(K_a^*K_b^*\): each \(K^*\) has
    dimension two and ghost number zero.  But such a term would make
    \(\delta\Gamma/\delta K_a^*\) depend on \(K^*\), contradicting the
  ...
  \begin{enumerate}
    ...
    \begin{split}
      s^2\phi
        &=\ii(s\omega^a)T_a\phi
          -\ii\omega^a T_a(s\phi)\\
        &=-\frac{\ii}{2}C_{abc}
            \omega^b\omega^c T_a\phi
          +\omega^a\omega^b T_a T_b\phi\\
        &=\frac12\omega^a\omega^b
          \left([T_a,T_b]
            -\ii C_{cab}T_c\right)\phi .
    \end{split}
    \]
    Thus \(s^2\phi=0\) for arbitrary ghosts precisely when
    \([T_a,T_b]=\ii C_{cab}T_c\).  Conversely, if
    nilpotence holds on a faithful collection of scalar fields, the
    coefficient of each independent
    \(\omega^a\omega^b\) vanishes and gives the representation
    relation.
    \item[(c)]
  ...
  \SupplementarySolution{4}{Gauge Fields with Adjoint-Scalar and Fundamental-Fermion Matter}
  \begin{enumerate}
    \item[(a)] With the convention used in the question, the finite
      transformations are
      \[
        A_\mu' =U^\dagger A_\mu U
          +\frac{\ii}{g}U^\dagger\partial_\mu U,
        \qquad
        F_{\mu\nu}'=U^\dagger F_{\mu\nu}U.
      \]
      The second formula follows either by direct substitution or from
      \([D_\mu,D_\nu]=-ig[F_{\mu\nu},\,\cdot\,]\).  Cyclicity of the trace
      then gives
      \[
        \operatorname{Tr}(F_{\mu\nu}'F'^{\mu\nu})
          ...
          =\operatorname{Tr}(F_{\mu\nu}F^{\mu\nu}).
      \]
      Before gauge fixing, the quadratic Yang--Mills kernel is
      proportional to
      \(p^2\eta_{\mu\nu}-p_\mu p_\nu\).  It annihilates \(p^\nu\), the
      linearized gauge direction, and therefore has no inverse.  A gauge
      condition removes this zero mode and makes a propagator definable.

    \item[(b)] Acting on the transformed scalar gives
      \[
      \begin{split}
        D_\mu'\phi'
          &=\partial_\mu(U^\dagger\phi U)
            -ig[A_\mu',U^\dagger\phi U]\\
          &=U^\dagger(\partial_\mu\phi-ig[A_\mu,\phi])U
           =U^\dagger(D_\mu\phi)U,
      \end{split}
      \]
      which proves covariance.  Put
      \([T_a,T_b]=if_{ab}{}^cT_c\).  Since
      \(U=\mathbf{1}+\ii\alpha+O(\alpha^2)\),
      \[
        \delta\phi=-\ii[\alpha,\phi]
          =\alpha^a\phi^b f_{ab}{}^cT_c,
        \qquad
        \boxed{\delta\phi^c=f_{ab}{}^c\alpha^a\phi^b.}
      \]

    \item[(c)]
      \begin{enumerate}
        \item[(i)] For definiteness, take a local, Lorentz-invariant
          two-derivative basis and retain polynomial interactions through
          four matter fields.  A convenient complete form, modulo
          integrations by parts, field redefinitions, and group-specific
          identities among invariant tensors, is
          \[
          \begin{split}
            \mathcal L={}&-\frac14\operatorname{Tr}F_{\mu\nu}F^{\mu\nu}
              +\frac12\operatorname{Tr}(D_\mu\phi D^\mu\phi)
              -\frac{m_\phi^2}{2}\operatorname{Tr}\phi^2
              +\bar\psi(\ii\slashed D-m_\psi)\psi\\
             &-\sum_r\frac{\lambda_r}{4!}\,
                I_r(\phi,\phi,\phi,\phi)
              -\sum_s h_s\,
                \bar\psi\,J_s(\phi,\phi)\psi
              -\sum_t G_t\,\mathcal O_{4\psi,t}.
          \end{split}                                      \tag{1}
          \]
          Here \(I_r\) ranges over independent invariant rank-four tensors
          in the adjoint representation; familiar representatives are
          \((\operatorname{Tr}\phi^2)^2\) and
          \(\operatorname{Tr}\phi^4\).  The matrices
          \(J_s(\phi,\phi)\) range over independent fundamental
          endomorphisms constructed from two adjoint scalars, for example
          \(\phi^2\) and \(\mathbf1\operatorname{Tr}\phi^2\), and
          \(\mathcal O_{4\psi,t}\) ranges over a Fierz-complete set of
          gauge-singlet four-fermion operators.  Equation~(1) makes clear
          that the scalar cubic and the Yukawa interaction
          \(\bar\psi\phi\psi\) are forbidden by
          \(\phi\mapsto-\phi\).  Odd-fermion operators are forbidden by
          \(\psi\mapsto-\psi\).  The gauge interactions
          \(A^3,A^4,A\phi^2,A^2\phi^2\), and \(A\bar\psi\psi\) are already
          fixed by the single coupling \(g\) inside \(F_{\mu\nu}\) and the
          covariant derivatives.

          The canonical dimensions are
          \[
            [A_\mu]=[\phi]=\frac{d-2}{2},\qquad
            [\psi]=\frac{d-1}{2},
          \]
          and hence
          \[
          \begin{array}{c|cccccc}
            \text{parameter}&g&m_\phi^2&m_\psi&\lambda_r&h_s&G_t\\ \hline
            \text{mass dimension}&(4-d)/2&2&1&4-d&3-d&2-d.
          \end{array}
          \]
          Positive-, zero-, and negative-dimension couplings are
          classically relevant, marginal, and irrelevant, respectively.
          Thus in four dimensions the masses are relevant, \(g\) and the
          scalar quartics are classically marginal and acquire logarithmic
          running, while the \(h_s\) and \(G_t\) interactions are
          irrelevant EFT operators.  In other dimensions the same table
          gives the leading classical term in each beta function; quantum
          anomalous dimensions and operator mixing then refine the flow.

        \item[(ii)] Use a solid line for \(\phi\), a wavy line for
          \(A_\mu\), and an arrowed line for \(\psi\).  Up to permutations
          of the four external scalars and the different invariant-tensor
          contractions, the one-loop 1PI topology list is
          \begin{center}
          \begin{tikzpicture}[scale=0.72,
            scalar/.style={thick},
            gauge/.style={decorate,decoration={snake,amplitude=1pt,segment length=4pt},thick},
            ...
            \begin{scope}[shift={(0,3.0)}]
              ...
              \draw[scalar] (l) arc (180:0:.65);
              \draw[scalar] (r) arc (0:-180:.65);
              \draw[scalar] (l)--(-1.35,.65); \draw[scalar] (l)--(-1.35,-.65);
              \draw[scalar] (r)--(1.35,.65); \draw[scalar] (r)--(1.35,-.65);
              \node at (0,-1.05) {two \(\phi^4\) vertices};
            \end{scope}
            \begin{scope}[shift={(4.1,3.0)}]
              \coordinate (l) at (-.65,0); \coordinate (r) at (.65,0);
              \draw[gauge] (l) arc (180:0:.65);
              \draw[gauge] (r) arc (0:-180:.65);
              \draw[scalar] (l)--(-1.35,.65); \draw[scalar] (l)--(-1.35,-.65);
              \draw[scalar] (r)--(1.35,.65); \draw[scalar] (r)--(1.35,-.65);
              \node at (0,-1.05) {two \(A^2\phi^2\) vertices};
            \end{scope}
            \begin{scope}[shift={(8.2,3.0)}]
              \coordinate (l) at (-.65,0); \coordinate (r) at (.65,0);
              \draw[fermion] (l) arc (180:0:.65);
              \draw[fermion] (r) arc (0:-180:.65);
              \draw[scalar] (l)--(-1.35,.65); \draw[scalar] (l)--(-1.35,-.65);
              \draw[scalar] (r)--(1.35,.65); \draw[scalar] (r)--(1.35,-.65);
              \node at (0,-1.05) {two \(\bar\psi\phi^2\psi\) vertices};
            \end{scope}
            \begin{scope}[shift={(1.3,-.1)}]
              \coordinate (a) at (0,.72); \coordinate (b) at (-.7,-.52);
              \coordinate (c) at (.7,-.52);
              \draw[gauge] (a)--(b); \draw[gauge] (a)--(c);
              \draw[scalar] (b)--(c);
              \draw[scalar] (a)--(-.55,1.35); \draw[scalar] (a)--(.55,1.35);
              \draw[scalar] (b)--(-1.35,-.85); \draw[scalar] (c)--(1.35,-.85);
              \node at (0,-1.62) {\(A^2\phi^2\) plus two \(A\phi^2\)};
            \end{scope}
            \begin{scope}[shift={(5.2,-.1)}]
              \coordinate (a) at (0,.72); \coordinate (b) at (-.7,-.52);
              \coordinate (c) at (.7,-.52);
              \draw[scalar] (a)--(b); \draw[scalar] (a)--(c);
              \draw[gauge] (b)--(c);
              \draw[scalar] (a)--(-.55,1.35); \draw[scalar] (a)--(.55,1.35);
              \draw[scalar] (b)--(-1.35,-.85); \draw[scalar] (c)--(1.35,-.85);
              \node at (0,-1.62) {\(\phi^4\) plus two \(A\phi^2\)};
            \end{scope}
            \begin{scope}[shift={(8.8,-.1)}]
              \coordinate (a) at (-.65,.65); \coordinate (b) at (.65,.65);
              \coordinate (c) at (.65,-.65); \coordinate (d) at (-.65,-.65);
              \draw[gauge] (a)--(b); \draw[scalar] (b)--(c);
              \draw[gauge] (c)--(d); \draw[scalar] (d)--(a);
              \draw[scalar] (a)--(-1.3,1.15); \draw[scalar] (b)--(1.3,1.15);
              \draw[scalar] (c)--(1.3,-1.15); \draw[scalar] (d)--(-1.3,-1.15);
              \node at (0,-1.62) {four \(A\phi^2\) vertices};
            \end{scope}
          \end{tikzpicture}
          \end{center}
          The first topology has the usual \(s,t,u\) channel assignments.
          The gauge bubble, the two mixed triangles, and the alternating
          gauge--scalar box also come with all inequivalent external-leg
          permutations.  The fermion bubble is present only when the
          allowed higher-dimensional \(h_s\) operators are retained; it is
          absent in the power-counting-renormalizable four-dimensional
          truncation.  External-leg self-energy insertions are one-particle
          reducible and therefore do not belong to the requested 1PI
          vertex.
      \end{enumerate}
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    Write
    \[
      V(\phi)=\ell_i\phi_i+\frac12m^2_{ij}\phi_i\phi_j
        +\frac1{3!}\kappa_{ijk}\phi_i\phi_j\phi_k
        +\frac1{4!}\lambda_{ijkl}\phi_i\phi_j\phi_k\phi_l .
    \]
    With \(\delta\phi_i=\ii\epsilon^a
    (T_a)_i{}^j\phi_j\), invariance gives
    \[
    \begin{aligned}
      &\ell_i(T_a)_i{}^j=0,\\
      &(T_a)_i{}^{i'}m^2_{i'j}
        +(T_a)_j{}^{j'}m^2_{ij'}=0,\\
      &(T_a)_i{}^{i'}\kappa_{i'jk}
        +(T_a)_j{}^{j'}\kappa_{ij'k}
        +(T_a)_k{}^{k'}\kappa_{ijk'}=0,
    \end{aligned}
    \]
    ...
    The variations are
    \[
      \delta\psi_R=\ii\epsilon^a t_a^R\psi_R,\quad
      \delta\bar\psi_L=-\ii\epsilon^a\bar\psi_Lt_a^L,\quad
      \delta\phi_i=\ii\epsilon^a(T_a)_i{}^j\phi_j .
    \]
    The coefficient of each \(\epsilon^a\phi_j\) vanishes when
    \[
      \boxed{
      t_a^LY^j-Y^jt_a^R
        -(T_a)_i{}^jY^i=0.}
  ...
  \begin{enumerate}
    ...
    \(a\leftrightarrow c\), whereas the structure constants are
    antisymmetric.
    \item[(c)]
    ...
    \[
      \delta f_a
        =\bar D^\mu\!\left[
          \bar D_\mu\epsilon_a
          -C_{abc}\epsilon_b A'_{c\mu}\right].
    \]
    Replacing \(\epsilon\) by \(\omega\) gives
    \[
      \mathcal L_{\rm gh}
        =\omega_a^*\bar D^\mu\!\left[
          \bar D_\mu\omega_a
          -C_{abc}\omega_b A'_{c\mu}\right],
    \]
    or, after a background-covariant integration by parts,
    \[
      \mathcal L_{\rm gh}
        =-(\bar D^\mu\omega_a^*)
         \left[\bar D_\mu\omega_a
         -C_{abc}\omega_b A'_{c\mu}\right].
    \]
    ...
    \(h_a=-\bar D^\mu A'_{a\mu}/\xi\) produces
    \(-f_a f_a/(2\xi)\), plus the ghost term found above.
    ...
    \item[(e)]
    For an infinitesimal parameter \(\epsilon_a(x)\), invariance of the
    action and measure gives
    \[
      \boxed{\bar D_\mu^{ab}[A]\,
         \frac{\delta\Gamma[A]}{\delta A_\mu^b}=0.}
    \]
    ...
    \[
      \int \dd^nx\,e^{-\frac12x^{\mathsf T}Mx}
        \propto(\det M)^{-1/2},\qquad
      \int \dd^n\bar\eta\,\dd^n\eta\,e^{-\bar\eta M\eta}
        =\det M .
    \]
  ...
  \begin{enumerate}
    ...
    Write
    \[
      A'_\mu=UA_\mu U^\dagger+\frac{\ii}{g}U\partial_\mu U^\dagger,
    ...
    \item[(c)]
    \begin{enumerate}
      \item[(i)] Put \(U=\mathbf{1}+ig\vartheta\omega\), where \(\vartheta\) is a
      ...
      \[
        \frac{\dd}{\dd\lambda}\langle{\cal O}\rangle_\lambda\Big|_{0}
        =\ii\bigl(\langle{\cal O}\,s\delta\Psi\rangle
  ...
  \begin{enumerate}
    ...
    \[
      \boxed{F'_{\mu\nu}=UF_{\mu\nu}U^\dagger.}
    \]
    For \(U=\mathbf{1}+\ii\alpha^cT_c+O(\alpha^2)\),
    \[
      \delta F_{\mu\nu}
       =\ii[\alpha^cT_c,F_{\mu\nu}^bT_b]
       =f_{bc}{}^aF_{\mu\nu}^b\alpha^cT_a,
    ...
    Infinitesimally,
    \[
      \delta\Psi=\ii[\alpha^aT_a,\Psi^bT_b]
  ...
  \begin{split}
    ...
    {\cal N}_{\mu\nu\rho}={}&
      ...
      &{}\times\eta^{\alpha\alpha'}\eta^{\beta\beta'}
       \eta^{\gamma\gamma'}.
  \end{split}
  \]
  ...
  integrals.  Abbreviating the regulated measure as
  \(\int_L\equiv\mu^{2\epsilon}\int \dd^dL/(2\pi)^d\),
  \[
  \begin{aligned}
    \int_L\frac1{(L^2+\Delta-\ii0)^2}
       &=\frac{\ii}{16\pi^2\epsilon}+O(1),\\
    \int_L\frac{L_\alpha L_\beta}{(L^2+\Delta-\ii0)^3}
       &=\frac{\ii\eta_{\alpha\beta}}{64\pi^2\epsilon}+O(1).
  \end{aligned}
  \]
  ...
  \begin{enumerate}
    ...
    by differentiating \(e^{\ii I}\), both sides acquire the same overall
    factor of \(i\).
    \item[(c)]
