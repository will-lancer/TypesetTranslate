...
\begin{enumerate}
  ...
  \[
    ...
    \qquad
    f_a=\ii\phi_n(t_a)_{nm}v_m.
  \]
  Under an infinitesimal gauge transformation,
  \(\delta\phi_n=\ii\epsilon_b(t_b)_{n\ell}\phi_\ell\), so
  \[
    \frac{\delta f_a(x)}{\delta\epsilon_b(y)}
      =-(t_a v)_n(t_b\phi(x))_n\,
        \delta^4(x-y).
  \]
  ...
  \(Y=0\).  Choose
  \(\langle\boldsymbol{\phi}\rangle=(0,0,v_T)\).  Its covariant derivative gives
  \[
    \frac12(D_\mu\boldsymbol{\phi})^2
  ...
  \[
    ...
    \Phi=\oint\mathbf A\cdot \dd\mathbf x
