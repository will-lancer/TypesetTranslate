...
\begin{enumerate}
  ...
  \WeinbergSolution{5}
  Write
  \(A_\mu=t_a A_{a\mu}\).  Successive transport along the sides of
  an infinitesimal parallelogram with side vectors \(u^\mu\) and \(v^\mu\)
  ...
  \[
    \Sigma^{\mu\nu}
      =\frac12\oint_\gamma(x^\mu\,\dd x^\nu-x^\nu\,\dd x^\mu)
      =\oint_\gamma x^\mu\,\dd x^\nu .
  \]
  Consequently
  \[
    \delta_\gamma\psi
  ...
  \[
    (\partial_\mu-\ii A_\mu)U(x,x_0)=0.
  \]
  ...
  The gauge variation of \(f_a=n^\mu A_{a\mu}\) is
  \[
    \delta f_a=n^\mu(D_\mu\lambda)_a,
  \]
  and hence
  \[
    \mathcal L_{\rm GH}
      =\omega_a^*n^\mu(D_\mu\omega)_a
      =\omega_a^*n^\mu\partial_\mu\omega_a
       +C_{acb}
        \omega_a^*n^\mu A_{c\mu}\omega_b .
  \]
  The free ghost propagator is the inverse of \(n\cdot\partial\):
  \[
    \Delta_{ab}(p)
      =\frac{\delta_{ab}}{\ii\,n\cdot p}
  \]
  with a compatible prescription.  With all momenta incoming, the
  ghost--antighost--gauge vertex is proportional to
  \(C_{acb}n^\mu\) (and acquires the conventional factor \(i\)
  when the usual \(\ii\mathcal L_{\rm int}\) Feynman rule is used).  In the
  ...
  \[
    \begin{split}
    \psi={}&
      z_{ab}\omega_a^*\partial_\mu A_b^\mu
      +\frac12\xi_{ab}\omega_a^*h_b
      +u_{abc}
       \omega_a^*A_b^\mu A_{c\mu}\\
      &+\frac14 v_{abc}
  ...
  \(u_{abc}=u_{acb}\), and
  \(v_{abc}=-v_{bac}\).  Thus the answer is
  \[
    \boxed{\mathcal L=\mathcal L_0+s\psi}
  \]
  up to total derivatives and field-independent constants.  The usual
  linear covariant gauge is obtained with \(z_{ab}\propto
  \delta_{ab}\), \(\xi_{ab}\propto\delta_{ab}\),
  and \(u=v=0\).  The last two terms display interactions allowed by the
  ...
  \[
    (O,S)=\ii\Delta O,
    \qquad\text{or}\qquad
    \sigma O\equiv(O,S)-\ii\Delta O=0,
  \]
  as also stated in the footnote following
  Eq.~\eqref{eq:15.9.36}.  The printed right-hand side \(\ii\Delta S\) does
  ...
  \[
    \delta_\Psi\langle O\rangle
      =\ii\bigl\langle \sigma(O\,\delta\Psi)\bigr\rangle
       -\ii\langle O\rangle\bigl\langle\sigma(\delta\Psi)\bigr\rangle .
  \]
  BV integration by parts makes the expectation value of every
  \(\sigma\)-exact insertion vanish when
  \[
    \frac12(S,S)-\ii\Delta S=0.
  \]
