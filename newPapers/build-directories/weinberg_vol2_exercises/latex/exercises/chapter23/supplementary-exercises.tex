...
\begin{enumerate}
  ...
  {Inspired by John McGreevy, Physics 215C, Assignment 2, the anomaly and domain-wall material (2025).}
  {mcgreevy-215c-s25-a2-p4-v2ch23-p13}
  A massive Dirac fermion in \(1+1\) dimensions has a slowly varying
  complex mass
  \[
    \mathcal L_m=-m\overline\psi e^{i\gamma^5\theta(x)}\psi
  \]
  and vector charge \(q\).  Remove the phase by a local chiral rotation and
  use the anomaly to derive the leading induced current
  \(j^\mu_{\rm ind}\).  For a static wall across which
  \(\Delta\theta=\pi\), calculate the localized charge.  Explain why the
  result is defined modulo an integer charge and what changes when
  \(\Delta\theta=2\pi\).

  \SupplementaryExercise{14}{The Polyacetylene Domain-Wall Zero Mode}
  {Inspired by John McGreevy, Physics 215C, Assignment 3, the polyacetylene and domain-wall material (2025).}
  {mcgreevy-215c-s25-a3-p1-v2ch23-p14}
  Consider the one-dimensional Dirac Hamiltonian
  \[
    H=-i\sigma_2\partial_x+\sigma_1m(x),
    \qquad m(-\infty)=-m_0,\quad m(+\infty)=m_0>0.
  \]
  Solve \(H\psi_0=0\), determine which constant spinor gives a normalizable
  state, and show that the mode is pinned to zero energy by the
  anticommuting symmetry \(\{H,\sigma_3\}=0\).  Derive the two possible
  charges of the wall in a spinless theory.  Then state how spin degeneracy
  produces the neutral spin-\(\tfrac12\) and charged spin-zero solitons of
  the continuum polyacetylene model.
