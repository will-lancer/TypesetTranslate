...
\begin{enumerate}
  ...
  The rotation \(\psi\mapsto e^{-i\gamma^5\theta/2}\psi\) makes the mass
  real.  Its anomalous Jacobian adds, to leading order in derivatives,
  \[
    I_{\rm top}=\frac{q}{2\pi}\int d^2x\,
      \theta\,\epsilon^{\mu\nu}\partial_\mu A_\nu .
  \]
  Varying with respect to the background field gives the Goldstone--Wilczek
  current
  \[
    \boxed{j^\mu_{\rm ind}
      =\frac{q}{2\pi}\epsilon^{\mu\nu}\partial_\nu\theta}
  \]
  up to the simultaneous convention choice for \(\epsilon^{01}\) and the
  sign of the gauge coupling.  For a static wall,
  \[
    Q=\int dx\,j^0
      =\frac{q}{2\pi}\bigl[\theta(+\infty)-\theta(-\infty)\bigr].
  \]
  Thus \(\Delta\theta=\pi\) binds \(Q=q/2\).

  A localized fermion level may be filled or emptied, shifting the wall
  charge by an integer multiple of \(q\); the fractional part is the
  invariant response.  For \(\Delta\theta=2\pi\), the induced value is
  \(q\), which is integral and can be removed by changing the occupation of
  one localized state.  The adiabatic cycle still represents quantized
  charge pumping even though a single wall has no fractional residue.

  \SupplementarySolution{14}{The Polyacetylene Domain-Wall Zero Mode}
  Multiplying the zero-mode equation by \(i\sigma_2\) gives
  \[
    \partial_x\psi_0=-m(x)\sigma_3\psi_0.
  \]
  If \(\sigma_3\chi_s=s\chi_s\), then
  \[
    \psi_s(x)=\mathcal N
      \exp\left(-s\int_0^x m(y)\,dy\right)\chi_s .
  \]
  Because the integral tends to \(+\infty\) at both spatial ends for the
  stated kink, only \(s=+1\) is normalizable.  Hence exactly one localized
  zero mode occurs.  Since \(\sigma_3H\sigma_3=-H\), every nonzero state has
  a partner of opposite energy; an unpaired state must remain at zero
  unless the spectral symmetry or the bulk gap is destroyed.

  Relative to the charge-conjugation-symmetric vacuum, the empty and filled
  zero mode carry
  \[
    \boxed{Q=-\frac q2,\qquad Q=+\frac q2}.
  \]
  With two spin species there are two zero modes.  Filling one gives total
  charge zero and spin \(1/2\); filling neither or both gives charge
  \(-q\) or \(+q\) and spin zero.  These are the neutral spinful and charged
  spinless solitons characteristic of the continuum polyacetylene
  description.
