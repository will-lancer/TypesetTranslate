...
\begin{enumerate}
  ...
  Put
  \[
    T=\frac12\int_{-\infty}^{\infty}(\phi')^2dx,
    \qquad
  ...
  \[
    0=\left.\frac{dE[\phi_\lambda]}{d\lambda}\right|_{\lambda=1}
      =T-V.
  \]
  Therefore
  \[
  ...
  The static field equation is \(\phi''=dU/d\phi\).  Integrating the
  ...
  \[
    E=\frac12\int dx\,[\phi'-(1-\phi^2)]^2
      +\int_{-1}^{1}(1-\phi^2)\,d\phi.
  \]
  Hence
  \[
    \phi'=1-\phi^2,\qquad
  ...
  \begin{split}
    L_0(\lambda)
      &=D_1+iD_2-\lambda(D_3-iD_\tau),\\
    L_1(\lambda)
      &=D_3+iD_\tau+\lambda(D_1-iD_2),
  \end{split}
  \qquad
  ...
  \begin{enumerate}
    ...
    Consequently
    \[
      A=J^{-1}\partial_{\bar w}J\,d\bar w
    ...
    \(J=e^{i\chi}\) with \(\chi\) real.  Commutators vanish and Yang's
    equation becomes
    \[
      i\bigl(\partial_w\partial_{\bar w}\chi
    ...
    Lie-algebra-valued logarithm \(\log J=i\chi\).
  \end{enumerate}
  ...
  \begin{split}
    \frac{dE}{dt}
      &=\int dx\,[\phi_t\phi_{tt}+\phi_x\phi_{xt}
                    +U'(\phi)\phi_t]\\
      &=\int dx\,\phi_t[\phi_{tt}-\phi_{xx}+U'(\phi)]
  ...
  \begin{enumerate}
    ...
    \[
      x-A=\int^\phi\frac{d\varphi}{\varphi^2(1-\varphi^2)}.
    \]
    Since
    \[
      \frac{d}{d\phi}\left[
  ...
  \[
    \langle i\phi,\dot\phi\rangle
      =\frac{i}{2}(\phi\dot{\bar\phi}-\bar\phi\dot\phi).
  \]
  ...
  \begin{enumerate}
    ...
    \]
    Therefore \(\phi=e^{-iEt}f_E\) solves (1) when \(\epsilon=0\).

    \item[(ii)]
    Put \(y=x-X(t)\) and
    \(\phi=f_E(y)e^{i\theta(t)+iu(t)y}\).  Direct differentiation gives
    \[
    \begin{split}
      i\phi_t={}&e^{i\theta+iuy}
       \left[-i\dot X f_E'
    ...
    \[
      \mathcal N(E)=\int_{-\infty}^{\infty}f_E^2dx=2k.
    \]
    The other useful integrals are
    \[
      \int(f_E')^2dx=\frac23k^3,
      \qquad
      \int f_E^4dx=\frac43k^3.
    \]
    ...
    \(\dot u\int y f_E^2dy\) make no contribution to the symplectic
    integral.  Define
    \[
    \begin{split}
  ...
  \[
    \mathcal I=\frac1{4\pi}\int_{S^2}
      \left(\frac{1+|z|^2}{1+|R|^2}
        \left|\frac{dR}{dz}\right|\right)^4d\Omega.
  \]
  ...
  \(\omega=e^{2\pi i/5}\).  Since \(\omega^5=1\),
  \[
  ...
  transformations generate \(D_{5d}\).  They are, in fact, contained in
  ...
  \[
    \boxed{z=0,\qquad
      z=\varphi_g^{-1}e^{2\pi ik/5},\qquad
      z=\varphi_g e^{(2k+1)\pi i/5},\qquad k=0,\ldots,4.}
  \]
