...
Spherical symmetry requires the invariant interval \(ds^2\) to depend only on
the rotational invariants
\[
  t,\quad dt,\quad r,\quad
  \mathbf{x}\cdot d\mathbf{x}=r\,dr,\quad
  d\mathbf{x}^2=dr^2+r^2(d\theta^2+\sin^2\theta\,d\varphi^2),
\]
so, in the binding signature, it can be written
\[
  \begin{aligned}
  ds^2={}&
  -C(r,t)\,dt^2+D(r,t)\,dr^2+2E(r,t)\,dr\,dt\\
  &+F(r,t)r^2(d\theta^2+\sin^2\theta\,d\varphi^2).
  \end{aligned}
\]
...
\(\eta(r,t_0+dt)\) for all \(r\).) The interval is then
\[
  ds^2
  =
  -\eta^{-2}C^{-1}\,dt'^2
