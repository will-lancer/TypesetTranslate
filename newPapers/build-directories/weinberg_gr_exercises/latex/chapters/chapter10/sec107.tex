...
\begin{equation}
  t_{0i}(\widehat{\mathbf{x}})
  =
  -\widehat x^{\,j}t_{ji},
  \qquad
  t_{00}(\widehat{\mathbf{x}})
  =
  \widehat x^{\,i}\widehat x^{\,j}t_{ij},
...
\(\Phi(\omega)\,d\omega\) between frequencies \(\omega\) and
\(\omega+d\omega\). If exposed to such radiation, the power absorbed by a
...
\input{figures/chapter10/fig10-01.tex}
