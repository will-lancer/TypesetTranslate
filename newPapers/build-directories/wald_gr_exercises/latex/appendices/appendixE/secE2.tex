...
element \({}^{(3)}\epsilon_{abc}=e_{\dd abc}n^d\), where \(n^d\) is the unit normal to
...
element \(\dd t\wedge \dd x^1\wedge \dd x^2\wedge \dd x^3\).] On each \(\Sigma_t\), we define
\({}^{(3)}e_{abc}=e_{\dd abc}t^d\). Unless otherwise stated, all integrals over \(M\)
...
\begin{equation}
  J=\int_{t_1}^{t_2}H\,\dd t
    =\int_{t_1}^{t_2}\dd t\int_{\Sigma_t}\mathcal H
    =-S+\int_{t_1}^{t_2}\dd t\int_{\Sigma_t}\pi\dot q .
...
\begin{align}
  \frac{\dd J}{\dd\lambda}
    &=\int_{t_1}^{t_2}\dd t\int_{\Sigma_t}
      \left[\frac{\delta H}{\delta q}\delta q
        +\frac{\delta H}{\delta\pi}\delta\pi\right]\notag\\
    &=\int_{t_1}^{t_2}\dd t\int_{\Sigma_t}
      [\pi\delta\dot q+\dot q\delta\pi]-\frac{\dd S}{\dd\lambda}\notag\\
    &=\int_{t_1}^{t_2}\dd t\int_{\Sigma_t}
      [-\dot\pi\delta q+\dot q\delta\pi]-\frac{\dd S}{\dd\lambda},
...
\({}^{(3)}e_{abc}=e_{\dd abc}t^d\) on \(\Sigma_t\). We note in analogy with the
