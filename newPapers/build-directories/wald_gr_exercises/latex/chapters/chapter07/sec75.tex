...
\begin{equation}
  \left.
    \frac{\dd}{\dd\lambda}\bigl[\mathcal E(g(\lambda))\bigr]
  \right|_{\lambda=0}=0 .
  \tag{7.5.3}\label{eq:7.5.3}
\end{equation}
Equation~\eqref{eq:7.5.3} is a linear equation for
\begin{equation}
  \gamma=\left.\frac{\dd g}{\dd\lambda}\right|_{\lambda=0} ;
...
where \(\gamma=(\dd g/\dd\lambda)|_{\lambda=0}\). However, the existence of a
...
\(\dot R_{ac}(\lambda)=(\dd R_{ac}/\dd\lambda)|_{\lambda=0}\), with respect to
...
\begin{equation}
  \dot C^c{}_{ab}
    =\left.\frac{\dd C^c{}_{ab}}{\dd\lambda}\right|_{\lambda=0} .
...
\begin{equation}
  h_{ab}
    =\left.\frac{\dd g_{ab}}{\dd\lambda}\right|_{\lambda=0}
...
\(R^b{}_{ca}{}^dh_{\dd b}\) is symmetric in \(c\) and \(a\) and
