...
\(\int T_{\mu\nu}\,\dd^3x=0\), where the integral is taken over a \(t=\text{constant}\)
...
\(M=\int T_{00}\,\dd^3x\), i.e., we obtain the usual formula of special relativity.
...
\[
  \int_\Sigma(\rho+3p)e^\phi\,\dd V
  =\int_\Sigma\rho\left(1-\frac{2m(r)}{r}\right)^{1/2}\,\dd V,
\]
where the integral is taken over a hypersurface \(\Sigma\) orthogonal to the static
Killing field \(\xi^a\), \(\dd V\) denotes the natural volume element on \(\Sigma\), and
...
energy, and \(K\equiv\tfrac32\int_\Sigma p\,\dd V\), so that in the case of a monatomic
