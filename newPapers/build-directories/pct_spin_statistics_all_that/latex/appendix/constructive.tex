...
interested reader has available the systematic review of Streater [1] as well
as the treatise of Bogolubov, Logunov, and Todorov [2], both of which have
...
particle dynamics.'' It was known in 1964 that the axioms given in Chapter 3
...
It was preceded by the pioneering studies of Friedrichs [3] and Segal [4] on
...
book on perturbation theory [5] was an inspiration for the work done on models
in the late 1960s. (The possibilities and some of the difficulties of such a
program were reviewed in [6] at the outset of constructive quantum field
theory. The survey [7] discusses the general significance of axiomatic and
...
respectively [8, 9], and $\bar\psi=\psi^\dagger\beta$. A cutoff version
...
Hamiltonian [10]. The self-adjointness $H_{V,\kappa}$ guarantees that
...
occurs, one can define more than one quantity (A.1).
...
$\varphi_\kappa$ than the free Hamiltonian, were studied in [11]. Here the
...
\item[(b)] Not all quantities associated with the theory may be expected to
  ...
  vacuum expectation values (A.1) and (A.2) are reasonable candidates for
...
$\exp(\ii tH_\infty)$ [13]. (This is more than one expects for general models.)
Furthermore, the vacuum expectation values (A.1) converge [14].
...
This problem was solved in a paper [15] which contained a number of
...
by a process referred to as \emph{wave function renormalization} [20]. (For a
discussion of a class of models which puts wave function renormalization in a
more general context, see [21].) The proof that the renormalized
...
The work quoted in [15--22] shows that a renormalized Hamiltonian in a box but
...
of the region where $g=1$ (see Figure~A.1). That means that
...
\item[(e)] To get the right representation one must construct a vacuum
  ...
  of Section 3.4; see [67]. The idea is to use the local Hamiltonians to
...
ideas is reviewed in [25] and [26]. It is sometimes referred to as the
...
is any polynomial bounded below [18, 27].
...
to Schwinger and Nakano [28, 29]. It was extensively explored by Symanzik
[30--32].
...
(For an introduction to the required notions of probability theory see [33].)
...
space, satisfying assumptions 0, I, II, and III of Chapter 3 (pp.~96--102).

% PCT-SOURCE: pdf=198 print=186 kind=prose id=appendix-reconstruction-theorem
Such a reconstruction theorem was provided in [34], where it was shown that
...
people to think Euclidean. In [35] it was shown that
...
expressed in appropriate Euclidean language. The second paper was [36], which
...
independently of the lattice spacing [37, 38].
...
mechanics of ferromagnets is that of [39] in which it is shown that for an
...
In [40, 41] it is shown that there is a unique vacuum for $\nu\neq0$.
...
[42]. Combined with the preceding result, this suggests that the phase diagram
...
to the Main Problem. The books [43] and [44] give a more complete survey, with
...
of assumptions 0, I, II, III of Chapter 3, i.e., the proof that the
...
established in [45] for any $\mathcal{P}$ bounded below and all sufficiently
small $\lambda$. For $\mathcal{P}$ even plus a linear term but for all
$\lambda$, it was established in [37, 38, 44]. An
...
generating functional for the Schwinger functions, was given in [46, 47]. This
...
$\lambda$ [45]. It was shown that the mass spectrum of the theory has the form
...
Haag--Ruelle theory of scattering [48, 49]. The Haag--Ruelle theory constructs
...
It was shown in [50] that when smeared with appropriate test functions, the
...
summable to the exact solution [51]. An additional argument is necessary for
the real time Green functions and the $S$-matrix elements themselves. This is
given in [52--54]. These results completely answer the first two questions:
...
result: $H(g)$ for $Y_2$ is self-adjoint [55]. The further development of the
$Y_2$ model theory followed the Hamiltonian strategy and culminated in the
construction of a theory (or theories) free of cutoffs [25, 26]. It was then
natural to ask how the $Y_2$ theory could be developed using the Euclidean
strategy. A crucial first step was taken in [57], where it was shown that
...
precisely, it was shown in [57] that the integrands of the two indicated
...
been verified for sufficiently weak coupling [58, 59]. For further results and
references see [60].
