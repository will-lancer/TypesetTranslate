...
from the nuclear theorem (Theorem \ref{thm:ch2-nuclear}) that this functional can be uniquely
extended to be a tempered distribution of the $n$ four-vectors
$x_1,\ldots,x_n$. It is this distribution that \eqref{eq:pct-3-19} symbolizes, and which,
...
denote by $\Wightman$:
\begin{equation}
 \Wightman(x_1,x_2,\ldots,x_n)
 =\bra{\Omega}\varphi_1(x_1)\varphi_2(x_2)\cdots
 \varphi_n(x_n)\ket{\Omega}.
 \tag{3-21}\label{eq:pct-3-21}
\end{equation}

With the same choice of labels we shall denote by $\Wightman_\pi$ the
“permuted” vacuum expectation value
\begin{equation}
 \Wightman_\pi(x_1,x_2,\ldots,x_n)
...
components $\varphi_j$ are different there are $n!$ different $\Wightman_\pi$
for each choice of components, and fewer if some coincide.

The argument used above to give a meaning to \eqref{eq:pct-3-19} as a distribution can
...
in \eqref{eq:pct-3-23}. To prove the convergence of \eqref{eq:pct-3-24}, consider
\begin{equation*}
 \begin{aligned}
  \left\|\ket{\Psi_m}-\ket{\Psi_n}\right\|^2
  &=\left\|
    \sum_{j=m}^{n}\varphi_1(f_1^j)\cdots\varphi_k(f_k^j)\ket{\Omega}
    \right\|^2\\
  &=\int \Wightman(y_k,\ldots,y_1,x_1,\ldots,x_k)\,
   \overline{(f_n-f_m)(y_1,\ldots,y_k)}\\[-2pt]
  &\phantom{={}}\times(f_n-f_m)(x_1,\ldots,x_k)\\[-2pt]
  &\phantom{={}}\times\dd^4x_1\cdots\dd^4x_k\,
    \dd^4y_1\cdots\dd^4y_k .
 \end{aligned}
\end{equation*}
where $\Wightman$ corresponds to the fields
$\varphi_k^\dagger\cdots\varphi_1^\dagger\varphi_1\cdots\varphi_k$ in that order.
Now the product of $\overline{f_n-f_m}$ and $f_n-f_m$ tends to zero in
$\Schwartz$, and since $\Wightman$ is a tempered distribution,
$\left\|\ket{\Psi_m}-\ket{\Psi_n}\right\|\to0$. Since $\mathcal{H}$ is a
complete space,\footnote{See Section~\ref{sec:2-6}.} $\{\ket{\Psi_m}\}$ has a limit
state $\ket{\Psi}$.

The set of all vectors symbolized by \eqref{eq:pct-3-23} will be denoted $D_1$. Clearly,
...
\begin{equation*}
 ...
 &\phantom{={}}\times\varphi_1(x_1)\cdots\varphi_k(x_k)\ket{\Omega}.
 \end{aligned}
\end{equation*}
...
\begin{theorem}\label{thm:ch3-1-vacuum-transformation-laws}
...
\begin{equation}
 ...
 \varphi_{\alpha'}\bigl(\Lambda(A)x+a\bigr).
...
\begin{align}
 ...
 \notag\\
 &\qquad=\bra{\Omega}\varphi_\alpha\bigl(\Lambda(A)x_1+a\bigr)
 \psi_\beta\bigl(\Lambda(A)x_2+a\bigr)\cdots
 \chi_\gamma\bigl(\Lambda(A)x_n+a\bigr)\ket{\Omega}.
...
distributions, by the nuclear theorem. Using \eqref{eq:pct-3-25} with $A$ replaced by
$A^{-1}$, and the property $U(a,A)\ket{\Omega}=\ket{\Omega}$ [Eq.~\eqref{eq:ch3-vacuum-invariant}],
\eqref{eq:pct-3-26} follows immediately.
\end{proof}

\begin{theorem}\label{thm:ch3-2-translation-invariance}
Suppose $\varphi_1,\varphi_2,\ldots,\varphi_n$ are any components of any
fields, and $\Wightman$ is defined as in \eqref{eq:pct-3-21}. Then
...
which satisfy
\begin{equation}
 \Wightman(x_1,\ldots,x_n)=W(\xi_1,\xi_2,\ldots,\xi_{n-1}).
 \tag{3-28}\label{eq:pct-3-28}
\end{equation}

The Fourier transforms of $\Wightman$ and $W$ are tempered distributions
defined by
\begin{equation}
 \widetilde{\Wightman}(p_1,p_2,\ldots,p_n)
 =\int \exp\!\left(-\ii\sum_{j=1}^{n}p_j\!\cdot x_j\right)
 \Wightman(x_1,\ldots,x_n)\,\dd^4x_1\cdots\dd^4x_n
...
and are related by
\begin{equation}
 \widetilde{\Wightman}(p_1,\ldots,p_n)
...
\begin{equation}
 \begin{aligned}
 \Wightman(x_1,x_2,\ldots,x_{j+1},x_j,\ldots,x_n)
 &=(-1)^m\Wightman(x_1,x_2,\ldots,x_j,x_{j+1},\ldots,x_n),\\
 &\qquad j=1,\ldots,n-1
 \end{aligned}
...
\begin{proof}
The argument given in Section \ref{sec:2-1} shows that translation invariance implies
that $\Wightman$ depends only on the differences
$\xi_1,\ldots,\xi_{n-1}$, or more precisely, that there exists a tempered
distribution $W$ satisfying \eqref{eq:pct-3-28}. Clearly
\begin{align*}
...
As explained in Section \ref{sec:2-6}, for any two states $\ket{\Phi}$, $\ket{\Psi}$,
we have
\begin{equation*}
...
\begin{theorem}\label{thm:ch3-3-positive-definiteness}
...
\begin{equation}
 ...
 \Wightman_{jk}(x_j,x_{j-1},\ldots,x_1,y_1,\ldots,y_k)
 f_k(y_1,\ldots,y_k)\\
...
In \eqref{eq:pct-3-35} $\Wightman_{jk}$ represents
\begin{equation*}
...
$f_0,f_1,\ldots$ we have the equality sign in \eqref{eq:pct-3-35}, then\footnote{It is
shown on p.~121 that this follows from \eqref{eq:pct-3-35} and the other axioms.} it must
...
\begin{proof}
The inequalities \eqref{eq:pct-3-35} express the fact that the norm of the state
\begin{align*}
...
test function $g$. Hence the expression \eqref{eq:pct-3-35} must give 0 also for the
sequence \eqref{eq:pct-3-36}.
\end{proof}
...
Section \ref{sec:ch3-reconstruction}.

\begin{theorem}\label{thm:ch3-4-cluster-decomposition}
\textbf{(\emph{Cluster Decomposition Property}).} If $a$ is a space-like
vector, then
\begin{equation}
 \begin{aligned}
  &\Wightman(x_1,\ldots,x_j,x_{j+1}+\lambda a,x_{j+2}+\lambda a,\ldots,\\[-2pt]
  &\qquad x_n+\lambda a)\\[-2pt]
  &\longrightarrow
  \Wightman(x_1,\ldots,x_j)\Wightman(x_{j+1},\ldots,x_n)
 \end{aligned}
...
but \eqref{eq:pct-3-37} still holds; the proof is more technical in this case. Equation
\eqref{eq:pct-3-37} can also be proved without assuming axiom III. See
\cite{pct-ch3-5,pct-ch3-7,pct-ch3-9,pct-ch3-10,pct-ch3-11} for further
information.
\end{pctremark}

\begin{proof}
The proof is based on a very simple argument due to D.~Ruelle. It exploits
the fact that for large $\lambda$ the operators occurring in \eqref{eq:pct-3-37} can be
...
distributions
\begin{align*}
 F_1={}&\Wightman(x_1,\ldots,x_j,x_{j+1}+\lambda a,\ldots,x_n+\lambda a)\\[-2pt]
 &-\Wightman(x_1,\ldots,x_j)\Wightman(x_{j+1},\ldots,x_n),\\
 F_2={}&\Wightman(x_{j+1}+\lambda a,\ldots,x_n+\lambda a,x_1,\ldots,x_j)\\[-2pt]
 &-\Wightman(x_1,\ldots,x_j)\Wightman(x_{j+1},\ldots,x_n)
\end{align*}
...
$\Wightman(x_1,\ldots,x_j)\allowbreak
\Wightman(x_{j+1},\ldots,x_n)$ cancels the contribution from this state at
...
$\lambda$, by locality. From Section \ref{sec:2-1}, $F_1\pm F_2$ is a finite sum of
...
so the required result \eqref{eq:pct-3-37} follows.
\end{proof}
...
If the fields have doubled components so that \eqref{eq:ch1-general-spinor-state-rules} makes sense, we can
...
fields entering in the state \eqref{eq:pct-3-23}, and requiring that all scalar products be
left invariant. This leads to further properties of the $\Wightman$'s. For
...
\end{equation}
holds for all products in all orders. Equation \eqref{eq:pct-3-38} is a further
restriction on the $\Wightman$'s, and should not be confused with the rule
for taking the hermitian conjugate, \eqref{eq:pct-3-33}. If $\Theta$ is a symmetry of the
...
\end{equation}
from \eqref{eq:ch1-general-spinor-pct-state}, since $\Theta$ is anti-unitary, where $F$ is the total number
of half-odd integer spin fields and $J$ is the total number of undotted
indices in the collections $(\alpha),\ldots,(\mu)$.

In the same way, equations similar to \eqref{eq:pct-3-38} and \eqref{eq:pct-3-39} can be found if
...
\begin{theorem}\label{thm:ch3-5-holomorphic-boundary-values}
The $\Wightman$ and $W$ are boundary values of holomorphic functions in the
following sense. There exist holomorphic functions $\Wightman$ and $W$,
such that
\begin{equation*}
 \Wightman(z_1,\ldots,z_n)
...
Section \ref{sec:ch2-laplace}.} in $\Tube_{n-1}$. The holomorphic functions $W$ possess a
...
\begin{equation*}
 ...
 \end{aligned}
\end{equation*}
is a holomorphic function, by Theorem \ref{thm:ch2-laplace-forward-cone}. The other assertions, except for
...
tube $\ExtendedTube_{n-1}$. Consider \eqref{eq:pct-3-26}:
\begin{align*}
 &\sum_{\alpha',\beta',\ldots,\gamma'}
 S^{(\varphi)}_{\alpha\alpha'}(A)S^{(\psi)}_{\beta\beta'}(A)\cdots
 S^{(\chi)}_{\gamma\gamma'}(A)
...
single-valued, by Theorem \ref{thm:ch2-11}. Thus $W(\xi_1,\ldots,\xi_{n-1})$ is
continuable into $\ExtendedTube_{n-1}$.
\end{proof}

An important special case is $\Lambda=-1$, which is connected in the complex
Lorentz group to $\Lambda=1$. By \eqref{eq:ch1-sl2-double-cover-signs}
...
\eqref{eq:pct-3-40} holds, even if $P$, $C$, and $T$ do not define symmetries, since it
...
\begin{theorem}\label{thm:ch3-6-free-field-wightman-functions}
Suppose that
\begin{equation*}
...
\begin{equation*}
 \Wightman'=\bra{\Omega}\varphi_n(x_n)\cdots\varphi_2(x_2)
...
the permuted extended tubes have a real environment in common (see
Figure~\ref{fig:2-4}).

The expectation values for a theory of a free field can be computed directly
from the definition of the field operators \eqref{eq:ch3-free-field-operator}. For example, for a
...
A repeated application of \eqref{eq:ch3-free-field-operator} gives
\begin{equation}
\begin{aligned}
...
An up-to-date summary of progress is to be found in \cite{pct-ch3-12}. The idea is
