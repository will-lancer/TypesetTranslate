...
We now return to the point made at the end of Section 1-2, namely, that if a
...
operators $P$, $C$, and $T$ given by (1-52).
...
By using (3-58) and (3-4), moving $\varphi$ to the right, we transform this
expression to
...
Since, anticipating Theorem 4-5, we may assume $\varphi$ is irreducible,
...
\end{equation}
where $|\omega|=1$.  Applying both sides of (3-60) to the vacuum and using
...
Substituting in (3-61) we see $\omega=1$, proving (3-59).
...
so extended, continues to satisfy (3-59).
\end{proof}

% PCT-SOURCE: pdf=140 print=128 kind=prose id=p128-discrete-inversions
In the same way the requirements (1-52) for $U(C)$ and $U(I_t)$ lead to the
...
Equations (3-59), (3-62), and (3-63) and the invariance of
...
Then, the results of Theorem 3-8 can be summarized: \emph{the requirement
...
example, the choices of phase in the definitions (1-45) of $P$, $C$, and $T$
...
multiple of the identity on every coherent subspace.  (See Section 1-1 for
...
Theorem 3-8 one concludes that $V$ commutes with $U(a,\Lambda)$ and leaves
...
in (1-45) was chosen so that it reverses the momenta of the particles in the
...
[that is, it is independent of time; cf. the remark after (1-3)].  In the
...
We now extend the proof of the reconstruction theorem (Theorem 3-7) to
...
like (3-38) and (3-39).  The construction of the corresponding $U(C)$ or
$\Theta$ follows the same lines as the construction of $U(a,\Lambda)$ in
Theorem 3-7.  We content ourselves with a discussion of the PCT operator.  If
we are given further identities among the $\mathcal{W}$'s, for $P$, $C$, and
$T$ separately, such as (3-38) for $C$, we can prove the existence of the
...
(3-39) holds for all vacuum expectation values of the fields, that is,
...
(defined in Section 3-4).  So defined, $\Theta$ is anti-unitary, by virtue of
(3-66), which leads to
...
Section 3-4).  For suppose $f\sim g$ in the sense that
...
using (3-66).  The new equation is then a statement that
...
Theorem 3-9 shows that the presence of PCT symmetry in a field theory is
equivalent to the validity of the identities (3-66).  It is shown in the next
chapter that the identities (3-66) hold in every local field theory.  This is
