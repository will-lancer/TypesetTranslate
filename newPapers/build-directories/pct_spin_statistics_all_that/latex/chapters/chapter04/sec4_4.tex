...
With the use of the relations (4-29) to (4-36) the proof goes just as
before.  We summarize:

% PCT-SOURCE: pdf=158 print=146 kind=theorem id=thm-ch4-7-general-spin-pct
\par\medskip\noindent\textbf{Theorem 4-7 (\emph{PCT Theorem for General Spin})}
\label{thm:ch4-7-general-spin-pct}\par\smallskip
...
the definition (1-53) by an extra factor $\pm1$.  This is discussed at the end
of the next section.
...
\par\medskip\noindent\textbf{Theorem 4-8}\label{thm:ch4-8}\par\smallskip
...
\par\medskip\noindent\emph{Proof:}\par\smallskip
...
commutation relations (4-37) imply that the left-hand side of (4-38) is
...
in a space-like direction, (4-39) approaches
...
and this is not positive.  Therefore, comparing with (4-38) we see that zero
is the only consistent value of the limit, and either
$\psi(g)\ket{\Omega}=0$ or $\varphi^\dagger(f)\ket{\Omega}=0$.  By Theorem 4-3
...
\par\medskip\noindent\textbf{Theorem 4-9 (\emph{Spin-Statistics Theorem for a Scalar Field})}
\label{thm:ch4-9-scalar-spin-statistics}\par\smallskip
...
The hypothesis (4-41) of the “wrong” connection of spin with statistics
implies, if $(x-y)^2>0$,
...
space-like.  Thus, (4-42) is a relation between holomorphic functions
...
Passing to the boundary values in (4-46) as $\eta\to0$, $\eta\in V_+$, we
...
We claim that this equation (4-47) implies $\varphi(x)\ket{\Omega}=0$.  To
...
For fields of general spin an analogue of Theorem 4-9 is

% PCT-SOURCE: pdf=162 print=150 kind=theorem id=thm-ch4-10-general-spin-statistics-heading
\par\medskip\noindent\textbf{Theorem 4-10 (\emph{Spin-Statistics Theorem for General Spin})}
\label{thm:ch4-10-general-spin-statistics}\par\smallskip
...
same lines as for a scalar field.  As the generalization of (4-42), the
relations (4-48) give directly
...
the sign being the same as in (4-48).  As in the proof of Theorem 4-9, there
are holomorphic functions $W$ and $\widehat W$ such that the vacuum expectation
values are given by (4-43).  The generalization of (4-44) is
...
Equation (4-51) is a consequence of the transformation law of $\widehat W$
under the group $SL(2,\C)\otimes SL(2,\C)$ and is a special case of (4-32).
...
the sign being the same as in (4-48).  Combining (4-52) and (4-50), we arrive
at (4-46), in exactly the same form as before.  From here on, the argument is
the same as for Theorem 4-9.\hfill$\blacksquare$
...
$\mathcal H_1$ and $\mathcal H_2$.  Recall from Section 1-1 that linear
...
symmetries we have discussed in Section 3-5, there is no unitary or anti-unitary
...
\par\medskip\noindent\textbf{Theorem 4-11}\label{thm:ch4-11}\par\smallskip
...
$a\to\infty$ is, by the cluster decomposition theorem (Theorem 3-4),
...
$\bra{\Omega}N\ket{\Omega}$ must vanish.\hfill$\blacksquare$
...
and applying Theorem (4-11), since $M$ and $N$ anti-commute we get for all $j$
...
(4-30)].  Clearly this and (4-56) together imply that the vacuum expectation
...
of the four subspaces is guaranteed by (4-56) together with the univalence
...
\par\medskip\noindent\textbf{Theorem 4-12}\label{thm:ch4-12}\par\smallskip
...
relations.  The set $\alpha$ in (4-57) (or several such sets) is determined by
the abnormal commutation relations, i.e., by applying Theorem 4-11.  The set
...
$[\varphi_1,\varphi_2]_\pm$ is unchanged by (4-57).  This disposes of
...
$[\varphi'_1,\varphi'_2]_\mp=0$, using (4-57) and the fact that if
$\varphi_j\in\alpha$, $p(\alpha)$ anti-commutes with $\varphi_j$, whereas if
$\varphi_j\notin\alpha$, $p(\alpha)$ and $\varphi_j$ commute.  We therefore
have the result: the transformation (4-57) alters the commutation relations
...
Obviously $\sigma_{ij}=\sigma_{ji}$; Theorem 4-10 asserts that a field always
...
number of fields from $\beta$, and from (4-58),
...
of the transformation (4-57) on $\sigma_{ij}$ can be calculated explicitly.
...
(4-57) if and only if one and only one of (4-61) holds.  Thus the effect of
(4-57) on $\sigma$ is
...
(4-57) by proving that $\sigma$ has the form given by the following theorem.

% PCT-SOURCE: pdf=170 print=158 kind=theorem id=thm-ch4-13
\par\medskip\noindent\textbf{Theorem 4-13}\label{thm:ch4-13}\par\smallskip
...
\par\medskip\noindent\emph{Proof of Theorem 4-12:}\par\smallskip

% PCT-SOURCE: pdf=170 print=158 kind=prose id=p158-proof-4-12-assume-4-13
Assuming the result of Theorem 4-13 we can proceed as follows.  Choose the set
...
apply (4-57) $N$ times using the $\alpha_k,\beta_k$, then $\sigma_{ij}$ is
transformed to
...
\par\medskip\noindent\emph{Proof of Theorem 4-13:}\par\smallskip

% PCT-SOURCE: pdf=170 print=158 kind=prose id=p158-proof-4-13-gamma
To prove Theorem 4-13 we must have a criterion for deciding whether a given
...
$s(M)\in\Gamma$, and by (4-59), $\sum_js_j(M)t_j(\alpha)=1\pmod2$,
...
contradicting Theorem 4-11.  Both $M$ and $N$ contain an even number of
half-odd integer
...
commutation relations.  Equivalent to this statement, according to (4-60), is
the condition
...
the monomials defined by the vectors $e^{(i)}$, and so, using (4-67) we see
...
$\sigma'_{ik}=0$ if $i,k\leq m$, as a consequence of (4-65).  Therefore
...
$j=1,\ldots,m$, span $\Gamma$.  Therefore, by (4-64), the set $\alpha_k$
defines an even-odd rule.\hfill$\blacksquare$
...
Theorem 4-12 from the given fields $\varphi_i$ to fields $\varphi'_i$
...
spin fields.  In (4-69), $\widehat f(x)=f(-x)$.  This will induce a
transformation of the original fields $\varphi_i$ which will, in general, not
be of the form (4-69) with the primes removed.  To see how this comes about
...
We conclude from Theorem 4-11 that all vacuum expectation values containing an
