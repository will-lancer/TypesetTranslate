...
is irreducible, then \eqref{eq:ch4-relative-locality-assumptions} implies \eqref{eq:ch4-relative-locality-conclusion}. The $\varphi_2$, $\varphi_3$ need
...
Section \ref{sec:ch3-2-independence} we have introduced the notion of a Wick polynomial in a free
field. It can be shown that the Wick polynomials in a given free field exhaust
the equivalence class of that free field \cite{ch4-27}. It is plausible for fields
...
Neumann algebras of open sets mentioned at the end of Section \ref{sec:ch4-2-polynomial-algebra}.
...
$\pi^0$, already discussed at the end of Section \ref{sec:ch4-2-polynomial-algebra}, in which the proton and
...
Since by Theorem \ref{thm:ch4-5-smeared-fields} $\varphi$ is irreducible, \eqref{eq:ch4-wlc-pct} determines $\Theta$ up
to a phase (the proof is in\textsuperscript{\(\dagger\)} Section \ref{sec:ch3-symmetries}). If there
...
same? The reconstruction theorem (Theorem \ref{thm:ch3-reconstruction}) and Theorem \ref{thm:ch4-7-general-spin-pct} show that if
...
Theorem \ref{thm:ch4-7-general-spin-pct}. It should be noted that neither field $\varphi$ nor $\psi$ of the
...
Using the invariance of \eqref{eq:ch4-wlc-interchange} under the complex Lorentz group we get for
...
the tube \eqref{eq:ch4-wlc-tube} we see that \eqref{eq:ch4-wlc-holomorphic} holds at all real points. If
\begin{equation}
  \ket{\Psi}
...
Using \eqref{eq:ch4-wlc-holomorphic} we get
\begin{equation*}
  \matrixel{\Psi}{\psi(x)}{\Phi}
  =\matrixel{\Theta\Phi}{\psi(-x)}{\Theta\Psi}.
\end{equation*}
which when compared with \eqref{eq:ch4-antiunitary-field} leads to
\begin{equation*}
  \Theta\psi(x)\Theta^{-1}=\psi(-x),
\end{equation*}
an equation valid on states of the form \eqref{eq:ch4-wlc-test-states}. Using the hermiticity of
$\psi$, we conclude (just as in the argument at the end of Section \ref{sec:ch4-global-locality}) that
...
\begin{proof}
From the proof of Theorem \ref{thm:ch4-18-weak-local-relative} it follows that $\varphi_1$ and $\varphi_2$
...
$\Theta$ carries in-fields into out-fields, \eqref{eq:ch4-asymptotic-out} follows from \eqref{eq:ch4-asymptotic-in}.
\end{proof}

For local fields there are stronger results, since, as remarked before,
\eqref{eq:ch4-asymptotic-in} can be proved using the Haag--Ruelle theory. The next theorem asserts
...
of Theorem \ref{thm:ch4-18-weak-local-relative}, and so $\varphi_2$ and $\varphi_3$ are weakly local, and
...
This is precisely the relation studied in the proof of Theorem \ref{thm:ch4-global-locality}, and just
...
Theorem \ref{thm:ch4-21-relative-locality-transitive} shows that relative locality is a transitive property.

As an application of Theorems \ref{thm:ch4-21-relative-locality-transitive} and \ref{thm:ch4-15-jost-schroer} consider the problem of the
...
\eqref{eq:ch4-unique-local-solution} follows but we shall not elaborate here.
