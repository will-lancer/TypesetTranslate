...
operators satisfy (1-1). In fact, all mappings \(\ket{\Phi}\to\ket{\Phi'}\)
lead essentially to a unique transformation \(\ket{\Phi}\to\ket{\Phi'}\)
satisfying (1-1), and this transformation is either unitary or anti-unitary
(Ref.~1).

% PCT-SOURCE: pdf=019 print=007 kind=theorem id=thm-ch1-symmetry
% Advance the native result counter so the printed theorem label and its
% cross-reference both carry the source identifier 1-1.
\refstepcounter{theorem}
\par\medskip\noindent\textbf{Theorem 1-1}\label{thm:ch1-symmetry}\par\smallskip
...
unitary and anti-unitary operator a symmetry operator and Theorem 1-1 shows
...
(1-3). For example, \(V\) might satisfy
