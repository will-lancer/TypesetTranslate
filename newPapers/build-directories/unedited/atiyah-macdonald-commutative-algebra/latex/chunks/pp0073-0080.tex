...
\item Let $A$ be a subring of a ring $B$ such that $B$ is integral over $A$, and let $f:A\longrightarrow\Omega$ be a homomorphism of $A$ into an algebraically closed field $\Omega$. Show that $f$ can be extended to a homomorphism of $B$ into $\Omega$. [Use \amref{5.10}.]
...
\item A ring homomorphism $f:A\longrightarrow B$ is said to have the \textit{going-up property} (resp. the \textit{going-down property}) if the conclusion of the going-up theorem \amref{5.11} (resp. the going-down theorem \amref{5.16}) holds for $B$ and its subring $f(A)$.
...
\item Let $f:A\longrightarrow B$ be a flat homomorphism of rings. Then $f$ has the going-down property. [Chapter 3, Exercise 18.]
