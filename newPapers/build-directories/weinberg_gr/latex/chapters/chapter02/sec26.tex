...
\[
  \int d^3x\,f(\mathbf x)\delta^3(\mathbf x-\mathbf y)=f(\mathbf y).
\]
...
\[
  J^\mu(x)=\int dt'\sum_n e_n
  \delta^4\bigl(x-x_n(t')\bigr)\frac{dx_n^\mu(t')}{dt'} .
\]
The differentials \(dt'\) cancel, and hence can be replaced with an invariant
\(d\tau\):
\begin{equation}
  J^\mu(x)=\int d\tau\sum_n e_n
  \delta^4\bigl(x-x_n(\tau)\bigr)\frac{dx_n^\mu(\tau)}{d\tau}.
  \tag{2.6.5}\label{eq:2.6.5}
\end{equation}
But \(\delta^4(x-x_n(\tau))\) is a scalar (because
\(\det\Lambda=1\)) and \(dx_n^\mu\) is a four-vector, so \(J^\mu\) is a
four-vector.
