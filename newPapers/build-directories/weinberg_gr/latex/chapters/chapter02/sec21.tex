...
they leave invariant the ``proper time'' \(d\tau\), defined by
\begin{equation}
...
will have \(\lvert d\mathbf{x}/dt\rvert\) equal to the speed of light, which in
...
\end{equation}
Performing a Lorentz transformation does not change \(d\tau\), so
\(d\tau'^2=0\), and therefore
\(\lvert d\mathbf{x}'/dt'\rvert=1\); that is, the speed of light in the new
...
\(d\tau^2\) invariant. (Nonsingular means that \(x'(x)\) and \(x(x')\) are
...
\(x\to x'\) will change \(d\tau\) into \(d\tau'\), given by
\[
\begin{aligned}
  d\tau'^2
  &=-\eta_{\mu\nu}\,dx'^\mu dx'^\nu \\
...
\end{aligned}
\]
If this is equal to \(d\tau^2\) for all \(dx^\rho\), we must have
\begin{equation}
  \eta_{\rho\sigma}
  =\eta_{\mu\nu}
    \frac{\partial x'^\mu}{\partial x^\rho}
...
assumed that the transformations \(x\to x'\) leave \(d\tau\) invariant when
\(d\tau=0\), that is, for a particle moving at the speed of light, then we would
...
\end{equation}
or, since \(d\mathbf{x}\) vanishes,
\begin{equation}
  dx'^i=\Lambda^i{}_0\,dt\qquad(i=1,2,3)
...
\end{equation}
Dividing \(d\mathbf{x}'\) by \(dt'\) gives the velocity \(\mathbf v\), so
\begin{equation}
