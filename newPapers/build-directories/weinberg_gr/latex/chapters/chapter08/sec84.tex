...
general \(d\tau\) is proportional to \(d\lambda\), so for a material
particle we could normalize \(\lambda\) so that \(\lambda=\tau\). However,
for a photon the proportionality constant \(d\tau/d\lambda\) vanishes, and
...
\begin{equation}
  ...
  &-\frac{r\sin^2\theta}{A(r)}
    \left(\frac{d\varphi}{d\lambda}\right)^2
  +\frac{B'(r)}{2A(r)}
...
\begin{equation}
  ...
  +\frac{2}{r}\frac{d\theta}{d\lambda}\frac{dr}{d\lambda}
  -\sin\theta\cos\theta
  \left(\frac{d\varphi}{d\lambda}\right)^2,
...
\begin{equation}
  0
  =
  \frac{d^2\varphi}{d\lambda^2}
  +\frac{2}{r}\frac{d\varphi}{d\lambda}\frac{dr}{d\lambda}
  +2\cot\theta
  \frac{d\varphi}{d\lambda}\frac{d\theta}{d\lambda},
  \tag{8.4.5}\label{eq:8.4.5}
\end{equation}
\begin{equation}
  0
  =
  \frac{d^2t}{d\lambda^2}
  +\frac{B'(r)}{B(r)}
  \frac{dt}{d\lambda}\frac{dr}{d\lambda}.
  \tag{8.4.6}\label{eq:8.4.6}
\end{equation}
(A prime denotes \(d/dr\).) We solve these equations by looking for
...
\eqref{eq:8.4.6} by \(d\varphi/d\lambda\) and \(dt/d\lambda\),
respectively, we next find
\begin{equation}
  \frac{d}{d\lambda}
  \left[
    \ln\frac{d\varphi}{d\lambda}+\ln r^2
  \right]=0,
...
\end{equation}
By multiplying this equation with \(2A(r)\,dr/d\lambda\), we may write it
as
\[
  \frac{d}{d\lambda}
  \left[
    A(r)\left(\frac{dr}{d\lambda}\right)^2
...
\(d\tau/d\lambda\) to be constant. We see that \(E\) must take the values
\begin{equation}
...
\((dr/dt)^2\), \(A-1\), and \(B-1\simeq2\phi\) will all be small, and
to first order in these quantities the above equations of motion become
\[
  r^2\frac{d\varphi}{dt}\simeq J,
\]
\[
  \frac12\left(\frac{dr}{dt}\right)^2
  +\frac{J^2}{2r^2}+\phi
  \simeq\frac{1-E}{2}.
\]
...
particle in a circular orbit at radius \(R\). Since \(dr/dt\) vanishes,
...
\(d\lambda\) from Eqs.~\eqref{eq:8.4.11} and \eqref{eq:8.4.13}; this
gives
\begin{equation}
  \frac{A(r)}{r^4}
