...
\begin{equation}
  ...
  =
  -dt^2+U(r,t)\,dr^2
  +V(r,t)(d\theta^2+\sin^2\theta\,d\varphi^2).
...
\begin{equation}
  ds^2
  =
  -\left(1-\frac{2MG}{\bar r}\right)d\bar t^2
  +\left(1-\frac{2MG}{\bar r}\right)^{-1}d\bar r^2
  +\bar r^2(d\bar\theta^2+\sin^2\bar\theta\,d\bar\varphi^2).
...
\end{equation}
In order to define a standard time coordinate such that \(ds^2\) does not
contain a cross-term \(d\bar r\,d\bar t\), we employ the ``integrating
...
\begin{equation}
  ...
  \int_{S(r,t)}^1
  \frac{d\alpha}{1-kr_\star^2/\alpha}
...
\[
  ds^2
  =
  -B(\bar r,\bar t)\,d\bar t^2
  +A(\bar r,\bar t)\,d\bar r^2
  +\bar r^2(d\bar\theta^2+\sin^2\bar\theta\,d\bar\varphi^2),
\]
with
\begin{align}
...
\(d\bar r/d\bar t\) given by Eq.~\eqref{eq:11.9.27} and the condition
\(ds^2=0\), so it will arrive at a distant point \(\bar r'\) at a time
\begin{equation}
  \bar t'
  =
  \bar t+
  \int_{r_\star a(t)}^{\bar r'}
  \left(1-\frac{2MG}{\bar r}\right)^{-1}d\bar r.
...
interval \(d\bar t'\) between arrivals of wave crests at \(\bar r'\) is the
