...
\begin{enumerate}
  ...
  \[
    e^{-\ii\Omega_L}=e^{-C}e^{2V_L},\qquad
    -\ii\Omega_L=\log\!\left(e^{-C}e^{2(C+\Delta)}\right),
    \tag{1}
  \]
  where we have made the convenient choice \(W=\ii C\).  A real part of
  \(W\) may be added; it is just the residual ordinary gauge
  transformation.

  The Baker--Campbell--Hausdorff formula through two commutators gives
  \[
    -\ii\Omega_L=C+2\Delta-[C,\Delta]
      +\frac12[C,[C,\Delta]]
  ...
  \[
    ...
    \tag{4}
  \]
  In ordinary components put \(X=M-\ii N\).  The chiral-slice Fierz identity
  \[
  ...
  \[
    W=\ii C,\qquad w=-\sqrt2\,\omega,\qquad
    \mathcal W=M-\ii N,
  \]
  ...
  \begin{gathered}
    ...
    V_{(\alpha)\mu}=-\ii\partial_\mu(\gamma_5\lambda)_\alpha,\quad
    M_{(\alpha)}=-\ii(\sl{\partial}\gamma_5\lambda)_\alpha,\quad
  ...
  are parallel:
  \[
    \boldsymbol\phi=e^{\ii\alpha}v\,\boldsymbol n,\qquad
    \boldsymbol n^2=1 .
    \tag{13}
  \]
  ...
  \[
    ...
    \tag{17}
  \]
  The relative phase in (16) agrees with the \(\ii\sqrt2\mathcal F\) and
  ...
  \[
    q_I=\int_{S_\infty^2}\dd S_i\,E_I^i,\qquad
    \mathcal M_I=\int_{S_\infty^2}\dd S_i\,B_I^i .
    \tag{20}
  \]
  ...
  \[
    Z_{12}=2\sqrt2\sum_{I=1}^2
       v_I\bigl(\ii q_I-\mathcal M_I\bigr).
    \tag{21}
  \]
