  ...
  \[
    (\bar\theta P_L\omega_B)(\bar\theta P_L\omega_D)
      =-\frac12(\bar\theta P_L\theta)
        (\omega_{BL}^{\rm T}\epsilon\omega_{DL})
  \]
  turns (4) into
  \[
    \mathcal W=X-\frac12[C,X]+\frac14[C,[C,X]]
      +\frac i6[t_B,[C,t_D]]
        (\omega_{BL}^{\rm T}\epsilon\omega_{DL})+O(g^3).
    \tag{5}
  \]
  ...
  \begin{aligned}
    \mathcal L={}&
      ...
      -\operatorname{Re}\!\left[
        m(\psi_{AL}^{\rm T}\epsilon\psi_{AL})\right]\\
    &-2\sqrt2\operatorname{Re}\!\left[
       g\epsilon_{ABC}
       (\lambda_{AL}^{\rm T}\epsilon\psi_{CL})\phi_B^*\right]
      +\frac12D_AD_A
