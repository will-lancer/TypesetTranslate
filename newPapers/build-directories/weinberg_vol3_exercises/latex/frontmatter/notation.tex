...
\[
  ...
  \end{pmatrix},
\]
where \(e\) is the \(2\times2\) antisymmetric matrix with \(e_{12}=+1\). The
...
run over the components of four-component spinors. Internal Lie-algebra
indices in Chapter 24 instead use lower-case Latin letters \(a,b,c,\ldots\).
To avoid confusion, I depart here from the conventions of Volume II, and use
upper-case letters \(A,B\), etc.\ to label the generators of a symmetry
algebra in later chapters. Components of two-component spinors are labelled
with indices \(a,b\), etc. If an internal adjoint index occurs in the same
formula, the non-colliding Latin family \(r,s,t,\ldots\) is used for it. In
particular, four-component supersymmetry generators are denoted \(Q_\alpha\),
while two-component generators (the bottom two components of \(Q_\alpha\))
are called \(Q_a\).
...
\(\beta=\ii\gamma^0=\gamma_4\). Where explicit matrices are needed, they are
...
\[
  ...
  \end{pmatrix},
\]
where \(\mathbf{1}\) is the unit \(2\times2\) matrix, \(0\) is the
\(2\times2\) matrix with elements zero, and the components of \(\boldsymbol
...
\[
  ...
  \end{pmatrix}.
\]
We also frequently make use of the \(4\times4\) block matrices
\[
  \gamma_5=
  \begin{pmatrix}
    \mathbf{1} & 0\\
  ...
  \end{pmatrix},
\]
where \(e\) is again the antisymmetric \(2\times2\) matrix \(\ii\sigma_2\).
