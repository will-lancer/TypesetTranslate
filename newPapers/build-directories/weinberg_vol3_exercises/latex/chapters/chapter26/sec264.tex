...
% Corrected the argument of f in Eq. (26.4.4) and a repeated inner sum in
% Eq. (26.4.15).
% Uncertainties: none.
...
\(\SuperQ_\alpha\) and hence \(\partial/\partial\theta_\alpha\) has
dimensionality \(1/2\), so \(\SuperD_\alpha\) has dimensionality \(+1/2\), and
...
pairs of spinor superderivatives \(\SuperD_\alpha\). As discussed in the
...
\[
  \begin{aligned}
  [\Phi_n^*\Phi_m]_{\theta^4}
  ={}&-\frac18(\bar\theta\gamma_5\theta)^2
    \left[\phi_n^*\Box\phi_m+(\Box\phi_m^*)\phi_n\right]
  \\
  &+(\bar\theta\gamma_5\theta)
    \left[
      (\bar\psi_n\theta)(\bar\theta\gamma^\mu\partial_\mu\psi_m)
      ((\partial_\mu\bar\psi_n)\gamma^\mu\theta)(\bar\theta\psi_m)
    \right]
  \\
  &+\frac14\mathcal{F}_n^*\mathcal{F}_m
    \bigl(\bar\theta(1-\gamma_5)\theta\bigr)
    \bigl(\bar\theta(1+\gamma_5)\theta\bigr)
  \\
  &-\frac14\partial^\mu\phi_n^*\partial^\nu\phi_m
    (\bar\theta\gamma_5\gamma_\mu\theta)
    (\bar\theta\gamma_5\gamma_\nu\theta)\,.
  \end{aligned}
\]
Using \eqref{eq:26.A.18} and \eqref{eq:26.A.19} allows us to convert the
\(\theta\) dependence of this expression to an over-all factor
...
\[
  ...
  \biggl[
    &\frac12\phi_n^*\Box\phi_m
    +\frac12(\Box\phi_m^*)\phi_n
    -(\bar\psi_n\gamma^\mu\partial_\mu\psi_m)
  \\
    &+((\partial_\mu\bar\psi_n)\gamma^\mu\psi_m)
    +2\mathcal{F}_n^*\mathcal{F}_m
    -\partial^\mu\phi_n^*\partial_\mu\phi_m
  \biggr]\,.
  \end{aligned}
\]
The \(D\)-term of a superfield is the coefficient of
\(-\frac14(\bar\theta\gamma_5\theta)^2\) minus \(\frac12\Box\) acting on the
\(\theta\)-independent term, which for \(\Phi_n^*\Phi_m\) is
\(\phi_n^*\phi_m\), so
\begin{equation}
  \begin{aligned}
  \frac12[K(\Phi,\Phi^*)]_D
  ...
  \\
    &-\frac12(\bar\psi_{nL}\gamma^\mu\partial_\mu\psi_{mL})
    +\frac12((\partial_\mu\bar\psi_{nL})\gamma^\mu\psi_{mL})
  \biggr]\,.
  \end{aligned}
...
\begin{equation}
  ...
  \\
    &-\frac12(\bar\psi_{nL}\gamma^\mu\partial_\mu\psi_{nL})
    +\frac12((\partial_\mu\bar\psi_{nL})\gamma^\mu\psi_{nL})
  \biggr]\,.
  \end{aligned}
...
The terms in Eq.~\eqref{eq:26.4.3} involving \(\phi_n\) and \(\psi_{nL}\) are
the correct kinematic Lagrangians for conventionally normalized complex scalar
and Majorana spinor fields. We will rewrite the fermion terms in a more
familiar form after we have had a chance to consider mass terms.
...
term of second order in \(\theta_L\):
\[
  \begin{aligned}
  [f(\Phi(x,\theta))]_{\theta_L^2}
  ={}&\sum_{nm}
    (\theta_L^{\mathsf T}\epsilon\psi_{nL}(x))
    (\theta_L^{\mathsf T}\epsilon\psi_{mL}(x))
...
\((\theta_R^{\mathsf T}\epsilon\gamma^\mu\theta_L)\) in Eq.~\eqref{eq:26.3.21}
vanishes when multiplied by an expression with two \(\theta_L\) factors.)
The \(\theta\) dependence of the first term on the right-hand side can be put
into a standard form by using Eq.~\eqref{eq:26.A.11} to write\footnote{Note
that \(\bar\psi_{nL}\) is the left-handed component of \(\bar\psi_n\), not
\(\overline{\psi_{nL}}\).}
\[
  \begin{aligned}
  (\theta_L^{\mathsf T}\epsilon\psi_{nL})(\theta_L^{\mathsf T}\epsilon\psi_{mL})
  &=
  \left(\psi_{nL}^{\mathsf T}\epsilon
    \left(\frac{1+\gamma_5}{2}\right)\theta\right)
  \left(\theta^{\mathsf T}\epsilon
    \left(\frac{1+\gamma_5}{2}\right)\psi_{mL}\right)
  \\
  &=-\frac12(\bar\psi_{nL}\psi_{mL})
    (\theta_L^{\mathsf T}\epsilon\theta_L)\,.
  \end{aligned}
\]
The \(\mathcal{F}\)-term of any left-chiral superfield is the coefficient of
\((\theta_L^{\mathsf T}\epsilon\theta_L)\), so here
\begin{equation}
  [f(\Phi)]_{\mathcal{F}}
  =-\frac12\sum_{nm}
...
\eqref{eq:26.3.17} of the remaining fields \(\psi_{nL}\) and \(\phi_n\):
\[
  \delta\psi_{nL}
  =\sqrt2\,\partial_\mu\phi_n\gamma^\mu\alpha_R
  -\sqrt2\left(\frac{\partial f(\phi)}{\partial\phi_n}\right)^*\alpha_L\,,
  \qquad
  \delta\phi_n=\sqrt2(\bar\alpha_R\psi_{nL})\,.
\]
This is because the expression \eqref{eq:26.4.6} does not obey the
transformation rule
\(\delta\mathcal{F}_n=\sqrt2(\bar\alpha_L\sl{\partial}\psi_{nL})\) for
...
the supersymmetry transformations of \(\phi_n\) and \(\psi_{nL}\) are no
...
\(-\ii(\bar\alpha Q)\) with any Heisenberg-picture quantum field \(\phi_n\) or
\(\psi_{nL}\) equals the change in that field under a supersymmetry
transformation with infinitesimal parameter \(\alpha\). With
\(\mathcal{F}_n\) given by Eq.~\eqref{eq:26.4.6} the commutator of
\(-\ii(\bar\alpha Q)\) with \(\mathcal{F}_n\) is given by
\(\delta\mathcal{F}_n=\sqrt2(\bar\alpha_L\sl{\partial}\psi_{nL})\), because in
the Heisenberg picture the quantum field \(\psi_{nL}\) satisfies the field
...
and \(\psi_{nL}\) do form a closed Lie superalgebra when the field equations
...
\(\delta\psi_{nL}\) is proportional to the vacuum expectation value of the
...
\begin{equation}
  ...
  \biggr]
  \\
  &-\frac12\sum_{nm}\mathcal{M}_{nm}(\bar\psi_{nL}\psi_{mL})
...
\begin{equation}
  ...
  \qquad
  \psi_{nL}=\sum_m\mathcal{U}_{nm}\psi'_{mL}\,,
...
\begin{equation}
  ...
  \biggr]
  \\
  &-\frac12\sum_n m_n(\bar\psi_{nL}\psi_{nL})
  -\frac12\sum_n m_n(\bar\psi_{nL}\psi_{nL})^*
  -\sum_n m_n^2\varphi_n^*\varphi_n\,.
  \end{aligned}
  \tag{26.4.14}\label{eq:26.4.14}
\end{equation}
To put the fermion mass terms in a more familiar form, we introduce fields
\(\psi_n(x)\) defined as the Majorana fields whose left-handed components are
\(\psi_{nL}(x)\). Then, using the symmetry properties \eqref{eq:26.A.7} of
Majorana bilinears:
\[
  \begin{aligned}
  &-\frac12(\bar\psi_{nL}\gamma^\mu\partial_\mu\psi_{nL})
  ...
  \\
  &\quad=
  -\frac12\left(\bar\psi_n\gamma^\mu
    \left(\frac{1+\gamma_5}{2}\right)\partial_\mu\psi_n\right)
  +\frac12\left((\partial_\mu\bar\psi_n)\gamma^\mu
    \left(\frac{1+\gamma_5}{2}\right)\psi_n\right)
  \\
  &\quad=
  -\frac12\left(\bar\psi_n\gamma^\mu
    \left(\frac{1+\gamma_5}{2}\right)\partial_\mu\psi_n\right)
  -\frac12\left(\bar\psi_n\gamma^\mu
    \left(\frac{1-\gamma_5}{2}\right)\partial_\mu\psi_n\right)
  \\
  &\quad=-\frac12(\bar\psi_n\gamma^\mu\partial_\mu\psi_n)\,,
  \end{aligned}
\]
while the reality properties \eqref{eq:26.A.21} give
\[
  (\bar\psi_{nL}\psi_{nL})
  +(\bar\psi_{nL}\psi_{nL})^*
  =2\operatorname{Re}\left(
    \bar\psi_n\left(\frac{1+\gamma_5}{2}\right)\psi_n
  \right)
  =(\bar\psi_n\psi_n)\,.
\]
...
\begin{equation}
  ...
  =\sum_n\left[
    -\partial_\mu\varphi_n^*\partial^\mu\varphi_n
    -m_n^2\varphi_n^*\varphi_n
    -\frac12(\bar\psi_n\gamma^\mu\partial_\mu\psi_n)
    -\frac{m_n}{2}(\bar\psi_n\psi_n)
  \right].
  \tag{26.4.15}\label{eq:26.4.15}
\end{equation}
The factor \(1/2\) in the fermion terms is correct because these are Majorana
fermion fields, while there is no factor \(1/2\) in the scalar terms because
...
\begin{equation}
  ...
  -\frac12\sum_{nm\ell}f_{nm\ell}\varphi_n
    \left(\bar\psi_m\left(\frac{1+\gamma_5}{2}\right)\psi_\ell\right)
  \\
  &-\frac12\sum_{nm\ell}f_{nm\ell}^*\varphi_n^*
    \left(\bar\psi_m\left(\frac{1-\gamma_5}{2}\right)\psi_\ell\right)
  \\
...
\begin{equation}
  f\equiv2\sqrt2\,e^{\ii\alpha}\lambda\,,
...
\begin{equation}
  \varphi\equiv e^{-\ii\alpha}\left(\frac{A+\ii B}{\sqrt2}\right).
...
\begin{equation}
  ...
  -\frac12m(\bar\psi\psi)
  -\lambda A(\bar\psi\psi)
  -\ii\lambda B(\bar\psi\gamma_5\psi)
  \\
  &-m\lambda A(A^2+B^2)
...
\begin{equation}
  ...
  \qquad
  \psi(x)\longrightarrow \ii\beta\psi(\Lambda_Px)\,,
