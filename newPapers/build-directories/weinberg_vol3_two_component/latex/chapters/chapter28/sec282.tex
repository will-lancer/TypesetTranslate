...
% Source notes: the prose after Eqs. (28.2.3)--(28.2.5) visibly refers to
% ``the beta functions in Eq. (28.2.1),'' although the renormalization-group
% equations are (28.2.2), and the prose following Eq. (28.2.7) visibly says
% ``no affect on the prediction.'' Both printed anomalies are preserved.
% Uncertainties: none.
...
\begin{equation}
  \mu\frac{d}{d\mu}g'(\mu)=\beta_1\bigl(g'(\mu)\bigr)\,,
  \qquad
  \mu\frac{d}{d\mu}g(\mu)=\beta_2\bigl(g(\mu)\bigr)\,,
  \qquad
  \mu\frac{d}{d\mu}g_s(\mu)=\beta_3\bigl(g_s(\mu)\bigr)\,,
...
Eq.~\eqref{eq:28.2.1}. These new fields introduce three major changes in the
calculations of the beta functions:

\noindent\textbf{1.}\quad
For every gauge boson, there is a Majorana gaugino with the same
\(SU(3)\cdot SU(2)\cdot U(1)\) quantum numbers. Equation
...
\end{equation}
For the adjoint representation, \((t_C)_{AB}=iC_{ABC}\), so \(C_1=C_2\),
...
\(SU(3)\cdot SU(2)\cdot U(1)\) quantum numbers. Following the same method
...
\begin{equation}
  ...
  =\frac{g_i^3C_{2i}}{48\pi^2}\,,
  \tag{28.2.7}\label{eq:28.2.7}
\end{equation}
where \(\operatorname{Tr}(t_At_B)=g_i^2C_{2i}\delta_{AB}\). This is \(1/4\)
...
shall see, in itself it would have no affect on the prediction of the
...
\(C_{2i}\) in Eq.~\eqref{eq:28.2.7} is
...
Weyl higgsinos contribute twice as much to the beta functions as
...
\end{equation}
It is convenient to take \(\mu=m_Z\), so that \(SU(2)\cdot U(1)\) can be
