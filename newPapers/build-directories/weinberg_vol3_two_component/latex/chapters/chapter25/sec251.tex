...
$[t_a,t_b]=\ii\sum_c C^c_{ab}t_c$. In much the same way, supersymmetry is
...
\begin{equation}
  \sum_d(-1)^{\eta_c\eta_a}C^d_{ab}C^e_{\dd c}
  +\sum_d(-1)^{\eta_a\eta_b}C^d_{bc}C^e_{\dd a}
  +\sum_d(-1)^{\eta_b\eta_c}C^d_{ca}C^e_{\dd b}
  =0\,.
