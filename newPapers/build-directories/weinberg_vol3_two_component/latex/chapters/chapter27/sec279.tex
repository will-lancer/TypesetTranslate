...
% Corrected two missing group-index sums in Eq. (27.9.33) and the power of
% the gauge coupling in Eq. (27.9.50).
% Uncertainties: none.
...
\eqref{eq:27.9.1} as one finite element \(\exp(\ii\pi\tau_2/2)\), but symmetry
...
\(\phi_A,\psi_{A\alpha},\mathcal{F}_A\) (with \(\psi_{A\alpha}\) a Weyl
field and \(\phi_A\) and \(\mathcal{F}_A\) both complex) for each \(N=1\)
gauge multiplet
...
\begin{equation}
  ...
  &-2\sqrt{2}\operatorname{Re}\sum_{ABC}
    C_{ABC}\left(\lambda_A\psi_C\right)\phi_B^*
  +\ii\sum_{ABC}C_{ABC}\phi_B^*\phi_CD_A
...
\((t_A)_{BC}=-\ii C_{ABC}\), where \(C_{ABC}\) is the real structure constant,
...
\begin{equation}
  \mathcal{F}_A=0\,,
  \qquad
  D_A=-\ii\sum_{BC}C_{ABC}\phi_B^*\phi_C\,.
...
\begin{equation}
  ...
  &+\sqrt{2}\sum_{ABC}C_{ABC}
    \left(\bar\psi_B\bar\lambda_A\right)\phi_C
  -\sqrt{2}\sum_{ABC}C_{ABC}
    \left(\lambda_A\psi_C\right)\phi_B^*
  -V(\phi,\phi^*)
  \\
  &-\frac14\sum_Af_{A\mu\nu}f_A^{\mu\nu}
  +\ii\sum_A\bar\lambda_A\bar\sigma^\mu(D_\mu\lambda)_A
...
\begin{equation}
  ...
  =\int \dd^3x\,\{Q_{1\alpha},S_{r\beta}^0(x)\}\,.
...
interacts.) The undotted usual \(N=1\) supersymmetry current (distinguished
now with a subscript 1) is given by Eq.~\eqref{eq:27.4.40} as
\begin{equation}
  \begin{aligned}
  S_{1\alpha}^\mu={}&
  -\ii\sum_A f_{A\rho\sigma}
    (\sigma^{\rho\sigma}\sigma^\mu\bar\lambda_A)_\alpha
  +ie\sum_{ABC}\epsilon_{ABC}
    (\sigma^\mu\bar\lambda_A)_\alpha\phi_B^*\phi_C
  \\
  &-\frac{1}{\sqrt{2}}\sum_A(D_\nu\phi_A)^*
    (\sigma^\nu\bar\sigma^\mu\psi_A)_\alpha .
  \end{aligned}
...
\begin{equation}
  ...
  S_{2\alpha}^\mu={}&
  +\ii\sum_A f_{A\rho\sigma}
    (\sigma^{\rho\sigma}\sigma^\mu\bar\psi_A)_\alpha
  -ie\sum_{ABC}\epsilon_{ABC}
    (\sigma^\mu\bar\psi_A)_\alpha\phi_B^*\phi_C
  \\
  &-\frac{1}{\sqrt{2}}\sum_A(D_\nu\phi_A)^*
    (\sigma^\nu\bar\sigma^\mu\lambda_A)_\alpha .
  \end{aligned}
  \tag{27.9.15}\label{eq:27.9.15}
\end{equation}
It will be enough for our purposes (and somewhat easier) to calculate only
the change in the dotted conjugate current under an \(N=1\)
...
\[
  \begin{aligned}
  \delta\bar S_2^{\mu\dot\alpha}={}&
  \sqrt{2}\sum_A f_{A\rho\sigma}(D_\nu\phi)_A
  \left[
    \bar\sigma^{\rho\sigma}\bar\sigma^\mu\sigma^\nu
    -\bar\sigma^\nu\sigma^\mu\bar\sigma^{\rho\sigma}
  \right]^{\dot\alpha}{}_{\dot\beta}\bar\alpha^{\dot\beta}
  \\
  &+\sqrt{2}e\sum_{ABC}\epsilon_{ABC}\phi_B^*\phi_C
    (D_\nu\phi)_A
...
can combine terms by using the sigma-matrix anticommutation relations and
...
\[
  ...
  \right]
  =-2f^{\mu\nu}\mathbf{1}
    -\ii\epsilon^{\mu\nu\rho\sigma}f_{\rho\sigma}\mathbf{1}
\]
and find
\[
  \begin{aligned}
  ...
  -2\sqrt{2}\sum_Af_A^{\mu\nu}(D_\nu\phi)_A
    \bar\alpha^{\dot\alpha}
  -\ii\sqrt{2}\sum_A\epsilon^{\mu\nu\rho\sigma}
    f_{A\rho\sigma}(D_\nu\phi)_A\bar\alpha^{\dot\alpha}
  \\
  &-2\sqrt{2}e\sum_{ABC}\epsilon_{ABC}\phi_B^*\phi_C
    (D^\mu\phi)_A\bar\alpha^{\dot\alpha}+\cdots\,.
  \end{aligned}
\]
...
\[
  ...
  \end{aligned}
\]
These allow us to write \(\delta\bar S_2^{\mu\dot\alpha}\) as a total
derivative
\begin{equation}
  \delta\bar S_2^{\mu\dot\alpha}
  =D_\nu X^{\mu\nu}\bar\alpha^{\dot\alpha}\,,
...
\begin{equation}
  \{\bar Q_{1\dot\alpha},\bar S_{2\dot\beta}^\mu\}
  =\ii\epsilon_{\dot\alpha\dot\beta}D_\nu X^{\mu\nu}\,.
...
\begin{equation}
  \sum_Af_A^{0\ii}\phi_A=-vE^i\,,
  \qquad
  \frac12\sum_A\epsilon^{0\ii\rho\sigma}f_{A\rho\sigma}\phi_A=vB^i\,,
...
mass terms \((\psi'_n\psi'_m)\) or
\((\psi''_n\psi''_m)\). The only
...
\begin{equation}
  ...
  -\sum_A(D_\mu\phi)_A^*(D^\mu\phi)_A
  \\
  &+\ii\sum_n\bar\psi'_n\bar\sigma^\mu(D_\mu\psi')_n
  +\ii\sum_n\bar\psi''_n\bar\sigma^\mu(D_\mu\psi'')_n
  +\ii\sum_A\bar\psi_A\bar\sigma^\mu(D_\mu\psi)_A
  +\ii\sum_A\bar\lambda_A\bar\sigma^\mu(D_\mu\lambda)_A
  \\
...
the transformation
\begin{equation}
  \lambda_{A\alpha}\longrightarrow-\psi_{A\alpha}\,,
  \quad
  \psi_{A\alpha}\longrightarrow+\lambda_{A\alpha}\,,
  \quad
...
provided that
\begin{equation}
  s_A=-2\sqrt{2}\ii\,t_A^{\prime\mathrm T}
  =+2\sqrt{2}\ii\,t''_A\,.
...
\begin{equation}
  ...
  \\
  &-4\operatorname{Re}\sum_{Anm}(t'_A\mu)_{nm}\phi_n^{\prime *}\phi'_m
  -4\operatorname{Re}\sum_{Anm}(\mu t'_A)_{nm}
...
Also setting \((t'_A)_{BC}\) equal to the generators \(-\ii C_{ABC}\) in the
...
\begin{equation}
  ...
  -\sum_A(D_\mu\phi)_A^*(D^\mu\phi)_A
  \\
  &+\ii\sum_A\bar\psi'_A\bar\sigma^\mu(D_\mu\psi')_A
  +\ii\sum_A\bar\psi''_A\bar\sigma^\mu(D_\mu\psi'')_A
  +\ii\sum_A\bar\psi_A\bar\sigma^\mu(D_\mu\psi)_A
  +\ii\sum_A\bar\lambda_A\bar\sigma^\mu(D_\mu\lambda)_A
  \\
  &-2\sqrt{2}\operatorname{Re}\sum_{ABC}C_{ABC}\phi_A
    \left(\psi'_B\psi''_C\right)
  -2\sqrt{2}\operatorname{Re}\sum_{ABC}C_{ABC}
    \left(\lambda_A\psi_C\right)\phi_B^*
  \\
  &-2\sqrt{2}\operatorname{Re}\sum_{ABC}C_{ABC}\phi'_B
    \left(\psi''_C\psi_A\right)
  -2\sqrt{2}\operatorname{Re}\sum_{ABC}C_{ABC}\phi''_C
    \left(\psi'_B\psi_A\right)
  \\
  &+2\sqrt{2}\operatorname{Re}\sum_{ABC}C_{ABC}
    \left(\psi'_B\lambda_A\right)\phi_C^{\prime *}
  +2\sqrt{2}\operatorname{Re}\sum_{ABC}C_{ABC}
    \left(\psi''_B\lambda_A\right)
    \phi_C^{\prime\prime *}
  \\
  &-\frac14\sum_Af_{A\mu\nu}f_A^{\mu\nu}
  +\frac{g^2\theta}{64\pi^2}\epsilon_{\mu\nu\rho\sigma}
...
notation for the fields. We assemble the undotted fermion fields into an
\(SU(4)\) vector:
\begin{equation}
  \psi_{1A\alpha}\equiv\psi_{A\alpha}\,,
  \qquad
  \psi_{2A\alpha}\equiv\lambda_{A\alpha}\,,
  \qquad
  \psi_{3A\alpha}\equiv\psi'_{A\alpha}\,,
  \qquad
  \psi_{4A\alpha}\equiv\psi''_{A\alpha}\,.
  \tag{27.9.38}\label{eq:27.9.38}
\end{equation}
In order for the fermion kinematic terms in the Lagrangian density to be
\(SU(4)\)-invariant, their dotted conjugates must form a contragredient
vector:
\begin{equation}
  \bar\psi_A^{1\dot\alpha}\equiv\bar\psi_A^{\dot\alpha}\,,
  \qquad
  \bar\psi_A^{2\dot\alpha}\equiv\bar\lambda_A^{\dot\alpha}\,,
  \qquad
  \bar\psi_A^{3\dot\alpha}\equiv\bar\psi_A^{\prime\dot\alpha}\,,
  \qquad
  \bar\psi_A^{4\dot\alpha}\equiv\bar\psi_A^{\prime\prime\dot\alpha}\,.
  \tag{27.9.39}\label{eq:27.9.39}
\end{equation}
Complex conjugation then takes the \(SU(4)\)-invariant form
\begin{equation}
  (\psi_{iA\alpha})^*=\bar\psi^i_{A\dot\alpha}\,,
...
\begin{equation}
  ...
  \\
  &+\ii\sum_{Ai}
    \bar\psi_A^i\bar\sigma^\mu(D_\mu\psi_i)_A
  \\
  &-\sqrt{2}\operatorname{Re}\sum_{ABCij}C_{ABC}\phi_A^{ij}
    \left(\psi_{iB}\psi_{jC}\right)-V
  \\
  &-\frac14\sum_Af_{A\mu\nu}f_A^{\mu\nu}
  +\frac{g^2\theta}{64\pi^2}\epsilon_{\mu\nu\rho\sigma}
    \sum_Af_A^{\mu\nu}f_A^{\rho\sigma}\,,
...
\(\mu\,\dd g/\dd\mu=\beta(g)\) is then given to all orders of perturbation theory
...
\begin{equation}
  ...
  &=g^2C_1\delta_{CD}\,,
  \\
  \left[\operatorname{Tr}(t_Ct_D)\right]_{\text{Weyl fermions}}
  &=g^2C_2^f\delta_{CD}\,,
  \\
...
\end{equation}
In general theories with \(N=2\) supersymmetry we have two Weyl fermions
\(\lambda_{A\alpha}\) and \(\psi_{A\alpha}\) in the adjoint representation
and \(H\) pairs of Weyl fermions \(\psi'_{n\alpha}\) and
\(\psi''_{n\alpha}\) in conjugate representations with generators \(t'_A\)
and \(-t_A^{\prime\mathrm T}\), so
\begin{equation}
  C_2^f=2C_1+2HC'_2\,,
...
\begin{equation}
  \beta(g)=-\frac{g^3}{8\pi^2}(C_1-HC'_2)\,.
...
\begin{equation}
  M=\sqrt{2}\left|v\left(ne+\frac{4\pi \ii m}{e}\right)\right|,
