...
\begin{equation}
  ...
  \longrightarrow{}&
  \exp\left(-\ii\sum_A t_A\Omega_A(x,\theta)\right)
  \exp\left(-2\sum_A t_A V_A(x,\theta)\right)
  \\
  &{}\cdot
  \exp\left(+\ii\sum_A t_A\Omega_A^*(x,\theta)\right),
...
\(D_\alpha\) acts on
\(\exp(+\ii\sum_A t_A\Omega_A(x,\theta))\) as well as on
...
\begin{equation}
  ...
  =\sum_A\biggl[
    &2\ii\bar\lambda_A\bar\sigma^\mu(D_\mu\lambda)_A
    -\frac12 f_{A\mu\nu}f_A^{\mu\nu}
  \\
...
\begin{equation}
  \left[
    \exp\left(\ii\int \dd^4x\,\mathcal{L}_\theta\right)
  \right]_\nu
  =\exp(\ii\nu\theta)\,,
