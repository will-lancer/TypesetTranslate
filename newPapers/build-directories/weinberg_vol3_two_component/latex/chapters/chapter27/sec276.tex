...
% Source notation preserved: Eq. (27.6.16) uses X^* and Y^*, while
...
\begin{equation}
  ...
  &+2\operatorname{Re}
  \left[
    \mathcal{B}_\lambda(\Phi,W_\alpha,X,Y)
  \right]_{\mathcal{F}}\,,
  \end{aligned}
...
gives \(W_\alpha\) itself in terms of \(\bar D^2\) acting on a
superfield \(\exp(-2V)D_\alpha\exp(2V)\), but this superfield is not
...
Section 26.3, for which \(\theta\) and \(\bar\theta\) are given the \(R\)
values \(+1\) and \(-1\), the superfields \(\Phi\), \(V\), and \(X\) are
\(R\)-neutral, and \(Y\) has the \(R\) value \(+2\). (Recall that
\(f_{\mathcal{F}}\) is the coefficient of \(\theta^2\) in \(f\), so in
order for \(f_{\mathcal{F}}\) to have the \(R\) value 0, \(f\) must have
the \(R\) value 2.) Because \(W_\alpha\) is given by \(\bar D^2D_\alpha\)
acting on \(R\)-neutral superfields, it has the \(R\)
...
order in \(W_\alpha\), with coefficients depending only on the \(R\)-neutral
...
\end{equation}
(Lorentz invariance requires the spinor indices on the \(W_\alpha\)s to be
contracted.) The other symmetry is
translation of \(X\) by an imaginary numerical constant,
\(X\longrightarrow X+\ii\xi\), with \(\xi\) real. This changes the Lagrangian
density \eqref{eq:27.6.4} by an amount proportional to
\(\operatorname{Im}\sum_AW_A^\alpha W_{A\alpha}\), which as
...
order contribution to \(\lambda\,\dd g_\lambda/\dd\lambda\) is the same function
...
\(\ii(\Omega-\Omega^*)/2\), and although the \(D\)-term of a chiral superfield
vanishes, the product of \(\ii(\Omega-\Omega^*)/2\) and \(\xi_\lambda\) is not
...
\IfFileExists{chapters/chapter27/figures/fig27-01.tex}{%
  \input{chapters/chapter27/figures/fig27-01.tex}%
}{%
  \input{../chapters/chapter27/figures/fig27-01.tex}%
}
...
\begin{equation}
  ...
  =
  \left[
    \exp\left(\ii\sum_A t_Az_A\right)
  \right]_{nm}
  \phi_m^{(0)}\,,
...
\(\delta\phi=\ii\sum_A\delta z_A t_A\phi\) with \(z_A\) real. But these are
