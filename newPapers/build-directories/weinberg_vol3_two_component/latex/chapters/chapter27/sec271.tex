...
the supersymmetry generators \(Q_\alpha,\bar Q_{\dot\alpha}\) invariant.
(For simple supersymmetry there is just one conjugate pair of Weyl-spinor
supersymmetry generators, which can only
...
\begin{equation}
  ...
  \phi_n(x)&\longrightarrow
    \sum_m\left[
      \exp\left(i\sum_A t_A\Lambda^A(x)\right)
    \right]_{nm}\phi_m(x)\,,
  \\
  \psi_{n\alpha}(x)&\longrightarrow
    \sum_m\left[
      \exp\left(i\sum_A t_A\Lambda^A(x)\right)
    \right]_{nm}\psi_{m\alpha}(x)\,,
  \\
...
confusion with spinor indices we label the gauge generators and gauge
...
\(\theta\) and the variable \(x_+\) defined by Eq.~\eqref{eq:26.3.23}.
It therefore has the transformation property
\begin{equation}
  \Phi_n(x,\theta)\longrightarrow
  \sum_m\left[
    \exp\left(i\sum_A t_A\Lambda^A(x_+)\right)
...
\(\int d^4x\,[f(\Phi)]_{\mathcal{F}}\) in Eq.~\eqref{eq:26.3.30}, then it
...
superfield \(\Upsilon\) with the transformation rule
\[
  \Upsilon(x,\theta)\longrightarrow
  \exp\left(i\sum_A t_A\Lambda^A(x_+)\right)
  \Upsilon(x,\theta)
  \exp\left(-i\sum_A t_A\Lambda^A(x_+)\right),
\]
...
anti-Hermitian part \((\Gamma-\Gamma^\dagger)/2i\).) Another simplification
...
\begin{equation}
  \begin{aligned}
  f^A(a,b)={}&a^A+b^A
  +\frac{i}{2}\sum_{BC}C^A{}_{BC}a^B b^C
...
\begin{equation}
  ...
  \sum_m\left[
    \exp\left(i\sum_A t_A\Omega^A(x,\theta)\right)
...
\end{equation}
where \(\Omega^A(x,\theta,\bar\theta)\) is an arbitrary left-chiral
superfield---that is, an arbitrary function of \(\theta\) as well as
\(x_+\). Under this
transformation,
\begin{equation}
...
\begin{equation}
  \begin{aligned}
  \Omega^A(x,\theta,\bar\theta)={}&
  W^A(x)
  -\sqrt{2}\,\theta w^A(x)
  +\theta^2\mathcal{W}^A(x)
  \\
  &-i\theta\sigma^\mu\bar\theta\,\partial_\mu W^A(x)
  +\frac{i}{\sqrt{2}}\theta^2
    \bar\theta\bar\sigma^\mu\partial_\mu w^A(x)
  \\
  &+\frac14\theta^2\bar\theta^2\Box W^A(x),
  \end{aligned}
  \tag{27.1.14}\label{eq:27.1.14}
\end{equation}
in which \(W^A(x)\) and \(\mathcal{W}^A(x)\) are arbitrary complex
functions of \(x^\mu\), and \(w^A_\alpha(x)\) are Weyl spinors, with
conjugates \(\bar w^A_{\dot\alpha}(x)\). Taking the complex conjugate of
Eq.~\eqref{eq:27.1.14}, using the two-component form of the conjugation
rule \eqref{eq:26.A.21}, gives
\begin{equation}
  \begin{aligned}
  \Omega^A(x,\theta,\bar\theta)^*={}&
  W^{A*}(x)
  -\sqrt{2}\,\bar\theta\bar w^A(x)
  +\bar\theta^2\mathcal{W}^{A*}(x)
  \\
  &+i\theta\sigma^\mu\bar\theta\,\partial_\mu W^{A*}(x)
  +\frac{i}{\sqrt{2}}\bar\theta^2
    \theta\sigma^\mu\partial_\mu\bar w^A(x)
  \\
  &+\frac14\theta^2\bar\theta^2\Box W^{A*}(x).
  \end{aligned}
...
\begin{equation}
  \begin{aligned}
  V^A(x,\theta,\bar\theta)={}&C^A(x)
  -i\theta\omega^A(x)+i\bar\theta\bar\omega^A(x)
  -\frac{i}{2}\bigl(\theta^2-\bar\theta^2\bigr)M^A(x)
  -\frac12\bigl(\theta^2+\bar\theta^2\bigr)N^A(x)
  \\
  &+\theta\sigma^\mu\bar\theta\,V^A_\mu(x)
  -i\theta^2\bar\theta
    \left[\bar\lambda^A(x)
      -\frac{i}{2}\bar\sigma^\mu\partial_\mu\omega^A(x)\right]
  \\
  &+i\bar\theta^2\theta
    \left[\lambda^A(x)
      -\frac{i}{2}\sigma^\mu\partial_\mu\bar\omega^A(x)\right]
...
real, and \(\omega^A_\alpha(x)\) and \(\lambda^A_\alpha(x)\) are Weyl
spinors, accompanied by their dotted conjugates.
...
\begin{equation}
  ...
  \\
  \omega^A_\alpha(x)&\longrightarrow
    \omega^A_\alpha(x)+\frac1{\sqrt{2}}w^A_\alpha(x)+\cdots,
  \\
  ...
  \\
  \lambda^A_\alpha(x)&\longrightarrow\lambda^A_\alpha(x)+\cdots,
  \\
...
\begin{equation}
  ...
  V^A(x,\theta,\bar\theta)={}&
  \theta\sigma^\mu\bar\theta\,V^A_\mu(x)
  +i\bar\theta^2\theta\lambda^A(x)
  -i\theta^2\bar\theta\bar\lambda^A(x)
  +\frac12\theta^2\bar\theta^2D^A(x).
  \end{aligned}
...
\(\mathcal{W}^A(x)=M^A(x)-iN^A(x)\). For Abelian gauge theories, in which
...
\eqref{eq:26.3.11}, but with no \(\psi_\alpha\)- or
\(\mathcal{F}\)-components, and with \(\phi\)-components given by
\textit{real} infinitesimal functions \(\Lambda^A(x)\):
\begin{equation}
  \Omega^A(x_+)=\Lambda^A(x)
  -i\theta\sigma^\mu\bar\theta\,\partial_\mu\Lambda^A(x)
  +\frac14\theta^2\bar\theta^2\Box\Lambda^A(x).
...
\[
  ...
  &=2\sum_A t_A\operatorname{Im}\Lambda^A(x_+)
   =-2\theta\sigma^\mu\bar\theta
     \sum_A t_A\partial_\mu\Lambda^A(x),
  \\
  b-a
  &=-2i\sum_A t_A\operatorname{Re}\Lambda^A(x_+)
   =-2i\sum_A t_A\left[
     \Lambda^A(x)+\frac14\theta^2\bar\theta^2
     \Box\Lambda^A(x)
   \right],
  \\
  X
  ...
  \end{aligned}
\]
Now, every term in \(X\) involves at least one factor of \(\theta\) and
at least one factor of \(\bar\theta\), while \(a+b\) has one factor of
each, so we can drop any terms in
...
\(\theta^2\bar\theta^2\), since when acted on by \(L_X\) it would
yield at least three factors of either \(\theta\) or \(\bar\theta\). Thus
...
\begin{equation}
  ...
  V^A(x,\theta)
  +\sum_{BC}C^A{}_{BC}V^B(x,\theta)\Lambda^C(x)
  +\theta\sigma^\mu\bar\theta\,\partial_\mu\Lambda^A(x).
  \end{split}
...
\end{equation}
\begin{equation}
  \lambda^A_\alpha(x)\longrightarrow
  \sum_{BC}C^A{}_{BC}\lambda^B_\alpha(x)\Lambda^C(x),
...
\(\lambda^A_\alpha(x)\) and \(D^A(x)\) transform as `matter' fields that
belong to the adjoint representation of the gauge group. The Weyl spinors
\(\lambda^A_\alpha\), together with their dotted conjugates, are known as
\textit{gaugino} fields, while the real scalars
...
\[
  \begin{aligned}
  \Gamma(x,\theta,\bar\theta)
  &=\exp\left(-2\sum_A t_AV^A(x,\theta,\bar\theta)\right)
  \\
  &=\mathbf{1}
  -2\theta\sigma^\mu\bar\theta
    \sum_A t_AV^A_\mu(x)
  \\
  &\quad+2(\theta\sigma^\mu\bar\theta)
    (\theta\sigma^\nu\bar\theta)
    \sum_{AB}t_At_BV^A_\mu(x)V^B_\nu(x)
  \\
  &\quad-2i\bar\theta^2\sum_A t_A(\theta\lambda^A(x))
    +2i\theta^2\sum_A t_A(\bar\theta\bar\lambda^A(x))
  -\theta^2\bar\theta^2\sum_A t_AD^A(x).
  \end{aligned}
\]
...
\[
  \begin{aligned}
  \Phi_n(x,\theta,\bar\theta)={}&
  \phi_n(x)-\sqrt{2}\theta\psi_n(x)
  +\theta^2\mathcal{F}_n(x)
  \\
  &-i\theta\sigma^\mu\bar\theta\,\partial_\mu\phi_n(x)
  +\frac{i}{\sqrt{2}}\theta^2\bar\theta\bar\sigma^\mu
    \partial_\mu\psi_n(x)
  +\frac14\theta^2\bar\theta^2\Box\phi_n(x),
  \end{aligned}
\]
...
\[
  \begin{aligned}
  \Phi_n(x,\theta,\bar\theta)^*={}&
  \phi_n^*(x)-\sqrt{2}\bar\theta\bar\psi_n(x)
  +\bar\theta^2\mathcal{F}_n^*(x)
  \\
  &+i\theta\sigma^\mu\bar\theta\,\partial_\mu\phi_n^*(x)
  +\frac{i}{\sqrt{2}}\bar\theta^2\theta\sigma^\mu
    \partial_\mu\bar\psi_n(x)
  \\
  &+\frac14\theta^2\bar\theta^2\Box\phi_n^*(x).
  \end{aligned}
\]
The term in this product proportional to \(\theta^2\bar\theta^2\) is
\[
  \begin{aligned}
  \left[\Phi^\dagger\Gamma\Phi\right]_{\theta^2\bar\theta^2}
  ...
  \left\{
    ...
    +[((\partial_\mu\bar\psi)\bar\sigma^\mu\theta)
      (\theta\psi)]
  \right\}
  \\
  &+\theta^2\bar\theta^2[\mathcal{F}^\dagger\mathcal{F}]
  \\
  &+(\theta\sigma^\mu\bar\theta)(\theta\sigma^\nu\bar\theta)
    [\partial_\mu\phi^\dagger\partial_\nu\phi]
  \\
  &+2i(\theta\sigma^\mu\bar\theta)(\theta\sigma^\nu\bar\theta)
    \sum_A V^A_\mu
    \left\{
    ...
    \right\}
  \\
  &+2(\theta\sigma^\mu\bar\theta)(\theta\sigma^\nu\bar\theta)
    \sum_{AB}V^A_\mu V^B_\nu[\phi^\dagger t_At_B\phi]
  \\
  &-4(\theta\sigma_\mu\bar\theta)
    \sum_A V^{A\mu}
    [(\bar\psi\bar\theta)t_A(\theta\psi)]
  \\
  &-2i\sqrt{2}(\theta^2-\bar\theta^2)
    \sum_A[(\bar\psi\bar\theta)t_A
      (\theta\lambda^A+\bar\theta\bar\lambda^A)\phi]
  \\
  &-2i\sqrt{2}(\theta^2-\bar\theta^2)
    \sum_A[\phi^\dagger
      (\lambda^A\theta+\bar\lambda^A\bar\theta)t_A(\theta\psi)]
  \\
  &-\theta^2\bar\theta^2
...
indices \(n,m\), and round brackets to indicate explicit two-component
spinor contractions. Just as in Section 26.4, we may use the
identities \eqref{eq:26.A.17}--\eqref{eq:26.A.19} to put all \(\theta\)
dependence in the form of an over-all factor \(\theta^2\bar\theta^2\):
\[
  \begin{aligned}
  \left[\Phi^\dagger\Gamma\Phi\right]_{\theta^2\bar\theta^2}
  =\theta^2\bar\theta^2\biggl\{&
  \frac14[\phi^\dagger\Box\phi]
  +\frac14[(\Box\phi^\dagger)\phi]
  \\
  &+\frac{i}{2}[\bar\psi\bar\sigma^\mu\partial_\mu\psi]
  -\frac{i}{2}[(\partial_\mu\bar\psi)\bar\sigma^\mu\psi]
  \\
  &+[\mathcal{F}^\dagger\mathcal{F}]
  -\frac12[\partial_\mu\phi^\dagger\partial^\mu\phi]
  \\
  &-i\sum_A V^A_\mu
  ...
  \\
  &-\sum_{AB}V^A_\mu V^{B\mu}
    [\phi^\dagger t_At_B\phi]
  +\sum_A V^A_\mu
    [\bar\psi\bar\sigma^\mu t_A\psi]
  \\
  &+i\sqrt{2}\sum_A[(\bar\psi t_A\bar\lambda^A)\phi]
  -i\sqrt{2}\sum_A[\phi^\dagger(\lambda^At_A\psi)]
  \\
  &-\sum_A D_A[\phi^\dagger t_A\phi]
  \biggr\}.
  \end{aligned}
...
\(\tfrac12\theta^2\bar\theta^2\) minus \(\tfrac12\Box\) acting
...
\[
  ...
  -i[(\partial_\mu\bar\psi)\bar\sigma^\mu\psi]
  +2[\mathcal{F}^\dagger\mathcal{F}]
  \\
  &-2i\sum_A V^A_\mu[\phi^\dagger t_A\partial^\mu\phi]
  +2i\sum_A V^A_\mu[(\partial^\mu\phi^\dagger)t_A\phi]
  \\
  &-2\sum_{AB}V^A_\mu V^{B\mu}
    [\phi^\dagger t_At_B\phi]
  +2\sum_A V^A_\mu[\bar\psi\bar\sigma^\mu t_A\psi]
  \\
  &+2i\sqrt{2}\sum_A[(\bar\psi t_A\bar\lambda^A)\phi]
  -2i\sqrt{2}\sum_A[\phi^\dagger(\lambda^At_A\psi)]
  \\
...
\begin{equation}
  ...
  +\frac{i}{2}[\bar\psi\bar\sigma^\mu D_\mu\psi]
  -\frac{i}{2}[(D_\mu\bar\psi)\bar\sigma^\mu\psi]
  +[\mathcal{F}^\dagger\mathcal{F}]
  \\
  &+i\sqrt{2}\sum_A[(\bar\psi t_A\bar\lambda^A)\phi]
  -i\sqrt{2}\sum_A[\phi^\dagger(\lambda^At_A\psi)]
...
\begin{equation}
  D_\mu\psi_\alpha\equiv
  \partial_\mu\psi_\alpha-i\sum_A t_AV^A_\mu\psi_\alpha,
  \qquad
  D_\mu\phi\equiv
  \partial_\mu\phi-i\sum_A t_AV^A_\mu\phi.
