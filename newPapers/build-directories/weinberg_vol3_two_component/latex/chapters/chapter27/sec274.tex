...
% Corrected field-name, omitted-factor, and repeated-index slips in the
% component expansion and mass-matrix relation.
% Uncertainties: none.
...
\begin{equation}
  ...
  -\frac12\operatorname{Re}\sum_A
    \left(W_A^\alpha W_{A\alpha}\right)_{\mathcal{F}}
  \\
...
\begin{equation}
  D_\mu\psi_\alpha
  \equiv\partial_\mu\psi_\alpha-\ii\sum_A t_A V_{A\mu}\psi_\alpha\,,
  \tag{27.4.2}\label{eq:27.4.2}
\end{equation}
\begin{equation}
  D_\mu\phi
  \equiv\partial_\mu\phi-\ii\sum_A t_A V_{A\mu}\phi\,,
...
\begin{equation}
  ...
  +\frac{\ii}{2}\sum_n
    \bar\psi_n\bar\sigma^\mu(D_\mu\psi)_n
  -\frac{\ii}{2}\sum_n
    (D_\mu\bar\psi)_n\bar\sigma^\mu\psi_n
  \\
  &-\frac12\sum_{nm}
    \frac{\partial^2f(\phi)}{\partial\phi_n\partial\phi_m}
    \left(\psi_n\psi_m\right)
  -\frac12\sum_{nm}
    \left(
      \frac{\partial^2f(\phi)}{\partial\phi_n\partial\phi_m}
    \right)^*
    \left(\bar\psi_n\bar\psi_m\right)
  \\
  &-\sum_n\left|
    \frac{\partial f(\phi)}{\partial\phi_n}
  \right|^2
  +\ii\sqrt{2}\sum_{Anm}
    \left(\bar\psi_n(t_A)_{nm}\bar\lambda_A\right)\phi_m
  \\
  &-\ii\sqrt{2}\sum_{Anm}
    \phi_n^*\left(\lambda_A(t_A)_{nm}\psi_m\right)
  -\frac12\sum_A
...
\end{equation}
Lorentz invariance requires the fields \(\psi_{n\alpha}\),
\(\lambda_{A\alpha}\), and
...
\(\langle\delta\psi_{n\alpha}\rangle_{\mathrm{VAC}}
=\sqrt{2}\langle\mathcal{F}_n\rangle_{\mathrm{VAC}}\alpha_\alpha\), and
Eq.~\eqref{eq:26.2.16} gives
\(\langle\delta\lambda_{A\alpha}\rangle_{\mathrm{VAC}}
=\ii\langle D_A\rangle_{\mathrm{VAC}}\alpha_\alpha\).
...
\(\phi\longrightarrow\exp(\ii\sum_A\Lambda_A t_A)\phi\), with
...
\(\phi^\Lambda\equiv\exp(\ii\sum_A\Lambda_A t_A)\phi_0\). On the other hand,
...
\begin{equation}
  ...
  =\ii\sum_C C_{ABC}\left(\phi_0^\dagger t_C\phi_0\right)
  =-\ii\left(\phi_0^\dagger\phi_0\right)
    \sum_C C_{ABC}\xi_C
  =0\,.
...
\begin{equation}
  ...
  =-\frac12\sum_{nm}\mathcal{M}_{nm}
    \left(\psi_n\psi_m\right)
  -\ii\sqrt{2}\sum_{Am}(t_A\phi_0)_m^*
    \left(\lambda_A\psi_m\right)
  +\mathrm{H.c.}
  \tag{27.4.28}\label{eq:27.4.28}
\end{equation}
We saw in Section 26.4 that if the fermion mass term in the Lagrangian for a
column \(\chi^i_\alpha\) of Weyl spinor fields is put in the form
\begin{equation}
  \mathcal{L}_{1/2}
  =-\frac12 M_{ij}\left(\chi^i\chi^j\right)
  +\mathrm{H.c.}\,,
...
eigenvector \eqref{eq:27.4.33}. Thus there is one Weyl field (equivalently,
one Majorana particle) of each mass \(m\) satisfying
Eq.~\eqref{eq:27.4.26}, two Weyl fields (equivalently, two Majorana
particles) of each non-zero mass \(\mu\) satisfying
Eq.~\eqref{eq:27.4.22}, and one massless Weyl field for each unbroken gauge
symmetry.
...
gauge-invariant derivatives, gives the undotted Noether supersymmetry
current (the dotted current is its complex conjugate):
\begin{equation}
  \begin{aligned}
  N^\mu_\alpha={}&
  -\ii\sum_Af_A^{\mu\nu}(\sigma_\nu\bar\lambda_A)_\alpha
  -\frac{\ii}{2}\sum_Af_{A\rho\sigma}
    (\sigma^{\rho\sigma}\sigma^\mu\bar\lambda_A)_\alpha
  -\frac12\sum_AD_A(\sigma^\mu\bar\lambda_A)_\alpha
  \\
  &+\frac1{\sqrt{2}}\sum_n\left[
    2(D^\mu\phi)_n^*\psi_{n\alpha}
    -(D_\nu\phi)_n^*
...
\begin{equation}
  \delta\mathcal{L}
  =\partial_\mu\left(
    \alpha^\beta K^\mu_\beta
    +\bar\alpha_{\dot\beta}\bar K^{\mu\dot\beta}\right),
...
\begin{equation}
  ...
  +\frac12\sum_AD_A(\sigma^\mu\bar\lambda_A)_\alpha
  \\
  &-\ii\sum_{Anm}(t_A)_{nm}
    \bigl[-\ii\sigma^\mu\bar\lambda_A\bigr]_\alpha\phi_n^*\phi_m
  \\
  &+\frac1{\sqrt{2}}\sum_n\left[
    (D_\nu\phi)_n^*
      (\sigma^\mu\bar\sigma^\nu\psi_n)_\alpha
...
\begin{equation}
  \begin{aligned}
  S^\mu_\alpha=N^\mu_\alpha+K^\mu_\alpha
  ={}&
  -\ii\sum_Af_{A\rho\sigma}
    (\sigma^{\rho\sigma}\sigma^\mu\bar\lambda_A)_\alpha
  -\sum_{Anm}(t_A)_{nm}
    (\sigma^\mu\bar\lambda_A)_\alpha\phi_n^*\phi_m
  \\
  &+\frac1{\sqrt{2}}\sum_n\left[
    -(D_\nu\phi)_n^*
      (\sigma^\nu\bar\sigma^\mu\psi_n)_\alpha
...
\begin{equation}
  ...
  \left[
    h_{AB}(\Phi)
    \left(W_A^\alpha W_{B\alpha}\right)
  \right]_{\mathcal{F}}\,,
  \end{aligned}
...
\[
  ...
  -\ii\theta^2(\sigma^\mu\partial_\mu\bar\lambda_A)_\alpha(x_+)
  -\ii\theta_\alpha D_A(x_+),
  \\
  \Phi_n(x,\theta,\bar\theta)={}&
  \phi_n(x_+)
  -\sqrt{2}\theta\psi_n(x_+)
  +\theta^2\mathcal{F}_n(x_+),
  \end{aligned}
\]
where \(x_+^\mu\) is the shifted coordinate \eqref{eq:26.3.23}. The terms of
second order in \(\theta\) (and independent of \(\bar\theta\)) in
\(\sum_{AB}h_{AB}(\Phi)W_A^\alpha W_{B\alpha}\) are then
\[
  \begin{aligned}
  -\left[
    \sum_{AB}h_{AB}(\Phi)
    W_A^\alpha W_{B\alpha}
  \right]_{\theta^2}
  ={}&
  \theta^2\sum_{AB}\left(\lambda_A\lambda_B\right)
  \left[
    \frac12\sum_{nm}
      \left(\psi_n\psi_m\right)
  ...
  \sum_{AB}h_{AB}(\phi)
  \left[
    2\ii\lambda_A\sigma^\mu\partial_\mu\bar\lambda_B
    -\frac12f_{A\mu\nu}f_B^{\mu\nu}
  \right.
  \\
  &\hspace{47mm}\left.
  ...
  &+\sqrt{2}\sum_{ABn}
  \frac{\partial h_{AB}(\phi)}{\partial\phi_n}
  \left(\theta\psi_n\right)
  \left[
    2\left(\lambda_B\sigma^{\mu\nu}\theta\right)f_{A\mu\nu}
  \right.
  \\
  &\hspace{54mm}\left.
    +2\ii\left(\lambda_B\theta\right)D_A
  \right],
  \end{aligned}
\]
...
writing \(\theta_\alpha\theta_\beta\) as
\(\tfrac12\epsilon_{\alpha\beta}
 \theta^2\), the third term on the right-hand
...
\[
  ...
  &\left(\theta\psi_n\right)
  \left[
    -\left(\lambda_B\sigma^\mu\bar\sigma^\nu\theta\right)f_{A\mu\nu}
    -2\ii\left(\lambda_B\theta\right)D_A
  \right]
  \\
  &\qquad
  =\frac12\theta^2
  \left[
    -\left(\lambda_B\sigma^\mu\bar\sigma^\nu\psi_n\right)f_{A\mu\nu}
    -2\ii\left(\lambda_B\psi_n\right)D_A
  \right].
  \end{aligned}
\]
The \(\mathcal{F}\)-term is the coefficient of \(\theta^2\), so
\[
  \begin{aligned}
  -\left[
    \sum_{AB}h_{AB}(\Phi)
  ...
  \right]_{\mathcal{F}}
  ={}&
  \sum_{AB}\left(\lambda_A\lambda_B\right)
  \left[
    \frac12\sum_{nm}
      \left(\psi_n\psi_m\right)
      \frac{\partial^2h_{AB}(\phi)}
...
\begin{equation}
  ...
  \left[
    \ii\bar\psi_m\bar\sigma^\mu(D_\mu\psi)_n
  ...
  &+\ii\sqrt{2}
    ...
    \left[
      (t_A\phi)_i(\bar\psi_j\bar\lambda_A)
      -(\phi^*t_A)_j(\psi_i\lambda_A)
    \right]
  \\
  &-\operatorname{Re}\sum_{nm\ell}
    \frac{\partial^3K(\phi,\phi^*)}
         {\partial\phi_n\partial\phi_m\partial\phi_\ell^*}
    \left(\psi_n\psi_m\right)\mathcal{F}_\ell^*
  \\
  &+\operatorname{Re}\sum_{nm\ell}
    \frac{\partial^3K(\phi,\phi^*)}
         {\partial\phi_n\partial\phi_m\partial\phi_\ell^*}
    \left[-\ii\psi_m\sigma^\mu\bar\psi_\ell\right]D_\mu\phi_n
  \\
  &+\frac14\sum_{nm\ell k}
    ...
    \left(\psi_n\psi_m\right)
    \left(\bar\psi_k\bar\psi_\ell\right)
  \\
  &-\operatorname{Re}\sum_{nm}
    \frac{\partial^2f(\phi)}{\partial\phi_n\partial\phi_m}
    \left(\psi_n\psi_m\right)
  ...
  &-\frac12\operatorname{Re}\sum_{ABn}
    \left(\lambda_A\lambda_B\right)\mathcal{F}_n
    \frac{\partial h_{AB}(\phi)}{\partial\phi_n}
  \\
  &+\operatorname{Re}\sum_{AB}h_{AB}(\phi)
  \left[
    +\ii\lambda_A\sigma^\mu(D_\mu\bar\lambda)_B
    -\frac14f_{A\mu\nu}f_B^{\mu\nu}
