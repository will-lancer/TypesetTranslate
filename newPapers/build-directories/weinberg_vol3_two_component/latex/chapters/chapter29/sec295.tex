...
% Source-visible anomalies preserved: the prose before Eq. (29.5.9)
% calls I_f the number of external fermion lines, although Eq. (29.5.11)
% uses E_f for external lines; the prose before Eq. (29.5.49) gives
% C_1=2e, without a square; the additive-constant footnote states
% -ih(ia)=h(a)-1; Eq. (29.5.75) omits an expected factor (u+u_0)
% outside the logarithm; Eq. (29.5.79), and the following prose, give
% 4/sqrt(pi); and the final discussion at u=-1 says the differences are
% O(u-1). Uncertainties: none.
...
\begin{equation}
  ...
  +i\sum_A\bar\psi_A\bar\sigma^\mu(D_\mu\psi)_A
  \\
...
\begin{equation}
  ...
  -\frac14 f_{\mu\nu}f^{\mu\nu}
  \\
  &+i\bar\lambda\bar\sigma^\mu\partial_\mu\lambda
  \Biggr]
  +\frac{\theta}{64\pi^2}
    \epsilon_{\mu\nu\rho\sigma}f^{\mu\nu}f^{\rho\sigma}\,,
  \end{aligned}
...
\begin{equation}
  ...
  -\frac12\operatorname{Re}\left\{
    \frac{\partial^3K(a,a^*)}{\partial^2a\,\partial a^*}
    \left(2i\psi\sigma^\mu\bar\psi\right)\partial_\mu a
  \right\}
  \\
  &+\frac14
  ...
  &+\frac14\operatorname{Re}\left\{
    ...
    \left(\psi^\beta\psi_\beta\right)\frac{d^2T(a)}{da^2}
  \right\}
  -\frac12\operatorname{Re}\left\{
    \left(\lambda^\alpha\lambda_\alpha\right)
    \mathcal{F}\frac{dT(a)}{da}
  \right\}
  \\
  &+\operatorname{Re}\left\{
    T(a)\left[
  ...
  &+\frac{\sqrt{2}}{4}\operatorname{Re}\left\{
    \frac{dT(a)}{da}\left[
      ...
      +2i\left(\lambda^\alpha\psi_\alpha\right)D
    \right]
  \right\}.
  \end{aligned}
...
\(i\bar\psi\bar\sigma^\mu\partial_\mu\psi\) and
\(i\bar\lambda\bar\sigma^\mu\partial_\mu\lambda\) should be equal is
\begin{equation}
  \frac{\partial^2K(a,a^*)}{\partial a\,\partial a^*}
  =\operatorname{Re}T(a).
...
\begin{equation}
  ...
  ={}&\frac{1}{4\pi}\operatorname{Im}\Biggl\{
    \left[\frac{dh}{da}\right]\Biggl[
      i\psi\sigma^\mu\partial_\mu\bar\psi
      +i\lambda\sigma^\mu\partial_\mu\bar\lambda
  \\
  &\hspace{31mm}
      -\partial_\mu a\,\partial^\mu a^*
  ...
  &+\frac{1}{4\pi}\operatorname{Im}\Biggl\{
    \left[\frac{d^2h}{da^2}\right]\Biggl[
      ...
      +\frac{\sqrt{2}}{2}i\left(\lambda^\alpha\psi_\alpha\right)D
  \\
  &\hspace{43mm}
      +\frac{\sqrt{2}}{2}
...
\eqref{eq:27.2.15}
\begin{equation}
  W_\alpha=\frac{i}{4}\bar D^2D_\alpha V\,,
...
\begin{equation}
  \widetilde W_\alpha
  =\frac{i}{4}\bar D^2D_\alpha\widetilde V.
...
\begin{equation}
  \theta\longrightarrow\exp(i\alpha)\theta\,,
  \qquad
  W_{A\alpha}\longrightarrow\exp(i\alpha)W_{A\alpha}\,,
  \qquad
...
\begin{equation}
  \psi_{A\alpha}\longrightarrow\sqrt{i}\,\psi_{A\alpha}\,,
  \qquad
  \lambda_{A\alpha}\longrightarrow\sqrt{i}\,\lambda_{A\alpha}\,,
  \qquad
...
\(-ih(ia)=h(a)-1\), which is a special case with \(b=-1\) of the
...
\(\mathbb{Z}_8\) transformations \(\phi_A\longrightarrow\pm i\phi_A\), and
...
\begin{equation}
  ...
  \qquad
  a_D\longrightarrow\frac{i}{\pi}\left[
...
\(\ln(2u/\Lambda^2)\) is shifted by \(2i\pi\), while \(\sqrt{2u}\) changes
...
\[
  h'(a)=4\pi i e^{-2}(a)
  =-\frac{i}{\pi}\ln\left(\frac{a}{\text{constant}}\right).
\]
...
\begin{equation}
  \frac{d\widehat h(\widehat a)}{d\widehat a}
  \longrightarrow-\frac{i}{\pi}
  \ln\left(\frac{\widehat a}{\text{constant}}\right),
  \tag{29.5.60}\label{eq:29.5.60}
\end{equation}
or, in other words,
\begin{equation}
  \frac{da}{d\bigl(h(a)+Na\bigr)}
  \longrightarrow+\frac{i}{\pi}
...
The solution is
\begin{equation}
  a(u)=a_0+\frac{i}{\pi}\bigl(h(u)+Na(u)\bigr)
...
\begin{equation}
  a(u)\longrightarrow-ia_0+\frac{c_0}{\pi}
  \ln\left(\frac{-c_0(u+u_0)}{\Lambda_0}\right).
...
\begin{equation}
  a_{\mathrm{SW}}(u)
  =\frac{\sqrt{2}}{\pi}\int_{-1}^{1}dx\,
    \sqrt{\frac{u-x}{1-x^2}}\,,
  \tag{29.5.77}\label{eq:29.5.77}
\end{equation}
\begin{equation}
  h_{\mathrm{SW}}(u)
  =\frac{i\sqrt{2}}{\pi}\int_1^u dx\,
    \sqrt{\frac{u-x}{x^2-1}}.
...
\begin{equation}
  ...
  a_{\mathrm{SW}}(u)
  &\longrightarrow
  \frac{\sqrt{2}}{\pi}\int_{-1}^{1}\frac{dx}{\sqrt{x+1}}
  +\frac{u-1}{2\pi}\int_{-1}^{1}
    \frac{dx}{\sqrt{(1-x)(u-x)}}
  \\
  &=\frac{4}{\sqrt{\pi}}
  ...
  \\
  &\longrightarrow
  \frac{4}{\sqrt{\pi}}-\frac{u-1}{2\pi}\ln\bigl(4(u-1)\bigr).
  \end{aligned}
...
\begin{equation}
  h_{\mathrm{SW}}(u)
  \longrightarrow\frac{i}{\pi}\int_1^u dx\,
  \sqrt{\frac{u-x}{x-1}}
  =\frac{i(u-1)}{2}.
  \tag{29.5.80}\label{eq:29.5.80}
\end{equation}
Eqs.~\eqref{eq:29.5.79} and \eqref{eq:29.5.80} agree with our previous
results \eqref{eq:29.5.71} and \eqref{eq:29.5.72}, with \(c_0=i\) and
\(a_0=4/\sqrt{\pi}\). Eq.~\eqref{eq:29.5.77} satisfies the
...
\begin{equation}
  h_{\mathrm{SW}}(u)
  \longrightarrow\frac{i\sqrt{2u}}{\pi}\int_1^u
    \frac{dx}{\sqrt{x^2-1}}
  \longrightarrow\frac{i\sqrt{2}}{\pi}\sqrt{u}\ln u.
