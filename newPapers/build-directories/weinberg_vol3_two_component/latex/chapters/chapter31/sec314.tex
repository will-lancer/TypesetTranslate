...
% supersymmetric standard model.'' In accordance with NOTATION.md, direct
% products of gauge groups are set with centered dots rather than the
% multiplication signs visible in the scan.
% Uncertainties: none.
...
\(SU(3)\mathbin{\cdot}SU(2)\mathbin{\cdot}U(1)\) quantum numbers of the
...
effects in the minimum supersymmetric standard model, those of first order
...
\begin{equation}
  ...
  =\frac{2\mathcal M}{3i}\frac{\partial}{\partial\mathcal M}
   [f(\Phi,W)]_{\theta=0}
  =\frac{2\mathcal M}{3i}
   \frac{\partial f(\phi,\lambda)}{\partial\mathcal M}.
...
\begin{equation}
  ...
  \left[
    \widetilde m_g^*
    \frac{\mathcal M\,\partial f(\phi,\lambda)}
         {\partial\mathcal M}
  \right].
...
\begin{equation}
  ...
  \sum_A[W_A^\alpha W_{A\alpha}]_{\mathcal F}.
...
\begin{equation}
  ...
  \left[
    \widetilde m_g^*
    \sum_A\lambda_A^\alpha\lambda_{A\alpha}
  \right],
  \tag{31.4.10}\label{eq:31.4.10}
\end{equation}
where \(\lambda_{A\alpha}\) is the Weyl gaugino field, normalized like
\(W_{A\alpha}\) by multiplication with the gauge coupling
...
\(\frac12\sum_A\lambda_A^\alpha\lambda_{A\alpha}\), or
\begin{equation}
  m_{\mathrm{gaugino}}
...
\(-\mu(\mathcal H_2^{\mathrm T}e\mathcal H_1)\) in
Eq.~\eqref{eq:28.1.7}, so there is a term in the Lagrangian given by
Eq.~\eqref{eq:31.4.7} as
\(2\operatorname{Re}
[\widetilde m_g^*\mu(\mathcal H_2^{\mathrm T}e\mathcal H_1)]\).
...
\([\Phi_r^\dagger e^{-V}\Phi_r]_D\) for any left-chiral superfield \(\Phi_r\).
