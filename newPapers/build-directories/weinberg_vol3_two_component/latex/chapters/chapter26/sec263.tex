...
\begin{equation}
  \delta C=\ii(\xi\omega-\bar\xi\bar\omega)\,,
  \tag{26.3.3}\label{eq:26.3.3}
\end{equation}
\begin{equation}
  \delta\omega_\alpha
  =(-M+\ii N)\xi_\alpha
   -(\sigma^\mu\bar\xi)_\alpha\partial_\mu(C+\ii Z)\,,
...
\begin{equation}
  C=A,\qquad
  \omega_\alpha=-\ii\psi_\alpha,\qquad
  \bar\omega_{\dot\alpha}
    =\ii\widetilde{\bar\psi}_{\dot\alpha},\qquad
  M=G,\qquad
  N=-F,\qquad
  Z=B\,.
...
\begin{equation}
  ...
  +\ii\theta\sigma^\mu\bar\theta\,\partial_\mu\widetilde{\phi}\\
  &+\frac{\ii}{\sqrt{2}}\bar\theta\bar\theta\,
    \theta\sigma^\mu\partial_\mu\widetilde{\bar\psi}
...
\begin{equation}
  \delta\psi_\alpha
  =-\ii\sqrt{2}\,\sigma^\mu_{\alpha\dot\alpha}
...
\begin{equation}
  \delta\mathcal{F}
  =-\ii\sqrt{2}\bar\xi\bar\sigma^\mu\partial_\mu\psi\,,
...
\begin{equation}
  ...
  =+\ii\sqrt{2}\,\xi^\alpha\sigma^\mu_{\alpha\dot\alpha}
...
reason, any superfield that \dd epends only on \(\theta\) and \(x_+^\mu\), but
not otherwise on \(\bar\theta\), must take the form
\eqref{eq:26.3.21}, and any superfield that \dd epends only on \(\bar\theta\)
...
entirely a matter of what the superfield is allowed to \dd epend on. It follows
...
a more formal way. Because \(\Phi\) \dd epends on \(\bar\theta\) only through
\(x_+\), and \(\widetilde{\Phi}\) \dd epends on \(\theta\) only through
\(x_-\), they satisfy
\begin{equation}
...
\begin{equation}
  ...
  -\ii\theta^\beta\sigma^\mu_{\beta\dot\alpha}\partial_\mu\,,
  \tag{26.3.25}\label{eq:26.3.25}
\end{equation}
\begin{equation}
  D_\alpha
  =-\frac{\partial}{\partial\theta^\alpha}
  +\ii\sigma^\mu_{\alpha\dot\beta}\bar\theta^{\dot\beta}\partial_\mu\,,
...
$\Phi$ by a \dd erivative, so that the integral of the $\mathcal{F}$-term of any
...
On what can $f$ and $K$ \dd epend? The function $f$ will be left-chiral if it
\dd epends only on left-chiral elementary superfields $\Phi_n$, but not their
...
\]
so, apart from spacetime \dd erivatives that \dd o not contribute to the action,
\((\bar D_{\dot\alpha}\bar D^{\dot\alpha})h\) is the coefficient of
\(-\bar\theta\bar\theta/4\) in \(h\). But, again apart from spacetime
\dd erivatives, $[f]_{\mathcal{F}}$ is the coefficient of
\(\theta\theta\) in \(f\), so
...
\end{equation}
Thus we \dd o not need to include terms in $f$ that \dd epend on left-chiral
...
superderivatives or spacetime \dd erivatives, it is known as the
\emph{superpotential}.
...
$\Phi_n^*$, as well as their superderivatives and spacetime \dd erivatives,
...
generality in supposing $K$ to \dd epend only on left-chiral superfields and
their complex conjugates.) However, not all $K$s obtained in this way will
yield \dd istinct actions. For one thing, chiral superfields have no $D$-terms,
so two $K$s that \dd iffer by a chiral superfield make the same contribution to
the action.
...
action because
\begin{equation}
  \int \dd^4x\,[D_\alpha S]_D
  =\int \dd^4x\,[\bar D_{\dot\alpha}S]_D=0\,.
  \tag{26.3.32}\label{eq:26.3.32}
\end{equation}
Using the \dd efinition \eqref{eq:26.2.26}, the two covariant \dd erivatives act
explicitly as
\[
  D_\alpha S
  =-\frac{\partial S}{\partial\theta^\alpha}
    +\ii\sigma^\mu_{\alpha\dot\alpha}\bar\theta^{\dot\alpha}
      \partial_\mu S,
  \qquad
  \bar D_{\dot\alpha}S
  =\frac{\partial S}{\partial\bar\theta^{\dot\alpha}}
    -\ii\theta^\alpha\sigma^\mu_{\alpha\dot\alpha}\partial_\mu S.
\]
Since \(S\) is a polynomial at most of second order in each of
\(\theta,\bar\theta\), the fermionic-coordinate \dd erivative in
\(D_\alpha S\) is at most cubic in the combined coordinates and
therefore cannot have a non-zero $D$-term that is not a \dd erivative, while the
remaining term in \(D_\alpha\) is a spacetime \dd erivative, so its \(D\)-term is
also a
spacetime \dd erivative, and therefore neither the first nor the second terms in
\(D_\alpha S\) can contribute to the integral in
Eq.~\eqref{eq:26.3.32}. Also, the superderivative acts \dd istributively, so it
follows from Eq.~\eqref{eq:26.3.32} that we can integrate by parts in
superspace: for any two bosonic superfields $S_1$ and $S_2$,
\begin{equation}
  \int \dd^4x\,[S_1D_\alpha S_2]_D
  =-\int \dd^4x\,[S_2D_\alpha S_1]_D\,,
  \tag{26.3.33}\label{eq:26.3.33}
\end{equation}
In Sections 26.4 and 26.8 we will consider in \dd etail the case where $f$ and
$K$ \dd epend only on elementary superfields, but not their superderivatives or
ordinary \dd erivatives.
...
\((\theta,\bar\theta)\rightarrow(-\ii\bar\theta,-\ii\theta)\), with spinor
...
\end{equation}
while \(\theta\rightarrow-\ii\sigma^0\bar\theta\) and
\(\bar\theta\rightarrow-\ii\bar\sigma^0\theta\). Thus space inversion takes left-chiral
...
annihilation operators for the same particles that are created and \dd estroyed
...
\begin{equation}
  ...
  =\Phi^\dagger(\Lambda_Px,-\ii\bar\theta,-\ii\theta)\,.
...
important in some models of spontaneous supersymmetry breaking \dd iscussed in
...
superpotential itself must have $R_f=+2$, so if $f$ \dd epends on a single
...
in order for the term $\int d^4x\,[g]_D$ in the action to conserve $R$ \ii t is
...
superfields. To learn the conditions \dd efining this sort of superfield, we note
...
\begin{equation}
  \omega'_\alpha
  =\lambda_\alpha-\ii\sigma^\mu_{\alpha\dot\alpha}
...
\end{equation}
A multiplet $S$ is said to be linear if the superfield $S'$ \dd efined in this
way vanishes
\begin{equation}
  \left(D^\alpha D_\alpha
