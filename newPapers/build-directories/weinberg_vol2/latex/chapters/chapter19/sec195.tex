...
=\boldsymbol{\theta}\times\boldsymbol{\zeta},
\qquad
\delta\sigma=0,
...
=2(\boldsymbol{\zeta}\times\boldsymbol{\epsilon})
\times\boldsymbol D_\mu
...
The transformation rules \eqref{eq:19.5.12} and \eqref{eq:19.5.14} specify what is called a \emph{non-linear realization} of the group $SU(2)\times SU(2)$~\hyperref[ch19-ref-25]{[25]}. The general theory of non-linear realizations of Lie groups is given in the next section; we shall show there that, up to field redefinitions, the transformation rules \eqref{eq:19.5.12} and \eqref{eq:19.5.14} provide the most general realization of $SU(2)\times SU(2)$ in which the isospin $SU(2)$ subgroup is realized linearly on $\boldsymbol{\zeta}$.
...
The factor $1/F$ acts as a coupling parameter that accompanies the interaction of each additional pion. Eq.~\eqref{eq:19.5.18} describes what is often called the `non-linear $\sigma$-model,' for the special case of $SU(2)\times SU(2)$ spontaneously broken to $SU(2)$.
...
\begin{equation}
S=\ii(2\pi)^4\delta^4(p_A+p_B-p_C-p_D)M
...
=-\overline N\left(
\sl{\partial}
+g[\phi_4+2\ii\boldsymbol t\cdot\boldsymbol{\phi}\gamma_5]
\right)N,
\tag{19.5.39}\label{eq:19.5.39}
\end{equation}
where $\boldsymbol t$ is the isospin matrix vector for isospin $1/2$ (that is, half the Pauli spinor $\boldsymbol{\tau}$.) This is invariant under chiral $SU(2)\times SU(2)$ transformations
\begin{equation}
\delta\boldsymbol{\phi}=2\boldsymbol{\epsilon}\phi_4,
\qquad
\delta\phi_4=-2\boldsymbol{\epsilon}\cdot\boldsymbol{\phi},
...
\end{equation}
\begin{equation}
\delta N=-2\ii\gamma_5\boldsymbol{\epsilon}\cdot\boldsymbol t\,N.
...
\begin{equation}
N\equiv
\frac{1-2\ii\gamma_5\boldsymbol t\cdot\boldsymbol{\zeta}}
...
\[
\overline N[\phi_4+2\ii\boldsymbol t\cdot\boldsymbol{\phi}\gamma_5]N
...
\left[
\sl{\partial}+g\sigma
+2\ii\frac{\boldsymbol t\cdot
(\boldsymbol{\zeta}\times\sl{\partial}\boldsymbol{\zeta})}
{1+\boldsymbol{\zeta}^2}
+2\ii\gamma_5\boldsymbol t\cdot\sl{\boldsymbol D}
\right]\widetilde N
...
\left[
\sl{\partial}+g\sigma
+\frac{2\ii\boldsymbol t\cdot
(\boldsymbol{\pi}\times\sl{\partial}\boldsymbol{\pi})}
{F_\pi^2[1+\boldsymbol{\pi}^2/F_\pi^2]}
+\frac{2\ii\gamma_5\boldsymbol t\cdot
...
\partial_\mu
+2\ii\frac{\boldsymbol t\cdot
(\boldsymbol{\zeta}\times\partial_\mu\boldsymbol{\zeta})}
{1+\boldsymbol{\zeta}^2}
\right]\widetilde N.
...
\right)\gamma_5
+\boldsymbol t\cdot
(\boldsymbol{\phi}\times\sl{\partial}\boldsymbol{\phi})
\right]N.
...
\[
\mathcal L'_N
=-8\ii\sigma^2g'\overline{\widetilde N}
\boldsymbol t\cdot\sl{\boldsymbol D}\gamma_5\widetilde N.
\]
Since $\sigma$ has a non-vanishing vacuum expectation value, this makes a contribution to the pion-nucleon coupling constant and to $g_A$ proportional to $g'$. Hence the value $g_A=1$ is \emph{not} dictated by the broken $SU(2)\times SU(2)$ symmetry alone; by including the interaction \eqref{eq:19.5.50} and adjusting $g'$, we can make $g_A$ anything we like.
...
\begin{equation}
\frac{-\ii(\sl{p}+\sl{q})+m_N}{(p+q)^2+m_N^2}
\underset{q\to0}{\longrightarrow}
\frac{-\ii\sl{p}+m_N}{2p\cdot q}.
...
\input{chapters/chapter19/figures/fig19-03.tex}
...
\frac{-\ii(\sl{p}\mathbin{\pm}\sl{q})+m_N}
{(p\mathbin{\pm}q)^2+m_N^2}
\gamma_5\sl{q}\,u
\\
&\quad=
\frac{m_\pi^2\overline u\gamma_5
\{-\ii\sl{p}(1\mathbin{\pm}m_\pi/m_N)+m_N\}
\gamma_5u}
{\mp2m_\pi m_N-m_\pi^2}
\\
&\quad=
\frac{\mp m_\pi}{2m_N\mathbin{\pm}m_\pi}
\overline u\{\ii\sl{p}(1\mathbin{\pm}m_\pi/m_N)+m_N\}u
=\frac{m_\pi^2}{2m_N\mathbin{\pm}m_\pi},
\end{align*}
where $u$ is a Dirac spinor with $\overline uu=1$. In contrast, the diagram of Figure~\ref{fig:19.3}(c) gives a contribution to $M$ of order $m_\pi$. We may write this amplitude as a $2\times2$ matrix in the nucleon isospin indices:
\[
M_{ba}
=-\frac{2\ii}{F_\pi^2}t_c\epsilon_{abc}
\overline u(-\ii\sl{q}-\ii\sl{q}')u,
\]
where $q'$ and $q$ are the final and initial pion four-momenta and $b$ and $a$ are the corresponding isovector indices. Using \eqref{eq:19.5.57} and the momentum-space Dirac equation $(\ii\sl{p}+m_N)u=0$, this becomes
\begin{equation}
M_{ba}
=-\frac{4\ii m_\pi}{F_\pi^2}t_c\epsilon_{abc}
=-\frac{4m_\pi}{F_\pi^2}
\boldsymbol t\cdot[\boldsymbol t^{(\pi)}]_{ba},
\tag{19.5.58}\label{eq:19.5.58}
\end{equation}
where $[t_c^{(\pi)}]_{ba}\equiv-\ii\epsilon_{bac}$ is the pion isovector matrix. The matrix $\boldsymbol t\cdot\boldsymbol t^{(\pi)}$ has eigenvalues in states of total isospin $T$ equal to $\tfrac12[T(T+1)-2-\tfrac34]$, so in the two isospin states, $T=1/2$ and $T=3/2$, the invariant amplitudes are~\hyperref[ch19-ref-30]{[30]}
\[
...
-4\ii\left(\frac{\boldsymbol{\zeta}}
{1+\boldsymbol{\zeta}^2}\right)
...
-\ii\left(\frac{\zeta_3}{1+\boldsymbol{\zeta}^2}\right)
...
\notag\\
&\qquad
\left(-\ii\frac{(2\pi)^4\,2g_A\gamma_5t_b\sl{q}}{F_\pi}\right)
\left(\frac{-\ii}{(2\pi)^4}
\frac{-\ii(\sl{p}+\sl{q})+m_N}{(p+q)^2+M^2}\right)
\left(\ii\frac{(2\pi)^4\,2g_A\gamma_5t_a\sl{q}}{F_\pi}\right)
\notag\\
&\qquad+
\left(\ii\frac{(2\pi)^4\,2g_A\gamma_5t_a\sl{q}}{F_\pi}\right)
\left(\frac{-\ii}{(2\pi)^4}
...
\left(-\ii\frac{(2\pi)^4\,2g_A\gamma_5t_b\sl{q}}{F_\pi}\right)
\notag\\
&\qquad-\frac{4\ii t_c\epsilon_{abc}\sl{q}}{F_\pi^2}
\Bigg]u.
...
\begin{align}
f_{\pi^+p}(\omega)
={}&R+\frac{\ii\omega}{4\pi}\sigma_{\pi^+p}(\omega)
\notag\\
...
[\sigma_{\pi^+p}(E)-\sigma_{\pi^-p}(E)]\,\dd E/E.
