...
$A_\alpha^\mu(x)$ (such as the $u$ and $d$ quark fields interacting with
...
the non-chiral generators $t_\alpha$, is not spoiled by any anomalies.
...
$A_{\alpha\mu}(x)$.) Rescaling the momentum $k^\mu$ by a factor $M$, this
is
\begin{equation}
...
\begin{equation}
 ...
 &=\frac14\{(D_x)^\mu,(D_x)^\nu\}\{\gamma_\mu,\gamma_\nu\}
 +\frac14[(D_x)^\mu,(D_x)^\nu][\gamma_\mu,\gamma_\nu]\\
 &=D_x^2-\frac14it_\alpha F_\alpha^{\mu\nu}
 [\gamma_\mu,\gamma_\nu].
 \end{aligned}
...
\begin{equation}
 ...
 F_\alpha^{\mu\nu}(x)F_\beta^{\rho\sigma}(x)
 \operatorname{tr}\{t_\alpha t_\beta t\},
...
$\operatorname{tr}\{t_\alpha t_\beta t\}$ is proportional to
$\delta_{\alpha\beta}$:
\begin{equation}
 \operatorname{tr}\{t_\alpha t_\beta t\}=N\delta_{\alpha\beta}.
...
$2\cdot2$ matrix with elements $+1$ for $u$ and $-1$ for $d$. If we
...
constant $g$ in \hyperref[eq:22.2.1]{Eq.~(22.2.1)} must have the value
\begin{equation}
...
$A_{4\alpha}=iA_\alpha^0$. The spacetime volume is then expressed as
...
\hyperref[eq:22.2.13]{(22.12.13)} for $\mathcal A(x)$. One great advantage
of the Euclidean approach is that with $x_4$ and $A_{4\alpha}$ real, the
Dirac operator $i\sl{D}$ in Eq.~\eqref{eq:22.2.13} is Hermitian:
\begin{equation}
 i\sl{D}=[i\partial_i+t_\alpha A_{i\alpha}]\gamma_i,
...
\end{equation}
where `$\mathbf{1}$' is the $4\cdot4$ unit matrix. Therefore the anomaly
...
\begin{equation}
 ...
 F_{ij\alpha}F_{kl\beta}\operatorname{tr}\{t_\alpha t_\beta t\},
...
orthornormal eigenfunctions $\varphi_u,\varphi_v$ of $i\sl{D}$ with
...
\hyperref[eq:22.2.44]{Eq.~(2.2.44)} then becomes
\begin{equation}
 \mathcal A(x)=-2\left[
 \sum_u t_u\bigl(\varphi_u^\dagger(x)\varphi_u(x)\bigr)
 -\sum_v t_v\bigl(\varphi_u^\dagger(x)\varphi_v(x)\bigr)
 \right].
...
\begin{equation}
 ...
 F_{\alpha ij}F_{\beta kl}\operatorname{tr}[t_\alpha t_\beta]
 =n_+-n_-,
