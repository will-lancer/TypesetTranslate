...
\end{equation}
and $\gamma_4\equiv \ii\gamma^0$ is the matrix used to define
...
\begin{equation}
 ...
 =\ii\alpha(x)[\gamma_5t]_{nm}\delta^4(x-y).
...
\begin{equation}
 [d\psi][d\bar\psi]\longrightarrow
 \exp\left\{\ii\int \dd^4x\,\alpha(x)\mathcal A(x)\right\}
 [d\psi][d\bar\psi],
...
weighted with a factor $\exp\{\ii\int \dd^4x\,\mathcal L(x)\}$, so the
factor\linebreak
$\exp\{\ii\int \dd^4x\,\alpha(x)\mathcal A(x)\}$ in the transformation rule
...
\[
 ...
 &=-2\int\frac{\dd^4k}{(2\pi)^4}
 \operatorname{Tr}\left\{\gamma_5t
 f\left(-[\ii\sl{k}+\sl{D}_x]^2/M^2\right)\right\}.
 \end{aligned}
\]
...
is
\begin{equation}
 \mathcal A(x)=-2M^4\int\frac{\dd^4k}{(2\pi)^4}
 \operatorname{Tr}\left\{\gamma_5t
 f\left(-[\ii\sl{k}+\sl{D}_x/M]^2\right)\right\}.
 \tag{22.2.17}\label{eq:22.2.17}
\end{equation}
The argument of the cut-off function may be written
\begin{equation}
 -\left[\ii\sl{k}+\frac{\sl{D}_x}{M}\right]^2
 =k^2-\frac{\ii k\cdot D_x}{M}
...
$f(-[\ii\sl{k}+\sl{D}_x/M]^2)$ that have no more than four factors of
...
\begin{equation}
 \mathcal A(x)=-\int\frac{\dd^4k}{(2\pi)^4}f''(k^2)
...
\begin{equation}
 \int \dd^4k\,f''(k^2)
 =\ii\int_0^\infty 2\pi^2\kappa^3\,\dd\kappa\,f''(\kappa^2).
...
\begin{equation}
 \int \dd^4k\,f''(k^2)
 =\ii\pi^2\int_0^\infty \dd s\,s f''(s)
 =-\ii\pi^2\int_0^\infty \dd s\,f'(s)
 =\ii\pi^2.
...
$\psi(x)\to\psi(x)+\ii t\gamma_5\alpha\psi(x)$ with constant infinitesimal
...
may be written as
$\delta I=\int \dd^4x\,J_5^\mu(x)\partial_\mu\alpha(x)$, where $J_5^\mu(x)$
...
$\delta\psi(x)=\ii t\gamma_5\alpha(x)\psi(x)$, the change in the path integral
over the fermionic fields is
\begin{equation}
 \begin{aligned}
 \delta\int[d\psi][d\bar\psi]e^{\ii I}
 =\ii\int \dd^4x\int[d\psi][d\bar\psi]\,
 \bigl[\mathcal A(x)\alpha(x)
 +J_5^\mu(x)\partial_\mu\alpha(x)\bigr]e^{\ii I}.
 \end{aligned}
...
\begin{equation}
 ...
 \frac{\int[d\psi][d\bar\psi]e^{\ii I}\mathcal O}
 {\int[d\psi][d\bar\psi]e^{\ii I}}.
...
\begin{equation}
 \delta u=\ii\alpha\gamma_5u,\qquad
 \delta d=-\ii\alpha\gamma_5d.
...
fourth coordinate $x_4=\ii x^0=-\ii x_0$, and correspondingly
$\partial_4=-\ii\partial_0$, $\gamma_4=\ii\gamma^0$, and
$A_{4a}=\ii A_a^0$. The spacetime volume is then expressed as
$\dd^4x=-\ii(\dd^4x)_E$, where $(\dd^4x)_E$ is the Euclidean volume element
$(\dd^4x)_E=\dd x_1\dd x_2\dd x_3\dd x_4$. In Euclidean spacetime the fields $\psi(x)$
and $\bar\psi(x)$ must be regarded as entirely independent, with local
chiral transformations defined by
$\delta\psi(x)=\ii\alpha(x)t\gamma_5\psi(x)$,
$\delta\bar\psi(x)=-\ii\alpha(x)\bar\psi(x)t\gamma_5$. The transformation of
...
Dirac operator $\ii\sl{D}$ in Eq.~\eqref{eq:22.2.13} is Hermitian:
\begin{equation}
 \ii\sl{D}=[\ii\partial_i+t_a A_{ia}]\gamma_i,
...
\begin{equation}
 \ii\sl{D}\varphi_\kappa=\lambda_\kappa\varphi_\kappa,
 \tag{22.2.41}\label{eq:22.2.41}
\end{equation}
\begin{equation}
 \int(\dd^4x)_E\,\varphi_\kappa(x)^\dagger\varphi_{\kappa'}(x)
 =\delta_{\kappa\kappa'},
 \tag{22.2.42}\label{eq:22.2.42}
\end{equation}
with real eigenvalues $\lambda_\kappa$. We also are assuming, as throughout
this section, that $t$ commutes with $\ii\sl{D}$, so that we can choose the
...
\[
 ...
 =16\epsilon^E_{ijkl},
\]
together with another factor $i$ from the replacement of $\dd^4k$ with
$(\dd^4k)_E$ in Eq.~\eqref{eq:22.2.20}.)

Now, given any eigenfunction $\varphi_\kappa(x)$ of $\ii\sl{D}$ and $t$ with
...
$\gamma_5=-\ii\gamma_1\gamma_2\gamma_3\gamma_0
...
$\gamma_5$ anticommutes with $\ii\sl{D}$, they can be chosen as simultaneous
orthonormal eigenfunctions $\varphi_u,\varphi_v$ of $\ii\sl{D}$ with
eigenvalue zero \emph{and} of $\gamma_5$ with eigenvalues $+1$ and $-1$,
respectively.
\begin{equation}
 \begin{aligned}
 \ii\sl{D}\varphi_u&=0,&\qquad \gamma_5\varphi_u&=\varphi_u,\\
 \ii\sl{D}\varphi_v&=0,& \gamma_5\varphi_v&=-\varphi_v.
 \end{aligned}
...
modes of the operator $\ii\sl{D}$, respectively. In particular, for the case
...
\begin{equation}
 -\frac{1}{32\pi^2}\int(\dd^4x)_E\,\epsilon^E_{ijkl}
