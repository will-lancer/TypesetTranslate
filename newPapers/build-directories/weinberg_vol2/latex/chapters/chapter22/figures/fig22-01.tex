...
\begin{figure}[htbp]
 ...
 \begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,
  ...
  \node[anchor=south] at (-5.72,0.18) {$j_a^\mu(x)$};
  \node[anchor=south west] at (-2.42,1.94) {$j_b^\nu(y)$};
  \node[anchor=north west] at (-2.42,-1.94) {$j_c^\rho(z)$};
  ...
  \node[anchor=south] at (0.78,0.18) {$j_a^\mu(x)$};
  \node[anchor=south west] at (4.08,1.94) {$j_c^\rho(z)$};
  \node[anchor=north west] at (4.08,-1.94) {$j_b^\nu(y)$};
 \end{tikzpicture}
 \renewcommand{\thefigure}{22.1}
 \caption{Two triangle diagrams for the anomaly in the current
 $j_a^\mu(x)$. Solid lines are fermions; wavy lines indicate
