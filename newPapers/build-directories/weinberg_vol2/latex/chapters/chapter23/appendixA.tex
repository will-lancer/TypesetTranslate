...
\begin{equation}
 ...
 =\int\left[\prod_a dq_a\right]F(q,q;\beta),
...
\end{equation}
Let us first calculate $\braket{q',t+dt}{q,t}$ for infinitesimal $dt$. Adopting the convention that $H(Q,P)$ is written with all $Q$s to the left of all $P$s, we have
\[
 \braket{q',t+dt}{q,t}
 =\bra{q',t}\exp\bigl(-H(q',P)dt\bigr)\ket{q,t}.
\]
\begingroup
...
\[
 \braket{q,t}{p,t}
 =\prod_a\frac{\exp(ip_aq_a)}{\sqrt{2\pi}},
 \qquad
 \braket{p,t}{q,t}
 =\prod_a\frac{\exp(-ip_aq_a)}{\sqrt{2\pi}},
\]
...
\[
 \braket{q',t+dt}{q,t}
 =\int\left(\prod_a\frac{dp_a}{2\pi}\right)
 \exp\left(
 i\sum_a p_a(q'_a-q_a)-H(q',p)dt
 \right).
\]
...
\begin{aligned}
 ...
 \left(\prod_{a,t}dq_a(t)\right)
 \int\left(\prod_{a,t}\frac{dp_a(t)}{2\pi}\right)\\
 &\quad{}\cdot
 \exp\left(
 \int_{-T/2}^{T/2}dt
 \left[
 i\sum_a\dot q_a(t)p_a(t)-H\bigl(q(t),p(t)\bigr)
 \right]
 \right).
\end{aligned}
...
\begin{aligned}
 ...
 \int_{\substack{q(-T/2)=q\\q(T/2)=q'}}
 \left(\prod_{a,t}dq_a(t)\right)
 \left(\operatorname{Det}\left[2i\pi\mathcal A(q)\right]\right)^{-1/2}\\
 &\quad{}\cdot
 \exp\left(
 \int_{-T/2}^{T/2}dt
 \left[
 i\sum_a\dot q_a(t)\bar p_a(t)
...
\begin{equation}
\begin{aligned}
 S[q]\equiv\int_{-T/2}^{T/2}dt\biggl[
 &\frac{1}{2}\sum_{a,b}A^{-1}_{ab}(q)\dot q_a\dot q_b
 +i\sum_{a,b}A^{-1}_{ab}(q)B_a(q)\dot q_b\\
