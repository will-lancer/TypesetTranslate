...
\end{equation}
where $A_i\equiv t_a A_{a i}$ and $g(\widehat x)$ is a
...
\begin{align}
\mathcal I[g]\equiv{}&
\int \dd\theta^1\,\dd\theta^2\,\dd\theta^3\,\epsilon^{abc}
...
A_{c i}F_{c jk}
-\frac13C_{abc}
A_{a i}A_{b j}A_{c k}
\right],
\tag{23.5.3}\label{eq:23.5.3}
\end{equation}
whose divergence is
\begin{equation}
\partial_lG_l=\frac12\epsilon^E_{ijkl}F_{a ij}F_{a kl}.
...
\begin{equation}
\operatorname{Tr}\{t_a t_b\}=\frac12N\delta_{ab},
...
$\epsilon^E_{ijkl}F_{a ij}F_{a kl}$ has a non-vanishing
integral.
...
\[
0\leq\int\left(
F_{a ij}\mp\frac12\epsilon^E_{ijkl}F_{a kl}
\right)^2\dd^4x
\]
(with squares indicating obvious index contractions), we have
\begin{equation}
S[A]\geq\frac18\left|
\epsilon^E_{ijkl}\int F_{a ij}F_{a kl}\,\dd^4x
\right|
=|\mathcal I[g]|/3N,
...
\begin{equation}
S[A]\equiv\frac14\int F_{a ij}F_{a ij}\,\dd^4x.
...
\begin{equation}
F_{a ij}=\pm\frac12\epsilon^E_{ijkl}F_{a kl}.
...
\begin{equation}
g_1(\widehat x)=\left(\frac{x_4+2\ii\mathbf x\cdot\mathbf t}{r}\right),
...
sign), we also have
\begin{equation}
\epsilon^E_{ijkl}\int F_{a ij}F_{a kl}\,\dd^4x=64\pi^2.
...
\[
\exp\left(-8\pi^2/g_{1/R}^2\right)=(R\Lambda)^{b_0}.
\]
...
$\psi\to\exp(\ii\gamma_5\alpha)\psi$, the measure for integration over
quark fields undergoes a change given by Eq.~\eqref{eq:22.2.10}:
\begin{equation}
[d\psi][d\overline\psi]\longrightarrow
\exp\left\{\ii\alpha\int\mathcal A(x)\,\dd^4x\right\}
...
F_{ija}F_{klb}\operatorname{tr}t_a t_b.
...
$A_{a4}(\mathbf x,x_4)=0$, the gauge field for
$x_4\to\pm\infty$ is expected to approach time-independent pure gauges
for $x_4\to\pm\infty$:
\begin{equation}
\ii\mathbf A(\mathbf x,x_4)\equiv
it_a\mathbf A_a(\mathbf x,x_4)
\longrightarrow
g_\pm^{-1}(\mathbf x)\boldsymbol{\nabla}g_\pm(\mathbf x),
\tag{23.5.24}\label{eq:23.5.24}
\end{equation}
where $g_\pm(\mathbf x)$ are group elements in the representation
generated by $t_a$. Assuming that
$\mathbf A_a(\mathbf x,x_4)$ vanishes for $\mathbf x\to\infty$, the
