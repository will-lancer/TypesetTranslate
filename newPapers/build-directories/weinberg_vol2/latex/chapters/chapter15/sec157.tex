...
\begin{equation}
 I_{\mathrm{MOD}}=I_{\mathrm{EFF}}+I_{GH}=\int \dd^4x\,\mathcal L_{\mathrm{MOD}},
 \tag{15.7.1}\label{eq:15.7.1}
\end{equation}
\begin{equation}
 \mathcal L_{\mathrm{MOD}}=\mathcal L-\frac{1}{2\xi}f_a f_a+\omega_a^*\Delta_a,
 \tag{15.7.2}\label{eq:15.7.2}
\end{equation}
where we have now introduced the quantity
\begin{equation}
 \Delta_a(x)=\int \dd^4y\,\mathcal F_{a x,b y}[A,\psi]\,\omega_b(y).
 \tag{15.7.3}\label{eq:15.7.3}
\end{equation}
This is for the choice
\begin{equation}
 B[f]\propto\exp\left(-\frac{\ii}{2\xi}\int \dd^4x\,f_a f_a\right)
...
\begin{equation}
 B[f]=\int\left[\prod_{a,x}\dd h_a(x)\right]
 \exp\left[\frac{\ii\xi}{2}\int h_a h_a\right]
 \exp\left[\ii\int \dd^4x\,f_a h_a\right].
 \tag{15.7.5}\label{eq:15.7.5}
\end{equation}
We must now do our path integrals over the field $h_a$ (often known as a `Nakanishi--Lautrup' field~\hyperref[ch15-ref-11a]{[11a]}) as well as over matter, gauge, ghost and antighost fields, with a new modified action
\begin{equation}
 I_{\mathrm{NEW}}=\int \dd^4x\left(\mathcal L+\omega_a^*\Delta_a+h_a f_a+\frac{1}{2}\xi h_a h_a\right).
 \tag{15.7.6}\label{eq:15.7.6}
\end{equation}
This modified action is not gauge-invariant---indeed, it had better not be, if we are to be able to use it in path integrals. However, it is invariant under a `BRST' symmetry transformation, parameterized by an infinitesimal constant $\theta$ that \emph{anticommutes} with $\omega_a$, $\omega_a^*$, and all fermionic matter fields. For a given $\theta$, the BRST transformation is
\begin{align}
 \delta_\theta\psi&=\ii t_a\theta\omega_a\psi,
 &&\tag{15.7.7}\label{eq:15.7.7}\\
 \delta_\theta A_{a\mu}&=\theta D_\mu\omega_a
 =\theta\bigl[\partial_\mu\omega_a+C_{abc}A_{b\mu}\omega_c\bigr],
 &&\tag{15.7.8}\label{eq:15.7.8}\\
 \delta_\theta\omega_a^*&=-\theta h_a,
 &&\tag{15.7.9}\label{eq:15.7.9}\\
 \delta_\theta\omega_a&=-\frac{1}{2}\theta C_{abc}\omega_b\omega_c,
 &&\tag{15.7.10}\label{eq:15.7.10}\\
 \delta_\theta h_a&=0.
 &&\tag{15.7.11}\label{eq:15.7.11}
\end{align}
(Recall that in fermionic path integrals, there is no connection between $\omega_a$ and $\omega_a^*$, so that Eq.~\eqref{eq:15.7.9} does not need to be the adjoint of Eq.~\eqref{eq:15.7.10}.) Because $h_a$ is BRST-invariant, we could if we like replace the Gaussian factor $\exp(\tfrac{1}{2}\ii\xi\int h_a h_a)$ in Eq.~\eqref{eq:15.7.5} with an arbitrary smooth functional of $h_a$, yielding an arbitrary functional $B[f]$, without affecting the BRST invariance of the action. However, for the purposes of diagrammatic calculation and renormalization it will help to leave $B[f]$ as a Gaussian.
...
\end{equation}
then\footnote{In the original work on BRST symmetry the functional $B[f]$ was left in the form \eqref{eq:15.7.4}, so that $h_a$ was replaced in Eq.~\eqref{eq:15.7.9} with $-f_a/\xi$, and the BRST transformation was only nilpotent when acting on functions of $\omega_a$ and the gauge and matter fields, but not of $\omega_a^*$.}
\begin{equation}
 \delta_\theta(sF)=0
...
\begin{align*}
 \delta_\theta s\psi
 &=\ii t_a\delta_\theta(\omega_a\psi)
 =-\frac{1}{2}iC_{abc}t_a\theta\omega_b\omega_c\psi
 -t_a t_b\omega_a\theta\omega_b\psi\\
 &=-\frac{1}{2}iC_{abc}t_a\theta\omega_b\omega_c\psi
 +t_a t_b\theta\omega_a\omega_b\psi.
\end{align*}
The product $\omega_a\omega_b$ in the second term on the right is antisymmetric in $a$ and $b$, so we can replace $t_a t_b$ in this term with $\tfrac{1}{2}[t_a,t_b]$, and this term thus cancels the first term:
\begin{equation}
 ss\psi=0.
...
\begin{align*}
 \delta_\theta sA_{a\mu}
 &=\delta_\theta D_\mu\omega_a\\
 &=\partial_\mu\delta_\theta\omega_a
 +C_{abc}\delta_\theta A_{b\mu}\omega_c
 +C_{abc}A_{b\mu}\delta_\theta\omega_c\\
 &=\theta\Bigl(-\frac{1}{2}C_{abc}\partial_\mu(\omega_b\omega_c)
 +C_{abc}(\partial_\mu\omega_b)\omega_c
 +C_{abc}C_{bde}A_{\dd\mu}\omega_e\omega_c
 -\frac{1}{2}C_{abc}C_{cde}A_{b\mu}\omega_d\omega_e\Bigr)\\
 &=\theta\Bigl(\frac{1}{2}C_{abc}(\partial_\mu\omega_b)\omega_c
 +\frac{1}{2}C_{abc}(\partial_\mu\omega_c)\omega_b
 -C_{abc}C_{cde}A_{\dd\mu}\omega_e\omega_b
 -\frac{1}{2}C_{abc}C_{cde}A_{b\mu}\omega_d\omega_e\Bigr).
\end{align*}
The first two terms of the final expression cancel because $C_{abc}$ is antisymmetric in $b$ and $c$, and the third and fourth terms cancel because of the Jacobi identity \eqref{eq:15.1.5}, so
\begin{equation}
 ssA_{a\mu}=0.
...
Finally,
\begin{align*}
 \delta_\theta s\omega_a
 &=-\frac{1}{2}C_{abc}\delta_\theta(\omega_b\omega_c)\\
 &=\frac{1}{4}\theta\left(C_{abc}C_{bde}\omega_d\omega_e\omega_c
 +C_{abc}C_{cde}\omega_b\omega_d\omega_e\right)\\
 &=\frac{1}{4}\theta C_{abc}C_{cde}
 \left[-\omega_d\omega_e\omega_b+\omega_b\omega_d\omega_e\right].
\end{align*}
But $\omega_b$ commutes with $\omega_d\omega_e$, so this too vanishes
\begin{equation}
 ss\omega_a=0.
...
\begin{align*}
 \delta_\theta f_a[x;A,\psi]
 &=\int\left.\frac{\delta f_a[x;A_\lambda,\psi_\lambda]}{\delta\lambda^b(y)}\right|_{\lambda=0}
 \theta\omega_b(y)\,\dd^4y\\
 &=\theta\int \mathcal F_{a x,b y}[A,\psi]\,\omega_b(y)\,\dd^4y
\end{align*}
or in terms of the quantity \eqref{eq:15.7.3}
\begin{equation}
 \delta_\theta f_a[x;A,\psi]=\theta\Delta_a(x;A,\psi,\omega).
 \tag{15.7.23}\label{eq:15.7.23}
\end{equation}
(Note that $\mathcal F$ is a bosonic quantity, so there is no sign change in moving $\theta$ to the left here.) Also recall that $\delta_\theta\omega_a^*=-\theta h_a$ and $\delta_\theta h_a=0$. Therefore the terms in the integrand of the `new' action \eqref{eq:15.7.6} other than $\mathcal L$ may be written
\begin{equation}
 \omega_a^*\Delta_a+h_a f_a+\frac{1}{2}\xi h_a h_a
 =s\left(\omega_a^*f_a+\frac{1}{2}\xi\omega_a^*h_a\right)
 \tag{15.7.24}\label{eq:15.7.24}
\end{equation}
or in other words
\begin{equation}
 I_{\mathrm{NEW}}=\int \dd^4x\,\mathcal L+s\Psi,
 \tag{15.7.25}\label{eq:15.7.25}
\end{equation}
where
\begin{equation}
 \Psi\equiv\int \dd^4x\left(\omega_a^*f_a+\frac{1}{2}\xi\omega_a^*h_a\right).
 \tag{15.7.26}\label{eq:15.7.26}
\end{equation}
The nilpotence of the BRST transformation tells us immediately that the term $s\Psi$ as well as $\int \dd^4x\,\mathcal L$ is BRST-invariant.

In a sense the converse of this result also applies: we shall see in Section 17.2 that a renormalizable Lagrangian that obeys BRST invariance and the other symmetries of the Lagrangian \eqref{eq:15.7.25} must take the form of Eq.~\eqref{eq:15.7.25}, aside from changes in the values of various constant coefficients. But this is not enough to establish the renormalizability of these theories. BRST symmetry transformations act non-linearly on the fields, and in this case there is no simple connection between the symmetries of the Lagrangian and the symmetries of matrix elements and Green's functions. Using the external field methods developed in the next chapter, it will be shown in Section 17.2 that the ultraviolet divergent terms in Feynman amplitudes (though not the finite parts) do obey a sort of renormalized BRST invariance, which allows the proof of renormalizability to be completed.

Eq.~\eqref{eq:15.7.25} shows that the physical content of any gauge theory is contained in the \emph{kernel} of the BRST operator (that is, in a general BRST-invariant term $\int \dd^4x\,\mathcal L+s\Psi$), modulo terms in the \emph{image} of the BRST transformation (that is, terms of the form $s\Psi$). The kernel modulo the image of any nilpotent transformation is said to form the \emph{cohomology} of the transformation. There is another sense in which the physical content of a gauge theory may be identified with the cohomology of the BRST operator~\hyperref[ch15-ref-12]{[12]}. It is a fundamental physical requirement that matrix elements between physical states should be independent of our choice of the gauge-fixing function $f_a$, or in other words, of the functional $\Psi$ in Eq.~\eqref{eq:15.7.25}. The change in any matrix element $\braket{\alpha}{\beta}$ due to a change $\widetilde\delta\Psi$ in $\Psi$ is
\begin{equation}
 \widetilde\delta\braket{\alpha}{\beta}
 =\ii\bra{\alpha}\widetilde\delta I_{\mathrm{NEW}}\ket{\beta}
 =\ii\bra{\alpha}s\widetilde\delta\Psi\ket{\beta}.
...
\]
For this to be satisfied for all operators $\Phi$, it is necessary for $Q^2$ either to vanish or be proportional to the unit operator. But $Q^2$ cannot be proportional to the unit operator, since it has a non-vanishing ghost quantum number\footnote{Recall that the ghost quantum number is defined as $+1$ for $\omega_a$, $-1$ for $\omega_a^*$, and 0 for all gauge and matter fields.}, so it must vanish:
\begin{equation}
 Q^2=0.
...
\begin{align}
 A^\mu(x)&=(2\pi)^{-3/2}\int\frac{\dd^3p}{\sqrt{2p^0}}
 \left[a^\mu(\mathbf p)e^{\ii p\cdot x}+a^{\mu*}(\mathbf p)e^{-\ii p\cdot x}\right],\notag\\
 \omega(x)&=(2\pi)^{-3/2}\int\frac{\dd^3p}{\sqrt{2p^0}}
 \left[c(\mathbf p)e^{\ii p\cdot x}+c^*(\mathbf p)e^{-\ii p\cdot x}\right],\tag{15.7.33}\label{eq:15.7.33}\\
 \omega^*(x)&=(2\pi)^{-3/2}\int\frac{\dd^3p}{\sqrt{2p^0}}
 \left[b(\mathbf p)e^{\ii p\cdot x}+b^*(\mathbf p)e^{-\ii p\cdot x}\right].\notag
\end{align}
Matching coefficients of $e^{\pm \ii p\cdot x}$ on both sides of Eq.~\eqref{eq:15.7.28} yields
\begin{align}
...
\begin{equation}
 \bra{\Omega}a_0(\mathbf p)a_0^*(\mathbf p')\ket{\Omega}=-\braket{\Omega}{\Omega}.
...
The Faddeev--Popov--De Witt formalism described so far necessarily yields an action that is bilinear in the ghost fields $\omega_a^*$ and $\omega_a$. This is adequate for renormalizable Yang--Mills theories with the gauge-fixing function $f_a=\partial_\mu A_a^\mu$, but not in more general cases. For instance, as we shall see in Section 17.2, in other gauges renormalizable Yang--Mills theories need $\omega^*\omega^*\omega\omega$ terms in the action Lagrangian density to serve as counterterms for the ultraviolet divergences in loop graphs with four external ghost lines.
...
Although we shall not use it here, there is a beautiful geometric interpretation~\hyperref[ch15-ref-14]{[14]} of the ghosts and the BRST symmetry that should be mentioned. The gauge fields $A_a^\mu$ may be written as one-forms $A_a=A_{a\mu}\dd x^\mu$, where $\dd x^\mu$ are a set of anticommuting c-numbers. (See Section 5.8.) This can be combined with the ghost to compose a one-form $\mathcal A_a=A_a+\omega_a$ in an extended space. Also, the ordinary exterior derivative $d=\dd x^\mu\partial/\partial x^\mu$ may be combined with the BRST operator $s$ to form an exterior derivative $\mathcal D=d+s$ in this space, which is nilpotent because $s^2=\dd^2=sd+\dd s=0$.
