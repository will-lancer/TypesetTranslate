...
\begin{equation}
 \Delta_a(x)=\int d^4y\,\mathcal F_{a x,b y}[A,\psi]\,\omega_b(y).
 \tag{15.7.3}\label{eq:15.7.3}
\end{equation}
This is for the choice
\begin{equation}
 B[f]\propto\exp\left(-\frac{i}{2\xi}\int d^4x\,f_a f_a\right)
...
\begin{equation}
 B[f]=\int\left[\prod_{a,x}dh_a(x)\right]
 \exp\left[\frac{i\xi}{2}\int h_a h_a\right]
 \exp\left[i\int d^4x\,f_a h_a\right].
...
\begin{equation}
 I_{\mathrm{NEW}}=\int d^4x\left(\mathcal L+\omega_a^*\Delta_a+h_a f_a+\frac{1}{2}\xi h_a h_a\right).
...
\begin{align}
 \delta_\theta\psi&=it_a\theta\omega_a\psi,
...
\begin{align*}
 ...
 \theta\omega_b(y)\,d^4y\\
 &=\theta\int \mathcal F_{a x,b y}[A,\psi]\,\omega_b(y)\,d^4y
\end{align*}
...
where
\begin{equation}
 \Psi\equiv\int d^4x\left(\omega_a^*f_a+\frac{1}{2}\xi\omega_a^*h_a\right).
...
\begin{equation}
 ...
 =i\bra{\alpha}\widetilde\delta I_{\mathrm{NEW}}\ket{\beta}
 =i\bra{\alpha}s\widetilde\delta\Psi\ket{\beta}.
...
Although we shall not use it here, there is a beautiful geometric interpretation~\hyperref[ch15-ref-14]{[14]} of the ghosts and the BRST symmetry that should be mentioned. The gauge fields $A_a^\mu$ may be written as one-forms $A_a=A_{a\mu}dx^\mu$, where $dx^\mu$ are a set of anticommuting c-numbers. (See Section 5.8.) This can be combined with the ghost to compose a one-form $\mathcal A_a=A_a+\omega_a$ in an extended space. Also, the ordinary exterior derivative $d=dx^\mu\partial/\partial x^\mu$ may be combined with the BRST operator $s$ to form an exterior derivative $\mathcal D=d+s$ in this space, which is nilpotent because $s^2=d^2=sd+ds=0$.
