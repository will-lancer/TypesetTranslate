...
\begin{enumerate}
  ...
  \item Suppose we use generalized Coulomb gauge in a non-Abelian gauge theory, taking the gauge-fixing function as $f_\alpha=\boldsymbol\nabla\cdot\mathbf A_\alpha$. Derive the ghost Lagrangian. What is the ghost propagator? (Take $B[f]=\exp(-i\int d^4x\,f_\alpha f_\alpha/2\xi)$.)
  ...
  \item Show that if a field $\psi_\ell(x)$ belonging to a representation of a gauge group with generator matrices $t_\alpha$ varies along a path $x^\mu=x^\mu(\tau)$ according to the differential equation
  \[
    \frac{d\psi(\tau)}{d\tau}
  ...
  \item Suppose we adopt BRST invariance instead of gauge invariance as a fundamental physical principle. Derive the most general Lagrangian density constructed from sums of products of fields and field derivatives of dimensionality $(\text{mass})^d$ with $d\leq4$, constructed from $A_{\alpha\mu}$, $\omega_\alpha$, $\omega_\alpha^*$, $h_\alpha$, and/or their derivatives, that is invariant (up to total derivatives) under Lorentz transformations, ghost number phase transformations ($\omega_\alpha\longrightarrow e^{i\theta}\omega_\alpha$, $\omega_\alpha^*\longrightarrow e^{-i\theta}\omega_\alpha^*$), global gauge transformations ($\epsilon_\alpha$ constant), and BRST invariance.
  ...
  \item Show that if a functional $O$ satisfies the condition $(O,S)=i\Delta S$ and the action $S$ satisfies the quantum master equation then the quantum average $\langle O\rangle$ is independent of the gauge-fixing functional $\Psi$.
\end{enumerate}
...
\begin{enumerate}
  ...
  The proof that \textbf{a} (with $g_{\alpha\beta}=\delta_{\alpha\beta}$) implies \textbf{b} and that \textbf{b} implies \textbf{c} was given by M.~Gell-Mann and S.~L.~Glashow, \textit{Ann. Phys. (N.Y.)} \textbf{15}, 437 (1961).
