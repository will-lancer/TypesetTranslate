...
\begin{enumerate}
  ...
  (Take $B[f]=\exp(-i\int d^4x\,f_a f_a/2\xi)$.) What is the ghost propagator?
  ...
  \item Suppose we adopt BRST invariance instead of gauge invariance as a fundamental physical principle. Derive the most general Lagrangian density constructed from sums of products of fields and field derivatives of dimensionality $(\text{mass})^d$ with $d\leq4$, constructed from $A_{a\mu}$, $\omega_a$, $\omega_a^*$, $h_a$, and/or their derivatives, that is invariant (up to total derivatives) under Lorentz transformations, ghost number phase transformations ($\omega_a\longrightarrow e^{i\theta}\omega_a$, $\omega_a^*\longrightarrow e^{-i\theta}\omega_a^*$), global gauge transformations ($\epsilon_a$ constant), and BRST invariance.
