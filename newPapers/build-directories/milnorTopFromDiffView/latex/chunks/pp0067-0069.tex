...
\textsc{Definition.} A map $f : I \to M$ is a \emph{parametrization by arc-length} if $f$ maps $I$ diffeomorphically onto an open subset$\dagger$ of $M$, and if the ``velocity vector'' $df_s(1) \in TM_{f(s)}$ has unit length, for each $s \in I$.\footnote{$\dagger$ Thus $I$ can have boundary points only if $M$ has boundary points.}
...
\IfFileExists{output/figures/fig19.tex}{\input{output/figures/fig19.tex}}{\input{figures/fig19.tex}}
