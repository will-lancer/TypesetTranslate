...
\(dx^1,\ldots,dx^n\).  [Thus, we stress that \(dx^\mu\) is merely the symbol for
...
the dual basis \(\{dx^\mu\}\), then from Eqs.~\eqref{eq:2.3.1} and
...
\end{equation}
Sometimes the notation \(ds^2\) is used in place of \(g\) to represent the metric
