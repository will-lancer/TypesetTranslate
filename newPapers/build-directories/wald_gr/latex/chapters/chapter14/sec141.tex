...
\(i\hbar\,\partial\Psi/\partial t=\widehat H\Psi\), where \(\widehat H\) is an operator corresponding
...
which ``interpolate'' between \(\psi_1\) and \(\psi_2\), and \(d\mu[\psi]\) denotes a measure
on the space of field configurations. The major difficulty which arises in the path
integral approach is the definition of the measure \(d\mu[\psi]\). One simply does not know
how to make mathematical sense of \(d\mu[\psi]\), except in the context of perturbation
...
precise meaning to \(d\mu[g_{ab}]\) so that the right-hand side of
...
\(t=-i\tau\), where \(\tau\) is real and positive (see Streater and Wightman 1964). It is
...
\end{equation}
where \(\tau=it\). This submanifold is referred to as a \emph{Euclidean section} of
...
\(S_E[\psi]=-iS[\psi]\rvert_{t=-i\tau}\), is positive definite and the analytically
...
\begin{equation}
  \partial_{AA'}\omega^B=-i\epsilon_A{}^B\pi_{A'}.
