...
\(\left.d\psi_\lambda/d\lambda\right|_{\lambda=0}\) by \(\delta\psi\). Suppose
\(dS/d\lambda\) at \(\lambda=0\) exists for all such one-parameter families
...
\(\left.dS/d\lambda\right|_{\lambda=0}=\chi[\delta\psi]\), we also say that \(S\)
...
\begin{align}
  \left.\frac{dS_{\mathrm{KG}}}{d\lambda}\right|_{\lambda=0}
...
\(T^{a\ldots b}{}_{c\ldots d}\) to be a tensor which can be expressed in the form
\begin{equation}
  T^{a\ldots b}{}_{c\ldots d}
    =\sqrt{-g}\,\widetilde T^{a\ldots b}{}_{c\ldots d},
  \tag{E.1.11}\label{eq:E.1.11}
\end{equation}
where \(\widetilde T^{a\ldots b}{}_{c\ldots d}\) is a tensor whose value does not
...
density \(\mathcal L\) be a scalar density. Similarly, in order for \(dS/d\lambda\)
...
variation we define \(\delta g^{ab}\) to be \(dg^{ab}/d\lambda\). However, in
...
\(\delta g_{ab}=dg_{ab}/d\lambda\). Thus, we warn the reader that, since
...
\begin{equation}
  \frac{dS_G}{d\lambda}
    =\int\frac{d\mathcal L_G}{d\lambda}e
    =\int\nabla^a v_a\sqrt{-g}\,e
...
\begin{equation}
  \frac{d\mathcal L}{d\lambda}=V^a\delta(\partial_a\psi)
...
\begin{equation}
  \delta\mathcal L\equiv\frac{d\mathcal L}{d\lambda}
