...
% Corrections: Eq. (30.2.3) sums on j; the infinite constant includes the
% complex-conjugate integration measure; and the electromagnetic solution
% consistently uses the free lambda index.
...
\begin{align}
  &\int\left[\prod_i \dd\phi_i\,\dd\phi_i^*\right]
    \exp\{\ii I_{\mathrm{quad}}[\phi]\}
    \phi_a\cdots\phi_b^*\cdots
  \notag\\
  &\quad={}
    \mathcal{J}\int \dd\phi'\,\dd\phi'{}^*
    \int\left[\prod_\nu \dd\phi'_\nu\,\dd\phi_\nu'{}^*\right]
    \exp\left\{-\ii\sum_\nu d_\nu|\phi'_\nu|^2\right\}
  \notag\\
  &\qquad\cdot
...
\(\mathcal{J}\int \dd\phi'\,\dd\phi'{}^*\). The integral over the \(\phi'_\nu\) then gives
\begin{align}
  &\int\left[\prod_i \dd\phi_i\,\dd\phi_i^*\right]
    \exp\{\ii I_{\mathrm{quad}}[\phi]\}
    \phi_a\cdots\phi_b^*\cdots
  \notag\\
  &\qquad\propto
    \sum_{\text{pairings}}\bigl[-\ii\Delta_{ab}\bigr]\cdots
    +\xi\text{-terms}\,,
...
\(A_\mu\) is a real field. The origin of the \(-\ii\epsilon\) in the
