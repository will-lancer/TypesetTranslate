...
% Source-visible anomaly preserved: after defining E^* as the number of
% external outgoing lines, the paragraph before Eq. (30.3.2) visibly calls
% them “incoming external lines.”
% Uncertainties: none.
...
potential superfields \(S_n(x,\theta)\) and their adjoints. Following the
...
\(x,\theta\), we include a c-number factor \(S_n(x,\theta)\) or
\(S_n^*(x,\theta)\), respectively (but no propagator). A vertex labelled
\(x,\theta\) with \(N\) incoming or \(N\) outgoing lines labelled
...
line of any type coming out of a vertex labelled \(x,\theta\) and going into a
vertex labelled \(x',\theta'\) yields a propagator factor, given by
Eqs.~\eqref{eq:30.2.10} and \eqref{eq:30.2.17} as
\begin{equation}
  -\frac{i}{4}\delta^4(\theta-\theta')\Delta_F(x-x')\,.
  \tag{30.3.1}\label{eq:30.3.1}
\end{equation}
In addition, as shown by Eq.~\eqref{eq:30.1.7}, a superderivative
\(\SuperD_R^2\) acts on the propagators or external line \(S\)-factors of all
but one of the internal or external lines coming into any vertex, and a
superderivative \(\SuperD_L^2\) acts on the propagators or external-line
\(S^*\)-factors of all but one of the lines coming out of any vertex. The
product of these factors must be integrated over all \(x\)s and \(\theta\)s;
the quantum effective action is the sum of these integrals for all
one-particle-irreducible graphs.

By integrating by parts in superspace, the \(\SuperD_L^2\) and/or
\(\SuperD_R^2\) operators accompanying any one propagator (say, one that
connects vertices labelled \(x,\theta\) and \(x',\theta'\)) may be moved to
other propagators or external line factors. This leaves the factor contributed
by this internal line proportional to a factor
\(\delta^4(\theta-\theta')\). Integrating over \(\theta'\) then eliminates
this delta function, and lets us replace \(\theta'\) with \(\theta\)
everywhere else. (In cases where several internal lines connect the same pair
of vertices, we need to use the property of fermionic delta functions that
\([\delta^4(\theta-\theta')]^2=0\).) Continuing in this way, we wind up with a
single four-dimensional \(\theta\) integral, with all \(\SuperD\)s acting on
the external line factors \(S_n\) and \(S_n^*\). That is, though not usually
local in spatial coordinates, \(\Gamma[S,S^*]\) is \emph{local in the
...
the external line factors \(S_n\) or \(S_n^*\) must be acted on by an operator
\(\SuperD_R^2\) or \(\SuperD_L^2\), respectively, with two possible
exceptions. The exceptions are that a term with only incoming or only outgoing
external lines, in which all but one of the external line factors \(S_n\) or
\(S_n^*\) are acted on by \(\SuperD_R^2\) or \(\SuperD_L^2\), respectively,
and by no other superderivatives, is still invariant under the transformation
\eqref{eq:30.2.4}, even though it cannot be expressed as a four-dimensional
\(\theta\)-integral of a functional only of the \(\Phi_n\) and/or
...
\(S_n\) or \(S_n^*\) that is not acted on by an operator \(\SuperD_R^2\) or
\(\SuperD_L^2\), and this change is eliminated if we use integration by parts
to move one of the other \(\SuperD_R^2\) or \(\SuperD_L^2\) operators to act
on it. As we saw in Section 30.1, such a term in \(\Gamma[S,S^*]\) is the
...
\(\Gamma[S,S^*]\) may be written as a four-dimensional \(\theta\)-integral
...
\(\SuperD_R^2\) or \(\SuperD_L^2\), respectively, and also by additional
\(\SuperD_R^2\) or \(\SuperD_L^2\) operators (such as in combinations like
\(\SuperD_R^2\SuperD_L^2\SuperD_R^2S_n\) or
\(\SuperD_L^2\SuperD_R^2\SuperD_L^2S_n^*\)) may also be written by
integration by parts as a term in which one of the extra \(\SuperD_R^2\) or
\(\SuperD_L^2\) acts on the previously undifferentiated external line
\(S_n\) or \(S_n^*\) factor. Such a term may therefore be expressed as a
four-dimensional \(\theta\)-integral of a functional only of the \(\Phi_n\)
and/or \(\Phi_n^*\), and therefore just makes another correction to the
\(D\)-term part of the action. The only terms in \(\Gamma[S,S^*]\) that
cannot be written in this way are those that have \(E\) incoming external
lines and only \(E-1\) operators \(\SuperD_R^2\) operating on the \(S_n\)
external line factors, or \(E^*\) incoming external lines and only \(E^*-1\)
operators \(\SuperD_L^2\) operating on the \(S_n^*\) external line factors.
...
\(\SuperD_R^2\) or \(\SuperD_L^2\) operators contributed by the supergraph
...
\end{equation}
The total numbers of \(\SuperD_R^2\) and \(\SuperD_L^2\) operators are then
\begin{equation}
  N_R
  =\sum_nV_n(n-1)
  =I+E-V
...
will have \(N_R\geq E\) and \(N_L\geq E^*\), so that there are at least enough
\(\SuperD_R^2\) operators to convert all \(S_n\) into
\(\Phi_n=\SuperD_R^2S_n\) and its derivatives, and enough
\(\SuperD_L^2\) operators to convert all \(S_n^*\) into
\(\Phi_n^*=\SuperD_L^2S_n^*\) and its derivatives.
\emph{Any graph with loops thus yields only a contribution to the
four-dimensional \(\theta\)-integral---in other words the \(D\)-term---of a
...
adjoint is to have just \(N_R=E-1\) operators \(\SuperD_R^2\) or just
\(N_L=E^*-1\) operators \(\SuperD_L^2\), respectively. According to
