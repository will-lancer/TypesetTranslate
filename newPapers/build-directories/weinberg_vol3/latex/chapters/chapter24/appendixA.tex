...
% Source note: the printed cross-reference to Sections 25.1 and 31.1 is retained.
...
construction of general supersymmetry algebras in Sections 25.1 and 31.1.
...
matrices $-\sigma_i^T$ and $-\lambda_A^T$ of the contragredient
...
$-(\sigma_i\lambda_A)^T=-(-\sigma_i^T)(-\lambda_A^T)$.} The $S_i$, $T_A$, and
$R_{iA}$ form the Lie algebra of the group $SU(6)$, with commutation relations
\begin{equation}
\begin{aligned}
 [S_i,S_j]&=i\sum_k\epsilon_{ijk}S_k,
 &\qquad [T_A,T_B]&=i\sum_C f_{ABC}T_C,
 \\[2pt]
 [S_i,T_A]&=0,
 & [S_i,R_{jA}]&=i\sum_k\epsilon_{ijk}R_{kA},
 \\[2pt]
 [T_A,R_{iB}]&=i\sum_C f_{ABC}R_{iC},
 \\[2pt]
 [R_{Ai},R_{Bj}]&=i\delta_{ij}\sum_C f_{ABC}T_C
 +\frac{3}{2}i\delta_{AB}\sum_k\epsilon_{ijk}S_k
 \\[-1pt]
...
$\mathbf{6}\cdot\overline{\mathbf{6}}$: the adjoint representation
