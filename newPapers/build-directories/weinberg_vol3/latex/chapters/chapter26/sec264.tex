...
\[
  ...
  ={}&\sum_{nm}
    (\theta_L^T\epsilon\psi_{nL}(x))
    (\theta_L^T\epsilon\psi_{mL}(x))
...
\((\theta_R^T\epsilon\gamma^\mu\theta_L)\) in Eq.~\eqref{eq:26.3.21}
...
\[
  \begin{aligned}
  (\theta_L^T\epsilon\psi_{nL})(\theta_L^T\epsilon\psi_{mL})
  &=
  \left(\psi_{nL}^T\epsilon
...
\((\theta_L^T\epsilon\theta_L)\), so here
\begin{equation}
  [f(\Phi)]_{\mathcal{F}}
  =-\frac12\sum_{nm}
...
\begin{equation}
  \mathcal{M}'=\mathcal{U}^T\mathcal{M}\mathcal{U}\,.
