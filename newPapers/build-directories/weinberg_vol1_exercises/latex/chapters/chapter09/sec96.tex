...
=H_M+\int \dd^3x\left[\frac12\boldsymbol\Pi_\perp^2
...
energy
\begin{equation}
V_{\mathrm{Coul}}(t)=\frac12\int \dd^3x\,\dd^3y\,
...
={}&\int\left[\prod_{x,\ii}\dd a_i(x)\prod_{x,\ii}\dd\pi_i(x)
 \prod_{x,\ell}\dd\psi_\ell(x)\right]
\notag\\
&\cdot\mathcal O_A\mathcal O_B\cdots
\notag\\
&\cdot\exp\Biggl\{\ii\int \dd^4x\Biggl[
 \boldsymbol\pi\cdot\dot{\mathbf a}-\frac12\boldsymbol\pi^2
 -\frac12(\boldsymbol\nabla\times\mathbf a)^2
\notag\\
&\qquad+\mathbf a\cdot\mathbf j+\mathcal L_M\Biggr]
 -\ii\int \dd t\,V_{\mathrm{Coul}}\Biggr\}
\notag\\
...
\begin{align}
\left\langle T\{\mathcal O_A\mathcal O_B\cdots\}\right\rangle_{\Omega}
={}&\int\left[\prod_{x,\ii}\dd a_i(x)\prod_{x,\ell}\dd\psi_\ell(x)\right]
\notag\\
&\cdot\mathcal O_A\mathcal O_B\cdots
\notag\\
&\cdot\exp\Biggl\{\ii\int \dd^4x\Biggl[
...
\notag\\
&\qquad+\mathbf a\cdot\mathbf j+\mathcal L_M\Biggr]
 -\ii\int \dd t\,V_{\mathrm{Coul}}+\ii\epsilon\text{ terms}\Biggr\}
\notag\\
&\cdot\left[\prod_x\delta(\boldsymbol\nabla\cdot\mathbf a(x))\right].
...
$-\int \dd t\,V_{\mathrm{Coul}}$ in the action with
\begin{equation}
\int \dd^4x\left[-a^0(x)j^0(x)+\frac12
 (\boldsymbol\nabla a^0(x))^2\right].
...
$-\int \dd t\,V_{\mathrm{Coul}}$. Hence we can rewrite the argument of the
...
\begin{align}
\left\langle T\{\mathcal O_A\mathcal O_B\cdots\}\right\rangle_{\Omega}
\propto{}&\int\left[\prod_{x,\mu}\dd a_\mu(x)\right]
 \left[\prod_{x,\ell}\dd\psi_\ell(x)\right]
...
\begin{equation}
I[a,\psi]=\int \dd^4x\left[-\frac14f_{\mu\nu}f^{\mu\nu}
 +a_\mu j^\mu+\mathcal L_M\right]+\ii\epsilon\text{ terms}.
...
$[\prod \dd a][\prod \dd\psi]$ are gauge-invariant.
...
\psi_{\ell\Lambda}(x)\equiv
 \exp(\ii q_\ell\Lambda(x))\psi_\ell(x)
\tag{9.6.12}\label{eq:9.6.12}
\end{equation}
with arbitrary finite $\Lambda(x)$. This step is a mathematical triviality,
like changing an integral $\int_{-\infty}^{\infty}f(x)\dd x$ to read
$\int_{-\infty}^{\infty}f(y)\dd y$, and does not require use of the postulated
...
&\left\langle T\{\mathcal O_A[A,\Psi],\mathcal O_B[A,\Psi],\ldots\}
 \right\rangle_{\Omega}
\notag\\
&\quad\propto\int\left[\prod_{x,\mu}\dd a_\mu(x)\right]
 \left[\prod_{x,\ell}\dd\psi_\ell(x)\right]
 \mathcal O_A[a,\psi]\mathcal O_B[a,\psi]\cdots
\notag\\
&\qquad\cdot\exp(\ii I[a,\psi])
 \prod_x\delta(\boldsymbol\nabla\cdot\mathbf a(x)
 +\boldsymbol\nabla^2\Lambda(x)).
...
\begin{equation}
B[\Lambda,a]=\exp\left(-\frac12\ii\alpha\int \dd^4x\,
...
\int\left[\prod_xd\Lambda(x)\right]
 \exp\left(-\frac12\ii\alpha\int \dd^4x\,
 (\boldsymbol\nabla^2\Lambda)^2\right).
...
&\int\left[\prod_xd\Lambda(x)\right]
 ...
 +\boldsymbol\nabla^2\Lambda)
\notag\\
&\qquad\propto\exp\left(-\frac12\ii\alpha\int \dd^4x\,
...
\propto{}&\int\left[\prod_{x,\mu}\dd a_\mu(x)\right]
 \left[\prod_{x,\ell}\dd\psi_\ell(x)\right]
 \mathcal O_A\mathcal O_B\cdots\exp(\ii I_{\mathrm{eff}}[a,\psi]),
\tag{9.6.17}\label{eq:9.6.17}
\end{align}
where
\begin{equation}
I_{\mathrm{eff}}[a,\psi]=I[a,\psi]
 -\frac12\alpha\int(\partial_\mu a^\mu)^2\dd^4x.
...
\begin{align}
I_0[a]
&=\int \dd^4x\left[-\frac14(\partial_\mu a_\nu-\partial_\nu a_\mu)
 (\partial^\mu a^\nu-\partial^\nu a^\mu)
 -\frac12\alpha(\partial_\mu a^\mu)^2+\ii\epsilon\text{ terms}\right]
\notag\\
&=-\frac12\int \dd^4x\,\dd^4y\,a^\mu(x)a^\nu(y)
 \mathcal D_{\mu x,\nu y},
...
&=\left[\eta_{\mu\nu}
 ...
 \delta^4(x-y)+\ii\epsilon\text{ terms}
\notag\\
&=(2\pi)^{-4}\int \dd^4q\left[
 \eta_{\mu\nu}q^2-(1-\alpha)q_\mu q_\nu-\ii\epsilon\eta_{\mu\nu}\right]
 e^{\ii q\cdot(x-y)}.
...
\begin{equation}
\Delta_{\mu x,\nu y}=(2\pi)^{-4}\int \dd^4q\left[
 \frac{\eta^{\mu\nu}}{q^2-\ii\epsilon}
 +\frac{1-\alpha}{\alpha}\frac{q^\mu q^\nu}{(q^2-\ii\epsilon)^2}\right]
 e^{\ii q\cdot(x-y)}.
...
=(2\pi)^{-4}\int \dd^4q\left[\frac{\eta^{\mu\nu}}{q^2-\ii\epsilon}\right]
 e^{\ii q\cdot(x-y)},
...
=(2\pi)^{-4}\int \dd^4q\left[
 \frac{\eta^{\mu\nu}}{q^2-\ii\epsilon}
 -\frac{q^\mu q^\nu}{(q^2-\ii\epsilon)^2}\right]e^{\ii q\cdot(x-y)}.
