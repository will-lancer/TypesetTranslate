...
\[
\braket{q'}{f}=(-)^N\left[
 \int\left(\widetilde{\prod_b}\dd q_b\right)
...
\[
\ket{f}=(-)^N\int\ket{q}
 \left(\widetilde{\prod_b}\dd q_b\right)\braket{q}{f},
\]
or as an operator equation
\begin{equation}
\mathbf{1}=\int\ket{q}
 \left(\widetilde{\prod_a}(-\dd q_a)\right)\bra{q}.
\tag{9.5.41}\label{eq:9.5.41}
\end{equation}
In exactly the same way, we can also show that
\begin{equation}
\mathbf{1}=\int\ket{p}
 \left(\widetilde{\prod_a}\dd p_a\right)\bra{p}.
...
\begin{equation}
\ket{q;t}\equiv\exp(\ii Ht)\ket{q},
\qquad
\ket{p;t}\equiv\exp(\ii Ht)\ket{p},
\tag{9.5.45}\label{eq:9.5.45}
\end{equation}
\begin{equation}
\bra{q;t}\equiv\bra{q}\exp(-\ii Ht),
\qquad
\bra{p;t}\equiv\bra{p}\exp(-\ii Ht).
...
=\bra{q'}\exp(-\ii H\,\dd\tau)\ket{q}.
\]
Now insert Eq. \eqref{eq:9.5.42} to the left of the operator $\exp(-\ii H\,\dd\tau)$. It is
convenient here to define the Hamiltonian operator $H(P,Q)$ with all $P$s to
the left of all $Q$s, so that (for $\dd\tau$ infinitesimal)
\[
\bra{p}\exp[-\ii H(P,Q)\dd\tau]\ket{q}
=\braket{p}{q}\exp[-\ii H(p,q)\dd\tau].
\]
...
\braket{q';\tau+\dd\tau}{q;\tau}
&=\int\braket{q'}{p}
 \left(\widetilde{\prod_a}\dd p_a\right)
 \bra{p}\exp(-\ii H\,\dd\tau)\ket{q}\\
&=\int\braket{q'}{p}
 \left(\widetilde{\prod_a}\dd p_a\right)\braket{p}{q}
 \exp[-\ii H(p,q)\dd\tau].
\end{align*}
...
\begin{equation}
\braket{q';\tau+\dd\tau}{q;\tau}
=\int\left(\widetilde{\prod_a}\ii\,\dd p_a\right)
 \exp\left[\ii\sum_ap_a(q'_a-q_a)-\ii H(p,q)\dd\tau\right].
...
step we have an equal number of $\dd p$s and $\dd q$s); and then introduce functions
...
&\quad=(-\ii)^N\chi_N
 ...
 \left(\widetilde{\prod_{a\tau}}\dd q_a(\tau)\,\dd p_a(\tau)\right)
 \notag\\
&\qquad\quad\cdot
...
$(-\ii)^N\chi_N$. But in fact these phases contribute only to the vacuum--vacuum
...
\begin{align}
&\OutBra{\Omega}T\{\mathcal{O}_A[P(t_A),Q(t_A)],
\notag\\[-2pt]
...
&\quad\propto\int
 ...
 \left[\prod_{\tau,\mathbf{x},m}\dd p_m(\mathbf{x},\tau)\right]
\notag\\
&\qquad\quad\cdot\mathcal{O}_A[p(t_A),q(t_A)]
 \mathcal{O}_B[p(t_B),q(t_B)]\cdots
\notag\\
&\qquad\quad\cdot\exp\left[\ii\int_{-\infty}^{\infty}\dd\tau
 \left\{\begin{aligned}
 &\int \dd^3x\sum_mp_m(\mathbf{x},\tau)\dot q_m(\mathbf{x},\tau)\\[-2pt]
 &\quad{}-H[q(\tau),p(\tau)]+\ii\epsilon\text{ terms}
 \end{aligned}\right\}\right],
\tag{9.5.49}\label{eq:9.5.49}
\end{align}
where the proportionality constant is the same for all operators
$\mathcal{O}_A,\mathcal{O}_B$, etc., and the `$\ii\epsilon$ terms' again arise
...
so that the quantity $\int \dd^3x\sum_mp_m\dot q_m-H$ in Eq. \eqref{eq:9.5.49} as it
...
&\int_{-\infty}^{\infty}\dd\tau
 \left\{\int \dd^3x\sum_mp_m(\mathbf{x},\tau)\dot q_m(\mathbf{x},\tau)
 -H_0[q(\tau),p(\tau)]+\ii\epsilon\text{ terms}\right\}
\notag\\
&\qquad=-\sum_{mn}\int \dd^4x\,\dd^4y\,
...
\notag\\
&\qquad\quad\cdot
 \exp\left(-\ii\sum_{mn}\int \dd^4x\,\dd^4y\,
...
={}&\int\left[\prod_{\mathbf{x},\tau,m}
 \dd q_m(\mathbf{x},\tau)\,\dd p_m(\mathbf{x},\tau)\right]
\notag\\
&\quad\cdot\exp\left\{\begin{aligned}
 &-\ii\sum_{mn}\int \dd^4x\,\dd^4y\,
 \mathcal{D}_{m\mathbf{x},n\mathbf{y}}p_m(x)q_n(y)\\[-2pt]
&-\ii\sum_m\int \dd^4x\,p_m(x)f_m(x)\\[-2pt]
&\qquad{}-\ii\sum_n\int \dd^4y\,g_n(y)q_n(y)
 \end{aligned}\right\},
...
\[
p'_m(x)=p_m(x)+\sum_n\int \dd^4y\,
 g_n(y)(\mathcal{D}^{-1})_{n\mathbf{y},m\mathbf{x}},
\]
\[
q'_n(y)=q_n(y)+\sum_m\int \dd^4x\,
...
&\quad\cdot\int\left[\prod_{\mathbf{x},\tau,m}
 \dd q'_m(\mathbf{x},\tau)\,\dd p'_m(\mathbf{x},\tau)\right]
\notag\\
&\quad\cdot\exp\left(-\ii\sum_{mn}\int \dd^4x\,\dd^4y\,
...
&\qquad\propto\sum_{\text{pairings}}\delta_{\text{pairing}}
 \prod_{\text{pairs}}
 \left(-\ii\mathcal{D}^{-1}\right)_{\text{paired }m\mathbf{x},n\mathbf{y}},
...
q_m(x)=\psi_m(x),
\qquad
p_m(x)=-[\bar\psi(x)\gamma^0]_m=\ii\psi_m^\dagger(x)
...
&=\left\{\gamma^0\left[\gamma^\mu\frac{\partial}{\partial x^\mu}
 +m-\ii\epsilon\right]\right\}_{mn}
 \delta^4(x-y)\notag\\
&=\int\frac{\dd^4k}{(2\pi)^4}
 \left\{\gamma^0[\ii\gamma^\mu k_\mu+m-\ii\epsilon]\right\}_{mn}
 e^{\ii k\cdot(x-y)}.
\tag{9.5.57}\label{eq:9.5.57}
\end{align}
(Though we shall not work it out in detail, the $\ii\epsilon$ term here arises
in much the same way as for the scalar field in Section 9.2.) The propagator is
then
\begin{equation}
(\mathcal{D}^{-1})_{m\mathbf{x},n\mathbf{y}}
=\int\frac{\dd^4k}{(2\pi)^4}
 \left\{[\ii\gamma^\mu k_\mu+m-\ii\epsilon]^{-1}[-\gamma^0]\right\}_{mn}
 e^{\ii k\cdot(x-y)},
...
\begin{align}
\OutBra{\Omega}\InKetWith{\Omega}{\Gamma}
\propto{}&\int
 \left[\prod_{\tau,\mathbf{x},m}\dd q_m(\mathbf{x},\tau)\right]
 \left[\prod_{\tau,\mathbf{x},m}\dd p_m(\mathbf{x},\tau)\right]
\notag\\
&\quad\cdot\exp\left\{-\ii\int \dd^4x\,
 p^{\mathsf T}\gamma^0[\gamma^\mu\partial_\mu+m+\Gamma-\ii\epsilon]q\right\}
...
from internal lines and vertices in this theory are $-\ii\mathcal{D}^{-1}$ and
$-\ii\mathcal{G}[\Gamma]$; the trace of the product of $n$ factors of
