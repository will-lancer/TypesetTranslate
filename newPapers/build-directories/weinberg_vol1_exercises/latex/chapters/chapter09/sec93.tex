...
\begin{align}
H[Q,P]={}&\frac12\sum_{nm}\int \dd^3x\,\dd^3y\,
 A_{xn,ym}[Q]P_n(\mathbf{x})P_m(\mathbf{y})
\notag\\
&+\sum_n\int \dd^3x\,B_{xn}[Q]P_n(\mathbf{x})+C[Q]
...
\begin{align}
&\int \dd\tau\left\{\int \dd^3x\sum_n p_n(\mathbf{x},\tau)
 \dot q_n(\mathbf{x},\tau)-H[q(\tau),p(\tau)]\right\}
\notag\\
&\quad=-\frac12\sum_{nm}\int \dd^3x\,\dd^3y\,\dd\tau\,\dd\tau'\,
 \mathcal A_{xn\tau,ym\tau'}[q]p_n(\mathbf{x},\tau)p_m(\mathbf{y},\tau')
\notag\\
&\qquad-\sum_n\int \dd^3x\int \dd\tau\,
...
\mathcal C[q]&\equiv\int \dd\tau\,C[q(\tau)].
...
\begin{align}
&\int_{-\infty}^{\infty}\left(\prod_s \dd\xi_s\right)
 \exp\left\{-\frac{\ii}{2}\sum_{sr}\mathcal A_{sr}\xi_s\xi_r
 -\ii\sum_s\mathcal B_s\xi_s-\ii\mathcal C\right\}
\notag\\
&\qquad=
 \left(\operatorname{Det}\left[\frac{\ii\mathcal A}{2\pi}\right]\right)^{-1/2}
 \exp\left\{-\frac{\ii}{2}\sum_{sr}\mathcal A_{sr}\bar\xi_s\bar\xi_r
 -\ii\sum_s\mathcal B_s\bar\xi_s-\ii\mathcal C\right\},
...
&\frac{\delta}{\delta p_n(\mathbf{x},\tau)}
 ...
 -H[q(\tau),p(\tau)]+\ii\epsilon\text{ terms}\right\}
\\
&\qquad=\dot q_n(\mathbf{x},\tau)
 -\frac{\delta}{\delta p_n(\mathbf{x},\tau)}H[q(\tau),p(\tau)].
\end{align*}
(The $\ii\epsilon$ terms depend only on the $q$s.) Thus the stationary `point'
...
\begin{equation}
L[q(t),\dot q(t)]\equiv\int \dd^3x\left(
...
&\OutBra{\Omega}T\left\{
 ...
 \InKet{\Omega}
\notag\\
&\quad=|\mathcal N|^2\int\prod_{\tau,\mathbf{x},n}\dd q_n(\mathbf{x},\tau)
 \left(\operatorname{Det}[2\ii\pi\mathcal A[q]]\right)^{-1/2}
\notag\\
&\qquad\cdot\mathcal O_A[q(t_A)]\mathcal O_B[q(t_B)]\cdots
\notag\\
&\qquad\cdot\exp\left[\ii\int_{-\infty}^{\infty}\dd\tau
 \left\{L[q(\tau),\dot q(\tau)]+\ii\epsilon\text{ terms}\right\}\right].
...
$\Phi(t+\dd\tau)$ and $\Phi(t)$, and then dividing by $\dd\tau$, with
$\dd\tau\to0$. Equivalently, as long as $t$ is not equal to any of the other
...
\[
H=\int \dd^3x\sum_n\left[\frac12\Pi_n^2+\frac12(\boldsymbol\nabla\Phi_n)^2
 +\mathbf J_n\cdot\boldsymbol\nabla\Phi_n+J_n^0\Pi_n
 +\frac12(J_n^0)^2\right]+\int \dd^3x\,V(\Phi).
\]
...
\]
The factor $\bigl(\operatorname{Det}[4\ii\pi\mathcal A[q]]\bigr)^{-1/2}$ in
...
\[
H=\int \dd^3x\left[\frac12\Pi_n(\mathbf{1}+U(\Phi))^{-1}_{nm}\Pi_m
...
$\sum_x\cdots=\Omega^{-1}\int \dd^4x\cdots$. The determinant factor here is
then
\[
\operatorname{Det}\mathcal A\propto
 \exp\left[-\Omega^{-1}\int \dd^4x\,
...
\begin{align*}
H=\int \dd^3x\sum_n\biggl[&\frac12\boldsymbol\Pi_n^2
 +\frac12(\boldsymbol\nabla\times\mathbf A_n)^2
...
Hamiltonian a term
\[
\Delta H=-\frac12\sum_n m_n^2\int \dd^3x
...
\begin{align*}
H+\Delta H=\int \dd^3x\sum_n\biggl[&\frac12\boldsymbol\Pi_n^2
 +\frac12(\boldsymbol\nabla\times\mathbf A_n)^2
...
$\sum_n\int \dd^3x\,\boldsymbol\Pi_n\cdot\dot{\mathbf A}_n-H-\Delta H$:
\[
\boldsymbol\Pi_n=\dot{\mathbf A}_n+\boldsymbol\nabla A_n^0.
\]
With $\boldsymbol\Pi_n$ eliminated in this way,
$\sum_n\int \dd^3x\,\boldsymbol\Pi_n\cdot\dot{\mathbf A}_n-H-\Delta H$ is just
...
&\OutBra{\Omega}T\left\{
 ...
 \InKet{\Omega}
\notag\\
&\quad\propto\int\prod_{\tau,\mathbf{x},n}\dd\psi_n(\mathbf{x},\tau)
 \mathcal O_A[\psi(t_A)]\mathcal O_B[\psi(t_B)]\cdots
\notag\\
&\qquad\cdot\exp\left[\ii\int_{-\infty}^{\infty}\dd\tau
 \left\{L[\psi(\tau),\dot\psi(\tau)]+\ii\epsilon\text{ terms}\right\}\right],
