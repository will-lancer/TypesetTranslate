...
instead has an adjoint $-\ii P_a$, in which case $\bra{\Omega}$ can be regarded as
...
\begin{equation}
Q_a\ket{a,b,c,\ldots}=\ii\ket{b,c,\ldots},
...
\begin{equation}
\bra{a,b,\ldots}\equiv\bra{\Omega}\cdots(-\ii Q_b)(-\ii Q_a).
...
&=\bra{\Omega}\cdots(-\ii Q_d)(-\ii Q_c)P_aP_b\cdots\ket{\Omega}\notag\\
&=\begin{cases}
0&\text{if }\{c,d,\ldots\}\ne\{a,b,\ldots\},\\
...
\begin{equation}
\ket{q}=\exp\left(-\ii\sum_aP_aq_a\right)\ket{\Omega}
...
&\equiv\bra{\Omega}\left(\prod_aQ_a\right)
 \exp\left(-\ii\sum_aq_aP_a\right)\notag\\
&=\bra{\Omega}\left(\prod_aQ_a\right)
 \exp\left(+\ii\sum_aP_aq_a\right),
...
\braket{q'}{q}
&=\bra{\Omega}\left(\prod_aQ_a\right)
 \exp\left(\ii\sum_bP_b(q'_b-q_b)\right)\ket{\Omega}\\
&=\bra{\Omega}\left(\prod_aQ_a\right)
 \left(\prod_b[1+\ii P_b(q'_b-q_b)]\right)\ket{\Omega}.
\end{align*}
Moving each $Q_a$ to the right (starting with the rightmost) yields factors
$\ii^2(q'_a-q_a)$, which we move to the right out of the scalar product, so
\begin{equation}
...
other), and
\begin{equation}
\ket{p}=\exp\left(-\ii\sum_aQ_ap_a\right)
 \left(\prod_bP_b\right)\ket{\Omega},
\tag{9.5.23}\label{eq:9.5.23}
\end{equation}
\begin{equation}
\bra{p}=\bra{\Omega}\exp\left(-\ii\sum_a p_aQ_a\right)
...
\begin{align*}
\braket{q}{p}
&=\bra{q}\exp\left(-\ii\sum_aQ_ap_a\right)
 \left(\prod_aP_a\right)\ket{\Omega}\\
&=\left(\prod_a\exp(-\ii q_ap_a)\right)
...
and so
\begin{equation}
\braket{q}{p}=\chi_N\exp\left(-\ii\sum_aq_ap_a\right)
=\chi_N\exp\left(\ii\sum_ap_aq_a\right),
...
\begin{equation}
\braket{p}{q}=\prod_a\exp(-\ii p_aq_a).
...
\begin{equation}
\int\left(\widetilde{\prod_n}\dd\xi_n\right)f(\xi)\equiv c
...
likewise antisymmetric under the interchange of any two $\dd\xi$s, so these
`differentials' effectively anticommute
\begin{equation}
\dd\xi_n\,\dd\xi_m+\dd\xi_m\,\dd\xi_n=0.
...
\[
\int \dd\xi_1\,f(\xi_1,\xi_2)=\xi_2c_{12}+c_1,
\qquad
\int \dd\xi_2\,f(\xi_1,\xi_2)=-\xi_1c_{12}+c_2,
\]
\[
\int \dd\xi_2\,\dd\xi_1\,f(\xi_1,\xi_2)=c_{12}.
\]
Note that the multiple integral is the same as a repeated integral:
\[
\int \dd\xi_2\,\dd\xi_1\,f(\xi_1,\xi_2)
=\int \dd\xi_2\left[\int \dd\xi_1\,f(\xi_1,\xi_2)\right],
\]
...
\[
\int \dd\xi_1\,f(\xi_1,\xi_2,\ldots)=c(\xi_2,\ldots).
\]
...
\begin{align}
\int\left(\widetilde{\prod_n}\dd\xi_n\right)[f(\xi)+g(\xi)]
={}&\int\left(\widetilde{\prod_n}\dd\xi_n\right)f(\xi)
+\int\left(\widetilde{\prod_n}\dd\xi_n\right)g(\xi)
\tag{9.5.31}\label{eq:9.5.31}
\end{align}
and also
\begin{equation}
\int\left(\widetilde{\prod_n}\dd\xi_n\right)[f(\xi)a(\xi')]
=\left[\int\left(\widetilde{\prod_n}\dd\xi_n\right)f(\xi)\right]a(\xi'),
...
and so
\begin{equation}
\int\left(\widetilde{\prod_n}\dd\xi_n\right)
[a((-1)^\nu\xi')f(\xi)]
=a(\xi')\int\left(\widetilde{\prod_n}\dd\xi_n\right)f(\xi).
\tag{9.5.33}\label{eq:9.5.33}
\end{equation}
It is therefore very convenient (though not strictly necessary) to take the
differentials $\dd\xi_n$ to anticommute with all anticommuting variables
(including the $\xi_n$):
\begin{equation}
(\dd\xi_n)\xi'_m+\xi'_m(\dd\xi_n)=0
\tag{9.5.34}\label{eq:9.5.34}
\end{equation}
in which case Eq. \eqref{eq:9.5.33} reads more simply
\begin{equation}
\int a(\xi')\left(\widetilde{\prod_n}\dd\xi_n\right)f(\xi)
=a(\xi')\int\left(\widetilde{\prod_n}\dd\xi_n\right)f(\xi).
...
\begin{equation}
\int\left(\widetilde{\prod_n}\dd\xi_n\right)f(\xi+\xi')
=\int\left(\widetilde{\prod_n}\dd\xi_n\right)f(\xi)
...
\begin{equation}
\int\left(\widetilde{\prod_n}\dd\xi'_n\right)f
=(\operatorname{Det}\mathcal{G})^{-1}
 \int\left(\widetilde{\prod_n}\dd\xi_n\right)f.
...
\begin{equation}
\ket{f}=\int\left(\widetilde{\prod_a}\dd q_a\right)\ket{q}f(q)
=\int\ket{q}\left(\widetilde{\prod_a}\dd q_a\right)f(q).
...
\braket{q'}{f}=\int\left(\prod_a(q_a-q'_a)\right)
 \left(\widetilde{\prod_b}\dd q_b\right)f(q).
\]
Moving every factor $(q_a-q'_a)$ to the right past every differential $\dd q_b$
yields a sign factor $(-)^{N^2}=(-)^N$, where $N$ is now the total number of
$q_a$ variables, so
\[
\braket{q'}{f}=(-)^N\int\left(\widetilde{\prod_b}\dd q_b\right)
