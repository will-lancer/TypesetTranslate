...
$\delta_{\sigma'\sigma}\exp(\ii\sigma\theta(\Lambda,p))$, where $\theta$ is
...
which $U(\Lambda,a)=\exp(\ii H\tau)$, Eq.~\eqref{eq:3.1.1} requires among other things
...
$U(\mathbf{1},-\tau)\ket{\Psi}=\exp(-\ii H\tau)\ket{\Psi}$. Thus the appearance
...
$\exp(-\ii H\tau)$ with $\tau\to-\infty$ or $\tau\to+\infty$, respectively. Of
course, if the state is really an energy eigenstate, then it cannot be
localized in time --- the operator $\exp(-\ii H\tau)$ yields an inconsequential
phase factor $\exp(-\ii E_\alpha\tau)$. Therefore, we must consider
wave-packets, superpositions $\int \dd\alpha\,g(\alpha)\ket{\alpha}$ of states,
...
\[
\exp(-\ii H\tau)\int \dd\alpha\,g(\alpha)
\InOutKet{\alpha}
=\int \dd\alpha\,e^{-\ii E_\alpha\tau}g(\alpha)
\InOutKet{\alpha}
\]
...
\InKet{\alpha}
&\longrightarrow
\int \dd\alpha\,e^{-\ii E_\alpha\tau}g(\alpha)\ket{\alpha}
&& (\tau\to-\infty),
\notag\\
\int \dd\alpha\,e^{-\ii E_\alpha\tau}g(\alpha)
\OutKet{\alpha}
...
\InOutKet{\alpha}
\longrightarrow
\exp(-\ii H_0\tau)\int \dd\alpha\,g(\alpha)\ket{\alpha}
\end{align*}
...
where
\begin{equation}
\Omega(\tau)\equiv\exp(+\ii H\tau)\exp(-\ii H_0\tau).
...
unitary operator $\exp(-\ii H\tau)$ act on a time-independent state, its norm is
...
\begin{align*}
&\int \dd\alpha\,\dd\beta\,
\exp\bigl(-\ii(E_\alpha-E_\beta)\tau\bigr)
g(\alpha)g^*(\beta)
...
\begin{align}
\InKet{\alpha}
&=\ket{\alpha}+(E_\alpha-H_0+\ii\epsilon)^{-1}
...
\begin{align}
\InKet{\alpha}
&=\ket{\alpha}+\int \dd\beta\,
\frac{T^{\mathrm{in}}_{\beta\alpha}\ket{\beta}}
{E_\alpha-E_\beta+\ii\epsilon},
\notag\\
\OutKet{\alpha}
&=\ket{\alpha}+\int \dd\beta\,
...
It remains to be shown that Eq.~\eqref{eq:3.1.17}, with a $+\ii\epsilon$ or
$-\ii\epsilon$ in the denominator, satisfies the condition \eqref{eq:3.1.12} for an
`in' or an `out' state, respectively. For this purpose, consider the
superpositions
\begin{equation}
\InOutKet{\Psi_g(t)}
\equiv\int \dd\alpha\,e^{-\ii E_\alpha t}g(\alpha)
\InOutKet{\alpha},
\tag{3.1.19}\label{eq:3.1.19}
\end{equation}
\begin{equation}
\ket{\Phi_g(t)}\equiv
\int \dd\alpha\,e^{-\ii E_\alpha t}g(\alpha)\ket{\alpha}.
...
\begin{align}
\InOutKet{\Psi_g(t)}
=\ket{\Phi_g(t)}+\int \dd\alpha\int \dd\beta\,
\frac{e^{-\ii E_\alpha t}g(\alpha)
T^{\mathrm{in/out}}_{\beta\alpha}\ket{\beta}}
{E_\alpha-E_\beta\mathbin{\pm}\ii\epsilon},
...
$\exp(-\ii E_\alpha t)$, which is exponentially small for $t\to-\infty$ and
...
$(E_\alpha-E_\beta\mathbin{\pm}\ii\epsilon)^{-1}$, which is in the upper
...
$(E_\alpha-E_\beta\mathbin{\pm}\ii\epsilon)^{-1}$ in Eq.~\eqref{eq:3.1.17}. In
general, we can write
\begin{equation}
(E\mathbin{\pm}\ii\epsilon)^{-1}
=\frac{\Pi_\epsilon}{E}\mp \ii\pi\delta_\epsilon(E),
