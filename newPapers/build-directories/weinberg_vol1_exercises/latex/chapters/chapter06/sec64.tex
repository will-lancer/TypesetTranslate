...
replaced with
\begin{equation}
H_{I,\epsilon}(t)=H_I(t)+\sum_a\int d^3x\,
...
picture
\begin{equation}
o_a(t)=\exp(iH_0t)o_a(0)\exp(-iH_0t),
\tag{6.4.2}
\label{eq:6.4.2}
\end{equation}
...
equal to $-i\epsilon_a(x)$ times whatever numerical factors appear in $o_a(x)$.
...
coefficient function like $(2\pi)^{-3/2}u_\ell(\mathbf p,\sigma)e^{ip\cdot x}$
or $(2\pi)^{-3/2}u_\ell^*(\mathbf p,\sigma)e^{-ip\cdot x}$ but a
\textit{propagator}, as well as a factor $-i$ from the vertex at the end of the
...
coefficient functions $u_\ell,u_\ell^*$, etc. and a factor $(-i)^r$.
...
picture
\begin{equation}
O_a(\mathbf x,t)=\exp(iHt)o_a(\mathbf x,0)\exp(-iHt)
=\Omega(t)o_a(\mathbf x,t)\Omega^{-1}(t),
\tag{6.4.4}
\label{eq:6.4.4}
\end{equation}
\begin{equation}
\Omega(t)\equiv e^{iHt}e^{-iH_0t},
\tag{6.4.5}
\label{eq:6.4.5}
\end{equation}
and $\ket{\beta}_{\mathrm{in}}$ and $\ket{\beta}_{\mathrm{out}}$ are ``in'' and
...
\right]_{\epsilon=0}\notag\\
&=\sum_{N=0}^{\infty}\frac{(-i)^{N+r}}{N!}
\int_{-\infty}^{\infty}d\tau_1\cdots d\tau_N
...
&\quad\cdot
\int_{x_1^0}^{\infty}d\tau_{01}\cdots d\tau_{0N_0}
\int_{x_2^0}^{x_1^0}d\tau_{11}\cdots d\tau_{1N_1}\cdots
\int_{-\infty}^{x_r^0}d\tau_{r1}\cdots d\tau_{rN_r}\\
&\quad\cdot\bra{\beta}
...
$(-i)^N$ equal to $N_0+N_1+\cdots+N_r$. This gives
\begin{align}
&\left[
\frac{\delta^rS[\epsilon]}
{\delta\epsilon_{a_1}(x_1^0)\cdots\delta\epsilon_{a_r}(x_r^0)}
\right]_{\epsilon=0}\notag\\
&=(-i)^r\bra{\beta}U(\infty,x_1^0)o_{a_1}(x_1)
...
where
\begin{equation}
U(t',t)=\sum_{N=0}^{\infty}\frac{(-i)^N}{N!}
\int_t^{t'}d\tau_1\cdots d\tau_N\,
...
\begin{equation}
U(t',t)=\exp(iH_0t')\exp\{-iH(t'-t)\}\exp(-iH_0t)
=\Omega^{-1}(t')\Omega(t)
\tag{6.4.11}
\label{eq:6.4.11}
\end{equation}
