...
\end{equation}
Let $\ii(2\pi)^4\Sigma^*(\sl{k})$ be the sum of all connected graphs, with one
...
$-\ii(2\pi)^{-4}$ and $[\ii\sl{k}+m-\ii\epsilon]^{-1}$ omitted. (Lorentz invariance
...
\begin{equation}
\begin{split}
S'(k)={}&[\ii\sl{k}+m-\ii\epsilon]^{-1}
+[\ii\sl{k}+m-\ii\epsilon]^{-1}\Sigma^*(\sl{k})
[\ii\sl{k}+m-\ii\epsilon]^{-1}
\\
&+[\ii\sl{k}+m-\ii\epsilon]^{-1}\Sigma^*(\sl{k})
[\ii\sl{k}+m-\ii\epsilon]^{-1}\Sigma^*(\sl{k})
[\ii\sl{k}+m-\ii\epsilon]^{-1}+\cdots
\\
={}&[\ii\sl{k}+m-\Sigma^*(\sl{k})-\ii\epsilon]^{-1}.
\end{split}
...
contributions:
\begin{equation}
\Sigma^*(\sl{k})=-(Z_2-1)[\ii\sl{k}+m]+Z_2\delta m
...
and hence
\begin{equation}
Z_2\delta m=-\Sigma^*_{\mathrm{LOOP}}(\ii m),
\tag{10.3.32}\label{eq:10.3.32}
\end{equation}
\begin{equation}
Z_2=1-\ii\left.\frac{\partial\Sigma^*_{\mathrm{LOOP}}(\sl{k})}
{\partial\sl{k}}\right|_{\sl{k}=\ii m}.
\tag{10.3.33}\label{eq:10.3.33}
\end{equation}
Just as for scalars, the vanishing of
$[\ii\sl{k}+m]^{-1}\Sigma^*(\sl{k})$ in the limit $\sl{k}\to \ii m$ tells us that
...
gauge transformations $\Psi_\ell\to\exp(\ii q_\ell\alpha)\Psi_\ell$ (with
...
time-independent:
\begin{equation}
\ii\frac{\dd}{\dd t}Q=[Q,H]=0,
\tag{10.4.3}\label{eq:10.4.3}
\end{equation}
where
\begin{equation}
Q\equiv\int \dd^3x\,J^0.
...
$\Psi_\ell\to\exp(\ii q_\ell\alpha)\Psi_\ell$, does nothing to fix the over-all
...
$[\partial_\mu-\ii q_\ell A_\mu]\Psi_\ell$, so that the current $J^\mu$ is
\begin{equation}
...
[\partial_\mu-\ii q_{\mathrm B\ell}A_{{\mathrm B}\mu}]\Psi_\ell\bigr).
\end{split}
...
\input{figures/chapter10/fig105.tex}

of the charged particle by

\begin{equation}
\begin{split}
&\int \dd^4x\,\dd^4y\,\dd^4z\,e^{-\ii p\cdot x}e^{-\ii k\cdot y}e^{+\ii\ell\cdot z}
\bra{\Omega}T\{J^\mu(x)\Psi_n(y)\overline\Psi_m(z)\}\ket{\Omega}
\\
&\qquad\equiv-\ii qS'_{nn'}(k)\Gamma^\mu_{n'm'}(k,\ell)
...
where
\begin{equation}
-\ii S'_{nm}(k)\delta^4(k-\ell)\equiv
\int \dd^4y\,\dd^4z\,
\bra{\Omega}T\{\Psi_n(y)\overline\Psi_m(z)\}\ket{\Omega}
e^{-\ii k\cdot y}e^{+\ii\ell\cdot z}.
