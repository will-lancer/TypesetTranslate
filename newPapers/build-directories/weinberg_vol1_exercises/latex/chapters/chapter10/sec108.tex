...
\begin{align}
  ...
  &\quad\cdot\int \dd^4x\int \dd^4y\,e^{-\ii k'\cdot y}e^{\ii k\cdot x}
  (\ii\Box_y)(\ii\Box_x)
...
differential operators $-\ii\Box_x$ and $-\ii\Box_y$ are inserted to supply
...
element as $S=-2\pi \ii\delta^4(k'-k)M(\omega)$, where
\begin{equation}
  M(\omega)=\frac{-\ii}{2\omega|N|^2}F(\omega),
  \tag{10.8.3}\label{eq:10.8.3}
\end{equation}
\begin{equation}
  F(\omega)\equiv\int \dd^4x\,e^{\ii\omega\ell\cdot x}
...
where
\begin{equation}
  F_A(\omega)\equiv\int \dd^4x\,\theta(-x^0)
  \bra{\alpha}[J^\dagger(0),J(x)]\ket{\alpha}
  e^{\ii\omega\ell\cdot x}+\mathrm{ETC},
  \tag{10.8.7}\label{eq:10.8.7}
\end{equation}
\begin{equation}
  F_R(\omega)\equiv-\int \dd^4x\,\theta(x^0)
  \bra{\alpha}[J^\dagger(0),J(x)]\ket{\alpha}
  e^{\ii\omega\ell\cdot x}+\mathrm{ETC},
  \tag{10.8.8}\label{eq:10.8.8}
\end{equation}
\begin{equation}
  F_+(\omega)\equiv\int \dd^4x\,
  \bra{\alpha}J(x)J^\dagger(0)\ket{\alpha}e^{\ii\omega\ell\cdot x},
  \tag{10.8.9}\label{eq:10.8.9}
\end{equation}
\begin{equation}
  F_-(\omega)\equiv\int \dd^4x\,
  \bra{\alpha}J^\dagger(0)J(x)\ket{\alpha}e^{\ii\omega\ell\cdot x}.
...
$e^{\ii\omega\ell\cdot x}$ provides a cutoff for the integral over $x^\mu$.
...
\begin{equation}
  \mathcal F(E+\ii\epsilon)-\mathcal F(E-\ii\epsilon)
...
\begin{equation}
  ...
  \frac{\mathcal F(z)\,\dd z}{(z-\omega)P(z)},
  \tag{10.8.13}\label{eq:10.8.13}
\end{equation}
where $\omega$ is any point off the real axis, and $C$ is a contour consisting
of two segments: one running just above the real axis from $-\infty+\ii\epsilon$
to $+\infty+\ii\epsilon$ and then around a large semi-circle back to
$-\infty+\ii\epsilon$, and the other just below the real axis from
$+\infty-\ii\epsilon$ to $-\infty-\ii\epsilon$ and then around a large
semi-circle back to $+\infty-\ii\epsilon$. Because the function
...
\begin{equation}
  \mathcal F(\omega)=Q(\omega)+\frac{P(\omega)}{2\pi \ii}
  \int_{-\infty}^{+\infty}
  \frac{F_-(E)-F_+(E)}{(E-\omega)P(E)}\,\dd E,
...
\begin{equation}
  F_A(\omega)=Q(\omega)+\frac{P(\omega)}{2\pi \ii}
  \int_{-\infty}^{+\infty}
  \frac{F_-(E)-F_+(E)}{(E-\omega-\ii\epsilon)P(E)}\,\dd E.
...
\begin{align}
  ...
  \notag\\
  &{}+\frac{P(\omega)}{2\pi \ii}\int_{-\infty}^{+\infty}
  \frac{F_-(E)-F_+(E)}{(E-\omega)P(E)}\,\dd E
...
\begin{equation}
  ...
  \left|\OutBra{\beta}J(0)^\dagger\ket{\alpha}\right|^2
  \delta^4(-p_\alpha+E\ell+p_\beta),
  \tag{10.8.17}\label{eq:10.8.17}
\end{equation}
\begin{equation}
  F_-(E)=(2\pi)^4\sum_\beta
  \left|\OutBra{\beta}J(0)\ket{\alpha}\right|^2
  \delta^4(p_\alpha+E\ell-p_\beta).
...
\begin{equation}
  -2\ii\pi M_{B^c+\alpha\to\beta}
  =\frac{(2\pi)^4}{(2\pi)^{3/2}\sqrt{2E_{B^c}}N}
  \OutBra{\beta}J^\dagger(0)\ket{\alpha},
  \tag{10.8.19}\label{eq:10.8.19}
\end{equation}
\begin{equation}
  -2\ii\pi M_{B+\alpha\to\beta}
  =\frac{(2\pi)^4}{(2\pi)^{3/2}\sqrt{2E_B}N}
  \OutBra{\beta}J(0)\ket{\alpha}.
...
\begin{align}
  M(\omega)={}&\frac{-\ii Q(\omega)}{2\omega|N|^2}
  -\frac{\ii}{2(2\pi)^3}\sigma_{\alpha+B}(\omega)\notag\\
  &{}-\frac{P(\omega)}{\omega(2\pi)^4}\int_0^\infty
  \left[\frac{\sigma_{\alpha+B}(E)}{(E-\omega)P(E)}
  +\frac{\sigma_{\alpha+B^c}(E)}{(E+\omega)P(-E)}\right]E\,\dd E.
...
by $f(\omega)=-4\pi^2\omega M(\omega)=2\pi^2\ii F(\omega)/|N|^2$, so
Eq.~\eqref{eq:10.8.23} now reads
\begin{align*}
  f(\omega)={}&R(\omega)+\frac{\ii\omega}{4\pi}
  \sigma_{\alpha+B}(\omega)\\
  &{}+\frac{P(\omega)}{4\pi^2}\int_0^\infty
  \left[\frac{\sigma_{\alpha+B}(E)}{(E-\omega)P(E)}
  +\frac{\sigma_{\alpha+B^c}(E)}{(E+\omega)P(-E)}\right]E\,\dd E,
\end{align*}
where $R(\omega)\equiv2\ii\pi^2Q(\omega)/|N|^2$. The optical theorem
\eqref{eq:3.6.4} tells us that the second term on the right-hand side equals
$\ii\operatorname{Im}f(\omega)$, so this can just as well be written in the more
conventional form
\begin{align}
...
\begin{align}
  ...
  +\frac{\sigma_{\alpha+B^c}(E)}{E+\omega}\right]\frac{\dd E}{E},
