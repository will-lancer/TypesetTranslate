...
$N$ soft photons into a volume $\prod_r d^3q_r$ of momentum space is given by
squaring this matrix element, summing over helicities, and multiplying with
$\prod_r d^3q_r$. We recall from Eq. \eqref{eq:8.5.7} that for $q^2=0$, the
...
for $|\mathbf q|d\Gamma^\lambda_{\beta\alpha}(\mathbf q)/
...
\begin{align}
d\Gamma^\lambda_{\beta\alpha}(\mathbf q_1,\ldots,\mathbf q_N)
={}&\Gamma^\lambda_{\beta\alpha}
\prod_{r=1}^{N}\frac{d^3q_r}{(2\pi)^3(2|\mathbf q_r|)}
...
\begin{equation}
d\Gamma^\lambda_{\beta\alpha}(\omega_1,\ldots,\omega_N)
=\Gamma^\lambda_{\beta\alpha}A(\alpha\to\beta)^N
\frac{d\omega_1}{\omega_1}\cdots\frac{d\omega_N}{\omega_N},
...
\prod_{r=1}^{N}\frac{d\omega_r}{\omega_r}.
...
\frac{d\omega}{\omega}e^{i\omega u}\right)
\Gamma^\lambda_{\beta\alpha}.
\tag{13.3.8}\label{eq:13.3.8}
\end{equation}
The integral in the exponent can be done in the limit $\lambda\ll E_T$ by
writing it as the sum of the integral of $(e^{i\omega u}-1)/\omega$, in which
...
\begin{align}
\mathcal F(x;A)
\equiv{}&\frac{1}{\pi}\int_{-\infty}^{\infty}du\,
\frac{\sin u}{u}\exp\left(A\int_0^x\frac{d\omega}{\omega}
(e^{i\omega u}-1)\right)\notag\\
={}&1-\frac{A^2\theta(x-\tfrac12)}{2}
\int_{1-x}^{x}\frac{d\omega}{\omega}
