...
\begin{align}
\psi_\ell(x)={}&(2\pi)^{-3/2}\int \dd^3p\sum_\sigma
\bigl[\kappa a_{\mathbf p,\sigma}u_\ell(\mathbf p,\sigma)e^{\ii p\cdot x}
+\lambda a^{c\dagger}_{\mathbf p,\sigma}
v_\ell(\mathbf p,\sigma)e^{-\ii p\cdot x}\bigr]
...
\sqrt{\frac{(\Lambda p)^0}{p^0}}\exp(\ii\sigma\theta(p,\Lambda))
...
must satisfy
\begin{align}
u_{\bar\ell}(\mathbf k,\sigma)\exp(\ii\sigma\theta(k,W))
&=\sum_\ell D_{\bar\ell\ell}(W)u_\ell(\mathbf k,\sigma),
\tag{5.9.10}\label{eq:5.9.10}\\
v_{\bar\ell}(\mathbf k,\sigma)\exp(-\ii\sigma\theta(k,W))
...
\begin{align}
u_{\bar\ell}(\mathbf k,\sigma)e^{\ii\sigma\theta}
&=\sum_\ell D_{\bar\ell\ell}(R(\theta))u_\ell(\mathbf k,\sigma),
\tag{5.9.12}\label{eq:5.9.12}\\
v_{\bar\ell}(\mathbf k,\sigma)e^{-\ii\sigma\theta}
...
\begin{align}
e^\mu(\mathbf k,\sigma)e^{\ii\sigma\theta}
&=R(\theta)^\mu{}_\nu e^\nu(\mathbf k,\sigma),
...
\begin{equation}
e^\mu(\mathbf k,\pm1)=(1,\pm \ii,0,0)/\sqrt2.
\tag{5.9.21}\label{eq:5.9.21}
\end{equation}
But then Eq.~\eqref{eq:5.9.20} would require also that $\alpha+\ii\beta=0$, which is
...
e^\nu(\mathbf k,\pm1)
\notag\\
&=\exp(\pm \ii\theta)\left\{e^\mu(\mathbf k,\pm1)
+\frac{\alpha\pm \ii\beta}{\sqrt{2}|\mathbf k|}k^\mu\right\}.
...
\begin{align}
a_\mu(x)={}&\int \frac{\dd^3 p}{\sqrt{(2\pi)^3 2p^0}} \,
\cdot\sum_{\sigma=\pm1}\bigl[e_\mu(\mathbf p,\sigma)e^{\ii p\cdot x}
a_{\mathbf p,\sigma}+e_\mu(\mathbf p,\sigma)^*e^{-\ii p\cdot x}
...
\begin{equation}
e^\mu(\mathbf p_\Lambda,\pm1)\exp(\pm \ii\theta(p,\Lambda))
=D^\mu{}_\nu(\Lambda)e^\nu(\mathbf p,\pm1)
...
\begin{equation}
u_{\mu\nu}(\mathbf p,\pm1)=\frac{\ii}{\sqrt{(2\pi)^3 2p^0}}
...
\notag\\
&\cdot\int \dd^3p\,(2p^0)^{-1}
\bigl[|\kappa|^2e^{\ii p\cdot(x-y)}-|\lambda|^2e^{-\ii p\cdot(x-y)}\bigr].
...
$\int \dd^3x\,\theta^{0\mu_2\cdots\mu_N}$ is a conserved quantity that
...
&=(J_2^{(A)}+\ii J_1^{(A)})_{aa'}u_{a'b}(\mathbf k,\sigma)
+(J_2^{(B)}-\ii J_1^{(B)})_{bb'}u_{ab'}(\mathbf k,\sigma),
\end{align*}
\begin{align*}
0&=(\mathcal J_{32}+\mathcal J_{02})_{ab,a'b'}
u_{a'b'}(\mathbf k,\sigma)\\
&=(-J_1^{(A)}+\ii J_2^{(A)})_{aa'}u_{a'b}(\mathbf k,\sigma)
+(-J_1^{(B)}-\ii J_2^{(B)})_{bb'}u_{ab'}(\mathbf k,\sigma),
\end{align*}
or more simply
\[
(J_1^{(A)}-\ii J_2^{(A)})_{aa'}u_{a'b}(\mathbf k,\sigma)=0,
\]
\[
(J_1^{(B)}+\ii J_2^{(B)})_{bb'}u_{ab'}(\mathbf k,\sigma)=0.
\]
