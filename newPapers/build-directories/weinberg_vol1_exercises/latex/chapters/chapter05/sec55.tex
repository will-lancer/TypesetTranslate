...
\begin{equation}
\psi_\ell^{(+)}(x)=(2\pi)^{-3/2}\sum_\sigma\int \dd^3p\,
u_\ell(\mathbf p,\sigma)e^{\ii p\cdot x}a_{\mathbf p,\sigma}
\tag{5.5.1}\label{eq:5.5.1}
\end{equation}
and
\begin{equation}
\psi_\ell^{(-c)}(x)=(2\pi)^{-3/2}\sum_\sigma\int \dd^3p\,
v_\ell(\mathbf p,\sigma)e^{-\ii p\cdot x}a^{c\dagger}_{\mathbf p,\sigma},
...
$D_{\ell\bar\ell}(\Lambda)$ in both cases taken as the $4\times4$ Dirac
...
$-\mathbf J^{(j)*}$ and the $2\times2$ matrices
...
\qquad
v_{m,\pm}(\mathbf0,\sigma)=\ii d_\pm(\sigma_2)_{m\sigma}.
...
\sum_\sigma\int \dd^3p\,u_\ell(-\mathbf p,\sigma)
e^{\ii p\cdot\mathcal P x}a_{\mathbf p,\sigma},
\tag{5.5.10}\label{eq:5.5.10}
\end{equation}
\begin{equation}
P\psi_\ell^{(-c)}(x)P^{-1}=\eta^c(2\pi)^{-3/2}
\sum_\sigma\int \dd^3p\,v_\ell(-\mathbf p,\sigma)
e^{-\ii p\cdot\mathcal P x}a^{c\dagger}_{\mathbf p,\sigma}.
...
=\int \dd^3p\bigl[|\kappa|^2N_{\ell\bar\ell}(\mathbf p)e^{\ii p\cdot(x-y)}
\mp|\lambda|^2M_{\ell\bar\ell}(\mathbf p)e^{-\ii p\cdot(x-y)}\bigr],
...
\]
We also recall that $\beta=-\ii\gamma^0$, so by using the Lorentz transformation
rule \eqref{eq:5.4.8} we have
\begin{equation}
D(L(\mathbf p))\beta D^{-1}(L(\mathbf p))
=-\ii L_\mu{}^0(\mathbf p)\gamma^\mu=-\ii p^\mu\gamma_\mu/m.
...
\begin{equation}
N(\mathbf p)=\frac1{2p^0}[-\ii p^\mu\gamma_\mu+b_um]\beta,
\tag{5.5.27}\label{eq:5.5.27}
\end{equation}
\begin{equation}
M(\mathbf p)=\frac1{2p^0}[-\ii p^\mu\gamma_\mu+b_vm]\beta.
...
\[
\Delta_+(x)\equiv\int\frac{\dd^3p}{2p^0(2\pi)^3}e^{\ii p\cdot x}.
\]
...
\end{equation}
Finally if we like we can replace $\psi$ with $\gamma_5\psi$, which changes the
...
\begin{align}
\psi_\ell(x)=(2\pi)^{-3/2}\sum_\sigma\int \dd^3p\,
\bigl[u_\ell(\mathbf p,\sigma)e^{\ii p\cdot x}a_{\mathbf p,\sigma}
+v_\ell(\mathbf p,\sigma)e^{-\ii p\cdot x}a^{c\dagger}_{\mathbf p,\sigma}\bigr]
...
The spin sums are
\begin{equation}
N(\mathbf p)=\frac1{2p^0}[-\ii p^\mu\gamma_\mu+m]\beta,
\tag{5.5.37}\label{eq:5.5.37}
\end{equation}
\begin{equation}
M(\mathbf p)=\frac1{2p^0}[-\ii p^\mu\gamma_\mu-m]\beta,
...
$v(\mathbf p,\sigma)$ are eigenvectors of $-\ii p^\mu\gamma_\mu/m$ with
eigenvalues $+1$ and $-1$, respectively:
\begin{equation}
(\ii p^\mu\gamma_\mu+m)u(\mathbf p,\sigma)=0,
\qquad
(-\ii p^\mu\gamma_\mu+m)v(\mathbf p,\sigma)=0.
...
\[
[\exp(\tfrac12\ii\mathcal{J}^{\mu\nu}\omega_{\mu\nu})]^*
=\beta\mathcal{C}\exp(\tfrac12\ii\mathcal{J}^{\mu\nu}\omega_{\mu\nu})
\mathcal{C}^{-1}\beta
\]
...
\[
\ket{\Phi}=\sum_{\sigma,\sigma'}\int \dd^3p\int \dd^3p'\,
...
is transformed into
\[
C\ket{\Phi}=\xi^*\xi^{c*}\sum_{\sigma,\sigma'}\int \dd^3p\int \dd^3p'\,
...
this as
\[
C\ket{\Phi}=-\sum_{\sigma,\sigma'}\int \dd^3p\int \dd^3p'\,
\chi(\mathbf p',\sigma';\mathbf p,\sigma)a^\dagger_{\mathbf p,\sigma}
a^{c\dagger}_{\mathbf p',\sigma'}\ket{\Omega}.
\]
...
\begin{align*}
T\psi_\ell(x)T^{-1}={}&(2\pi)^{-3/2}\sum_\sigma\int \dd^3p\,
(-1)^{1/2-\sigma}\\
&\cdot[\zeta^*u_\ell^*(\mathbf p,\sigma)e^{-\ii p\cdot x}a_{-\mathbf p,-\sigma}
+\zeta^c v_\ell^*(\mathbf p,\sigma)e^{\ii p\cdot x}
...
that $\mathcal{J}^{i0}$ anticommutes with $\beta$ and commutes with $\gamma_5$
together with our former result for $D(L(\mathbf p))^*$ to write
\[
D^*(L(-\mathbf p))=\gamma_5\beta D^*(L(\mathbf p))\beta\gamma_5
=\gamma_5\mathcal{C}D(L(\mathbf p))\mathcal{C}^{-1}\gamma_5.
\]
Also, Eqs.~\eqref{eq:5.4.36} and \eqref{eq:5.5.35}--\eqref{eq:5.5.36} give
\[
\gamma_5\mathcal{C}^{-1}u(\mathbf0,-\sigma)
=(-1)^{1/2-\sigma}u(\mathbf0,\sigma),
\]
\[
\gamma_5\mathcal{C}^{-1}v(\mathbf0,-\sigma)
=(-1)^{1/2-\sigma}v(\mathbf0,\sigma),
\]
so
\begin{equation}
(-1)^{1/2+\sigma}u^*(-\mathbf p,-\sigma)
=-\gamma_5\mathcal{C}u(\mathbf p,\sigma),
\tag{5.5.51}\label{eq:5.5.51}
\end{equation}
\begin{equation}
(-1)^{1/2+\sigma}v^*(-\mathbf p,-\sigma)
=-\gamma_5\mathcal{C}v(\mathbf p,\sigma).
...
and in this case
\begin{equation}
T\psi(x)T^{-1}=-\zeta^*\gamma_5\mathcal{C}\psi(-\mathcal T x).
...
$\mathcal{J}^{\mu\nu}$, $\gamma_5\gamma^\mu$, or $\gamma_5$ yields a bilinear
...
replaced with any one of the five covariant types of $4\times4$ matrices
$\mathbf{1}$, $\gamma^\mu$, $\mathcal{J}^{\mu\nu}$,
$\gamma_5\gamma^\mu$, or $\gamma_5$. (As discussed in Chapter 2, we are
...
$\bar\psi_pM\psi_n\,\bar\psi_eM\gamma_5\psi_\nu$, with $M$ running over the
matrices $\mathbf{1}$, $\gamma^\mu$, $\mathcal{J}^{\mu\nu}$,
$\gamma_5\gamma^\mu$, or $\gamma_5$.
...
\begin{align}
C(\bar\psi M\psi)C^{-1}
&=(\beta\mathcal{C}\psi)^{\mathsf T}\beta M(\beta\mathcal{C}\psi^*)
=-(\beta\mathcal{C}\psi^*)^{\mathsf T}M^{\mathsf T}\mathcal{C}\psi
\notag\\
&=\bar\psi\mathcal{C}^{-1}M^{\mathsf T}\mathcal{C}\psi
=\mathbin{\pm}\bar\psi M\psi,
\tag{5.5.58}\label{eq:5.5.58}
\end{align}
the sign in the last expression being $+$ for the matrices $\mathbf{1}$,
$\gamma_5\gamma_\mu$, and $\gamma_5$, and $-$ for $\gamma_\mu$ and
