...
As in Section 10.3, we define $\ii(2\pi)^2[\Sigma^*(p)]_{\beta,\alpha}$ as the sum
...
\begin{align}
[-\ii(2\pi)^{-4}S'(p)]={}&[-\ii(2\pi)^{-4}S(p)]\notag\\
&+[-\ii(2\pi)^{-4}S(p)][\ii(2\pi)^4\Sigma^*(p)]
[-\ii(2\pi)^{-4}S(p)]+\cdots,
\tag{11.4.1}\label{eq:11.4.1}
\end{align}

\input{chapters/chapter11/figures/fig11-06.tex}

where
\begin{equation}
S(p)=\frac{-\ii\sl{p}+m_e}{p^2+m_e^2-\ii\epsilon}.
\tag{11.4.2}\label{eq:11.4.2}
\end{equation}
The sum is trivial, and gives
\begin{equation}
S'(p)=[\ii\sl{p}+m_e-\Sigma^*(p)-\ii\epsilon]^{-1}.
...
Figure~\ref{fig:11.6}:
\begin{align*}
\ii(2\pi)^4\Sigma^*_{\text{1 loop}}(p)
={}&\int \dd^4k\left[\frac{-\ii}{(2\pi)^4}
\frac{\eta_{\rho\sigma}}{k^2-\ii\epsilon}\right]
\cdot[(2\pi)^4e\gamma^\rho]\notag\\
&\cdot\left[\frac{-\ii}{(2\pi)^4}
\frac{-\ii\sl{p}+\ii\sl{k}+m_e}{(p-k)^2+m_e^2-\ii\epsilon}\right]
[(2\pi)^4e\gamma^\sigma]
\end{align*}
or more simply
\begin{equation}
\Sigma^*_{\text{1 loop}}(p)=\frac{ie^2}{(2\pi)^4}\int \dd^4k
\left[\frac{1}{k^2-\ii\epsilon}\right]
\left[\frac{\gamma^\rho(-\ii\sl{p}+\ii\sl{k}+m_e)\gamma_\rho}
...
and Villars~\hyperref[ref:11.8]{[8]}. We replace the photon propagator $(k^2-\ii\epsilon)^{-1}$ with
\[
...
-\frac{1}{k^2+\mu^2-\ii\epsilon}\right] \cdot \left[\frac{\gamma^\rho(-\ii\sl{p}+\ii\sl{k}+m_e)\gamma_\rho}
{(p-k)^2+m_e^2-\ii\epsilon}\right].
...
\Sigma^*_{\text{1 loop}}(p)={}&\frac{ie^2}{(2\pi)^4}\int \dd^4k
[2\ii(\sl{p}-\sl{k})+4m_e]\notag\\
&\cdot\int_0^1\dd x\left[
\frac{1}{\{(k-px)^2+p^2x(1-x)+m_e^2x-\ii\epsilon\}^2}
\right.\notag\\[-2pt]
&\hspace{37mm}\left.
-\frac{1}{\{(k-px)^2+p^2x(1-x)+m_e^2x+\mu^2(1-x)-\ii\epsilon\}^2}
\right].
...
\int_0^1\dd x\,[2\ii(1-x)\sl{p}+4m_e]\int_0^\infty \dd\kappa\,\kappa^3
\notag\\
&\cdot\left[
...
\int_0^1\dd x\,[2\ii(1-x)\sl{p}+4m_e]
...
$-(Z_2-1)(\ii\sl{p}+m_e)+Z_2\delta m_e$ in $\Sigma^*(p)$, with $Z_2$ and
$\delta m_e$ determined by the condition that the complete propagator $S'(p)$
regarded as a function of $\ii\sl{p}$ should have a pole at
$\ii\sl{p}=-m_e$ with residue unity. (As we shall see in the next chapter,
...
\notag\\
={}&\frac{2m_e\pi^2e^2}{(2\pi)^4}\int_0^1\dd x\,[1+x]
\ln\left(\frac{m_e^2x^2+\mu^2(1-x)}{m_e^2x^2}\right),
\tag{11.4.9}\label{eq:11.4.9}
\end{align}
\begin{align}
Z_2-1={}&-\ii\left.\frac{\partial\Sigma^*_{\text{1 loop}}}
{\partial\sl{p}}\right|_{i\sl{p}=-m_e}\notag\\
={}&\frac{-2\pi^2e^2}{(2\pi)^4}\int_0^1\dd x
...
\end{equation}
\begin{equation}
\delta m_e=\frac{2m_e\pi^2e^2}{(2\pi)^4}\int_0^1\dd x\,[1+x]
\ln\left(\frac{\mu^2(1-x)}{m_e^2x^2}\right),
\tag{11.4.12}\label{eq:11.4.12}
\end{equation}
\begin{equation}
Z_2-1=\frac{-2\pi^2e^2}{(2\pi)^4}\int_0^1\dd x
...
={}&\Sigma^*_{\text{1 loop}}(p)-(Z_2-1)(\ii\sl{p}+m_e)+Z_2\delta m_e
\notag\\
={}&\frac{2\pi^2e^2}{(2\pi)^4}\int_0^1\dd x
\left\{[\ii(1-x)\sl{p}+2m_e]
\ln\left(\frac{m_e^2(1-x)}{p^2x(1-x)+m_e^2x}\right)\right.\notag\\
&\quad-m_e[1+x]\ln\left(\frac{1-x}{x^2}\right)\notag\\
&\quad\left.-(\ii\sl{p}+m_e)\left[(1-x)
