...
\[
 \frac{-i}{p^2-i0},\qquad -i\lambda,\qquad
 -i(\delta Zp^2+\delta m^2),\qquad -i\delta\lambda.
\]
...
symmetry factor \(1/2\)), and the four-point counterterm.  The massless
...
\[
 \Gamma_4=-i\lambda
 +\frac{i\lambda^2}{2}\bigl[B(-s)+B(-t)+B(-u)\bigr]
...
\begin{aligned}
 ...
 ={}&-i\lambda
 +\frac{i\lambda^2}{32\pi^2}\int_0^1dx\left[
...
\(-i(-i\lambda)^2=+i\lambda^2\).

Choose \(\mu=M\).  Since
\(\int_0^1dx\,[-\ln(x(1-x))]=2\), a momentum-subtraction counterterm that
...
\(\int d^dx\,\partial\phi\cdot\partial\chi=0\).
...
where
\[
 S_{\chi,0}=\frac12\int d^dx\,\partial_\mu\chi\partial^\mu\chi
\]
and
\[
 \boxed{\;
...
\[
 ...
 \exp\left[-S_{\chi,0}-\int d^dx\,J\chi\right],
 \qquad Z_\chi[0]=1.
\]
...
\[
 ...
 =-i\mu^{\epsilon/2}gZ_1\phi\bar\psi\psi,
...
proportional to
\[
 2\int_0^1dx\,(1-x)i\slashed p=i\slashed p.
\]
...
\[
 ...
 \qquad
 I_\Lambda=\int_{|k|<\Lambda}\frac{d^4k}{(2\pi)^4}\frac1{k^2}
...
\]
For positive couplings, \(0<r<1\) gives \(dr/d\ell>0\), while
\(1<r<3\) gives \(dr/d\ell<0\).  Hence every trajectory in
...
standard convention \(\beta_r=\mu,dr/d\mu\), the symmetric direction is
