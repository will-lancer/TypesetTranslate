...
\(d\chi/d\phi=\sqrt{Z(\phi)}\) restores a canonical kinetic term.  Thus an
...
\(L_{\rm div}=(Z_2-1)_{\rm div}\), so the same coefficient is already fixed
...
\[
 t_G\bar R I_G=A_{\rm div}+B_{\rm div}\,i\sl p.
\]
...
\[
 \Pi_E^{(1)}(q)
 =4g^2\int_0^1dx\int\frac{d^4\ell}{(2\pi)^4}
...
where
\[
 A=4g^2\int_0^1dx\int\frac{d^4\ell}{(2\pi)^4}
       \frac{m_\psi^2-\ell^2}{u^2},
\]
\[
 B=4g^2\int_0^1dx\int\frac{d^4\ell}{(2\pi)^4}
       \frac{a(3\ell^2-m_\psi^2)}{u^3},
\]
and
\[
 \boxed{\;
 \Pi_{\rm conv}(q^2)=4g^2\int_0^1dx\int\frac{d^4\ell}{(2\pi)^4}
 \left[
...
Analytic continuation \(q_E^2=-q^2-i0\) gives the requested Minkowski
