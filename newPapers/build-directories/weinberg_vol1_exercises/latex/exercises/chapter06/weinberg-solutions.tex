...
\[
 N(q)=-i\gamma_\mu q^\mu+M,\qquad
 D_F(q)=\frac{N(q)}{q^2+M^2-i0}.
\]
The factors \(\beta=i\gamma^0\) in the propagator convert the external
...
connected graphs for antifermion--boson scattering give
\[
 \begin{split}
 \mathcal A_{\bar F B}
...
\[
 S^c_{fi}=-i(2\pi)^{-2}\delta^4(P_f-P_i)
...
\]
up to the same overall \(i\mathcal M\) convention used at first order.
Equivalently,
\[
 S^{(2)c}_{fi}
 =\frac{-i(2\pi)^{-2}\delta^4(P_f-P_i)}
 {\sqrt{16E_1E_2E'_1E'_2}}\,
 \mathcal M^{(2)} .
\]
...
\[
 ...
 =-i\,\partial_{x^\mu}\Delta_{\ell m}(x-y).
\]
For a Dirac field,
\[
 \Delta_{\ell m}(x-y)
 ...
 e^{iq\cdot(x-y)}}{q^2+M^2-i0}.
\]
Therefore the line with the differentiated end contributes
\[
 -i\int\frac{d^4q}{(2\pi)^4}\,
 \frac{iq_\mu
 \bigl[(-i\gamma_\nu q^\nu+M)\beta\bigr]_{\ell m}}
 {q^2+M^2-i0}\,e^{iq\cdot(x-y)}.
\]
Thus the ordinary Dirac propagator factor is simply multiplied by
\(iq_\mu\) when the momentum \(q\) flows from \(y\) to \(x\).  No extra
...
\[
 ...
 e^{-i(g/3!)\int d^4z\,\phi(z)^3}\}\ket{\Omega}}
 {\bra{\Omega}T\{e^{-i(g/3!)\int d^4z\,\phi(z)^3}\}\ket{\Omega}} .
\]
...
\[
 ...
 =-\frac{ig}{2}\int d^4z\,D(x-z)D(0)+O(g^3).\quad}
\]
