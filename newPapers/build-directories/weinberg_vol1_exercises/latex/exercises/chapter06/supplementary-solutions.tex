...
\begin{enumerate}
\SupplementarySolution{1}{Lorentz Form of the Scalar Propagator}
...
\SupplementarySolution{2}{The Propagator as a Green Function}
...
\SupplementarySolution{3}{Recovering Time Ordering by Contours}
...
\SupplementarySolution{4}{Oriented Complex-Scalar Lines}
...
\SupplementarySolution{5}{A Linear Source and Functional Differentiation}
With \(H_{I,J}(t)=\int d^3x\,J(x)\phi(x)\),
\[
 Z[J]=\bra{\Omega}T\exp\!\left[-i\int d^4x\,J(x)\phi(x)\right]
 \ket{\Omega}.
\]
The linear term vanishes and Wick's theorem gives
\[
 Z[J]=1+\frac{(-i)^2}{2}\int d^4x\,d^4y\,
 J(x)J(y)D(x-y)+O(J^4),
\quad D(x-y)=-i\Delta_F(x-y).
\]
There are two ways for the derivatives to hit the two sources, so
\[
 \left.\frac{\delta^2Z}{\delta J(x)\delta J(y)}\right|_{J=0}
 =(-i)^2D(x-y).
\]
Multiplication by \(i^2\) recovers the time-ordered two-point function,
as required by the source-vertex factor \(-iJ\phi\).

\SupplementarySolution{6}{Why an Adiabatic Source Makes No Particles}
The first Dyson term is
\[
 \bra{\mathbf p}S^{(1)}\ket{\Omega}
 =-\frac{i}{(2\pi)^{3/2}\sqrt{2E_{\mathbf p}}}
 \widetilde J(E_{\mathbf p},\mathbf p),
\]
so only the restriction of \(\widetilde J(q)\) to
\(q^0=E_{\mathbf p}\), \(q^2=-m^2\), creates a particle.  For an
adiabatic source \(J_T(t,\mathbf x)=f(t/T)j(\mathbf x)\),
\[
 \widetilde J_T(E,\mathbf p)
 =T\,\widetilde f(ET)\widetilde j(\mathbf p).
\]
If \(f\) is smooth, \(\widetilde f(\omega)\) decreases faster than any
power.  Since \(E_{\mathbf p}\geq m>0\), this expression tends to zero as
\(T\to\infty\).  No on-shell Fourier weight remains.

\SupplementarySolution{7}{Counting a Four-Point Wick Contraction}
...
\SupplementarySolution{8}{The One-Vertex Tadpole Factor}
...
\SupplementarySolution{9}{The Cubic Self-Energy Factor}
...
\SupplementarySolution{10}{A Vacuum Bubble Cancels in the Ratio}
...
\SupplementarySolution{11}{A Vertex Delta Function from Fourier Phases}
...
\SupplementarySolution{12}{Independent Momentum in the Fish Graph}
...
\SupplementarySolution{13}{Dirac Propagator from the Scalar One}
...
\SupplementarySolution{14}{The Dirac Green Equation}
...
\SupplementarySolution{15}{A Derivative at Either Fermion Endpoint}
...
\SupplementarySolution{16}{On-Shell Residues of the Spinor Numerator}
...
\SupplementarySolution{17}{Fermionic Time-Ordering Sign}
By definition,
\[
 T\{\psi(x)\bar\psi(y)\}
...
\SupplementarySolution{18}{The Minus Sign of a Closed Fermion Loop}
...
\SupplementarySolution{19}{The Scalar Pole Prescription}
...
\SupplementarySolution{20}{Three Channels from One Cubic Interaction}
...
\SupplementarySolution{21}{Why a Quartic Vertex Has No Factorial}
The first Dyson term is
\[
...
\SupplementarySolution{22}{Reading a Yukawa Fermion Line Backwards}
...
\SupplementarySolution{23}{Crossing a Fermion into an Antifermion}
...
\SupplementarySolution{24}{Exchange of Identical External Fermions}
...
\SupplementarySolution{25}{Pairings from a Gaussian Generator}
For
\[
...
\SupplementarySolution{26}{First Coefficients of a Quartic Integral}
...
\SupplementarySolution{27}{Automorphisms and Symmetry Factors}
...
\SupplementarySolution{28}{Connected Diagrams from a Logarithm}
...
\SupplementarySolution{29}{Amputation of an Off-Shell Scalar Leg}
...
\SupplementarySolution{30}{The Normalized Gell-Mann--Low Expansion}
