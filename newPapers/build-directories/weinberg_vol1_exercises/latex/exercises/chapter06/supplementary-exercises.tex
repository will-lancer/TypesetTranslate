...
\[
 U_y=e^{-\ii P^\mu y_\mu}=e^{\ii Hy^0-\ii P^iy_i},
...
\]
\[
 \phi^\dagger(x)=\int\frac{\dd^3\mathbf k}{(2\pi)^3\sqrt{2\omega_{\mathbf k}}}
 \left[b(\mathbf k)e^{\ii k\cdot x}
       +a^\dagger(\mathbf k)e^{-\ii k\cdot x}\right],
...
\[
 \begin{aligned}
 U_y a(\mathbf k)U_y^\dagger&=e^{\ii k\cdot y}a(\mathbf k),&
 U_y a^\dagger(\mathbf k)U_y^\dagger&=e^{-\ii k\cdot y}a^\dagger(\mathbf k),\\
 U_y b(\mathbf k)U_y^\dagger&=e^{\ii k\cdot y}b(\mathbf k),&
 U_y b^\dagger(\mathbf k)U_y^\dagger&=e^{-\ii k\cdot y}b^\dagger(\mathbf k).
 \end{aligned}
\]
The operator generating a Lorentz transformation is
\[
 U_\Lambda=e^{\frac{\ii}{2}\omega_{\mu\nu}M^{\mu\nu}},
 \qquad \omega_{\mu\nu}=-\omega_{\nu\mu},
\]
...
and define
\[
 J(p)=\int \dd^4x\,e^{-\ii p\cdot x}J(x).
\]
...
\[
 ...
 \left[a_{\mathrm{in}}(\mathbf k)e^{\ii k\cdot x}
 +a_{\mathrm{in}}^\dagger(\mathbf k)e^{-\ii k\cdot x}\right],
...
\[
 ...
 \int\frac{\dd^3\mathbf k}{(2\pi)^3\sqrt{2\omega_{\mathbf k}}}
 \left[a_{\mathrm{out}}(\mathbf k)e^{\ii k\cdot x}
 +a_{\mathrm{out}}^\dagger(\mathbf k)e^{-\ii k\cdot x}\right],
 \qquad x^0\longrightarrow+\infty,
\]
with
\[
...
\[
 \phi(x)=\phi_0(x)+\ii\int \dd^4x'\,G(x-x')J(x'),
\]
where
\[
 (-\Box+m^2)\phi_0(x)=0,
 \qquad
 (-\Box+m^2)G(x-x')=-\ii\delta^4(x-x').
\]
...
\[
 \lambda=\int\frac{\dd^3\mathbf k}{(2\pi)^3}\,
...
\[
 S=e^{\ii B},\qquad B=\int \dd^4x\,J(x)\phi_{\mathrm{in}}(x).
\]
...
\[
 S=e^{\ii B}=e^F e^G e^{-\lambda/2},
\]
where
\[
 F=\ii\int\frac{\dd^3\mathbf k}{(2\pi)^3\sqrt{2\omega_{\mathbf k}}}
 J(k)a_{\mathrm{in}}^\dagger(\mathbf k),
 \qquad
 G=\ii\int\frac{\dd^3\mathbf k}{(2\pi)^3\sqrt{2\omega_{\mathbf k}}}
...
\item The probability \(\dd P\) of finding exactly \(n\) particles, with one
particle in each momentum cell \(\dd^3\mathbf k_i\) around \(\mathbf k_i\), is
\[
 \dd P=
 \left|\OutBra{\mathbf k_1,\ldots,\mathbf k_n}\InKet{0}\right|^2
 \prod_{i=1}^n
 \left(\frac{\dd^3\mathbf k_i}{(2\pi)^3\,2\omega_{\mathbf k_i}}\right).
\]
...
\SupplementaryExercise{4}{Position-Space Feynman Rules in Cubic Scalar Theory}
  {(Adapted from J. McGreevy, Physics 215A, Fall 2023, Assignment 7, Problem 2.)}
  {mcgreevy-215a-f23-hw07-p2-v1ch06-s04}
...
\]
\begin{enumerate}
\item Briefly state its Feynman rules in position space, emphasizing how
they differ from those of \(\phi^4\) theory.
\item Draw every diagram that corrects the normalized position-space
two-point function at order \(g^2\).
\item Find the symmetry factor of each diagram and verify the factors
directly by counting Wick contractions.
\end{enumerate}
The source writes the free action with the mostly-minus metric.  It has been
translated here to the mostly-plus convention of this volume; the
interaction and all three requested tasks are unchanged.
...
the chapter's \(-\ii0\) denominator convention.  Explain how their
...
\SupplementaryExercise{10}{Pair Creation Through Scalar Exchange}
  {(Adapted from University of Cambridge, Part III Quantum Field Theory, Paper 301, 2023, Question 2.)}
  {cambridge-partiii-qft-2023-p301-q2-v1ch06-s10}
Consider two complex scalar fields \(\Psi_1\) and \(\Psi_2\), of masses
\(M_1\) and \(M_2\), interacting with a real scalar field \(\phi\) of
mass \(\mu\).  Let \(\mathcal L=\mathcal L_0+\mathcal L_{\rm int}\),
where \(\mathcal L_0\) is the sum of the three free Lagrangians and
\[
 \mathcal L_{\rm int}
 =-g\phi\sum_{i=1,2}\Psi_i^*\Psi_i.
\]
Using Dyson's formula and Wick's theorem, calculate the tree-level
scattering amplitude for the pair-creation process
\[
 \Psi_1(p)+\overline\Psi_1(q)
 \longrightarrow
 \Psi_2(p')+\overline\Psi_2(q'),
\]
where every displayed four-momentum is on shell.  Then work in the
center-of-momentum frame, \(\mathbf p+\mathbf q=0\), and find the minimum
value of \(|\mathbf p|\) for which the scattering amplitude is
kinematically nonzero.  Give the answer for both possible orderings of
\(M_1\) and \(M_2\).
 ...
 \foreach \sx/\loopleg in {0/0,3/1,6/2,9/3,12/4}{
  \begin{scope}[xshift=\sx cm]
   ...
   \ifnum\loopleg=1
    ...
    \draw[line] (v)..controls(.516,.428)and(-.532,1.476)..(v); \node[vertex] at (v){};
   \fi
   \ifnum\loopleg=2
    \coordinate (v) at (.48,.48);
    \draw[line] (v)..controls(.532,1.476)and(-.516,.428)..(v); \node[vertex] at (v){};
   \fi
   \ifnum\loopleg=3
    \coordinate (v) at (-.48,-.48);
    \draw[line] (v)..controls(-.532,-1.476)and(.516,-.428)..(v); \node[vertex] at (v){};
...
\begin{tikzpicture}[
 ...
 \begin{scope}[xshift=0cm]
  ...
  \draw[line] (nw)..controls(-.612,1.959)and(-1.959,.612)..(nw);
  \draw[line] (ne)..controls(1.959,.612)and(.612,1.959)..(ne);
  \draw[line] (sw)..controls(-1.959,-.612)and(-.612,-1.959)..(sw);
  \draw[line] (se)..controls(.612,-1.959)and(1.959,-.612)..(se);
