...
\[
 -\ii\Delta'(k)\delta^4(k-\ell)
 =\int \dd^4y\,\dd^4z\,e^{-\ii k\cdot y+\ii\ell\cdot z}
 \bra{\Omega}T\{\Phi(y)\Phi^\dagger(z)\}\ket{\Omega},
\]
\[
 \begin{split}
 &\int \dd^4x\,\dd^4y\,\dd^4z\,
 e^{-\ii p\cdot x-\ii k\cdot y+\ii\ell\cdot z}
 \bra{\Omega}T\{J^\mu(x)\Phi(y)\Phi^\dagger(z)\}\ket{\Omega}\\
 &\qquad=-\ii q\,\Delta'(k)\Gamma^\mu(k,\ell)
...
\[
 ...
 =\ii\Delta'^{-1}(k)-\ii\Delta'^{-1}(\ell).\;}
\]
At zero momentum transfer this becomes the ordinary Ward identity
\[
 \boxed{\;
 \Gamma^\mu(k,k)
 =-\ii\frac{\partial}{\partial k_\mu}\Delta'^{-1}(k).\;}
\]
For a free scalar \(\Delta'^{-1}(k)=k^2+m^2\), so
\(\Gamma^\mu(k,\ell)=-\ii(k+\ell)^\mu\), which verifies both formulas and
...
=\ii(m_2+m_1)\bar u_2\gamma_5u_1\).  Again
\(\widetilde F_1(0)=0\) for a nonsingular transition matrix element.

\WeinbergSolution{4}
Define
\[
 G^{\mu\nu}(q)=\int \dd^4x\,e^{-\ii q\cdot x}
...
\[
 \boxed{\;
 S'(p)=\int_0^\infty \dd\mu^2\,
 \frac{-\ii\slashed p\,\rho_1(\mu^2)+\rho_2(\mu^2)}
 {p^2+\mu^2-\ii\epsilon}.\;}
\]
...
and hence
\[
 S'(p)=Z\frac{-\ii\slashed p+m}{p^2+m^2-\ii\epsilon}
 +\text{continuum}.
\]
For a canonically normalized bare field, the equal-time
anticommutator gives the sum rule
\(\int_0^\infty\rho_1(\mu^2)\,\dd\mu^2=1\), subject to the usual
...
\[
 \operatorname{Re}f(0)
 =\frac{1}{2\pi^2}\int_0^\infty\sigma_{\gamma\alpha}(E)\,\dd E.
\]
...
\[
 ...
 \equiv\Delta'(z+\ii0)-\Delta'(z-\ii0)
 =2\pi \ii\,\rho(z),\qquad \rho(z)\geq0.
\]
...
\[
 ...
 \int_0^\infty
 \frac{\rho(\mu^2)\,\dd\mu^2}
 {(\mu^2+\mu_0^2)^n(p^2+\mu^2-\ii\epsilon)}.
\]
...
\(\int\rho(\mu^2)\dd\mu^2=1\).  The unsubtracted form is then
\[
 \boxed{\;
 \Delta'(p)=\int_0^\infty
 \frac{\rho(\mu^2)\,\dd\mu^2}
...
\[
 ...
 \frac{\sigma_{\rm KN}(E)}{E^2-\omega^2}\,\dd E
