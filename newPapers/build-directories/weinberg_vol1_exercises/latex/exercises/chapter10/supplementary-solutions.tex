...
\[
 ...
 =e^{\ii p\cdot x}\bra{\Omega}\phi(0)\ket{n,p}.
\]
...
Thus
\[
 D(p)=\frac{Z}{p^2+m^2-\ii0}
 +\int_{M_{\rm th}^2}^{\infty}
 \frac{\rho_c(\mu^2)\,\dd\mu^2}{p^2+\mu^2-\ii0}.
\]
...
\(-\ii\delta^3(\mathbf x-\mathbf y)\).  Comparing with the canonical
commutator gives
\[
 \boxed{\int_0^\infty\rho(\mu^2)\,\dd\mu^2=1.}
\]
Writing
...
\[
 \begin{split}
 -\ii D_\beta(t,\mathbf x)
 =\frac1{Z_\beta}\sum_{n,m}|O_{nm}|^2
 e^{-\ii\omega_{mn}t+\ii\mathbf q_{mn}\cdot\mathbf x}
...
\(F(\omega,\mathbf p)=\int \dd^4x\,
e^{\ii\omega t-\ii\mathbf p\cdot\mathbf x}F(x)\), it becomes
\[
 \begin{split}
 -\ii D_\beta(\omega,\mathbf p)
 ={}&\frac{\ii}{Z_\beta}\sum_{n,m}(2\pi)^3
 \delta^3(\mathbf p-\mathbf q_{mn})|O_{nm}|^2\\
 &\times\left[
 \frac{e^{-\beta E_n}}{\omega-\omega_{mn}+\ii0}
 -\frac{e^{-\beta E_m}}{\omega-\omega_{mn}-\ii0}
 \right].
 \end{split}
\]
...
\[
 -\ii D_\beta(\omega,\mathbf p)
 =\ii\int_{-\infty}^{\infty}\frac{\dd\omega'}{2\pi}\,
 \rho_\beta(\omega',\mathbf p)
 \left[
 \frac{1+n_B(\omega')}{\omega-\omega'+\ii0}
 -\frac{n_B(\omega')}{\omega-\omega'-\ii0}
 \right].
\]
...
\[
 \begin{split}
 -\ii D(\omega,\mathbf p)=\ii\sum_m(2\pi)^3\Bigg[
...
\[
 \delta\psi=\ii\alpha\psi,\qquad
 \delta\bar\psi=-\ii\alpha\bar\psi,\qquad
 \delta\mathcal L_E=\ii(\partial_\mu\alpha)
...
\[
 \delta\mathcal O
 =\ii[\alpha(x_1)-\alpha(x_2)]\mathcal O,
\qquad
 \delta S_E
 =-\ii\int \dd^4x\,\alpha(x)\partial_\mu j^\mu(x).
\]
...
\[
 \ii q_\mu G(\slashed p_1)V_3^\mu(q,p_1,p_2)G(\slashed p_2)
...
\(\delta\phi=-\ii\varepsilon\phi\) and
\(\delta\psi=-\ii\varepsilon\psi\), the unit-normalized Noether currents are
\[
 \boxed{J_\phi^\mu
 =-\ii\bigl[\phi^\dagger D^\mu\phi
 -(D^\mu\phi)^\dagger\phi\bigr],
 \qquad
 J_\psi^\mu=\ii\bar\psi\gamma^\mu\psi.}
\]
...
give \((\Box-M^2)D_F=\ii\delta^{(4)}\) and
\((\gamma^\mu\partial_\mu+m)S_F=-\ii\delta^{(4)}\), consistent with the
...
\[
 ...
 e^{-\ii p\cdot x-\ii q\cdot y+\ii k\cdot z}
 G_{\alpha\beta}(x,y,z).
\]
...
amputated invariant vertex by \(\Gamma=-\ii\mathcal V\),
\[
 \boxed{\begin{aligned}
 \OutBra{\psi(p,s),\bar\psi(q,s')}S\InKet{\phi(k)}
 &={}\,\ii(2\pi)^4\delta^{(4)}(k-p-q)\mathcal A_{s,s'},\\
 \mathcal A_{s,s'}
...
\[
 \widetilde G_c(p,q;k)
 =(2\pi)^4\delta^{(4)}(k-p-q)
 S_F(p)(-\ii\lambda)S_F(-q)D_F(k)+O(\lambda^3).
\]
...
\[
 \boxed{\begin{aligned}
 \OutBra{\psi(p,s),\bar\psi(q,s')}S\InKet{\phi(k)}
 ={}&-\ii\lambda(2\pi)^4\delta^{(4)}(k-p-q)\\
...
\(-\ii\lambda\mathbf1\) by \(g\gamma_5\).  Hence
\[
...
\[
 ...
 \chi'^\dagger \ii\mathbf q\times\boldsymbol\sigma\,\chi .
\]
Hence
\[
 \boxed{\boldsymbol\mu
...
\[
 ...
 =\ii(m_2+m_1)\bar u_2\gamma_5u_1.
\]
Therefore current conservation reads
\[
 \ii(m_2+m_1)\widetilde F
...
\(\operatorname{Disc}F(s)=2\ii\operatorname{Im}F(s+\ii0)\).  The result is
\[
 \boxed{\;
 F(z)=F(z_0)+\frac{z-z_0}{\pi}
 \int_{s_0}^{\infty}
 \frac{\operatorname{Im}F(s+\ii0)\,\dd s}
 {(s-z_0)(s-z-\ii0)}.\;}
\]
...
\[
 F^{(n)}(z_0)=\frac{n!}{\pi}
 \int_{s_0}^{\infty}
 \frac{\operatorname{Im}F(s)\,\dd s}{(s-z_0)^{n+1}},
 \qquad n\geq1.
\]
...
\[
 \begin{split}
 \operatorname{Im}\Pi_\mu(s+\ii0)
 &=\frac{Q_\mu^2}{2\pi}
   \int_{x_-}^{x_+}\dd x\,x(1-x)\\
...
\[
 ...
 -\frac{Q_\mu^2}{2\pi^2}\int_0^1\dd x\,x(1-x)
 \log\!\left[m_\mu^2-sx(1-x)-\ii0\right],
\]
...
\[
 \sigma(e^+e^-\to\mu^+\mu^-)
 =\frac{Q_e^2}{s}\operatorname{Im}\Pi_\mu(s+\ii0)
...
forward amplitude.
\item
Write \(S=\mathbf1+\ii T\).  Unitarity gives
\[
...
\[
 ...
 =\sum_f\int \dd\Phi_f\,|\mathcal M(a\to f)|^2\geq0.
\]
...
\[
 ...
 \text{numerator pair}&
 \eta_{\mu\alpha}\eta_{\nu\beta}
 \operatorname{tr}\bigl[(\slashed{k}_1'+\ii m)
 N^{\mu\nu}(\slashed{k}_1+\ii m)\bar N^{\alpha\beta}\bigr]\\ \hline
...
\[
 ...
 {E^2-\omega^2}\,\dd E
 +\frac{\ii\omega}{4\pi}\sigma_{\rm KN}(\omega)
 +O(e^6).\;}
\]
At small \(\omega\),
\[
...
\(\int_0\sigma_{\rm KN}(E)\dd E/E^2\) diverges.  Using
