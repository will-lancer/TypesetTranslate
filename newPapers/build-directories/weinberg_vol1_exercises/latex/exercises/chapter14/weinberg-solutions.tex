...
\[
 ...
 a_Nu_N(\mathbf x)e^{-iE_Nt}
 {}+b_N^\dagger v_N(\mathbf x)e^{iE_Nt}\right].
\]
...
and \([\phi(\mathbf x),\pi(\mathbf y)]=i\delta^3(\mathbf x-\mathbf y)\)
...
\[
 \boxed{\begin{aligned}
 c_N&=\int d^3y\,u_N^*(\mathbf y)
       \epsilon_N^{(+)}(\mathbf y)f(\mathbf y),\\
 d_N&=\int d^3y\,v_N^*(\mathbf y)
       \epsilon_N^{(-)}(\mathbf y)f(\mathbf y).
 \end{aligned}}
\]
...
\[
 ...
 \int_0^\infty dr\,r^3R_{10}(r)R_{21}(r)
...
\[
 -\frac{g^2}{2}\int\frac{d^3k}{(2\pi)^3\omega_k}
 \left\langle N\left|
...
\(e^{i\mathbf k\cdot\mathbf x}=1+i\mathbf k\cdot\mathbf x+\cdots\)
leaves
\[
 \delta E_N^{\rm soft}
