\chapterbackmatter{Solutions to Supplementary Exercises}

\begin{enumerate}
\SupplementarySolution{1}{M{\o}ller Operators and Scattering States and \(S\)-Matrix as an In--Out Overlap}
\begin{enumerate}
\item
For fixed \(s\), shifting the limiting variable gives
\[
 e^{\ii Hs}\Omega_\pm e^{-\ii H_0s}
 =\lim_{t\to\mp\infty}e^{\ii H(t+s)}e^{-\ii H_0(t+s)}
 =\Omega_\pm .
\]
Differentiation at \(s=0\) yields the intertwining relation
\[
 H\Omega_\pm=\Omega_\pm H_0.
\]
Thus an \(H_0\)-eigenstate of energy \(E_\alpha\) is mapped to a
generalized \(H\)-eigenstate of the same energy.  With the signs in the
question,
\[
 \InKet{\alpha}=\Omega_+\ket{\alpha}_0,\qquad
 \OutKet{\alpha}=\Omega_-\ket{\alpha}_0.
\]
On the scattering subspace the wave operators are isometries.  The
operator acting on free asymptotic labels is
\[
 S=\Omega_-^\dagger\Omega_+,\qquad
 \braket{\beta}{S\alpha}_0=\OutBra{\beta}\InKet{\alpha}.
\]
If their ranges exhaust the continuous spectral subspace, \(S\) is
unitary there; bound states lie outside this scattering subspace.
\item
The total momentum acts on the asymptotic states with eigenvalues
\(p_\alpha\) and \(p_\beta\).  Translation by \(a\) therefore gives
\[
 \OutBra{\beta}\InKet{\alpha}
 =e^{\ii(p_\beta-p_\alpha)\cdot a}
 \OutBra{\beta}\InKet{\alpha}
\]
for every \(a\).  The overlap consequently has support only at
\(p_\beta=p_\alpha\).  Isolating free propagation in the book's
normalization gives
\[
 \OutBra{\beta}\InKet{\alpha}
 =\delta(\beta-\alpha)
 -2\pi \ii\,\delta^4(p_\beta-p_\alpha)M_{\beta\alpha}.
\]
The identity term contains delta functions matching every outgoing
particle with an incoming particle.  A connected matrix element retains
only the single delta function required by conservation of the total
four-momentum.  Extra delta functions matching proper subsets signal
disconnected spectator pieces.
\end{enumerate}

\SupplementarySolution{2}{Completeness on the Scattering Subspace}
Symmetrized two-boson states obey
\[
 \braket{\mathbf k,\mathbf l}{\mathbf p,\mathbf q}
 =\delta^3(\mathbf k-\mathbf p)\delta^3(\mathbf l-\mathbf q)
 +\delta^3(\mathbf k-\mathbf q)\delta^3(\mathbf l-\mathbf p).
\]
Hence
\[
 I_2=\frac12\int \dd^3p\,\dd^3q\,
 \ket{\mathbf p,\mathbf q}\bra{\mathbf p,\mathbf q}
\]
acts on \(\ket{\mathbf k,\mathbf l}\) as one half times two equal terms,
and is the identity.  Equivalently one may integrate over only one
ordering of the two momenta and omit \(1/2!\).

Applying the wave operators gives
\[
 I_{2,\rm in}=\frac12\int \dd^3p\,\dd^3q\,
 \InKet{\mathbf p,\mathbf q}\InBra{\mathbf p,\mathbf q},
\quad
 I_{2,\rm out}=\frac12\int \dd^3p\,\dd^3q\,
 \OutKet{\mathbf p,\mathbf q}\OutBra{\mathbf p,\mathbf q}.
\]
These are identities on the corresponding two-particle asymptotic
subspaces.  Asymptotic completeness asserts that the sum over every
open particle-number channel supplies the full scattering projector.
For a general multiparticle state the divisor is
\[
 S(n)=\prod_a N_a!,
\]
where \(N_a\) is the number of identical particles of species and
polarization type \(a\).  It removes the permutations already generated
by the creation-operator algebra.

\SupplementarySolution{3}{Invariant Flux, Two-Body Phase Space, and Decay Widths}
\begin{enumerate}
\item
With \(p_i^2=-m_i^2\), the numerator
\[
 F=\sqrt{(p_1\cdot p_2)^2-m_1^2m_2^2}
\]
is invariant.  The ratio \(F/(E_1E_2)\) is the relative velocity that
combines with the density factors implicit in the book's
\(\delta^3\)-normalized states.  If \(p_2=(m_2,\mathbf0)\), then
\[
 F=m_2|\mathbf p_1|,\qquad
 u=\frac{|\mathbf p_1|}{E_1}=|\mathbf v_1|.
\]
For collinear velocities,
\[
 \frac{F^2}{E_1^2E_2^2}
 =(1-v_1v_2)^2-(1-v_1^2)(1-v_2^2)
 =(v_1-v_2)^2.
\]
Thus \(u=|v_1-v_2|\), including the case of oppositely directed
particles through the signs of \(v_1,v_2\).
\item
Momentum conservation sets
\(\mathbf p'_2=-\mathbf p'_1\).  Write \(q=|\mathbf p'_1|\).  The
remaining energy delta function is
\[
 \delta(E-E'_1(q)-E'_2(q)),
\quad
 \frac{d(E'_1+E'_2)}{\dd q}
 =\frac{qE}{E'_1E'_2}.
\]
Consequently
\[
 \delta^4(p_\beta-p_\alpha)\dd\beta
 \longrightarrow \frac{k'E'_1E'_2}{E}\,\dd\Omega,
\]
which is Eq.~\eqref{eq:3.4.28}.  Squaring
\(E=E'_1+E'_2\) and solving for the positive momentum gives
\[
 k'=\frac{\sqrt{[E^2-(m'_1+m'_2)^2]
 [E^2-(m'_1-m'_2)^2]}}{2E}.
\]
The expression vanishes at threshold and is real precisely when the
final channel is open.
\item
Equation \eqref{eq:3.4.29} gives
\[
 \frac{\dd\Gamma}{\dd\Omega}
 =\frac{2\pi k'E'_1E'_2}{M}|M_{fi}|^2.
\]
For an angle-independent matrix element,
\[
 \Gamma
 =\frac{8\pi^2k'E'_1E'_2}{M}|M_{fi}|^2,
\]
where
\[
 k'=\frac{\sqrt{[M^2-(m_1+m_2)^2]
 [M^2-(m_1-m_2)^2]}}{2M},
\quad
 E'_{1,2}=\frac{M^2+m_{1,2}^2-m_{2,1}^2}{2M}.
\]
These factors are specific to the book's \(\delta^3\) state normalization.
For two identical daughters, integrating over the full labeled phase
space double-counts every physical state, so the result is divided by
\(2!\).
\end{enumerate}

\SupplementarySolution{4}{The Optical Theorem and Forward Scattering}
\begin{enumerate}
\item
Substitution of \(S=1-\ii\mathcal T\) into unitarity yields
\[
 \ii(\mathcal T-\mathcal T^\dagger)=\mathcal T^\dagger\mathcal T.
\]
Between arbitrary states,
\[
 \ii(\mathcal T_{fi}-\mathcal T_{if}^*)
 =\sum_n\mathcal T_{nf}^*\mathcal T_{ni},
\]
where the sum includes all discrete labels and phase-space integrals.
For \(f=\ii\),
\[
 2\,\operatorname{Im}\mathcal T_{ii}
 =\sum_n|\mathcal T_{ni}|^2.
\]
Every term on the right is a squared modulus, so the absorptive part of
the forward matrix element is nonnegative.  This is the optical theorem
before the common energy--momentum delta functions are removed.
\item
In the book's convention,
\[
 \mathcal T_{\beta\alpha}
 =2\pi\delta^4(p_\beta-p_\alpha)M_{\beta\alpha}.
\]
Removing the common factor
\(\delta^4(0)\) from the forward optical theorem gives
\[
 \operatorname{Im}M_{\alpha\alpha}
 =\pi\int \dd\beta\,
 \delta^4(p_\beta-p_\alpha)|M_{\beta\alpha}|^2.
\]
On the other hand, integration of Eq.~\eqref{eq:3.4.15} gives
\[
 \sigma_{\rm tot}(\alpha)
 =\frac{(2\pi)^4}{u_\alpha}
 \int \dd\beta\,\delta^4(p_\beta-p_\alpha)
 |M_{\beta\alpha}|^2.
\]
Therefore
\[
 \boxed{\;
 \sigma_{\rm tot}(\alpha)
 =\frac{16\pi^3}{u_\alpha}
 \operatorname{Im}M_{\alpha\alpha}.
 \;}
\]
The numerical factor changes when covariantly normalized one-particle
states and the more common invariant amplitude are used; the content of
the theorem does not.
\end{enumerate}

\SupplementarySolution{5}{Exact Scattering from a Delta Potential}
For incidence from the left, take
\[
 \psi_L(x)=
 \begin{cases}
 e^{\ii kx}+r e^{-\ii kx},&x<0,\\
 t e^{\ii kx},&x>0.
 \end{cases}
\]
Continuity gives \(t=1+r\), while
\(\psi'(0^+)-\psi'(0^-)=2mV_0\psi(0)\) gives, with
\(\lambda=mV_0/k\),
\[
 t=\frac1{1+\ii\lambda},\qquad
 r=\frac{-\ii\lambda}{1+\ii\lambda}.
\]
Reflection symmetry gives the right-incident solution by \(x\mapsto-x\).
For this real, time-reversal-invariant potential, complex conjugating the
two incoming eigenfunctions gives the corresponding outgoing basis.
Thus all four boundary-condition states solve the full stationary
Schr\"odinger equation; neither a lone left-incident state nor free plane
waves form a complete scattering basis.
In the basis of incoming waves from the left and right,
\[
 S=\begin{pmatrix}t&r\\ r&t\end{pmatrix}.
\]
Direct calculation gives
\[
 |t|^2+|r|^2=1,\qquad tr^*+rt^*=0,
\]
so \(S^\dagger S=1\).  As \(V_0\to0\), \(t\to1\); as
\(V_0\to\infty\), \(r\to-1\), as expected.

\SupplementarySolution{6}{Disconnected Spectators}
Let \(s\) denote the spectator and \(\widehat\alpha,\widehat\beta\) the
interacting parts.  With the book's normalization, a disconnected term
has the form
\[
 \OutBra{\mathbf p'_s,s;\widehat\beta}
 \left.\InKet{\mathbf p_s,s;\widehat\alpha}\right|_{\rm \dd isc}
 =\delta^3(\mathbf p'_s-\mathbf p_s)\,
 \OutBra{\widehat\beta}\InKet{\widehat\alpha},
\]
with an additional Kronecker delta for the spectator's spin and species.
The reduced overlap contains its own conservation delta function.  Thus
the full term is more singular than a connected amplitude, because a
proper subset of momenta is separately conserved.  Defining connected
matrix elements by cumulant subtraction removes every such factorization,
leaving only the one delta function for total four-momentum conservation.

\SupplementarySolution{7}{Mandelstam Variables in Massless Kinematics}
\begin{enumerate}
\item
Momentum conservation gives \(p_2=p_3+p_4-p_1\).  Expanding the
definitions,
\[
 \begin{split}
 s+t+u
 &=-3p_1^2-2p_1\cdot(p_2-p_3-p_4)-p_2^2\\
 &=-p_1^2-p_2^2-p_3^2-p_4^2,
 \end{split}
\]
where the second line follows after using conservation once more.
Because the mostly-plus mass shells obey \(p_i^2=-m_i^2\),
\[
 s+t+u=m_1^2+m_2^2+m_3^2+m_4^2.
\]
This proof does not require any particular Lorentz frame.
\item
Choose
\[
 p_1=\frac E2(1,0,0,1),\quad
 p_2=\frac E2(1,0,0,-1),\quad
 p_3=\frac E2(1,\sin\theta,0,\cos\theta),
\]
with \(p_4\) opposite to \(p_3\) and \(E=E_{\rm cm}\).  In the definitions
of S.3.7,
\[
 s=E^2,\qquad
 t=-\frac{E^2}{2}(1-\cos\theta),\qquad
 u=-\frac{E^2}{2}(1+\cos\theta).
\]
They sum to zero.  Since \(-1\leq\cos\theta\leq1\),
\[
 -s\leq t\leq0,\qquad -s\leq u\leq0,
\]
and \(t+u=-s\).
\end{enumerate}

\SupplementarySolution{8}{Yukawa Decay, Exchange, and Heavy-Field Matching}
\begin{enumerate}
\item
The supplied mostly-plus rules are
\[
 D_\phi(p)=\frac{-\ii}{p^2+M^2-\ii0},\qquad
 D_\psi(p)=\frac{-\ii}{p^2+m^2-\ii0},\qquad
 V_{\phi\psi\psi^*}=-ig.
\]
The vertex gives the covariant invariant amplitude
\(\mathcal M=-g\), so \(|\mathcal M|^2=g^2\).  In the rest frame of
\(\phi\),
\[
 |\mathbf p|=\frac M2\sqrt{1-\frac{4m^2}{M^2}}.
\]
The standard two-body phase-space integral gives
\[
 \Gamma(\phi\to\psi\psi^*)
 =\frac{|\mathbf p|}{8\pi M^2}|\mathcal M|^2
 =\frac{g^2}{16\pi M}
 \sqrt{1-\frac{4m^2}{M^2}}.
\]
The daughters have opposite charge and are distinguishable, so there is
no \(1/2!\) factor.
\item
Charge flow permits annihilation through the \(s\)-channel and exchange
through the \(t\)-channel.  In the usual covariant amplitude convention,
\[
 \mathcal M=-g^2\left(
 \frac1{s-M^2+\ii0}+\frac1{t-M^2+\ii0}\right).
\]
There is no \(u\)-channel graph for these assigned charges.  When
\(|s|,|t|\ll M^2\),
\[
 \mathcal M
 =\frac{2g^2}{M^2}
 +O\!\left(\frac{s+t}{M^4}\right).
\]
The leading term is momentum independent and hence can be reproduced by
a local four-field interaction.
\item
Write \(X=\psi^*\psi\).  At momenta small compared with \(M\), the heavy
field equation is
\[
 (\Box-M^2)\phi=gX,\qquad
 \phi=-\frac g{M^2}X+O(M^{-4}\Box X).
\]
Substitution into the mass and interaction terms gives
\[
 \mathcal L_{\rm eff}
 =\frac{g^2}{2M^2}X^2+O(M^{-4})
 =-\lambda_{\rm eff}(\psi^*\psi)^2+\cdots,
\qquad
 \lambda_{\rm eff}=-\frac{g^2}{2M^2}.
\]
Differentiating \(X^2\) with respect to two \(\psi\)'s and two
\(\psi^*\)'s supplies a factor \(2!\,2!=4\).  The contact vertex is
\(-4\ii\lambda_{\rm eff}=2ig^2/M^2\), exactly \(\ii\mathcal M\) from the
leading heavy-exchange result.
\end{enumerate}

\SupplementarySolution{9}{The Linear \(O(N)\) Sigma Model}
\begin{enumerate}
\item
Canonical quantization gives
\[
 [a_i(\mathbf p),a_j^\dagger(\mathbf q)]
 =(2\pi)^3 2\omega_{\mathbf p}\delta_{ij}\delta^3(\mathbf p-\mathbf q),
\qquad
 \bra{0}T\Phi_i(x)\Phi_j(y)\ket{0}=\delta_{ij}D_F(x-y).
\]
\item
Four differentiations of the interaction give
\[
 -2\ii\lambda(\delta_{ij}\delta_{kl}+\delta_{ik}\delta_{jl}
 +\delta_{il}\delta_{jk}).
\]
\item
The three invariant amplitudes are \(-2\lambda,-2\lambda,-6\lambda\).
Since \(\dd\sigma/\dd\Omega=|\mathcal M|^2/(64\pi^2s)\), and identical
final particles contribute \(1/2!\),
\[
 \sigma_{12\to12}=\frac{\lambda^2}{4\pi s},\qquad
 \sigma_{11\to22}=\frac{\lambda^2}{8\pi s},\qquad
 \sigma_{11\to11}=\frac{9\lambda^2}{8\pi s}.
\]
\item
The minima obey \(\Phi_i\Phi_i=v^2\), \(v=\mu/\sqrt\lambda\).
Choose \(\Phi_N=v+\sigma\), \(\Phi_a=\pi_a\),
\(a=1,\ldots,N-1\).
\item
With \(\pi^2=\pi_a\pi_a\), the expanded theory is
\[
 \mathcal L=-\frac12(\partial\sigma)^2-\frac12(\partial\pi_a)^2
 -\mu^2\sigma^2-\lambda v\sigma^3-\frac{\lambda}{4}\sigma^4
 -\lambda v\sigma\pi^2-\frac{\lambda}{2}\sigma^2\pi^2
 -\frac{\lambda}{4}(\pi^2)^2+\frac{\mu^4}{4\lambda}.
\]
Thus \(m_\sigma^2=2\mu^2\) and \(m_\pi^2=0\), and the displayed
coefficients exhaust the cubic and quartic interactions.
\item
The \(\sigma\) and pion propagators are
\(-\ii/(p^2+m_\sigma^2-\ii0)\) and
\(-\ii\delta_{ab}/(p^2-\ii0)\).  The
\(\sigma^3,\sigma^4,\pi_a\pi_b\sigma,\pi_a\pi_b\sigma\sigma\) rules are
\[
 -6\ii\lambda v,\quad-6\ii\lambda,\quad
 -2\ii\lambda v\delta_{ab},\quad-2\ii\lambda\delta_{ab},
\]
and the four-pion rule is
\(-2\ii\lambda(\delta_{ab}\delta_{cd}+\delta_{ac}\delta_{bd}
+\delta_{ad}\delta_{bc})\).
\item
For each flavor the decay rule is \(-2\ii\lambda v\); after the
identical-particle factor and flavor sum,
\[
 \Gamma_\sigma=(N-1)\frac{(2\lambda v)^2}{32\pi m_\sigma}
 =\frac{(N-1)\lambda\mu}{8\pi\sqrt2}.
\]
At the stated values this is \(5.63\times10^{-2}\,\mathrm{GeV}\), so
\(\tau=\hbar/\Gamma_\sigma=1.17\times10^{-23}\,\mathrm{s}\).
\item
Using \(2\lambda v^2=m_\sigma^2\), contact and pole terms combine into
\[
 \mathcal M_{ij\to kl}=-2\lambda\left[
 \delta_{ij}\delta_{kl}\frac{s}{s-m_\sigma^2}
 +\delta_{ik}\delta_{jl}\frac{t}{t-m_\sigma^2}
 +\delta_{il}\delta_{jk}\frac{u}{u-m_\sigma^2}\right].
\]
\item
At vanishing external spatial momenta the pions are massless, so every
external four-momentum tends to zero and hence \(s,t,u\to0\); the
amplitude vanishes.  More strongly, taking only one external pion soft
on shell also sends the three channel invariants multiplying its possible
flavor tensors to zero.  Every term in the last expression therefore
vanishes, which is the Adler zero.
\item
The shifted vacuum \(v_a\) obeys
\[
 -\mu^2v_a+\lambda v_a^3-a=0.
\]
Its transverse curvature is
\[
 m_\pi^2=-\mu^2+\lambda v_a^2=\frac{a}{v_a}
 =\frac{a}{v}+O(a^2).
\]
\end{enumerate}

\SupplementarySolution{10}{A Connected \(3\to3\) Tree Amplitude}
The connected contribution first occurs in the second Dyson term:
\[
 \frac{(-\ii)^2}{2!}\left(\frac{\lambda}{4!}\right)^2
 \int \dd^4x\,\dd^4y\,
 T\{\phi^4(x)\phi^4(y)\}.
\]
Three external legs contract at each vertex and the remaining field at
each vertex forms the internal line.  Taking all six external momenta
\(k_a\) incoming, a chosen split \(A\cup A^c\), \(|A|=3\), contributes
\[
 \ii\mathcal M_A=(-\ii\lambda)^2
 \frac{-\ii}{q_A^2+m^2-\ii0},\qquad
 q_A=\sum_{a\in A}k_a
\]
in the volume's mostly-plus convention.  The replacement
\(A\leftrightarrow A^c\) describes the same graph, so the number of
distinct labeled trees is
\[
 \frac12\binom63=10.
\]
The Dyson \(1/2!\) is cancelled by interchanging the two vertices, and
the \(4!\) factors are cancelled by their Wick contractions.

\SupplementarySolution{11}{Exponentiation of Vacuum Bubbles}
At one vertex the three pairings of four fields give the
double-tadpole with symmetry factor \(8\).  At two vertices, \(4!=24\)
contractions join all four legs between the vertices, giving the
four-line basketball with symmetry factor \(48\).  Choosing two legs at
each vertex and pairing them across gives
\(\binom42^2 2!=72\) contractions; the remaining two legs at each vertex
self-contract, so this connected topology has symmetry factor \(16\).
Finally, the \(3^2=9\) contractions that self-pair both vertices form a
disconnected pair of one-vertex bubbles.

Let \(C_1\) denote the connected one-vertex contribution and \(C_2\) the
sum of the two connected two-vertex topologies, including their integrals
and the symmetry factors just found.  The Dyson \(1/2!\) makes the
disconnected \(9\)-contraction term exactly \(\frac12C_1^2\).
Wick's theorem therefore organizes the vacuum amplitude as
\[
 \braket{0}{S|0}
 =1+C_1+\left(C_2+\frac12C_1^2\right)+O(\lambda^3).
\]
The \(1/2\) divides permutations of the two identical disconnected
components.  This is precisely
\[
 \exp(C_1+C_2+\cdots)
 =1+C_1+C_2+\frac12C_1^2+\cdots.
\]
The same component-counting argument works at every order: a collection
of \(n_r\) connected bubbles of type \(r\) carries \(1/n_r!\).
Normalized matrix elements divide by \(\braket{0}{S|0}\), so the common
vacuum exponential cancels and no vacuum bubbles remain in scattering
amplitudes.

\SupplementarySolution{12}{An \(O(3)\)-Symmetric Scattering Theory}
\begin{enumerate}
\item
The canonical commutator is diagonal in flavor, so
\[
 \bra{0}T\phi_i(x)\phi_j(y)\ket{0}=\delta_{ij}D_F(x-y).
\]
\item
An internal scalar line therefore carries
\(-\ii\delta_{ij}/(p^2+m^2-\ii0)\).
An invariant rank-four tensor with the required Bose symmetries is a
multiple of
\[
 \delta_{ij}\delta_{kl}+\delta_{ik}\delta_{jl}
 +\delta_{il}\delta_{jk}.
\]
Four differentiation of
\(-(\lambda/8)(\phi_a\phi_a)^2\) fixes the vertex to
\[
 -\ii\lambda\left(
 \delta_{ij}\delta_{kl}+\delta_{ik}\delta_{jl}
 +\delta_{il}\delta_{jk}\right).
\]
\item
Thus
\[
 \mathcal M_{ij\to kl}
 =-\lambda\left(
 \delta_{ij}\delta_{kl}+\delta_{ik}\delta_{jl}
 +\delta_{il}\delta_{jk}\right).
\]
The tensor is unchanged by any \(O(3)\) rotation and is symmetric under
interchange within either the incoming or outgoing pair.
\end{enumerate}

\SupplementarySolution{13}{Fermion and Antifermion Yukawa Scattering}
\begin{enumerate}
\item
An ordered two-fermion out-state is created by
\(b^\dagger_3b^\dagger_4\ket0\).  Interchanging its labels gives
\[
 b^\dagger_4b^\dagger_3\ket0
 =-b^\dagger_3b^\dagger_4\ket0.
\]
Accordingly the two contractions that attach the outgoing fermions to
the incoming lines enter with opposite signs.  For scalar Yukawa
exchange one may write
\[
 \mathcal M_{\psi\psi}=-\lambda^2\left[
 \frac{(\bar u_3u_1)(\bar u_4u_2)}{t-\mu^2+\ii0}
 -\frac{(\bar u_4u_1)(\bar u_3u_2)}{u-\mu^2+\ii0}\right].
\]
Under \(3\leftrightarrow4\), the two terms exchange and the amplitude
changes sign.  This antisymmetry, rather than a manually appended rule,
is the direct consequence of the external fermionic Fock states.
\item
For \(\psi(p_1)\bar\psi(p_2)\to\psi(p_3)\bar\psi(p_4)\),
\[
 \mathcal M_{\psi\bar\psi}=-\lambda^2\left[
 \frac{(\bar u_3u_1)(\bar v_2v_4)}{t-\mu^2+\ii0}
 -\frac{(\bar v_2u_1)(\bar u_3v_4)}{s-\mu^2+\ii0}\right],
\]
up to the common overall sign of the chosen \(S\)-matrix convention.
The first term is \(t\)-channel elastic exchange and the second is
\(s\)-channel annihilation.  Reordering the external fermion operators
into the fixed canonical order supplies their relative sign.
\end{enumerate}

\SupplementarySolution{14}{Three Routes to a Half-Line Fourier Transform}
\begin{enumerate}
\item
Set \(\Delta=\omega_1-\omega_0\).  Transforming the Gaussian first
gives the well-defined pairing
\[
 \langle\widehat g,\varphi_\sigma\rangle
 =\int_0^\infty \dd t\,e^{\ii\Delta t-\sigma^2t^2/2}
 =\frac{\sqrt\pi}{\sqrt2\,\sigma}e^{-\Delta^2/(2\sigma^2)}
 \left[1+\operatorname{erf}
 \left(\frac{\ii\Delta}{\sqrt2\,\sigma}\right)\right].
\]
\item
For \(\epsilon>0\) the ordinary transform is
\[
 \widehat g_\epsilon(\omega)
 =\frac{\ii}{\omega-\omega_0+\ii\epsilon}.
\]
Its Gaussian pairing is equivalently
\(\int_0^\infty \dd t\,e^{\ii\Delta t-\sigma^2t^2/2-\epsilon t}\).
Dominated convergence produces exactly the result of part 1.
\item
With a sharp endpoint,
\[
 \widehat g_T(\omega)
 =\frac{e^{\ii(\omega-\omega_0)T}-1}{\ii(\omega-\omega_0)}.
\]
After pairing this becomes
\(\int_0^Tdt\,e^{\ii\Delta t-\sigma^2t^2/2}\), which tends to the same
answer as \(T\to\infty\).  Sokhotski--Plemelj therefore identifies the
common distribution as
\[
 \widehat g(\omega)=\pi\delta(\omega-\omega_0)
 +\ii\,\operatorname{P.V.}\frac1{\omega-\omega_0}.
\]
\item
Fix the Gaussian in the question and put
\(\Delta=\omega_1-\omega_0\).  Let
\[
 T_\epsilon=\epsilon^{-1},\qquad w_\epsilon=\epsilon^3,\qquad
 A_\epsilon=w_\epsilon^{-1}
 \exp\!\left(\frac{\sigma^2T_\epsilon^2}{2}-\ii\Delta T_\epsilon\right),
\]
and define
\[
 g_\epsilon(t)=g(t)\left[
 1+A_\epsilon\,
 \mathbf 1_{[T_\epsilon,T_\epsilon+w_\epsilon]}(t)\right].
\]
For every fixed \(t\), the moving interval eventually lies to its right,
so \(g_\epsilon(t)\to g(t)\).  The extra pulse is not uniformly
dominated.  Its Gaussian pairing is
\[
 A_\epsilon\int_{T_\epsilon}^{T_\epsilon+w_\epsilon}
 \dd t\,e^{\ii\Delta t-\sigma^2t^2/2}=1+o(1).
\]
Thus this regulator adds a nonzero constant to the limiting functional,
even though it converges pointwise.  This exhibits why a domination
hypothesis, not pointwise convergence alone, is needed.
\end{enumerate}

\SupplementarySolution{15}{Partial-Wave Unitarity and Cross-Section Bounds}
\begin{enumerate}
\item
Orthogonality of angular momentum channels makes unitarity diagonal:
\(|S_\ell|=1\).  With
\[
 S_\ell=1+2\ii kf_\ell,
\]
the identity \(S_\ell^*S_\ell=1\) gives
\[
 \operatorname{Im}f_\ell=k|f_\ell|^2.
\]
Every unit-modulus complex number can be written
\(S_\ell=e^{2\ii\delta_\ell}\) with real \(\delta_\ell\).  Hence
\[
 f_\ell=\frac{e^{2\ii\delta_\ell}-1}{2\ii k}
 =\frac{e^{\ii\delta_\ell}\sin\delta_\ell}{k}.
\]
This form satisfies the unitarity relation identically and makes the
phase-shift ambiguity \(\delta_\ell\sim\delta_\ell+\pi\) explicit.
\item
Using
\(\int \dd\Omega\,P_\ell(\cos\theta)P_{\ell'}(\cos\theta)
=4\pi\delta_{\ell\ell'}/(2\ell+1)\), one finds
\[
 \sigma_{\rm el}
 =4\pi\sum_\ell(2\ell+1)|f_\ell|^2
 =\frac{4\pi}{k^2}
 \sum_\ell(2\ell+1)\sin^2\delta_\ell.
\]
For purely elastic scattering this also equals the total cross section,
as follows independently from the forward optical theorem.  Since
\(\sin^2\delta_\ell\leq1\), one partial wave obeys
\[
 \sigma_\ell\leq\frac{4\pi}{k^2}(2\ell+1).
\]
The bound is saturated at
\(\delta_\ell=\pi/2\pmod{\pi}\).
\end{enumerate}

\SupplementarySolution{16}{The \(K\)-Matrix and Inelastic Partial Waves}
\begin{enumerate}
\item
Equating the two representations,
\[
 e^{2\ii\delta}=\frac{1-\ii K}{1+\ii K},
\]
and solving gives
\[
 K=-\tan\delta.
\]
The minus sign follows from the chapter's convention
\(S=1-2\ii K+\cdots\); the opposite convention for the transition operator
would give \(K=\tan\delta\).  At a resonance the phase passes through
\(\delta=\pi/2\), so \(K\) has a real-axis pole while \(S=-1\) remains
finite.  If \(K=K^*\), then
\[
 S^*=\frac{1+\ii K}{1-\ii K}=S^{-1},
\]
which proves one-channel unitarity without further constraints.
\item
Unitarity of the full multichannel matrix implies that the squared norm
of any one diagonal entry cannot exceed one.  Thus
\[
 S_\ell=\eta_\ell e^{2\ii\delta_\ell},\qquad
 0\leq\eta_\ell\leq1.
\]
With \(f_\ell=(S_\ell-1)/(2\ii k)\), the three standard partial-wave
cross sections are
\[
 \begin{split}
 \sigma_{{\rm el},\ell}
 &=\frac{\pi}{k^2}(2\ell+1)
 |1-\eta_\ell e^{2\ii\delta_\ell}|^2,\\
 \sigma_{{\rm reac},\ell}
 &=\frac{\pi}{k^2}(2\ell+1)(1-\eta_\ell^2),\\
 \sigma_{{\rm tot},\ell}
 &=\frac{2\pi}{k^2}(2\ell+1)
 (1-\eta_\ell\cos2\delta_\ell).
 \end{split}
\]
Direct expansion verifies
\(\sigma_{\rm tot}=\sigma_{\rm el}+\sigma_{\rm reac}\).
\end{enumerate}

\SupplementarySolution{17}{Breit--Wigner Phase Motion}
For real \(E\), numerator and denominator are complex conjugates, so
\(|S_\ell|=1\).  Put \(x=E-E_R\) and \(\gamma=\Gamma/2\).  A continuous
choice increasing from \(0\) to \(\pi\) obeys
\[
 \tan\delta_\ell=-\frac{\gamma}{x},
\qquad
 \delta_\ell(E_R)=\frac{\pi}{2}.
\]
Therefore
\[
 \sin^2\delta_\ell
 =\frac{\gamma^2}{x^2+\gamma^2}
 =\frac{\Gamma^2/4}
 {(E-E_R)^2+\Gamma^2/4}.
\]
Using
\(\lim_{\gamma\to0^+}\gamma/(x^2+\gamma^2)=\pi\delta(x)\),
\[
 \sin^2\delta_\ell
 \longrightarrow\pi\gamma\,\delta(E-E_R)
 =\frac{\pi\Gamma}{2}\delta(E-E_R).
\]
The peak reaches the elastic unitarity limit independently of its width,
while its integrated area is proportional to \(\Gamma\).
\end{enumerate}
