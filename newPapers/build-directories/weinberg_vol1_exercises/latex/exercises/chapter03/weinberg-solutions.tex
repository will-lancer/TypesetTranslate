...
\[
 ...
 =\delta_{\beta\alpha}
 \frac{g\,u_\beta}{E_\alpha-E_\beta\pm i0}
...
\[
 T^+_{\beta\alpha}
 =\langle\beta|H_{\rm int}\InKet{\alpha}
...
\[
 \left|\frac{d(E_3+E_4)}{dq}\right|
 =\left|\frac q{E_3}+\frac{q-p\cos\theta}{E_4}\right|.
\]
It follows directly that
\[
 \boxed{\;
 \frac{d\sigma}{d\Omega}_{\rm lab}
...
\[
 \frac1{E_\alpha-E_\gamma+i0}
 =-i\int_0^\infty d\tau\,
 e^{i(E_\alpha-E_\gamma)\tau},
\]
and represent the energy-conserving factor in the \(S\)-matrix by
\[
 2\pi\delta(E_\beta-E_\alpha)
 =\int_{-\infty}^{\infty}dt\,e^{i(E_\beta-E_\alpha)t}.
\]
...
\[
 ...
 {E_\alpha-E_\gamma+i0}\\
 &\qquad=(-i)^2\int_{-\infty}^{\infty}dt_1
 \int_{-\infty}^{t_1}dt_2\,
 \langle\beta|H_I(t_1)H_I(t_2)|\alpha\rangle .
 \end{split}
\]
...
\[
 (-i)^n\int_{-\infty}^{\infty}dt_1
 \int_{-\infty}^{t_1}dt_2\cdots
 \int_{-\infty}^{t_{n-1}}dt_n\,
 \langle\beta|H_I(t_1)\cdots H_I(t_n)|\alpha\rangle .
\]
...
\[
 S_{\ell\pm}^{I}=e^{2i\delta_{\ell\pm}^{I}},\qquad
...
\[
 \begin{split}
 f_I(\theta)&=\frac1{2ik}\sum_{\ell=0}^\infty
 \left[(\ell+1)(S_{\ell+}^{I}-1)
 +\ell(S_{\ell-}^{I}-1)\right]P_\ell(\cos\theta),\\
 g_I(\theta)&=\frac1{2ik}\sum_{\ell=1}^\infty
...
\[
 \frac{d\sigma_I}{d\Omega}=|f_I|^2+|g_I|^2.
\]
The state \(\pi^+p\) has \(I_3=3/2\), and hence is pure \(I=3/2\):
\[
 \frac{d\sigma(\pi^+p)}{d\Omega}
 =|f_{3/2}|^2+|g_{3/2}|^2.
\]
