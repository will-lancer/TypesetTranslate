...
\[
 K(x,t;x_1,t_1)=
 \left(\frac{m\omega}{2\pi \ii\sin\omega T}\right)^{1/2}
 \exp(\ii S_{\rm cl}).
\]
...
\[
 \boxed{\quad
 \dd P=|K(x,t;x_1,t_1)|^2\,\dd x
 =\frac{m\omega}{2\pi|\sin\omega T|}\,\dd x .
 \quad}
\]
...
\[
 ...
 =(2\pi)^{-3/2}\int \dd^3x\,
 e^{-\ii\mathbf p\cdot\mathbf x}\phi(\mathbf x),
...
\(\Pi(\mathbf x)=-\ii\delta/\delta\phi(\mathbf x)\).  Taking the adjoint of
...
\[
 ...
 \InKet{\mathbf p}&
 (2\pi)^{-3/2}(2\omega_{\mathbf p})^{-1/2}e^{\ii p\cdot x}\\[2pt]
 \OutBra{\mathbf p}&
 (2\pi)^{-3/2}(2\omega_{\mathbf p})^{-1/2}e^{-\ii p\cdot x}
 \end{array}
\]
...
\mathcal E_{ij}(\mathbf p)v_j(\mathbf p)\,\dd^3p]\), the functional
...
Thus
\[
 \Psi_0[v]=\mathcal N\exp\left[-\frac12\int \dd^3p\,
...
quadratic action as
\[
 I_0=-\int\frac{\dd^4p}{(2\pi)^4}\,
 \bar\psi^\mu(-p)\mathcal K_{\mu\nu}(p)\psi^\nu(p),
\]
where
\[
 \mathcal K_{\mu\nu}=
 (\ii\gamma\cdot p+m)\eta_{\mu\nu}
 +\frac{\ii}{3}(\gamma_\mu p_\nu+\gamma_\nu p_\mu)
 -\frac13\gamma_\mu(\ii\gamma\cdot p-m)\gamma_\nu .
\]
...
\[
 \boxed{\quad
 \Delta^{\mu\nu}(p)=
 \frac{-\ii\gamma\cdot p+m}{p^2+m^2-\ii\epsilon}
 \left[
...
\(\beta=\ii\gamma^0\); the displayed expression is the conventional
\(T\{\psi^\mu(x)\bar\psi^\nu(y)\}\) propagator.
...
\(-\ii\epsilon\) displacement as for the Dirac propagator.
\end{enumerate}
