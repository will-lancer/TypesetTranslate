...
=\delta^3(\mathbf p+\mathbf q)/(2E_{\mathbf p})\).

For example, an incoming particle inserts
\(\sqrt{2E_{\mathbf p}}\widetilde\phi(-\mathbf p)\) in the initial boundary
...
\[
 ...
 \InKet{\mathbf p}&
 (2\pi)^{-3/2}(2E_{\mathbf p})^{-1/2}e^{ip\cdot x}\\[2pt]
 \OutBra{\mathbf p}&
 (2\pi)^{-3/2}(2E_{\mathbf p})^{-1/2}e^{-ip\cdot x}
 \end{array}
\]
with \(p^0=E_{\mathbf p}\).  The conjugate factors describe absorption at
...
\[
 A=P^T+\frac{E_{\mathbf p}^2}{m^2}P^L,
\qquad
 B=E_{\mathbf p}^2P^T+m^2P^L.
\]
For a Gaussian
...
\[
 ...
 =E_{\mathbf p}P^T_{ij}
 +\frac{m^2}{E_{\mathbf p}}P^L_{ij}
 =E_{\mathbf p}\left(
 \delta_{ij}-\frac{p_ip_j}{E_{\mathbf p}^2}\right).
 \quad}
\]
Thus
\[
