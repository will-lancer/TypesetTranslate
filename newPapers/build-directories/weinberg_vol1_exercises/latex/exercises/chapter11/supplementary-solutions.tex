...
\[
 \prod_i dt_i
 =T^{n-1}dT\left(\prod_i dx_i\right)
...
\begin{aligned}
 \frac{1}{\prod_iA_i^{\nu_i}}
 ={}&\frac{1}{\prod_i\Gamma(\nu_i)}
 \int_0^1\!\left(\prod_i dx_i\right)
 \delta\!\left(1-\sum_i x_i\right)
 \prod_i x_i^{\nu_i-1}\\
 &{}\times\int_0^\infty dT\,T^{\nu-1}
 e^{-T\sum_i x_iA_i}.
\end{aligned}
\]
...
\(A=k^2-i0\) and \(B=(k+p)^2-i0\).  Equation (8) for two factors gives
\[
\begin{aligned}
 I_B(a,b;p^2)
 ...
 \frac{1}{[x(k^2-i0)+(1-x)((k+p)^2-i0)]^{a+b}}.
\end{aligned}
\]
Since
\[
...
\[
 ...
 \frac{1}{(\ell^2+\Delta-i0)^N}
 =i\frac{\Omega_d}{(2\pi)^d}
 \int_0^\infty dr\,\frac{r^{d-1}}{(r^2+\Delta-i0)^N}.
\]
With \(u=r^2/(\Delta-i0)\), the radial integral is
\[
 \frac12(\Delta-i0)^{d/2-N}
...
\]
and
\((4\pi)^{-d/2}(p^2-i0)^{-\varepsilon}
=(4\pi)^{-2}[(p^2-i0)/(4\pi)]^{-\varepsilon}\).  The result is exactly
...
\[
 \boxed{\;
 \Sigma(p^2)=ie^2\int\frac{d^dk}{(2\pi)^d}
 \frac{\gamma_\rho[-i(\slashed k+\slashed p)]\gamma^\rho}
 {(k^2-i0)((k+p)^2-i0)}
 \;}
\]
and
\[
 \boxed{\;
 \Pi^{\mu\nu}(p^2)=-ie^2\int\frac{d^dk}{(2\pi)^d}
 \frac{\operatorname{tr}\!\left[
 (-i\slashed k)\gamma^\mu
 (-i(\slashed k+\slashed p))\gamma^\nu\right]}
 {(k^2-i0)((k+p)^2-i0)}.
 \;}
\]
...
\[
 ...
 -\eta^{\mu\nu}k\cdot(k+p)\right].
\]
The two factors \(-i\) in the fermion numerators turn the prefactor into
\(+ie^2\).  Let
\[
 \begin{aligned}
 I^\mu&=\int\frac{d^dk}{(2\pi)^d}
...
\begin{aligned}
 \Gamma(1-\varepsilon)
 &=\int_0^\infty dx\,x^{-\varepsilon}e^{-x}\\
 &=\int_0^\infty dx\,e^{-x}
...
Set
\[
 L_p\equiv\log\!\left(\frac{p^2-i0}{4\pi}\right),
 \qquad
 \left(\frac{p^2-i0}{4\pi}\right)^{-\varepsilon}
...
\(L_p\) by \(\log[(p^2-i0)/(4\pi\mu^2)]\).  This restores a dimensionless
