...
\SupplementarySolution{4}{One-Loop Self-Energies in Massless QED}
\begin{enumerate}
\item
With \(\int_k=\mu^{2\varepsilon}\int d^dk/(2\pi)^d\),
\[
 \Sigma(p)=-ie^2\int_k
 \frac{\gamma_\mu(\slashed p-\slashed k)\gamma^\mu}
 {(k^2+i0)((p-k)^2+i0)},
\]
\[
 i\Pi^{\mu\nu}(q)=-(-ie)^2\int_k
 \frac{\operatorname{tr}[\gamma^\mu\slashed k
 \gamma^\nu(\slashed k+\slashed q)]}
 {(k^2+i0)((k+q)^2+i0)}.
\]
The first minus sign in the photon expression is the closed-loop sign.
\item
...
mass is induced.
\item
The displayed tensor obeys \(q_\mu\Pi^{\mu\nu}=0\).  Its pole is canceled
by the \(Z_3F_{\mu\nu}F^{\mu\nu}\) counterterm, and the electron pole by
\((Z_2-1)\bar\psi i\slashed\partial\psi\), with the values found in
parts 2 and 3.
\end{enumerate}
