...
order are included.
\item
For \(q=p_1+p_2\), representative regulated subgraph integrals are
\[
 \ii\Pi^{\mu\nu}(q)=-(-ie)^2\mu^{2\varepsilon}\int_k
 \frac{\operatorname{tr}[
 \gamma^\mu(\slashed k+m_f)\gamma^\nu(\slashed k+\slashed q+m_f)]}
 {(k^2-m_f^2+\ii0)((k+q)^2-m_f^2+\ii0)},
\]
\[
 \Lambda^\mu_f=-e^2\mu^{2\varepsilon}\int_k
 \frac{\gamma^\alpha(\slashed k+\slashed p'+m_f)\gamma^\mu
 (\slashed k+\slashed p+m_f)\gamma_\alpha}
 {(k^2+\ii0)((k+p')^2-m_f^2+\ii0)((k+p)^2-m_f^2+\ii0)},
\]
and
\[
 \Sigma_f(\slashed p)=-ie^2\mu^{2\varepsilon}\int_k
 \frac{\gamma^\alpha(\slashed p-\slashed k+m_f)\gamma_\alpha}
 {(k^2+\ii0)((p-k)^2-m_f^2+\ii0)}.
\]
The box has two photon and two fermion denominators; exchanging the
photon-attachment order gives the crossed box.  Inserting these tensors
between external currents supplies every class in part 1.  Local
\(\delta Z_1,\delta Z_2,\delta Z_3,\delta m\) insertions subtract the
subdivergences, while the on-shell LSZ condition removes explicit
external self-energy factors.
\end{enumerate}

\SupplementarySolution{2}{Feynman Parameters}
\begin{enumerate}
\item
For \(\Re\nu>0\), rescale the defining Gamma-function integral by
\(y=Ax\), initially taking \(\Re A>0\):
\[
 \Gamma(\nu)=\int_0^\infty \dd y\,y^{\nu-1}e^{-y}
 =A^\nu\int_0^\infty \dd x\,x^{\nu-1}e^{-Ax}.
\]
Dividing by \(A^\nu\Gamma(\nu)\) proves (7).  Both sides are analytic in
their common convergence domain, and the formula elsewhere is defined by
analytic continuation with the branch of \(A^{-\nu}\) fixed by the
prescription in the propagator denominator.

\item
Apply (7) independently to all \(n\) factors:
\[
 \frac{1}{\prod_iA_i^{\nu_i}}
 =\frac{1}{\prod_i\Gamma(\nu_i)}
 \int_0^\infty\prod_i(\dd t_i\,t_i^{\nu_i-1})
 \exp\!\left[-\sum_i t_iA_i\right].
\]
Introduce an overall scale \(T=\sum_i t_i\) and simplex coordinates
\(x_i=t_i/T\).  The measure is
\[
 \prod_i \dd t_i
 =T^{n-1}\dd T\left(\prod_i \dd x_i\right)
...
\begin{aligned}
 \frac{1}{\prod_iA_i^{\nu_i}}
 ={}&\frac{1}{\prod_i\Gamma(\nu_i)}
 \int_0^1\!\left(\prod_i \dd x_i\right)
 \delta\!\left(1-\sum_i x_i\right)
 \prod_i x_i^{\nu_i-1}\\
 &{}\times\int_0^\infty \dd T\,T^{\nu-1}
 e^{-T\sum_i x_iA_i}.
\end{aligned}
\]
The last integral is
\(\Gamma(\nu)(\sum_i x_iA_i)^{-\nu}\), giving precisely (8).  No
integer-exponent step has been used.  As a check, when every
\(\nu_i=1\), the prefactor becomes \(\Gamma(n)=(n-1)!\).

\item
For the bubble choose
\(A=k^2-\ii0\) and \(B=(k+p)^2-\ii0\).  Equation (8) for two factors gives
\[
\begin{aligned}
 I_B(a,b;p^2)
 ={}&\frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}
 \int_0^1\dd x\,x^{a-1}(1-x)^{b-1}\\
 &{}\times\int\frac{\dd^dk}{(2\pi)^d}
 \frac{1}{[x(k^2-\ii0)+(1-x)((k+p)^2-\ii0)]^{a+b}}.
\end{aligned}
\]
Since
\[
 ...
 =[k+(1-x)p]^2+x(1-x)p^2,
\]
the shift \(\ell=k+(1-x)p\) yields the requested single denominator:
\[
 \boxed{\begin{aligned}
 I_B(a,b;p^2)
 ={}&\frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}
 \int_0^1\dd x\,x^{a-1}(1-x)^{b-1}\\
 &{}\times\int\frac{\dd^d\ell}{(2\pi)^d}
 \frac{1}{[\ell^2+x(1-x)p^2-\ii0]^{a+b}}.
 \end{aligned}}
\]
Dimensional regularization justifies the translation of loop momentum by
analytic continuation from a convergence domain.

\item
Write \(N=a+b\) and \(\Delta=x(1-x)p^2\).  Wick rotation in the
mostly-plus convention gives
\[
 \int\frac{\dd^d\ell}{(2\pi)^d}
 \frac{1}{(\ell^2+\Delta-\ii0)^N}
 =\ii\frac{\Omega_d}{(2\pi)^d}
 \int_0^\infty \dd r\,\frac{r^{d-1}}{(r^2+\Delta-\ii0)^N}.
\]
With \(u=r^2/(\Delta-\ii0)\), the radial integral is
\[
 \frac12(\Delta-\ii0)^{d/2-N}
 \frac{\Gamma(d/2)\Gamma(N-d/2)}{\Gamma(N)}.
\]
Using (9), the loop integral therefore becomes
\[
 \frac{\ii}{(4\pi)^{d/2}}
 \frac{\Gamma(N-d/2)}{\Gamma(N)}
 (\Delta-\ii0)^{d/2-N}.
\]
Substitution in the preceding part leaves a beta integral,
\[
 \int_0^1\dd x\,
 x^{d/2-b-1}(1-x)^{d/2-a-1}
 =\frac{\Gamma(d/2-b)\Gamma(d/2-a)}
 {\Gamma(d-a-b)}.
\]
Consequently
\[
 ...
 \;}
\]
For \(a=b=1\) and \(d=4-2\varepsilon\), use
\[
 \Gamma(\varepsilon)=\frac{\Gamma(1+\varepsilon)}{\varepsilon},
 \qquad
 \Gamma(2-2\varepsilon)=(1-2\varepsilon)
 \Gamma(1-2\varepsilon),
\]
and
\((4\pi)^{-d/2}(p^2-\ii0)^{-\varepsilon}
=(4\pi)^{-2}[(p^2-\ii0)/(4\pi)]^{-\varepsilon}\).  The result is exactly
the required check (11):
\[
 I_B(1,1;p^2)
 ...
 \Gamma(1-2\varepsilon)}.
\]
The Gamma function \(\Gamma(\varepsilon)\) displays the logarithmic
ultraviolet pole.
\end{enumerate}
...
\[
 \int \dd^dk\,(k^2)^2f(k^2)=A(\dd^2+2d),
\]
so
\[
 \boxed{\;
...
immediately gives
\[
 I^\mu\equiv\int\frac{\dd^dk}{(2\pi)^d}
...
\[
 \dd A+p^2B=\int\frac{\dd^dk}{(2\pi)^d}\frac1{(k+p)^2}=0.
\]
...
\SupplementarySolution{4}{Self-Energy Corrections in QED}
\begin{enumerate}
\item
Use the momentum routing printed on the two diagrams.  The volume's
Feynman-gauge rules give
\[
 \boxed{\;
 \Sigma(p^2)=ie^2\int\frac{\dd^dk}{(2\pi)^d}
 \frac{\gamma_\rho[-\ii(\slashed k+\slashed p)]\gamma^\rho}
 {(k^2-\ii0)((k+p)^2-\ii0)}
 \;}
\]
and
\[
 \boxed{\;
 \Pi^{\mu\nu}(p^2)=-ie^2\int\frac{\dd^dk}{(2\pi)^d}
 \frac{\operatorname{tr}\!\left[
 (-\ii\slashed k)\gamma^\mu
 (-\ii(\slashed k+\slashed p))\gamma^\nu\right]}
 {(k^2-\ii0)((k+p)^2-\ii0)}.
 \;}
\]
The overall minus sign in the second expression is the closed-fermion-loop
sign.  No external spinor or polarization occurs, so the first expression
is a spinor matrix and the second carries its two free Lorentz indices.

\item
In \(d\) dimensions,
\[
 \gamma_\rho\gamma_\alpha\gamma^\rho=(2-d)\gamma_\alpha.
\]
Writing \(I_B=I_B(1,1;p^2)\), the supplied rank-one reduction gives
\[
\begin{aligned}
 \Sigma(p^2)
 &=e^2(2-d)\int\frac{\dd^dk}{(2\pi)^d}
 \frac{\slashed k+\slashed p}
 {(k^2-\ii0)((k+p)^2-\ii0)}\\
 &=\frac{e^2(2-d)}2\slashed p\,I_B
 =-e^2(1-\varepsilon)\slashed p\,I_B.
\end{aligned}
\]
Substituting the supplied exact bubble yields
\[
 \boxed{\;
 \Sigma(p^2)
 =-\frac{ie^2}{(4\pi)^2}\slashed p
 \left(\frac{p^2-\ii0}{4\pi}\right)^{-\varepsilon}
 \frac{(1-\varepsilon)\Gamma(1-\varepsilon)^2
 \Gamma(1+\varepsilon)}
 {(1-2\varepsilon)\varepsilon\Gamma(1-2\varepsilon)}.
 \;}
\]

For the photon, first take the trace:
\[
 \operatorname{tr}[\slashed k\gamma^\mu
 (\slashed k+\slashed p)\gamma^\nu]
 =4\left[k^\mu(k+p)^\nu+k^\nu(k+p)^\mu
 -\eta^{\mu\nu}k\cdot(k+p)\right].
\]
The two factors \(-\ii\) in the fermion numerators turn the prefactor into
\(+ie^2\).  Let
\[
 \begin{aligned}
 I^\mu&=\int\frac{\dd^dk}{(2\pi)^d}
 \frac{k^\mu}{(k^2-\ii0)((k+p)^2-\ii0)},\\
 I^{\mu\nu}&=\int\frac{\dd^dk}{(2\pi)^d}
 \frac{k^\mu k^\nu}{(k^2-\ii0)((k+p)^2-\ii0)}.
 \end{aligned}
\]
Scaleless tadpoles give
\(I^\rho{}_{\rho}=0\), while the supplied formulas give
\(I^\mu=-p^\mu I_B/2\) and
\[
 I^{\mu\nu}=\frac{\dd p^\mu p^\nu-p^2\eta^{\mu\nu}}
 {4(d-1)}I_B.
...
\begin{aligned}
 \Pi^{\mu\nu}(p^2)
 &=4ie^2\left[2I^{\mu\nu}+p^\mu I^\nu+p^\nu I^\mu
 -\eta^{\mu\nu}(I^\rho{}_{\rho}+p\cdot I)\right]\\
 &=ie^2\frac{2(d-2)}{d-1}
 (p^2\eta^{\mu\nu}-p^\mu p^\nu)I_B.
\end{aligned}
\]
With \(d=4-2\varepsilon\), this is
\[
 \boxed{\;
 \Pi^{\mu\nu}(p^2)
 =-\frac{e^2}{(4\pi)^2}
 (p^2\eta^{\mu\nu}-p^\mu p^\nu)
 \left(\frac{p^2-\ii0}{4\pi}\right)^{-\varepsilon}
 \frac{4(1-\varepsilon)\Gamma(1-\varepsilon)^2
 \Gamma(1+\varepsilon)}
 {(3-2\varepsilon)(1-2\varepsilon)\varepsilon
 \Gamma(1-2\varepsilon)}.
 \;}
\]
The exact tensor is transverse, which checks both the tensor reduction and
the relative numerator terms.
...
\begin{aligned}
 \Gamma(1-\varepsilon)
 &=\int_0^\infty \dd x\,x^{-\varepsilon}e^{-x}\\
 &=\int_0^\infty \dd x\,e^{-x}
 \left[1-\varepsilon\log x+O(\varepsilon^2)\right]\\
 &=1+\gamma_E\varepsilon+O(\varepsilon^2),
\end{aligned}
\]
where the last equality is exactly definition (21).  This proves (22).

\item
Equation (22), with \(\varepsilon\) replaced by \(-\varepsilon\) or
\(2\varepsilon\), gives
\[
 \Gamma(1+\varepsilon)=1-\gamma_E\varepsilon+O(\varepsilon^2),
 \qquad
 \Gamma(1-2\varepsilon)=1+2\gamma_E\varepsilon
 +O(\varepsilon^2).
\]
Thus
\[
 \frac{\Gamma(1-\varepsilon)^2\Gamma(1+\varepsilon)}
 {\Gamma(1-2\varepsilon)}
 =1-\gamma_E\varepsilon+O(\varepsilon^2).
\]
Set
\[
 L_p\equiv\log\!\left(\frac{p^2-\ii0}{4\pi}\right),
 \qquad
 \left(\frac{p^2-\ii0}{4\pi}\right)^{-\varepsilon}
 =1-\varepsilon L_p+O(\varepsilon^2).
\]
The rational factors expand as
\[
 \frac{1-\varepsilon}{1-2\varepsilon}
 =1+\varepsilon+O(\varepsilon^2),
 \qquad
 \frac{4(1-\varepsilon)}{(3-2\varepsilon)(1-2\varepsilon)}
 =\frac43\left(1+\frac53\varepsilon\right)
 +O(\varepsilon^2).
\]
Therefore, after dropping terms that vanish with \(\varepsilon\),
\[
 \boxed{\;
 \Sigma(p^2)
 =-\frac{ie^2}{(4\pi)^2}\slashed p
 \left[\frac1\varepsilon+1-\gamma_E-L_p\right]
 +O(\varepsilon).
 \;}
\]
and
\[
 ...
 =-\frac{e^2}{12\pi^2}
 (p^2\eta^{\mu\nu}-p^\mu p^\nu)
 \left[\frac1\varepsilon+\frac53-\gamma_E-L_p\right]
 +O(\varepsilon).
 \;}
\]
If one inserts the conventional dimensional-regularization scale through
\(e^2\mapsto e^2\mu^{2\varepsilon}\), replace
\(L_p\) by \(\log[(p^2-\ii0)/(4\pi\mu^2)]\).  This restores a dimensionless
logarithm without changing the source's requested Laurent coefficients.
\end{enumerate}
...
\[
 \int_{\mathbb R^d}\dd^dx\,e^{-x^2}
 =\left(\int_{-\infty}^{\infty}e^{-u^2}\dd u\right)^d
 =\pi^{d/2}.
\]
In radial coordinates the same integral is
\[
 \Omega_d\int_0^\infty \dd r\,r^{d-1}e^{-r^2}
...
\begin{aligned}
 ...
 \dd q\,\frac{q^{d-1}}{(q^2+m^2)^n}\\
 &=\frac{\Omega_d}{2(2\pi)^d}(m^2)^{d/2-n}
 \int_0^\infty \dd u\,\frac{u^{d/2-1}}{(1+u)^n}.
\end{aligned}
\]
The last integral is
\(B(d/2,n-d/2)\), so
...
\[
 \Pi^{\mu\nu}(p)=-e^2\int\frac{\dd^dk}{(2\pi)^d}
 \frac{\operatorname{Tr}[
 \gamma^\mu(-\ii\sl k+m)\gamma^\nu(-\ii(\sl k+\sl p)+m)]}
 {(k^2+m^2)((k+p)^2+m^2)}.
\]
...
\[
 \begin{aligned}
 \Pi^{\mu\nu}(p)
  &=-4e^2\int_0^1\dd x\int\frac{\dd^d\ell}{(2\pi)^d}
...
\[
 \int_0^1x^2(1-x)^2\dd x=\frac1{30},\qquad
 \int_0^1x^3(1-x)^3\dd x=\frac1{140}.
\]
Therefore
\[
 \boxed{\;
...
\[
 ...
 -\ii\delta_1eA_\mu\bar\psi\gamma^\mu\psi.
\]
...
\SupplementarySolution{8}{One-Loop QED Renormalization Constants}
\begin{enumerate}
\item
In the \(d=4-\epsilon\) convention, direct extraction of the pole parts
gives
\[
 \boxed{
 Z_1=Z_2=1-\frac{\xi e^2}{8\pi^2\epsilon},\qquad
 Z_3=1-\frac{e^2}{6\pi^2\epsilon},\qquad
 Z_m=1-\frac{e^2}{2\pi^2\epsilon}}
 +O(e^4).
\]
The longitudinal photon propagator produces the \(\xi\)-dependent
kinetic and vertex poles; the mass and transverse photon poles are gauge
independent.
\item
...
\item
In Lorenz gauge \(\xi=0\), the first result reduces to
\[
 Z_1=Z_2=1+O(e^4).
\]
Moreover \(Z_1Z_2^{-1}=1\), so the bare-charge relation depends only on
\(Z_3^{-1/2}\).
\end{enumerate}
...
At one loop,
\[
 \frac{\dd e}{\dd\ln\mu}=\frac{e^3}{12\pi^2}.
\]
Therefore
\[
 \frac{\dd}{\dd\ln\mu}\frac1{e^2}=-\frac1{6\pi^2},
\]
...
\[
 \mathcal L_1=-\frac1{8\pi^2}\int_0^\infty\frac{\dd s}{s^3}e^{-m_e^2s}
...
Since \(\int_0^\infty \dd s\,s\,e^{-m_e^2s}=m_e^{-4}\), one obtains
\[
 \boxed{\;
 \mathcal L_{\rm EH}
...
\(\psi=e^{-\ii mt}(\Phi,X)^{\mathsf T}\), and put
...
\[
 \frac1{A_1A_2A_3}
 =2\int_0^1\prod_{i=1}^3\dd x_i\,
 \frac{\delta(1-x_1-x_2-x_3)}
 {(x_1A_1+x_2A_2+x_3A_3)^3}.
\]
Take \(A_1=(k+p')^2+m^2-\ii0\),
\(A_2=(k+p)^2+m^2-\ii0\), and \(A_3=k^2-\ii0\), with
\(p^2=p'{}^2=-m^2\) and \(q=p'-p\).  Shifting
\(\ell=k+x_1p'+x_2p\) gives
\[
 \boxed{\;
 x_1A_1+x_2A_2+x_3A_3
 =\ell^2+m^2(x_1+x_2)^2+q^2x_1x_2-\ii0.
 \;}
\]
Since \(x_1+x_2=1-x_3\), this is
\(\ell^2+m^2(1-x_3)^2+q^2x_1x_2-\ii0\).  The common form makes the finite
...
spinors and use
\((\ii\sl p+m)u(p)=0\) and
\(\bar u(p')(\ii\sl p'+m)=0\).  The part independent of \(\ell\) that is not
proportional to \(\gamma^\mu\) reduces to
\[
 \boxed{\;
 N^\mu_{\rm Pauli}
 =2\ii m(x_1+x_2)x_3(p^\mu+p'{}^\mu).
 \;}
\]
...
\[
\begin{aligned}
 \int\frac{\dd^4k}{(2\pi)^4}\frac1{(k^2+\Delta)^3}
 &=\frac{2\pi^2}{(2\pi)^4}
 \int_0^\infty \dd k\,\frac{k^3}{(k^2+\Delta)^3}\\
...
\begin{aligned}
 F_2(0)
 &=\frac{e^2}{4\pi^2}\int_0^1\dd x_3\,
 (1-x_3)\frac{x_3}{1-x_3}\\
 &=\frac{e^2}{4\pi^2}\int_0^1x_3\,\dd x_3
 =\frac{e^2}{8\pi^2}
