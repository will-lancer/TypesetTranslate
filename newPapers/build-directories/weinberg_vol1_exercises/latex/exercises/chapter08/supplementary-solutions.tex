...
\(E_i=F_{0\ii}\) and \(B_i=\frac12\epsilon_{ijk}F_{jk}\),
\[
 \boxed{
 T_{00}
...
For
\[
 A^\mu(x)=\epsilon^\mu(p)e^{-\ii p\cdot x},
\]
...
\[
 \boxed{
 \gamma^\mu D_\mu\psi-\ii\lambda C^{\mu\nu}F_{\mu\nu}\psi=0,}
\]
\[
 \boxed{
 (D_\mu\bar\psi)\gamma^\mu
 +\ii\lambda\bar\psi C^{\mu\nu}F_{\mu\nu}=0,}
 \qquad
...
\(\delta\psi=-\ii\epsilon\psi\), Noether's theorem gives
\[
 \boxed{j^\mu=\ii\bar\psi\gamma^\mu\psi,qquad
 Q=\int \dd^3\mathbf x\,j^0.}
\]
Its divergence is
\[
 \partial_\mu j^\mu
 =\ii\left[(D_\mu\bar\psi)\gamma^\mu\psi
...
\[
 J_{\rm Max}^\nu
 =e j^\nu+2\ii\lambda\partial_\mu B^{\mu\nu}.
\]
...
\(\int \dd^3\mathbf x\,J_{\rm Max}^0=eQ\), even though the local source
...
transforms as \(\bar\psi\mapsto\bar\psi e^{\ii\alpha\gamma_5}\).  In
contrast, \([\gamma_5,C^{\mu\nu}]=0\), so
\[
 \bar\psi C^{\mu\nu}\psi
 \longmapsto
 \bar\psi C^{\mu\nu}e^{2\ii\alpha\gamma_5}\psi.
\]
...
\(e^{2\ii\alpha\gamma_5}\) factor, so one also needs \(m=0\).

For \(\alpha=\alpha(x)\), the kinetic term produces
\[
 \delta\mathcal L
 =-\ii(\partial_\mu\alpha)
...
\]
Using \(\int \dd\Omega\,\cos^2\theta=4\pi/3\), the total cross section is
\[
 \boxed{
 \sigma(e^+e^-\to\mu^+\mu^-)
...
\[
 ...
 \frac{-\ii[-\ii\slashed q+m]}{q^2+m^2-\ii0},\\
...
\[
 G(q)\equiv\frac{-\ii\slashed q+m}{q^2+m^2-\ii0},
 \qquad
...
used to define \(S=\mathbf1+\ii T\).  Replacing either external photon
...
Write
\[
 D_\mu\Phi=(\partial_\mu-\ii qA_\mu)\Phi,\qquad
 (D_\mu\Phi)^*=(\partial_\mu+\ii qA_\mu)\Phi^* .
\]
\begin{enumerate}
\item
...
\[
 \boxed{
 j^0=\ii q\left(\Pi_\Phi\Phi-\Phi^*\Pi_{\Phi^*}\right),}
\]
\[
 \boxed{
 \mathbf j
 =\ii q\left[\Phi^*\boldsymbol\nabla\Phi
 -(\boldsymbol\nabla\Phi^*)\Phi\right]
 +2q^2\mathbf A|\Phi|^2 .}
\]
...
\[
 D'_\mu\Phi'
 =(\partial_\mu-\ii qA_\mu-\ii q\partial_\mu\Omega)
   e^{\ii q\Omega}\Phi
 =e^{\ii q\Omega}D_\mu\Phi .
\]
...
\[
 \Phi'\!=e^{\ii q\Omega}\Phi,\quad
 \Pi'_\Phi=e^{-\ii q\Omega}\Pi_\Phi,\quad
 \Phi'^*\!=e^{-\ii q\Omega}\Phi^*,\quad
 \Pi'_{\Phi^*}=e^{\ii q\Omega}\Pi_{\Phi^*},
\]
...
\[
 F=\dd A=(\partial_\theta A_\varphi)\,\dd\theta\wedge \dd\varphi
 =\frac{n}{2e}\sin\theta\,\dd\theta\wedge \dd\varphi.
\]
...
\[
 \boxed{
 \int_{S^2}F
 =\frac{n}{2e}\int_0^\pi \dd\theta\,\sin\theta
...
\[
 ...
 =\pm \ii\delta^3(\mathbf x-\mathbf y),
\]
...
\[
 \chi_\mu(x)=C\,k_\mu e^{\ii k\cdot x}.
\]
Choose
\[
 \lambda(x)=\frac{C}{\ii}e^{\ii k\cdot x}.
\]
...
\[
 \chi_\mu^{(f)}(x)
 =\int \dd^3k\,C(\mathbf k)f(\mathbf k)
 k_\mu e^{\ii k\cdot x}.
\]
Define
\[
 \lambda_f(x)=\int \dd^3k\,
 \frac{C(\mathbf k)f(\mathbf k)}{\ii}e^{\ii k\cdot x}.
\]
...
\[
 ...
 \frac{\dd^3k_1}{(2\pi)^3\,2E_1'}
 \frac{\dd^3k_2}{(2\pi)^3\,2E_2'}.
\]
...
\[
 \dd\Phi_2=\frac{|\mathbf k|}{16\pi^2\sqrt s}\,\dd\Omega.
\]
...
\qquad \frac{\dd\sigma}{\dd\Omega}=|f(\mathbf q)|^2.
\]
\item
For \(V(r)=Z\alpha/r\),
...
\[
 \bra{p,s}H_I\ket{p',s'}
 =q\int \dd^3x\,e^{-\ii(\mathbf p-\mathbf p')\cdot\mathbf x}
...
of \(F_{\mu\nu}\) into
\[
 \Delta H_{\rm EDM}=-\dd\,\boldsymbol\sigma\cdot\mathbf E
 =-2\dd\,\mathbf S\cdot\mathbf E,
\]
