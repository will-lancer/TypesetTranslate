...
\[
 Q(t)=\int \dd^3x\,j^0(t,\mathbf x).
\]
Current conservation implies
\[
 \dot Q=-\int \dd^3x\,\boldsymbol\nabla\cdot\mathbf j
       =-\oint_{S_\infty}\dd\mathbf S\cdot\mathbf j,
\]
...
\(\delta\phi=\ii\epsilon[Q,\phi]\), and conservation is equivalent to
...
\qquad
 \widetilde T^{0\ii}=(\mathbf E\times\mathbf B)^i.
\]
\item
At \(j^\mu=0\), vary
\[
 S[A,g]=-\frac14\int \dd^4x\,\sqrt{|g|}\,
...
The measure \(\dd^4x\), the contraction
...
\item
Under
\(\delta\phi=\ii\epsilon\phi\) and
\(\delta\phi^\dagger=-\ii\epsilon\phi^\dagger\), the chapter's Noether
normalization gives
\[
...
\[
 ...
 =\ii\delta_{\alpha\beta}\delta^d(\mathbf x-\mathbf y),
\qquad
...
Thus
\[
 \delta_i\Phi=\ii\epsilon_i t_i\Phi,\qquad
 \delta_i\Phi^\dagger=-\ii\epsilon_i\Phi^\dagger t_i,
\]
or, finitely, \(\Phi\mapsto U\Phi\) with
\(U=\exp(\ii\epsilon_i t_i)\in SU(2)\).  Both
\(\partial_\mu\Phi^\dagger\partial^\mu\Phi\) and
...
\(\ii\varepsilon_{ijk}Q^k\).  Direct evaluation of the bilinear charges
has no central remainder and gives
\[
 [Q^i,Q^j]=\ii\varepsilon_{ijk}Q^k.
\]
...
Then
\[
 \delta_A\Phi=\ii\epsilon_A t_A\Phi,\qquad
 J_A^\mu=\frac{\ii}{2}\left[
...
Its momenta are
\[
 \Pi_i=2\widetilde aF_{0\ii},\qquad \Pi_0=0.
\]
...
\]
\item
On \(e^{-\ii k\cdot x}\), the mostly-plus wave operator acts as
...
\[
 \frac{\ii}{k^2(\widetilde c k^2+m^2)}
 =\frac{\ii}{m^2}\left(
...
Consequently
\[
 [P^i,\Psi(\mathbf x)]=\ii\partial_i\Psi(\mathbf x),
 \qquad
 [P^i,\Psi^\dagger(\mathbf x)]
 =\ii\partial_i\Psi^\dagger(\mathbf x).
\]
Thus
\(e^{-\ii\boldsymbol\epsilon\cdot\mathbf P}\Psi(\mathbf x)
e^{\ii\boldsymbol\epsilon\cdot\mathbf P}
...
\[
 \begin{split}
 P^i\ket{\psi}
 &=\int \dd^d\mathbf x\,\psi(\mathbf x)
 [P^i,\Psi^\dagger(\mathbf x)]\ket{\Omega}\\
 &=\ii\int \dd^d\mathbf x\,\psi(\mathbf x)
 \partial_i\ket{\mathbf x}\\
 &=-\ii\int \dd^d\mathbf x\,
...
\[
 ...
 -2\ii\lambda\sum_{r=1}^{3}(p_r^2+m^2),
\]
and
\[
 \text{quartic vertex:}\qquad
 -4\ii\lambda^2\left(\sum_{r=1}^{4}p_r^2+3m^2\right).
\]
...
\[
 4\ii\lambda\sum_{r<s}p_r\cdot p_s-6\ii\lambda m^2
 =-2\ii\lambda\sum_r(p_r^2+m^2),
\]
...
\(-2\ii\lambda(q_x^2+m^2)\).  The contribution of one channel is
therefore
\[
 \bigl[-2\ii\lambda(q_x^2+m^2)\bigr]^2
 \frac{-\ii}{q_x^2+m^2-\ii0}
 =4\ii\lambda^2(q_x^2+m^2).
\]
Summing the three exchange graphs gives
\[
 \mathcal A_{\rm exch}
 =4\ii\lambda^2(q_s^2+q_t^2+q_u^2+3m^2)
 =-4\ii\lambda^2m^2.
\]
The on-shell contact rule is
\[
 \mathcal A_{\rm contact}
 =-4\ii\lambda^2(-4m^2+3m^2)
 =4\ii\lambda^2m^2.
\]
Consequently
\[
...
\[
 ...
 +\frac{\epsilon^2}{2}\int \dd^4x\,\dd^4y\,
...
\(\omega_{\mathbf p}^2=\mathbf p^2+m^2\).  Hence
\[
...
\]
where
\(E_{\rm vac}=\delta^{D-1}(0)\int d^{D-1}\mathbf p\,\omega_{\mathbf p}\)
