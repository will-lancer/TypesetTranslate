...
Here is a figure-independent complete list at fourth order.  Label the
four cubic vertices \(1,\ldots,4\); write \(ij^r\) for \(r\) parallel
lines between distinct vertices and \(ii\) for a tadpole at \(i\).
Relabelings of \(1,\ldots,4\) do not make a new diagram.  The five
connected topologies are
\[
\begin{array}{c|l|c}
...
\end{array}
\]
and the three disconnected topologies are
\[
\begin{array}{c|l|c}
V_6&12^3;\ 33,34,44&96\\
V_7&14^3;\ 23^3&288\\
V_8&11,14;\ 22,23,33,44&128 .
\end{array}
\]
The semicolon separates components.  The last column, though not
required in this part, supplies a check and will be useful below.
\item
...
its coefficient is \((i\lambda)^n/S_\Gamma\), where \(n\) is the number
...
\[
 \dot\phi=i[H,\phi]=\pi,\qquad
 \dot\pi=i[H,\pi]=(\boldsymbol\nabla^2-m^2)\phi+J.
\]
Consequently
\[
 \ddot\phi=(\boldsymbol\nabla^2-m^2)\phi+J,
...
\[
 a(\mathbf p,0)=\int d^3x\,e^{-i\mathbf p\cdot\mathbf x}
 \left[
 \sqrt{\frac{E_{\mathbf p}}2}\,\phi(\mathbf x,0)
 +\frac{i}{\sqrt{2E_{\mathbf p}}}\,\pi(\mathbf x,0)
 \right].
\]
...
\item
Let
\(\alpha(\mathbf p)=i\widetilde j(\mathbf p)/
\sqrt{2E_{\mathbf p}}\).  Since the pre-impulse vacuum obeys
\(a(\mathbf p,0^-)\ket0=0\),
\[
...
\(|\widetilde j(\mathbf p)|^2/(2E_{\mathbf p})\), with the
...
\[
 (\Box-m^2)\left[i\int d^4y\,G(x-y)J(y)\right]
 =i^2J(x)=-J(x).
\]
Thus
\[
 \phi(x)=\phi_{\rm hom}(x)
 +i\int d^4y\,G(x-y)J(y),
\qquad
...
\[
 \int d\tau\,D_F(\tau,\mathbf R)
 =-i\int\frac{d^3p}{(2\pi)^3}
 \frac{e^{i\mathbf p\cdot\mathbf R}}{\mathbf p^2+m^2}
 =-\frac{i}{4\pi R}e^{-mR}.
\]
Consequently
\[
...
\item
Comparison with \(Z_{\rm int}=e^{-iV(R)T}\) gives the Yukawa potential
\[
