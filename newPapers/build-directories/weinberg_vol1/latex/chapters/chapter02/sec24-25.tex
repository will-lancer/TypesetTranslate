...
=\mathbf{1}+\frac{\ii}{2}\omega_{\rho\sigma}J^{\rho\sigma}
-\ii\epsilon_\rho P^\rho+\cdots\,.
...
\begin{align}
[J_i,J_j]&=\ii\epsilon_{ijk}J_k\,, \tag{2.4.18}\label{eq:2.4.18}\\
[J_i,K_j]&=\ii\epsilon_{ijk}K_k\,, \tag{2.4.19}\label{eq:2.4.19}\\
[K_i,K_j]&=-\ii\epsilon_{ijk}J_k\,, \tag{2.4.20}\label{eq:2.4.20}\\
[J_i,P_j]&=\ii\epsilon_{ijk}P_k\,, \tag{2.4.21}\label{eq:2.4.21}\\
[K_i,P_j]&=\ii H\delta_{ij}\,, \tag{2.4.22}\label{eq:2.4.22}\\
[J_i,H]&=[P_i,H]=[H,H]=0\,, \tag{2.4.23}\label{eq:2.4.23}\\
[K_i,H]&=\ii P_i\,, \tag{2.4.24}\label{eq:2.4.24}
\end{align}
...
\begin{equation}
U(\mathbf{1},a)=\exp(-\ii P^\mu a_\mu)\,.
...
\begin{equation}
U(R_{\boldsymbol\theta},0)=\exp(\ii\mathbf{J}\cdot\boldsymbol\theta)\,.
...
[K_i,P_j]=\ii M\delta_{ij}\,,\\
[J_i,W]=[P_i,W]=0\,,\qquad [K_i,W]=\ii P_i\,,\\
...
\[
\exp(\ii\mathbf{K}\cdot\mathbf{v})\exp(-\ii\mathbf{P}\cdot\mathbf{a})
=\exp(\ii M\mathbf{a}\cdot\mathbf{v}/2)
 \exp\!\left(\ii(\mathbf{K}\cdot\mathbf{v}-\mathbf{P}\cdot\mathbf{a})\right).
\]
The appearance of the phase factor $\exp(\ii M\mathbf{a}\cdot\mathbf{v}/2)$ shows
...
under translations:
\[
U(\mathbf{1},a)\ket{p,\sigma}=e^{-\ii p\cdot a}\ket{p,\sigma}\,.
\]
...
\begin{equation}
\dd^3\mathbf{p}/\sqrt{\mathbf{p}^2+M^2}\,.
...
\begin{aligned}
F(\mathbf{p})
&=\int F(\mathbf{p}')\delta^3(\mathbf{p}-\mathbf{p}')\dd^3\mathbf{p}'\\
&=\int F(\mathbf{p}')
\left[\sqrt{\mathbf{p}'^2+M^2}\,
\delta^3(\mathbf{p}'-\mathbf{p})\right]
\frac{\dd^3\mathbf{p}'}{\sqrt{\mathbf{p}'^2+M^2}}\,,
\end{aligned}
\]
...
=\delta_{\sigma'\sigma}
+\frac{\ii}{2}\Theta_{ik}(J^{(j)}_{ik})_{\sigma'\sigma}\,,
\tag{2.5.20}\label{eq:2.5.20}
\end{equation}
\begin{equation}
\begin{aligned}
(J^{(j)}_{23}\pm \ii J^{(j)}_{31})_{\sigma'\sigma}
&=(J^{(j)}_1\pm \ii J^{(j)}_2)_{\sigma'\sigma}\\
...
\begin{equation}
U(W(\theta,\alpha,\beta))=\mathbf{1}+\ii\alpha A+\ii\beta B+\ii\theta J_3\,,
...
\begin{align}
[J_3,A]&=+\ii B\,, \tag{2.5.35}\label{eq:2.5.35}\\
[J_3,B]&=-\ii A\,, \tag{2.5.36}\label{eq:2.5.36}\\
...
$\beta$ to
\begin{equation}
U(S(\alpha,\beta))=\exp(\ii\alpha A+\ii\beta B)
\tag{2.5.40}\label{eq:2.5.40}
\end{equation}
and for finite $\theta$ to
\begin{equation}
U(R(\theta))=\exp(\ii J_3\theta)\,.
...
\[
U(W)\ket{k,\sigma}
=\exp(\ii\alpha A+\ii\beta B)\exp(\ii\theta J_3)\ket{k,\sigma}
=\exp(\ii\theta\sigma)\ket{k,\sigma}
\]
and therefore Eq.~\eqref{eq:2.5.8} gives
\[
D_{\sigma'\sigma}(W)=\exp(\ii\theta\sigma)\delta_{\sigma'\sigma}\,,
\]
...
U(\Lambda)\ket{p,\sigma}
=\sqrt{\frac{(\Lambda p)^0}{p^0}}
\exp(\ii\sigma\theta(\Lambda,p))\ket{\Lambda p,\sigma}
...
\begin{equation}
U(R(\hat{\mathbf{p}}))=\exp(\ii\phi J_3)\exp(\ii\theta J_2)\,,
...
$\exp(\ii\sigma\theta)$ in Eq.~\eqref{eq:2.5.42}, a state formed as a linear superposition
