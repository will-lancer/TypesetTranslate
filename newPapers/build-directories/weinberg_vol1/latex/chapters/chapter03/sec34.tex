...
\begin{equation}
 ...
 \frac{1}{(2\pi)^3}\int_V \dd^3x\,e^{\ii(\mathbf{p}-\mathbf{p}')\cdot\mathbf{x}}
...
\begin{equation}
 ...
 \exp\bigl(\ii(E_\alpha-E_\beta)t\bigr)\,\dd t.
...
one-particle box states in a momentum-space volume $\dd^3p$ is $Vd^3p/(2\pi)^3$, because this
is the number of triplets of integers $n_1,n_2,n_3$ for which the momentum \eqref{eq:3.4.1} lies in the
momentum-space volume $\dd^3p$ around $\mathbf{p}$. We shall define the final-state interval
$\dd\beta$ as a product of $\dd^3p$ for each final particle, so the total number of states in this
range is
\begin{equation}
 \dd\mathcal{N}_\beta=\left[V/(2\pi)^3\right]^{N_\beta}\dd\beta.
 \tag{3.4.8}\label{eq:3.4.8}
\end{equation}
Hence the total probability for the system to wind up in a range $\dd\beta$ of final states is
\begin{equation}
 \dd P(\alpha\to\beta)=P(\alpha\to\beta)\dd\mathcal{N}_\beta
 =\left[(2\pi)^3/V\right]^{N_\alpha}|S_{\beta\alpha}|^2\dd\beta.
...
\begin{equation}
 S_{\beta\alpha}=-2\pi \ii\,\delta_V^3(\mathbf{p}_\beta-\mathbf{p}_\alpha)
...
\begin{align*}
 \dd P(\alpha\to\beta)={}&(2\pi)^2\left[(2\pi)^3/V\right]^{N_\alpha-1}
 (T/2\pi)|M_{\beta\alpha}|^2\\
 &\cdot\delta_V^3(\mathbf{p}_\beta-\mathbf{p}_\alpha)
 \delta_T(E_\beta-E_\alpha)\dd\beta.
\end{align*}
...
\begin{align}
 \dd\Gamma(\alpha\to\beta)&\equiv \dd P(\alpha\to\beta)/T\notag\\
...
where now
\begin{equation}
 S_{\beta\alpha}=-2\pi \ii\delta^4(p_\beta-p_\alpha)M_{\beta\alpha}.
...
factor $\delta^4(p_\alpha-p_\beta)\dd\beta$ later in this section.

There are two cases of special importance:

\noindent$\mathbf{N}_{\alpha}=1:$
...
\begin{equation}
 \dd\Gamma(\alpha\to\beta)=2\pi|M_{\beta\alpha}|^2
 \delta^4(p_\beta-p_\alpha)\dd\beta.
...
\begin{equation}
 \dd\sigma(\alpha\to\beta)\equiv \dd\Gamma(\alpha\to\beta)/\Phi_\alpha
...
\[
 \sum_{\mathrm{spins}}\dd\Gamma(\alpha\to\beta)
 =2\pi E_\alpha^{-1}R_{\beta\alpha}\delta^4(p_\beta-p_\alpha)\dd\beta
 /\prod_\beta E.
\]
The factor $\dd\beta/\prod_\beta E$ may be recognized as the product of the Lorentz-invariant
...
\[
 \sum_{\mathrm{spins}}\dd\sigma(\alpha\to\beta)
 =(2\pi)^4u_\alpha^{-1}E_1^{-1}E_2^{-1}R_{\beta\alpha}
 \delta^4(p_\alpha-p_\beta)\dd\beta/\prod_\beta E,
\]
...
$\dd\beta/\prod_\beta E$ are already Lorentz-invariant, so this means that we must define the
...
$\delta^4(p_\beta-p_\alpha)\dd\beta$, which appears in the general formula \eqref{eq:3.4.11} for transition
...
\begin{equation}
 ...
 \delta(E'_1+E'_2+\cdots-E)\dd^3p'_1\dd^3p'_2\cdots,
...
\begin{equation}
 ...
 \delta(E'_1+E'_2+\cdots-E)\dd^3p'_2\cdots
...
\begin{equation}
 ...
 |\mathbf{p}'_1|^2d|\mathbf{p}'_1|\dd\Omega,
 \tag{3.4.23}\label{eq:3.4.23}
\end{equation}
where $\mathbf{p}'_2=-\mathbf{p}'_1$ and $\dd\Omega\equiv\sin\theta\,\dd\theta\,\dd\phi$ is the
...
\begin{equation}
 \frac{\dd\Gamma(\alpha\to\beta)}{\dd\Omega}
...
\[
 \delta^4(p_\beta-p_\alpha)\dd\beta\longrightarrow \dd^3p'_2\dd^3p'_3
...
\[
 \dd^3p'_2\dd^3p'_3=|\mathbf{p}'_2|^2d|\mathbf{p}'_2|
 |\mathbf{p}'_3|^2d|\mathbf{p}'_3|\dd\Omega_3\dd\phi_{23}\dd\cos\theta_{23},
\]
