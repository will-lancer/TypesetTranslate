...
\begin{equation}
-\ii\Delta_{\ell m}(x,y)=\theta(x-y)(\psi_\ell(x)\psi_m^\dagger(y))_0
...
\begin{equation}
-\ii\Delta_{\ell m}(x,y)=(T\{\psi_\ell(x)\psi_m^\dagger(y)\})_0,
\tag{6.2.31}
\label{eq:6.2.31}
\end{equation}
...
$\exp(-\ii p'\cdot x)$, and for an initial particle line with momentum $p^\mu$
entering a vertex with spacetime coordinate $x^\mu$, we get a factor
proportional to $\exp(+\ii p\cdot x)$. In Section 6.2 we saw that the factor
...
proportional to $\exp(\ii q\cdot(x-y))$. We can think of $q^\mu$ as the
...
\setcounter{figure}{8}
\begin{figure}[htbp]
\input{figures/chapter06/fig69.tex}
...
\begin{equation}
-\ii(2\pi)^4g_j\delta^4\left(\sum p+\sum q-\sum p'-\sum q'\right)
\tag{6.3.2}
\label{eq:6.3.2}
\end{equation}
...
$-\ii\Delta_{\ell m}(q)$:
\begin{equation}
-\ii(2\pi)^{-4}\frac{P_{\ell m}(q)}{q^2+m_\ell^2-\ii\epsilon}.
\tag{6.3.3}
\label{eq:6.3.3}
\end{equation}
...
&=\sum_{k'\ell'm'k\ell m}(-\ii)^2(2\pi)^8g_{\ell'm'k'}g_{m\ell k}
u_{\ell'}^*(\mathbf p'_1\sigma'_1n'_1)u_\ell(\mathbf p_1\sigma_1n_1)\\
&\quad\cdot\int \dd^4q\left(-\ii(2\pi)^{-4}
\frac{P_{m'm}(q)}{q^2+m_m^2-\ii\epsilon}\right)(2\pi)^{-6}\\
&\quad\cdot\Bigl[
...
&S_{\mathbf p'_1\sigma'_1n'_1\,\mathbf p'_2\sigma'_2n'_2,\,
\mathbf p_1\sigma_1n_1\,\mathbf p_2\sigma_2n_2}\notag\\
&=\ii(2\pi)^{-2}\delta^4(p_1+p_2-p'_1-p'_2)
\sum_{k'\ell'm'k\ell m}g_{\ell'm'k'}g_{m\ell k}
u_{\ell'}^*(\mathbf p'_1\sigma'_1n'_1)u_\ell(\mathbf p_1\sigma_1n_1)
\notag\\
&\quad\cdot\left[
\frac{P_{m'm}(p'_1-p_1)}{(p'_1-p_1)^2+m_m^2-\ii\epsilon}
u_{k'}^*(\mathbf p'_2\sigma'_2n'_2)u_k(\mathbf p_2\sigma_2n_2)\right.
\notag\\
&\qquad\left.{}+
\frac{P_{m'm}(p'_2-p_1)}{(p'_2-p_1)^2+m_m^2-\ii\epsilon}
...
\frac{P_{k'k}(p'_1-p_1)}{(p'_1-p_1)^2+m_k^2-\ii\epsilon}
\notag\\
...
&S_{\mathbf p'_1\sigma'_1n'_1\,\mathbf p'_2\sigma'_2n'_2,\,
\mathbf p_1\sigma_1n_1\,\mathbf p_2\sigma_2n_2}\notag\\
&=\ii(2\pi)^{-2}\delta^4(p_1+p_2-p'_1-p'_2)\sum_{k'k}
\Biggl[\left(u^\dagger(\mathbf p'_1\sigma'_1n'_1)\Gamma_{k'}
\frac{P(p'_1-p_1)}{(p'_1-p_1)^2+M^2-\ii\epsilon}
...
\frac{P(p'_2-p_1)}{(p'_2-p_1)^2+M^2-\ii\epsilon}
\Gamma_k u(\mathbf p_1\sigma_1n_1)\right)\notag\\
...
\mathbf p_1\sigma_1n_1\,\mathbf p_2\sigma_2n_2}\notag\\
&=\ii(2\pi)^{-2}\delta^4(p_1+p_2-p'_1-p'_2)
\sum_{k'k}\frac{P_{k'k}(p'_1-p_1)}{(p'_1-p_1)^2+m_k^2-\ii\epsilon}\notag\\
&\quad\cdot
...
&\quad\cdot\int \dd^4q\,\operatorname{Tr}\left\{
\Gamma_{k'_2}\frac{P(q)}{q^2+M^2-\ii\epsilon}
\Gamma_{k'_1}\frac{P(q+p'_1)}{(q+p'_1)^2+M^2-\ii\epsilon}\right.\notag\\
&\qquad\left.\cdot
\Gamma_{k_1}\frac{P(q+p'_1-p_1)}{(q+p'_1-p_1)^2+M^2-\ii\epsilon}
\Gamma_{k_2}\frac{P(q-p'_2)}{(q-p'_2)^2+M^2-\ii\epsilon}
\right\}+\cdots.
\tag{6.3.10}
\label{eq:6.3.10}
\end{align}
...
$-\ii$ is inserted to make this interaction Hermitian for real coupling
constants $g$.) Recall that the polynomial $P(q)$ for the scalar is just unity,
while for the spinor it is $[-\ii\gamma_\mu q^\mu+M]\beta$. Also, the $u$ for a
...
&\cdot\frac{1}{(p'_1-p_1)^2+m^2-\ii\epsilon}-[1'\rightleftarrows2'],
\\[4pt]
S_{\mathbf p'_1\,\mathbf p'_2,\,\mathbf p_1\,\mathbf p_2}
={}&-(2\pi)^{-6}g^2(16E_1E_2E'_1E'_2)^{-1/2}
\delta^4(p_1+p_2-p'_1-p'_2)\\
&\cdot\int \dd^4q\,\operatorname{Tr}\Biggl\{
\gamma_5\frac{-\ii\gamma_\mu q^\mu+M}{q^2+M^2-\ii\epsilon}
\gamma_5\frac{-\ii\gamma_\mu(q+p'_1)^\mu+M}{(q+p'_1)^2+M^2-\ii\epsilon}\\
&\qquad\cdot\gamma_5
\frac{-\ii\gamma_\mu(q+p'_1-p_1)^\mu+M}{(q+p'_1-p_1)^2+M^2-\ii\epsilon}
\gamma_5\frac{-\ii\gamma_\mu(q-p'_2)^\mu+M}{(q-p'_2)^2+M^2-\ii\epsilon}
\Biggr\}+\cdots,
\end{align*}
