...
\begin{equation}
\mathcal H_I(x)=\sum_i g_i\mathcal H_{I,\ii}(x),
\tag{6.1.2}
\label{eq:6.1.2}
\end{equation}
each term $\mathcal H_{I,\ii}$ being a product of definite numbers of fields and
...
\begin{align}
\psi_\ell(x)={}&\sum_\sigma(2\pi)^{-3/2}\int \dd^3p\,
\bigl[u_\ell(\mathbf p,\sigma,n)a_{\mathbf p,\sigma,n}e^{\ii p\cdot x}
\notag\\
&\hspace{43mm}
+v_\ell(\mathbf p,\sigma,n)a^\dagger_{\mathbf p,\sigma,n^c}e^{-\ii p\cdot x}
\bigr].
\tag{6.1.3}
\label{eq:6.1.3}
\end{align}
Here $n^c$ denotes the antiparticle of the species $n$, and
$\exp(\pm \ii p\cdot x)$ is calculated with $p^0$ set equal to
...
terms $\mathcal H_{I,\ii}$ in the polynomial
...
$\mathcal H_{I,\ii}(x)$ yields a factor
\begin{equation}
\left[a_{\mathbf p'\sigma'n'},\psi_\ell^\dagger(x)\right]_{\mp}
=(2\pi)^{-3/2}e^{-\ii p'\cdot x}u_\ell^*(\mathbf p',\sigma',n').
\tag{6.1.9}
\label{eq:6.1.9}
\end{equation}
...
=(2\pi)^{-3/2}e^{-\ii p'\cdot x}v_\ell(\mathbf p',\sigma',n').
\tag{6.1.10}
\label{eq:6.1.10}
\end{equation}
...
$\psi_\ell^\dagger(x)$ in $\mathcal H_{I,\ii}(x)$ yields a factor
\begin{equation}
...
$\mathcal H_{I,\ii}(x)$ with a field adjoint $\psi_m^\dagger(y)$ in
...
\notag\\
&\hspace{48mm}\equiv-\ii\Delta_{\ell m}(x,y),
\tag{6.1.14}
\label{eq:6.1.14}
\end{align}
...
\begin{align}
\psi_\ell^{(+)}(x)
&=(2\pi)^{-3/2}\int \dd^3p\sum_\sigma
u_\ell(\mathbf p\sigma n)e^{\ii p\cdot x}a_{\mathbf p\sigma n},
\tag{6.1.15}\label{eq:6.1.15}\\
\psi_\ell^{(-)}(x)
&=(2\pi)^{-3/2}\int \dd^3p\sum_\sigma
v_\ell(\mathbf p\sigma n)e^{-\ii p\cdot x}a^\dagger_{\mathbf p\sigma n^c}.
\tag{6.1.16}
\label{eq:6.1.16}
\end{align}
...
\setcounter{figure}{0}
\begin{figure}[htbp]
\input{figures/chapter06/fig61.tex}
...
called \emph{vertices}, each representing one of the $\mathcal H_{I,\ii}(x)$, and
...
one of the $\mathcal H_{I,\ii}(x)$ is represented by a line running from the vertex
representing that $\mathcal H_{I,\ii}(x)$ upwards out of the diagram, carrying an
arrow pointed upwards.

\noindent\textbf{(b)} The pairing of a final antiparticle with a field in one
of the $\mathcal H_{I,\ii}(x)$ is also represented by a line running from the vertex
representing that $\mathcal H_{I,\ii}(x)$ upwards out of the diagram, but carrying
...
of the $\mathcal H_{I,\ii}(x)$ is represented by a line running into the diagram from
below, ending in the vertex representing that $\mathcal H_{I,\ii}(x)$, carrying an
arrow pointed upwards.

\noindent\textbf{(d)} The pairing of an initial antiparticle with a field in
one of the $\mathcal H_{I,\ii}(x)$ is also represented by a line running into the
diagram from below, ending in the vertex representing that $\mathcal H_{I,\ii}(x)$,
...
\noindent\textbf{(f)} The pairing of a field in $\mathcal H_{I,\ii}(x)$ with a
field adjoint in $\mathcal H_{I,j}(y)$ is represented by a line joining the
vertices representing $\mathcal H_{I,\ii}(x)$ and $\mathcal H_{I,j}(y)$,
...
create a particle destroyed by a field in $\mathcal H_{I,\ii}(x)$, or destroy an
antiparticle created by a field in $\mathcal H_{I,\ii}(x)$. Note also that since
every field or field adjoint in $\mathcal H_{I,\ii}(x)$ must be paired with
something, the total number of lines at a vertex of type $i$, corresponding to
a term $\mathcal H_{I,\ii}(x)$ in Eq. \eqref{eq:6.1.2}, is just equal to the total number
of field or field adjoint factors in $\mathcal H_{I,\ii}(x)$. Of these lines, the
...
order $N_i$ in each of the interaction terms $\mathcal H_{I,\ii}(x)$ in Eq.
...
\noindent\textbf{(ii)} For each vertex of type $i$, include a factor $-\ii$
(from the $(-\ii)^N$ in Eq. \eqref{eq:6.1.1}) and a factor $g_i$ (the coupling constant
multiplying the product of fields in $\mathcal H_{I,\ii}(x)$). For each line
...
\noindent\textbf{(v)} Suppose that an interaction $\mathcal H_{I,\ii}(x)$
...
\setcounter{figure}{1}
\begin{figure}[htbp]
\input{figures/chapter06/fig62.tex}
...
$\mathcal H_{I,\ii}(x)$; the second with any one of the remaining $M-1$
...
explicit factor $1/M!$ appears in any $\mathcal H_{I,\ii}(x)$ containing $M$
...
interaction $\mathcal H_{I,\ii}(x)$ are paired with $M$ corresponding field
