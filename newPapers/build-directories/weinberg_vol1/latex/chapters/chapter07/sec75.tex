...
&=\int d^3x\Bigl[\Pi(\Pi+J^0)+\frac12(\boldsymbol\nabla\Phi)^2
...
\end{equation}
\begin{equation}
H_{\mathrm{int}}=\int d^3x\left[\Pi J^0+\mathbf J\cdot\boldsymbol\nabla\Phi
...
\begin{equation}
H_I(t)=\int d^3x\left[J^\mu(\mathbf x,t)\partial_\mu\phi(\mathbf x,t)
+\frac12\bigl[J^0(\mathbf x,t)\bigr]^2
...
$H=\int d^3x\,(\boldsymbol\Pi\cdot\dot{\mathbf V}-\mathcal L)$ for this
...
so
\begin{align*}
H=\int d^3x\Bigl[&\boldsymbol\Pi^2
...
$J^\mu$):
\begin{equation}
H_0=\int d^3x\left[\frac12\boldsymbol\pi^2
...
\end{equation}
\begin{equation}
H_I(t)=\int d^3x\left[\mathbf J\cdot\mathbf v
-m^{-2}J^0\boldsymbol\nabla\cdot\boldsymbol\pi
...
a Fourier transform
\begin{align}
v^\mu(x)=(2\pi)^{-3/2}\sum_\sigma\int d^3p\,(2p^0)^{-1/2}
\Bigl\{&e^\mu(\mathbf p,\sigma)a_{\mathbf p,\sigma}e^{ip\cdot x}\notag\\
&+e^{\mu*}(\mathbf p,\sigma)a^\dagger_{\mathbf p,\sigma}e^{-ip\cdot x}\Bigr\},
...
\begin{align}
[v^i(\mathbf x,t),\pi^j(\mathbf y,t)]&=i\delta_{ij}\delta^3(\mathbf x-\mathbf y),
\notag\\
...
$\sum_\sigma\int d^3p\,p^0a^\dagger_{\mathbf p,\sigma}a_{\mathbf p,\sigma}$.
Finally, using Eq.~\eqref{eq:7.5.22} in Eq.~\eqref{eq:7.5.19} yields the interaction in the
interaction picture
\begin{equation}
H_I(t)=\int d^3x\left[J_\mu v^\mu+\frac1{2m^2}(J^0)^2\right].
...
\[
H=\int d^3x\,[\Pi\dot\Psi-\mathcal L]
=\int d^3x\left[\Pi\gamma^0(\boldsymbol\gamma\cdot\boldsymbol\nabla+m)\Psi
+\mathcal H_I\right].
\]
We write this as
\begin{equation}
...
where
\begin{equation}
H_0=\int d^3x\,\Pi\gamma^0(\boldsymbol\gamma\cdot\boldsymbol\nabla+m)\Psi,
\tag{7.5.37}\label{eq:7.5.37}
\end{equation}
\begin{equation}
H_{\mathrm{int}}=\int d^3x\,\mathcal H_I(\bar\Psi,\Psi).
...
a Fourier transform
\begin{equation}
\psi(x)=(2\pi)^{-3/2}\int d^3p\sum_\sigma
\left\{u(\mathbf p,\sigma)e^{ip\cdot x}a_{\mathbf p,\sigma}
+v(\mathbf p,\sigma)e^{-ip\cdot x}b^\dagger_{\mathbf p,\sigma}\right\},
...
\begin{equation}
(i\gamma^\mu p_\mu+m)u(\mathbf p,\sigma)=0
\tag{7.5.43}\label{eq:7.5.43}
\end{equation}
and likewise
\begin{equation}
(-i\gamma^\mu p_\mu+m)v(\mathbf p,\sigma)=0
\tag{7.5.44}\label{eq:7.5.44}
\end{equation}
normalized so that\footnote{The matrix $i\gamma^\mu p_\mu$ has eigenvalues
$\pm m$, so $\sum u\bar u$ and $\sum v\bar v$ must be proportional to the
projection matrices $(-i\gamma^\mu p_\mu+m)/2m$ and
$(i\gamma^\mu p_\mu+m)/2m$, respectively. The proportionality factor may be
...
&=i(\gamma^0)_{\alpha\beta}\delta^3(\mathbf x-\mathbf y),
