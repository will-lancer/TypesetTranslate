...
diverge, like $\int^\infty k^{D-1}\,\dd k$. If we differentiate $D+1$ times
...
\[
  \mathcal{J}(q)=\int_0^\infty\frac{\dd k}{k+q}
\]
with $D=1-1=0$. Differentiating once gives
\[
  \mathcal{J}'(q)=-\int_0^\infty\frac{\dd k}{(k+q)^2}=-\frac1q,
\]
so
\[
  \mathcal{J}(q)=-\ln q+c.
...
integral
\[
  \int_0^\infty\frac{k\,\dd k}{k+q}=a+bq+q\ln q
\]
...
contribution as
\begin{equation}
  \Sigma^{*(\ell)}(p)=A-(\ii\sl{p}+m)B+\Sigma^{(f)}(\sl{p}),
...
\[
  \Gamma^\mu(p,p)=\gamma^\mu+\ii\frac{\partial}{\partial p_\mu}\Sigma(p)
\]
and therefore
\begin{equation}
  L\gamma_\mu+\Gamma_\mu^{(f)}(p,p)
  =B\gamma_\mu+\ii\frac{\partial\Sigma^{(f)}(p)}{\partial p^\mu}.
...
contribution $Z_2\delta m-(Z_2-1)(\ii\sl{p}+m)$ to $\Sigma^*(p)$. The
...
\input{chapters/chapter12/figures/fig12-02.tex}
...
$-(Z_3-1)(q^2\eta_{\mu\nu}-q_\mu q_\nu)$ for each $\Pi_{\mu\nu}^*(q)$; a
term $Z_2\delta m-(Z_2-1)(\ii\sl{p}+m)$ for each $\Sigma^*(p)$; and a term
...
\input{chapters/chapter12/figures/fig12-03.tex}
...
\begin{align}
 ...
 ={}&-\frac{e^4}{(2\pi)^8}\int \dd^4p\int \dd^4p'\,
 \frac{1}{(p-p')^2-\ii\epsilon}
 \notag\\
 &\quad\cdot\operatorname{Tr}\!\left\{
 S(p')\gamma_\nu S(p'+q)\gamma^\rho
 S(p+q)\gamma_\mu S(p)\gamma_\rho\right\}
 \notag\\
 &-2(Z_2-1)_2\frac{\ii e^2}{(2\pi)^4}\int \dd^4p\,
...
\end{align}
where $S(p)\equiv[-\ii\sl{p}+m]/[p^2+m^2-\ii\epsilon]$;
...
\begin{equation}
  ...
  =\frac{\ii e^2}{(2\pi)^4}\int\frac{\dd^4p'}{p'^2-\ii\epsilon}\,
...
\begin{align}
 ...
 \Biggr]
 \notag\\
 &-2R_2\frac{\ii e^2}{(2\pi)^4}\int \dd^4p\,
 \operatorname{Tr}\!\left\{\gamma_\nu S(p+q)\gamma_\mu S(p)\right\}
 \notag\\
 &-(Z_3-1)_{\mathrm{overlap}}
 (q^2\eta_{\mu\nu}-q_\mu q_\nu).
...
\begin{equation}
  S(q_1q_2\to q'_1q'_2)
  =\frac{-\ii(2\pi)^4\delta^4(p'_1+p'_2-p_1-p_2)}
...
where
\begin{align}
 -\ii(2\pi)^4F(q_1q_2\to q'_1q'_2)
 ={}&-\ii(2\pi)^4g
 +\frac12\bigl[-\ii(2\pi)^4g\bigr]^2
 \left[\frac{-\ii}{(2\pi)^4}\right]^2
 \notag\\
 &\cdot\int \dd^4k\Biggl[
 \frac{1}{[(q_1+k)^2+m^2-\ii\epsilon][(q_2-k)^2+m^2-\ii\epsilon]}
 \notag\\
...
this is
\begin{align}
 F={}&g-\frac{g^2}{16\pi^2}\int_0^\infty k^3\,\dd k\int_0^1\dd x
...
\begin{align}
 F=g-\frac{g^2}{32\pi^2}\int_0^1\dd x\Biggl\{
...
\begin{equation}
  ...
  -\int_0^1\dd x\,\ln\!\left(\frac{4x(1-x)}3+\frac{m^2}{\mu^2}\right)\right]
  +\cdots.
...
\begin{align}
 F=g_R-\frac{g_R^2}{32\pi^2}\int_0^1\dd x\Biggl\{
