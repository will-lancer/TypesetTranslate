...
\begin{align}
-i\Delta_{\mu\nu}(x-y)
=\int\frac{d^3p}{(2\pi)^3 2\lvert\mathbf p\rvert}
P_{\mu\nu}(\mathbf p)\Bigl[
e^{ip\cdot(x-y)}\theta(x^0-y^0)
+e^{ip\cdot(y-x)}\theta(y^0-x^0)\Bigr],
...
\notag\\
P_{0i}(\mathbf p)&=P_{i0}(\mathbf p)=P_{00}(\mathbf p)=0.
...
\[
-i\frac12\int d^4x\int d^4y\,
[-ij^0(x)][-ij^0(y)]\frac{-i}{(2\pi)^4}
\int\frac{d^4q}{\lvert\mathbf q\rvert^2}e^{iq\cdot(x-y)}.
\]
