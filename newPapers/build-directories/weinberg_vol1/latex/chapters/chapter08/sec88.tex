...
calculated using ordinary rather than covariant derivatives~\hyperref[ref:8.5]{[5]}.} $dt$ by
taking the derivative and then antisymmetrizing with respect to all indices:
\begin{align}
(dt)_{\mu_1\mu_2\cdots\mu_{p+1}}
...
$A_{\mu_1\cdots\mu_p}$ a $p$-form, because for $F=dA$ to be a tensor it is
...
condition $dF=0$ infer the existence of a $p$-form $A$ with $F=dA$.) By
...
simply
\begin{equation}
d\mathcal F=\mathcal J,\qquad d\mathcal J=0.
...
condition $d\mathcal F=d\mathcal S$ simply tells us that $\mathcal F$ and
...
For $p\leq D-2$, the homogeneous 'Maxwell' equations $dF=0$ read
\begin{equation}
...
\begin{equation}
\partial_{\mu_1}(d\phi)^{\mu_1\cdots\mu_{D-p}}
=-(d\mathcal S)^{\mu_2\cdots\mu_{D-p}}.
\tag{8.8.15}\label{eq:8.8.15}
\end{equation}
This is invariant under a new set of gauge transformations
$\phi\to\phi+d\omega$, except that where $D-p-2=0$ the gauge transformation
...
$(D-p-2)$-form gauge field $\phi$, with current $-d\mathcal S$.}
