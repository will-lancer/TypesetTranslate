...
\[
P(E)=P(E\mid F_1)P(F_1)+P(E\mid F_2)P(F_2)+\cdots+P(E\mid F_n)P(F_n).
\]
...
event that it splits into three. For event $F_1$, $P(E\mid F_1)=1$ since no amoeba is left.
$P(E\mid F_2)=P(E)$ since the state is the same as the beginning. For $F_3$, there are two
...
both will die out is $P(E)^2$. Similarly we have $P(E\mid F_4)=P(E)^3$. Plug in all the numbers,
the equation becomes $P(E)=1/4\times1+1/4\times P(E)+1/4\times P(E)^2+1/4\times P(E)^3$.
...
$P(T_g=60)=\dfrac{30}{60}$. Conditioned on $T_g=60$, we need $P(T_r<T_b\mid T_g=60)$. Among the 30
red and blue candies, each candy is again equally likely to be the last candy and there are
20 blue candies, so $P(T_r<T_b\mid T_g=60)=\dfrac{20}{30}$ and $P(T_r<T_b<T_g)=\dfrac{30}{60}\times\dfrac{20}{30}$. Similarly,
we have $P(T_r<T_g<T_b)=\dfrac{20}{60}\times\dfrac{30}{40}$.

Hence,

\[
P(T_r<T_b\cap T_r<T_g)=P(T_r<T_b<T_g)+P(T_r<T_g<T_b)
=\frac{30}{60}\times\frac{20}{30}+\frac{20}{60}\times\frac{30}{40}=\frac{7}{12}.
\]
...
becomes the first one to toss an $H$. In that case, $A$ has $(1-P(A\mid H))$ probability of
winning. So $P(A\mid H)=1/2\times0+1/2\times(1-P(A\mid H))\Rightarrow P(A\mid H)=1/3$

Combining all the available information, we have

\[
P(A)=1/2\times1/3+1/2(1-P(A))\Rightarrow P(A)=4/9.
\]

\textbf{Sanity check:} we can see that $P(A)<1/2$, which is reasonable since $A$ cannot win in his
first toss, yet $B$ has $1/4$ probability to win in her first toss.

\problem{Russian roulette series}
...
of losing $1-p$. That happens with probability $5/6$. That gives us $p=1\times1/6+(1-p)\times5/6$
...
\[
4!\times\frac{48!}{12!12!12!12!}\div\frac{52!}{13!13!13!13!}
=\frac{52}{52}\times\frac{39}{51}\times\frac{26}{50}\times\frac{13}{49}.
\]
