...
\begin{align*}
\lim_{n\to\infty}\underbrace{x^{x^{x^{\cdots^x}}}}_{n\ \mathrm{terms}}=2
\quad\Longleftrightarrow\quad
\lim_{n\to\infty}\underbrace{x^{x^{x^{\cdots^x}}}}_{n-1\ \mathrm{terms}}=2.
\end{align*}
...
corners have the same color. If you put a 1\(\times\)2 brick on the board, it will always cover one
