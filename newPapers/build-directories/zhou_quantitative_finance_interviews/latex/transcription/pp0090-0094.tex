...
The priors are $P(A)=1/1000$ and $P(A^c)=999/1000$. If the coin is unfair, it always turns up heads, so $P(B\mid A)=1$. If the coin is fair, each time it has $1/2$ probability turning

% ZHOU-SOURCE-PAGE: 91 PRINTED: 75
up heads. So $P(B\mid A^c)=(1/2)^{10}=1/1024$. Plug in all the available information and we have the answer:

\[
P(A\mid B)=\frac{P(B\mid A)P(A)}{P(B\mid A)P(A)+P(B\mid A^c)P(A^c)}
=\frac{1/1000\times1}{1/1000\times1+999/1000\times1/1024}\simeq 0.5.
\]

\problem{Fair probability from an unfair coin}
...
\textbf{1st throw} & A & B & A & C & B & C\\
\textbf{2nd throw} & B & A & C & A & C & B\\
\textbf{3rd throw} & C & C & B & B & A & A\\
\end{tabular}
\end{center}
...
&\times p(\text{yours among those $n-1$ birthdays})
\end{aligned}
\]
\[
=\frac{365\times364\times\cdots\times(365-n+2)}{365^{n-1}}\times\frac{n-1}{365}.
\]
...
\[
P(n-1)=\frac{365}{365}\times\frac{364}{365}\times\cdots\times\frac{365-(n-3)}{365}\times\frac{n-2}{365}
\]
\[
...
\left\{
\begin{aligned}
P(n)>P(n-1)&\Rightarrow \frac{365-(n-2)}{365}\times\frac{n-1}{365}>\frac{n-2}{365}\\
P(n)>P(n+1)&\Rightarrow \frac{n-1}{365}>\frac{365-(n-1)}{365}\times\frac{n}{365}
\end{aligned}
\right.
\Rightarrow
\left\{
...
\textbf{You should be the 20th person in line.}\footnote[14]{\emph{Hint:} If you are the $n$-th person in line, to get the free ticket, the first $(n-1)$ people in line must not have the same birthday and you must have the same birthday as one of them.}
...
\[
P=P(\text{different numbers in all three throws})\times P(\text{increasing order}\mid3\text{ different numbers})
\]
\[
=(1\times\frac{5}{6}\times\frac{4}{6})\times\frac{1}{6}=\frac{5}{54}.
\]
