...
\problem{D. Can you estimate the value of an at-the-money call on a non-dividend paying stock?
...
\[
N(d_1)-N(d_2)=\int_{d_2}^{d_1}\frac{1}{\sqrt{2\pi}}e^{-x^2/2}\,dx,
\]
...
\[
\therefore\quad N(d_1)-N(d_2)\approx
\frac{1}{\sqrt{2\pi}}(d_1-d_2)=\frac{\sigma\sqrt{\tau}}{\sqrt{2\pi}}
\approx0.4\sigma\sqrt{T-t}\Rightarrow c\approx0.4\sigma S\sqrt{\tau}.
\]

\noindent\textbf{In practice, this approximation is used by some volatility traders to estimate
...
\problem{What happens to the gamma of an at-the-money European option when it approaches
its maturity?}
...
\noindent\textbf{So for plain vanilla call and put options, gamma is always positive.}
...
\problem{A. When will a European option have positive theta?}
...
\problem{B. You just entered a long position for a call option on GM and hedged the position
...
\problem{A. Explain implied volatility and volatility smile. What is the implication of volatility}
