...
\(e^{\ii\pi}=\cos\pi+\ii\sin\pi=-1.\)
When \(\theta=\pi/2\), the equation becomes
\(e^{\ii\pi/2}=\cos(\pi/2)+\ii\sin(\pi/2)=\ii.\)\footnote[3]{Clearly they satisfy equation \((e^{\ii\pi/2})^2=\ii^2=e^{\ii\pi}=-1\).}
So \(\ln \ii=\ln(e^{\ii\pi/2})=\ii\pi/2.\)

Hence, \(\ln(i^i)=\ii\ln \ii=\ii(\ii\pi/2)=-\pi/2\Rightarrow i^i=e^{-\pi/2}.\)
