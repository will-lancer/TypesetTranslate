...
\noindent\textbf{Random walk:} The process $\{S_n;n\geq1\}$ is called a random walk if
...
called a \textbf{simple random walk} with parameter $p$. Furthermore, if $p=\frac{1}{2}$, the
process $S_n$ is a \textbf{symmetric random walk}. For symmetric random walk, it's easy
to show that
...
\noindent\textbf{Martingale:} a martingale $\{Z_n,n\geq1\}$ is a stochastic process with the
properties that $E[|Z_n|]<\infty$ for all $n$ and

\[
E[Z_{n+1}\mid Z_n=z_n,Z_{n-1}=z_{n-1},\ldots,Z_1=z_1]=z_n.
\]

The property of a martingale can be extended to

\[
E[Z_m,m>n\mid Z_n=z_n,Z_{n-1}=z_{n-1},\ldots,Z_1=z_1]=z_n,
\]
...
\[
E[S_{n+1}\mid S_n=s_n,\ldots,S_1=s_1]=s_n.
\]
...
\noindent\textbf{Stopping rule:} For an experiment with a set of IID random variables
...
\noindent\textbf{Wald's Equality:} Let $N$ be a stopping rule for IID random variables
...
\end{aligned}\footnotemark[7]
\]
\footnotetext[7]{For detailed proof and applications of Wald's Equality, please refer to the book
\textit{Discrete Stochastic Processes} by Robert G. Gallager.}

\noindent\textbf{A martingale stopped at a stopping time is a martingale.}
...
\begin{gathered}
\left.\begin{aligned}
E[S_N]&=p_\alpha\times83-(1-p_\alpha)\times17=S_0=0,\\
E[S_N^2-N]&=E\left[p_\alpha\times83^2+(1-p_\alpha)\times17^2\right]-E[N]
 =S_0^2-0=0
\end{aligned}\right\}\\
...
$E[N]=1/p=2$. For each roll, the expected face value is $E[X]=7/2$. The total expected
payoff is $E[S_N]=E[X]E[N]=7/2\times2=7$.
