...
and $T(0)=0$, then $V(i,j)=T(j)-T(i-1)$, $\forall 1\leq i\leq j\leq n$. Clearly for
any fixed $j$, when $T(i-1)$ is minimized, $V(i,j)$ is maximized. So the maximum
...
whether the integer $x$ is a power of 2.
...
result could be wrong if \texttt{<<} causes an overflow.}
...
You've got 1000 bottles of wines for a birthday party. Twenty hours before the party,
...
\problem{A. Explain how you can use Monte Carlo simulation to price a European call option?}
...
$N$ should be large.} So $\Delta t=\dfrac{T-t}{N}$ and $t_i=t+\Delta t\times i$, for
$i=0,1,2,\ldots,N$. We then simulate the stock price paths under risk-neutral probability
using equation
...
\[
C=e^{-r(T-t)}\frac{\displaystyle\sum_{k=1}^{M}\max(S_{T,k}-K,0)}{M}.
\]

\problem{B. How do you generate random variables that follow $N(\mu,\sigma^2)$ (normal
...
\[
U=F(X)=\int_{-\infty}^{x}\frac{1}{\sqrt{2\pi}}e^{-x^2/2}\,dx.
\]
