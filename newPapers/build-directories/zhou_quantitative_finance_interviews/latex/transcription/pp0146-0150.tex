...
\problem{B. What is the correlation of a Brownian motion and its square?}
...
\problem{C. Let $B_t$ be a Brownian motion. What is the probability that $B_1>0$ and
$B_2<0$?}
...
P(B_1>0,B_2<0)
 ...
 &=\int_0^\infty\int_{-\infty}^{-x}
   \frac{1}{2\pi}e^{-(x^2+y^2)/2}\,dy\,dx\\
 &=\int_{3\pi/2}^{7\pi/4}\int_0^\infty
...
\problem{A. What is the mean of the stopping time for a Brownian motion to reach either
$-1$ or $1$?}
...
\begin{aligned}
d(B_t^2-t)
 &=\frac{\partial(B_t^2-t)}{\partial B_t}\,dB_t
   +\frac{\partial(B_t^2-t)}{\partial t}\,dt
   +\frac{1}{2}\frac{\partial^2(B_t^2-t)}{\partial B_t^2}\,dt\\
 &=2B_t\,dB_t-dt+dt=2B_t\,dB_t.
\end{aligned}
\]

So $d(B_t^2-t)$ has no drift term and is a martingale. Let
...
\problem{B. Let $W(t)$ be a standard Wiener process and $\tau_x$ ($x>0$) be the first
...
P(\tau_x\leq t)
 ...
 &=2P(W(t)\geq x)=2\int_x^\infty
   \frac{1}{\sqrt{2\pi t}}e^{-w^2/2t}\,dw.
\end{aligned}
\]
...
\[
\therefore\quad P(\tau_x\leq t)=2\int_x^\infty\frac{1}{\sqrt{2\pi t}}e^{-w^2/2t}\,dw
=2\int_{x/\sqrt{t}}^\infty\frac{1}{\sqrt{2\pi}}e^{-v^2/2}\,dv
...
\[
\begin{aligned}
f_{\tau_x}(t)&=\frac{dP\{\tau_x\leq t\}}{dt}\\
&=\frac{dP\{\tau_x\leq t\}}{d(x/\sqrt{t})}
  \frac{d(x/\sqrt{t})}{dt}\\
&=2N'\left(\frac{x}{\sqrt{t}}\right)\times\frac{x}{2}t^{-3/2}\\
...
$E[\tau_x]=x\times\infty=\infty$. Although we have
...
\problem{C. Suppose that $X$ is a Brownian motion with no drift, i.e.
$dX(t)=dW(t)$. If $X$ starts at 0, what is the probability that $X$ hits 3 before
hitting $-5$? What if $X$ has drift $m$, i.e. $dX(t)=m\,dt+dW(t)$?}
...
\[
P_3\exp(-2m\times3)+(1-P_3)\exp(-2m\times-5)=1
\quad\Rightarrow\quad
\frac{e^{10m}-1}{e^{10m}-e^{-6m}}.
\]

\problem{D. Suppose that $X$ is a generalized Weiner process $dX=dt+dW(t)$, where
...
$dX(t)=\beta(t,X_t)dt+\gamma(t,X_t)dW$ and $f(x)$ be a function of $X$. Define function
$V(t,x)=E[f(X_T)\mid X_t=x]$, then $V(t,x)$ is a martingale process that satisfies the
