...
\[
\Rightarrow E[f(n+1)]=2\times E[f(n)]+2=2^{n+2}-2.
\]

\textbf{General Martingale approach:} Let's use $HH\cdots H_n$ to explain a
...
have after the $i$-th game, then $(x_i-i)$ is a martingale.
...
time is a martingale. So we still have $E[(x_i-i)]=0$. If the sequence stops after
the $i$-th toss ($i\geq n$), the $(i-n+1)$-th player is the (first) player who
gets $n$ heads in a roll with payoff $2^n$. So all the $(i-n)$ players before him
went bankrupt; the $(i-n+2)$-th player gets $(n-1)$ heads in a roll with payoff
...
If the sequence stops after the $i$-th toss, the $(i-5)$th gambler gets the
$HHTTH$ with payoff

% ZHOU-SOURCE-PAGE: 137 PRINTED: 121

$2^6$. All the $(i-6)$ players before him went bankrupt; the $(i-4)$th player
loses in the second toss ($HT$); the $(i-3)$th player and the $(i-2)$th player
lose in the first toss ($T$); the $(i-1)$th player gets sequence $HH$ with payoff
...
\noindent\textbf{1. The underlying discrete-time dynamic system}
...
\noindent\textbf{2. A cost (or profit) function that is additive over time.}
...
\[
J_{\pi^*}(x_0)=\min_{\pi}E\left\{g_N(x_N)+\sum_{k=0}^{N-1}g_k(x_k,u_k,w_k)\right\}.
\]

\textbf{Dynamic programming (DP) algorithm}

The dynamic programming algorithm relies on an idea called the \textbf{Principle of
...
\textbf{DP algorithm:} To solve the basic problem
\[
J_{\pi^*}(x_0)=\min_{\pi}E\left\{g_N(x_N)+\sum_{k=0}^{N-1}g_k(x_k,u_k,w_k)\right\},
\]
...
\[
J_k(x_k)=\min_{u_k\in U_k}E\left\{g_k(x_k,u_k,w_k)+J_{k+1}\bigl(f(x_k,u_k,w_k)\bigr)\right\},
\qquad k=0,\ldots,N-1.
\]
...
is $3/6\times3.5+1/6\times(4+5+6)=\$4.25$.
...
$4/6\times4.25+1/6\times(5+6)=\$14/3$.
...
$(f(i,j)+y)$ if the Red Sox wins and the state goes to $(i+1,j)$, or
...
\begin{cases}
f(i,j)=\dfrac{f(i+1,j)+f(i,j+1)}{2},\\[6pt]
...
$y=(f(i+1,j)-f(i,j+1))/2$, we can also calculate the bet we need to place at each
