...
\[
I(x)y=\int I(x)Q(x)\,dx \Rightarrow y=\frac{\int I(x)Q(x)\,dx}{I(x)}.
\]

The integrating factor, $I(x)$, must satisfy
$\dfrac{dI(x)}{dx}=I(x)P(x)$, which means $I(x)$ is a separable differential equation
with general solution $I(x)=e^{\int P(x)\,dx}$.\footnote[8]{The constant $c$ is not needed in this case since it just scales both sides of the equation by a factor.}

\problem{Solve ordinary differential equation $y'+\dfrac{y}{x}=\dfrac{1}{x^2}$, $y(1)=1$, where $x>0$.}
\solution
This is a typical example of first-order linear equations with $P(x)=\dfrac{1}{x}$ and
$Q(x)=\dfrac{1}{x^2}$. So $I(x)=e^{\int P(x)\,dx}=e^{\int(1/x)\,dx}=e^{\ln x}=x$
...
Taking integration on both sides, $xy=\int(1/x)\,dx=\ln x+c$
...
\item If $r_1$ and $r_2$ are real and $r_1=r_2=r$, then the general solution is
      $y=c_1e^{rx}+c_2xe^{rx}$;
\item If $r_1$ and $r_2$ are complex numbers $\alpha\pm i\beta$, then the general
...
\problem{What is the solution of ordinary differential equation $y''+y'+y=0$?}
\solution
...
$a\dfrac{d^2y}{dx^2}+b\dfrac{dy}{dx}+cy=0$, a nonhomogeneous linear equation
$a\dfrac{d^2y}{dx^2}+b\dfrac{dy}{dx}+cy=d(x)$ has no closed-form solution. But if we
can find a particular solution $y_p(x)$ for
$a\dfrac{d^2y}{dx^2}+b\dfrac{dy}{dx}+cy=d(x)$, then $y=y_p(x)+y_g(x)$, where
$y_g(x)$ is the general solution of the homogeneous equation
$a\dfrac{d^2y}{dx^2}+b\dfrac{dy}{dx}+cy=0$, is a general solution of the nonhomogeneous
equation $a\dfrac{d^2y}{dx^2}+b\dfrac{dy}{dx}+cy=d(x)$.
...
\problem{What is the solution of ODE $y''+y'+y=1$ and $y''+y'+y=x$?}
\solution
...
discuss several topics that have significant \textbf{applications} in statistics and numerical
methods.
...
\noindent\textbf{Inner product/dot product:} the inner product (or dot product) of two $R^n$
vectors $x$ and $y$ is defined as
\[
\sum_{i=1}^{n}x_i y_i=x^Ty.
\]

\noindent\textbf{Euclidean norm:}
\[
...
\problem{There are 3 random variables $x$, $y$ and $z$. The correlation between $x$ and $y$
...
\[
0.8\times1.2=1\times h\Rightarrow h=0.96
\]
\[
...
\noindent\textbf{QR decomposition:} For each non-singular $n\times n$ matrix $A$, there is a
...
\problem{If the programming language you are using does not have a function for the linear
