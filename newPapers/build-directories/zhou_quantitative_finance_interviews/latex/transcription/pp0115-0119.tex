...
&=0.5+0.3-\bigl(0.5\times0.3-\rho_{AB}\sigma_A\sigma_B\bigr)\\
...
\subsubsection{Expected value of max and min}
...
\[
E[Z_n]=\int_0^1xf_{Z_n}(x)\,dx=\int_0^1nx^n\,dx
...
\[
E[Y_n]=\int_0^1nx(1-x)^{n-1}\,dx
=\int_0^1n(1-y)y^{n-1}\,dy
=\left[y^n\right]_0^1-\frac{n}{n+1}\left[y^{n+1}\right]_0^1=\frac{1}{n+1}.
\]

\subsubsection{Correlation of max and min}
...
\[
\operatorname{corr}(Y,Z)=\frac{\operatorname{cov}(Y,Z)}{\operatorname{std}(Y)\times\operatorname{std}(Z)}
=\frac{E[YZ]-E[Y]E[Z]}{\sqrt{E[Y^2]-E[Y]^2}\times\sqrt{E[Z^2]-E[Z]^2}}.
\]
...
$E[Z]=\dfrac{2}{2+1}=\dfrac{2}{3}$. From the pdfs of $Y$ and $Z$,
$f_Y(x)=n(1-x)^{n-1}=2(1-x)$ and $f_Z(z)=nz^{n-1}=2z$, we can also get
$E[Y^2]=\int_0^1 2(1-y)y^2\,dy=\dfrac{2}{3}-\dfrac{2}{4}=\dfrac{1}{6}$ and
$E[Z^2]=\int_0^1 2z^3\,dz=\dfrac{2}{4}$, which give us the variances:
...
To calculate $E[YZ]$, we can use $E[YZ]=\int_0^1\int_0^z yz f(y,z)\,dy\,dz$. To solve
...
\\
\text{and}\quad E[YZ]&=\int_0^1\int_0^z2yz\,dy\,dz\\
&=\int_0^1\left[zy^2\right]_0^z\,dz\\
&=\int_0^1z^3\,dz=\frac{1}{4}.
\end{aligned}
\]
...
\[
\therefore\quad E[YZ]=\int_0^1\int_0^1x_1x_2\,dx_1\,dx_2
=E[X_1]E[X_2]=\frac{1}{2}\times\frac{1}{2}=\frac{1}{4}.
\]

Hence $\operatorname{cov}(Y,Z)=E[YZ]-E[Y]E[Z]=\dfrac{1}{36}$ and
$\operatorname{corr}(Y,Z)=\dfrac{\operatorname{cov}(Y,Z)}{\sqrt{\operatorname{var}(Y)}\times
\sqrt{\operatorname{var}(Z)}}=\dfrac{1}{2}$.

\textbf{Sanity check:} That $Y$ and $Z$ have positive autocorrelation make sense since
when $Y$ becomes large, $Z$ tends to become large as well ($Z\geq Y$).

\subsubsection{Random ants}
...
\textbf{When two ants collide head-on, both immediately change directions. What does it
