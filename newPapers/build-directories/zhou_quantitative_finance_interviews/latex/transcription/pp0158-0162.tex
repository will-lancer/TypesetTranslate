...
For this specific problem, we should have $8/9\times P(90)=8/9\times9=8>P(80)$. Since the
...
\[
90>S_T\ge 80,\qquad \text{payoff}=8\times(90-S_T)>0\quad\text{(Puts with }K=90\text{ are exercised.)}
\]

\[
S_T<80,\qquad \text{payoff}=8\times(90-S_T)-9\times(80-S_T)=S_T>0\quad\text{(All puts are exercised.)}
\]
...
$dS=\mu Sdt+\sigma SdW(t)$, and the derivative $V=V(S,t)$ is a function of $S$ and $t$,
then applying Ito's lemma yields:

\[
\begin{aligned}
dV&=\left(\frac{\partial V}{\partial t}+\mu S\frac{\partial V}{\partial S}
+\frac{1}{2}\sigma^2S^2\frac{\partial^2V}{\partial S^2}\right)dt\\
&+\sigma S\frac{\partial V}{\partial S}dW(t),\\
...
+\frac{1}{2}\sigma^2S^2\frac{\partial^2V}{\partial S^2}\right)dt
+\sigma S\frac{\partial V}{\partial S}dW(t)
-\frac{\partial V}{\partial S}\left(\mu Sdt+\sigma SdW(t)\right)
\]

\[
=\left(\frac{\partial V}{\partial t}+\frac{1}{2}\sigma^2S^2\frac{\partial^2V}{\partial S^2}\right)dt.
\]

It is apparent that this portfolio is risk-free since it has no diffusion term. It should
have risk-free rate of return as well: $d\Pi=r\left(V-\dfrac{\partial V}{\partial S}S\right)dt$.
...
$dX(t)=\beta(t,X)dt+\gamma(t,X)dW(t)$ and $f(x)$ be a function of $X$. Define function
$V(t,x)=E\left[e^{-r(T-t)}f(X_T)\mid X_t=x\right]$, then $V(t,x)$ is a martingale process
that satisfies the partial differential equation

\[
\frac{\partial V}{\partial t}+\beta(t,x)\frac{\partial V}{\partial x}
+\frac{1}{2}\gamma^2(t,x)\frac{\partial^2V}{\partial x^2}=rV(t,x)
\]

and boundary condition $V(T,x)=f(x)$ for all $x$.

Under risk-neutral measure, $dS=rSdt+\sigma SdW(t)$. Let $S=X$, $\beta(t,X)=rS$ and
...
\problem{A. What are the assumptions behind the Black-Scholes formula?}
...
volatility $\sigma$: $dS=\mu Sdt+\sigma SdW(t)$.
...
\problem{B. How can you derive the Black-Scholes formula for a European call on a non-dividend
...
interest rate $r(t)$: $dS=r(t)Sdt+\sigma SdW(t)$. Risk-neutral measure allows the option to be
priced as the discounted value of its expected payoff with the risk-free interest rate:

\[
\begin{aligned}
V(t)&=E\left[ e^{-\int_t^T r(u)\,du}V(T)\mid S(t)\right],\\
...
\[
\begin{aligned}
E[V(T)\mid S]&=E[\max(S_T-K,0)\mid S]\\
...
e^{-(\epsilon-\sigma\sqrt{\tau})^2/2}\,d\epsilon
-K\int_{-d_2}^{\infty}\frac{1}{\sqrt{2\pi}}e^{-\epsilon^2/2}\,d\epsilon.
\end{aligned}
\]

Let $\tilde{\epsilon}=\epsilon-\sigma\sqrt{\tau}$, then
$d\epsilon=d\tilde{\epsilon}$, $\epsilon=-d_2\Rightarrow
\tilde{\epsilon}=-d_2-\sigma\sqrt{\tau}=-d_1$ and we have

\[
Se^{r\tau}\int_{-d_2}^{\infty}\frac{1}{\sqrt{2\pi}}
e^{-(\epsilon-\sigma\sqrt{\tau})^2/2}\,d\epsilon
=Se^{r\tau}\int_{-d_1}^{\infty}\frac{1}{\sqrt{2\pi}}e^{-\tilde{\epsilon}^2/2}\,d\tilde{\epsilon}
=Se^{r\tau}N(d_1),
\]
...
\problem{C. How do you derive the Black-Scholes formula for a European call option on a
...
$\dfrac{\partial V}{\partial S}=\dfrac{\partial V}{\partial y}\dfrac{dy}{dS}
...
\[
\frac{\partial V}{\partial t}+rS\frac{\partial V}{\partial S}
+\frac{1}{2}\sigma^2S^2\frac{\partial^2V}{\partial S^2}-rV=0
\]
...
+\frac{1}{2}\sigma^2\frac{\partial^2V}{\partial y^2}-rV=0.
\]
...
+\frac{1}{2}\sigma^2\frac{\partial^2u}{\partial y^2}=0.
\]
...
\int_{-\infty}^{\infty}u_0(\psi)\exp\left(-\frac{(x-\psi)^2}{2\sigma^2\tau}\right)d\psi.\footnotemark[7]
\]
