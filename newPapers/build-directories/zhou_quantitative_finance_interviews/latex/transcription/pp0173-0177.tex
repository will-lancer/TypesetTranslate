...
\begin{problem}{B. You have to price a European call option either with a constant volatility 30\%
...
\begin{problem}{C. The Black-Scholes formula for non-dividend paying stocks assumes that the stock
...
\begin{solution}
The payoff of a European call at its maturity date is $\max(S_T-K,0)$.
Therefore under risk-neutral measure, we have

\[
c=e^{-r\tau}\int_K^\infty(s-K)f_{S_T}(s)\,\dd s,
\]
...
\(\displaystyle \frac{\partial}{\partial z}\int_{a(z)}^{b(z)}f(x,z)\,\dd x
=\int_{a(z)}^{b(z)}\frac{\partial f(x,z)}{\partial z}\,\dd x
+f(b(z),z)\frac{\partial b}{\partial z}-f(a(z),z)\frac{\partial a}{\partial z}.\)} we have

\[
\begin{aligned}
\frac{\partial c}{\partial K}
&=e^{-r\tau}\frac{\partial}{\partial K}\int_K^\infty(s-K)f_{S_T}(s)\,\dd s\\
&=e^{-r\tau}\int_K^\infty\frac{\partial(s-K)}{\partial K}f_{S_T}(s)\,\dd s\\
&\quad-e^{-r\tau}(K-K)\cdot1\\
&=e^{-r\tau}\int_K^\infty-f_{S_T}(s)\,\dd s
\end{aligned}
\]
...
&=e^{-r\tau}\frac{\partial}{\partial K}\int_K^\infty-f_{S_T}(s)\,\dd s\\
&=e^{-r\tau}f_{S_T}(K).
\end{aligned}
\]
...
\begin{problem}{What are the price boundaries for a bull call spread?}
\end{problem}
...
\begin{problem}{Explain what a straddle is and when you want to purchase a straddle.}
\end{problem}
...
variance swap pays $N\cdot(\sigma_r^2-K_{\mathrm{var}})$, where $N$ is the notional
...
\begin{problem}{What is the price of a binary (cash-or-nothing digital) European call option on a
...
\begin{problem}{How would you price an exchange call option that pays $\max(S_{T,1}-S_{T,2},0)$ at
...
\[
\dd S_1=\mu_1S_1\dd t+\sigma_1S_1\dd W_{t,1}
\]

\[
\dd S_2=\mu_2S_2\dd t+\sigma_2S_2\dd W_{t,2}.
\]
