...
\[
\begin{aligned}
\frac{\partial f}{\partial S_1}&=-\frac{S_2}{S_1^2},\\
\frac{\partial f}{\partial S_2}&=\frac{1}{S_1},\\
\frac{\partial^2 f}{\partial S_1^2}&=\frac{2S_2}{S_1^3},\\
...
\[
\begin{aligned}
\dd f={}&\frac{\partial f}{\partial S_1}\dd S_1+\frac{\partial f}{\partial S_2}\dd S_2
+\frac{1}{2}\frac{\partial^2 f}{\partial S_1^2}(\dd S_1)^2
+\frac{1}{2}\frac{\partial^2 f}{\partial S_2^2}(\dd S_2)^2
+\frac{\partial^2 f}{\partial S_1\partial S_2}\dd S_1\dd S_2\\
={}&-\mu_1\frac{S_2}{S_1}\dd t-\sigma_1\frac{S_2}{S_1}\dd W_{t,1}
+\mu_2\frac{S_2}{S_1}\dd t+\sigma_2\frac{S_2}{S_1}\dd W_{t,2}
+\sigma_1^2\frac{S_2}{S_1}\dd t-\rho\sigma_1\sigma_2\frac{S_2}{S_1}\dd t\\
={}&\left(\mu_2-\mu_1+\sigma_1^2-\rho\sigma_1\sigma_2\right)f\dd t
-\sigma_1f\,\dd W_{t,1}+\sigma_2f\,\dd W_{t,2}\\
={}&\left(\mu_2-\mu_1+\sigma_1^2-\rho\sigma_1\sigma_2\right)f\dd t
+\sqrt{\sigma_1^2-2\rho\sigma_1\sigma_2+\sigma_2^2}\cdot f\,\dd W_{t,3}.
\end{aligned}
\]
...
$S_1C_s=S_2N(d_1)-S_1N(d_2)$.\qedhere
...
\begin{problem}{You are constructing a simple portfolio using two stocks $A$ and $B$. Both have the same
...
&\quad+2\rho_{A,B}\sigma_A\sigma_B(1-w_A)\\
&\quad-2\rho_{A,B}\sigma_A\sigma_Bw_A=0.
\end{aligned}
\]
...
&=\frac{0.09-0.5\cdot0.2\cdot0.3}
{0.04-2\cdot0.5\cdot0.2\cdot0.3+0.09}=\frac{6}{7}.
\end{aligned}
\]

So we should invest 6/7 of the money in stock $A$ and 1/7 in stock $B$.\qedhere

\end{solution}

\subsubsection*{Value at risk}

\begin{problem}{Briefly explain what VaR is. What is the potential drawback of using VaR to measure the
risk of derivatives?}
\end{problem}
...
\[
\alpha=\int_{\mathrm{VaR}}^{\infty}f(x)\,\dd x,
\]
...
confidence level, $\operatorname{VaR}(A)=0$ since the probability of default is less than 5\%.
...
have $\operatorname{VaR}(C)\leq\operatorname{VaR}(A)+\operatorname{VaR}(B)$. For example, if we add a short position in a credit
...
$\operatorname{VaR}(B)=0$. When $A$ and $B$ form a portfolio $C$, the probability that at least one bond
will default becomes $1-(1-3\%)(1-3\%)\approx5.9\%$. So $\operatorname{VaR}(C)=\$1M>\operatorname{VaR}(A)+\operatorname{VaR}(B)$.
...
\[
\begin{aligned}
T\uparrow&\Rightarrow D\uparrow\quad c\uparrow\Rightarrow D\downarrow\quad y\uparrow\Rightarrow D\downarrow\\
T\uparrow&\Rightarrow C\uparrow\quad c\uparrow\Rightarrow C\downarrow\quad y\uparrow\Rightarrow C\downarrow.
\end{aligned}
\]

Another important concept is dollar duration: $\$D=-\dfrac{\dd P}{\dd y}=P\cdot D$. Many
market participants use a concept called DV01: $DV01=-\dfrac{\dd P}{10,000\cdot\dd y}$,
...
\begin{problem}{What are the price and duration of an inverse floater with face value \$100 and annual
