...
% Source-visible anomalies preserved: the prose after Eq. (29.3.7) assigns
% R=2 to W_{A alpha L}; Eqs. (29.3.9) and (29.3.32) visibly print
% W_{B beta B}; Eq. (29.3.15) gives its last F-term without a preceding
% factor 2 Re; after deriving C_1-C_2=N_c-N_f=1/nu, the paragraph following
% Eq. (29.3.16) prints N_c=N_f-1; and the dimensional-analysis paragraph
% refers to the ``second term'' of Eq. (29.3.11).
% Uncertainties: none.
...
non-vanishing divergence. With \(\theta_L\) and \(\theta_R\) having
\(R=+1\) and \(R=-1\), respectively, and \(V_A\) and \(\Phi_n\)
\(R\)-neutral, the fermion fields \(\lambda_{AL}\) and \(\psi_{nL}\)
...
\begin{equation}
  \theta_L\longrightarrow e^{i\varphi}\theta_L\,,
  \qquad
  \Phi\longrightarrow\Phi\,,
  \qquad
  V_A\longrightarrow V_A\,,
  \qquad
  Y\longrightarrow e^{2i\varphi}Y\,,
...
\end{equation}
Because \(W_{A\alpha L}\) has \(R=2\), the whole theory including
...
\(\exp(2i\pi T)\), which for \(T=\tau\) is periodic in \(\theta\), has
\(R=2(C_1-C_2)\).
...
\begin{equation}
  ...
  \left[
    ...
    +\mathcal{B}_\lambda(\Phi,W_L,T,Y)
  \right]_{\mathcal{F}}\,,
  \end{aligned}
...
powers of \(\exp(2i\pi T)\), which have definite \(R\) values.

Furthermore, it is only \emph{positive} powers of \(\exp(2i\pi T)\) that
...
operators \(\exp(2i\pi aT)\) that have positive-definite, zero, or
...
\(\mathcal{B}_\lambda\) contains a factor \(W_{A\alpha L}\), then it must
...
\(T\), or one factor of \(\exp(2i\pi T/(C_1-C_2))\) and no dependence
on \(W\) or \(Y\):
\begin{equation}
  \begin{aligned}
  \mathcal{B}_\lambda
  ...
  Yf_\lambda(\Phi)
  +\sum_{\alpha\beta AB}
  \epsilon_{\alpha\beta}W_{A\alpha L}W_{B\beta B}
  \ell_{\lambda AB}(\Phi)
  \\
  &+\exp\left(\frac{2i\pi T}{C_1-C_2}\right)v_\lambda(\Phi)\,.
  \end{aligned}
...
non-supersymmetric gauge theories with fermions, Eq.~\eqref{eq:18.7.2}
...
\begin{equation}
  ...
  +2\operatorname{Re}
  \left[
    \frac{\tau_\lambda}{8\pi i}
    \sum_{A\alpha\beta}
    \epsilon_{\alpha\beta}W_{A\alpha L}W_{A\beta L}
  \right]_{\mathcal{F}}
  \\
  &+2\operatorname{Re}\left[f(\Phi)\right]_{\mathcal{F}}
  +\exp\left(\frac{2i\pi\tau_\lambda}{C_1-C_2}\right)
  \left[v_\lambda(\Phi)\right]_{\mathcal{F}}\,,
  \end{aligned}
  \tag{29.3.15}\label{eq:29.3.15}
\end{equation}
where
\begin{equation}
  \tau_\lambda=\frac{4\pi i}{g_\lambda^2}+\frac{\theta}{2\pi}\,.
...
\begin{equation}
  ...
  \longrightarrow
  \mathcal{L}
  -\sum_d\frac{n(d)C_{2d}}{32\pi^2}
  \sum_A\epsilon_{\mu\nu\rho\sigma}
  f_A^{\mu\nu}f_A^{\rho\sigma}\varphi_d\,,
  \tag{29.3.18}\label{eq:29.3.18}
\end{equation}
where \(C_{2d}\) is the contribution to \(C_2\) of any one left-chiral
...
property
\begin{equation}
  T\longrightarrow T+n(d)C_{2d}\varphi_d/\pi\,.
  \tag{29.3.19}\label{eq:29.3.19}
\end{equation}
Since \(v_\lambda(\Phi)\) is accompanied in Eq.~\eqref{eq:29.3.9} by a
factor \(\exp(2i\pi T/(C_1-C_2))\), which undergoes the transformation
\begin{equation}
  \exp\left(\frac{2i\pi T}{C_1-C_2}\right)
  \longrightarrow
  \prod_d
  \exp\left(\frac{+2i n(d)C_{2d}\varphi_d}{C_1-C_2}\right)
  \exp\left(\frac{2i\pi T}{C_1-C_2}\right),
...
\(-2n(d)C_{2d}/(C_1-C_2)\)}. (For instance, in generalized supersymmetric
...
\(n(d)=N_f\) and \(C_{2d}=1/2\), so \(v_\lambda\) is a homogeneous
...
In general \(C_2=\sum_d n(d)C_{2d}\), so \(v_\lambda(\Phi)\) is a
...
the bare coupling \(g\) with \(g_\lambda\) in the second term of
Eq.~\eqref{eq:29.3.11} it cannot depend on the ultraviolet cut-off \(K\)
...
\(-2n(d)C_{2d}/(C_1-C_2)\) in the \(\Phi\)s that belong to each
...
\begin{equation}
  ...
  v_\lambda(\Phi)
  =
  \exp\left(\frac{i\theta}{C_1-C_2}\right)
  \Lambda^{(3C_1-C_2)/(C_1-C_2)}H(\Phi)\,.
...
transformations: first rotate \(\bar q_{1i}\) into the 1-direction; then
keeping the 1-direction fixed, rotate in the space perpendicular to this
direction to put \(\bar q_{2i}\) in the 2-direction; and so on; and then
...
\begin{equation}
  ...
  \equiv
  \mathcal{J}
  \exp\left(\frac{i\theta}{N_c-N_f}\right)
...
\(SU(N_f)\mathbin{\cdot}\overline{SU(N_f)}\) transformation used to put
...
factor \(\exp(2i\pi n/N_c)\), with \(n\) an integer ranging from 0 to
...
symmetry with \(q_{ai}\longrightarrow e^{i\pi}q_{ai}\) and
\(\bar q_{ai}\longrightarrow e^{-i\pi}\bar q_{ai}\).) The fact that there
...
For \(C_2=C_1\) the function \(\exp(2i\pi T)\) has \(R=0\), so that its
...
\begin{equation}
  \mathcal{B}_\lambda
  =
  Yf_\lambda\bigl(\Phi,\exp(2i\pi T)\bigr)
  +\sum_{\alpha\beta AB}
  \epsilon_{\alpha\beta}W_{A\alpha L}W_{B\beta B}
  \ell_{\lambda AB}\bigl(\Phi,\exp(2i\pi T)\bigr).
...
\begin{equation}
  \Phi_n\longrightarrow e^{i\varphi}\Phi_n\,,
  \qquad
...
Here the \(R\) value \(2(C_1-C_2)\) of \(\exp(2i\pi T)\) is
\emph{negative}, so positive powers of \(\exp(2i\pi T)\) can compensate
